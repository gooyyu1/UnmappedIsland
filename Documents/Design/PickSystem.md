...
`pick` は、既存の `effects` が付けられる場所（無条件 / `when` / `on` / stage 内、GameElementDefinition.md 8.2 節・6.4 節）に
そのまま付けられる、新しいトリガー体系を必要としない仕組みとして設計します。

- 探索時のアイテム発見（`on: explore` 発火時に、複数アイテムから重み付きで1つを選ぶ）
- 攻撃の結果（`on: attack` 発火時に、命中/ミス/クリティカルを重み付きで選ぶ）
- 生水を飲んで腹を下すか（`on: drink_raw_water` 発火時に、発症/無事を重み付きで選ぶ）
  ...
  attack:
    effects:
      - on: attack
        ...
        pick:
          - weight: 10
            add: { target.hp: -20 }   # クリティカル(現時点では固定重み)
          - weight: { path: self.accuracy }
            add: { target.hp: -10 }   # 通常命中
          - weight: 40
            add: {}                    # ミス
...
- `pick` は新しいトリガー体系を必要としない。既存の `effects` の付け所（無条件 / `when` / `on` / stage 内）にそのまま乗る
...
- `modify` のターゲットが現状 `self`/`parent`/`child`（8.2 節）に限定されている点。自分のツリーに属さない対象の重みを
  外部から書き換えたいケース（例: 「幸運のお守り」が探索の宝物発見率を上げる）は、8.2 節の既存 TODO（`target` への
  `ancestor`/`sibling`/`descendant` 追加）と合わせて解決する必要がある
