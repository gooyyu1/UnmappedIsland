...
**環境エリア**と**キャラクターエリア**の 2 つのサブエリアで構成されます。

##### 環境エリア (Environment Area)

情報エリアの上半分を占め、現在の自然環境に関する情報を集約するエリアです。
**天候エリア**と**日時エリア**で構成されます。
...
└── ダッシュボードエリア (Dashboard Area)
    ...
    │   ├── 環境エリア (Environment Area)
    │   │   ├── 天候エリア (Weather Area)
    │   │   ├── 環境エリア (Conditions Area)
    │   │   └── 日時エリア (DateTime Area)
    │   └── キャラクターエリア (Character Area) [左右 1:1 分割]
    │       ├── キャラクター表示エリア (Character Display Area) [上下 2:1 分割]
    │       │   ├── ポートレイトエリア (Portrait Area)
    │       │   └── 装備・状態エリア (Equipment/Condition Area) [左右 1:1 分割]
    │       │       ├── 装備エリア (Equipment Area)
    │       │       └── 状態エリア (Condition Area)
