...
**状況エリア**と**キャラクターエリア**の 2 つのサブエリアで構成されます。

##### 状況エリア (Situation Area)

情報エリアの上半分を占め、現在の自然環境と時間に関する情報を集約するエリアです。
**天候エリア**、**条件エリア**、**日時エリア**で構成されます。
...
###### 条件エリア (Conditions Area)

現在の自然環境の条件（日差し、虫の多さ、湿度、風など）を表示するエリアです。
...
左右に 1:1 で分割され、左がキャラクター表示エリア、右がステータスエリアです。

###### キャラクター表示エリア (Character Display Area)

キャラクターの表示領域です。さらに上下に 2:1 で分割されます。

####### ポートレイトエリア (Portrait Area)

キャラクターのポートレイト（2/3 の高さ）をカード形式で表示するエリアです。

####### 装備・怪我エリア (Equipment/Injury Area)

キャラクターの装備と怪我情報を表示する領域です（1/3 の高さ）。
さらに左右に 1:1 で分割されます。

######## 装備エリア (Equipment Area)

キャラクターが装備している品目を示す**装備カード**を表示するエリアです。

######## 怪我エリア (Injury Area)

キャラクターが負っている傷病を示す**怪我カード**を表示するエリアです。

###### ステータスエリア (Status Area)

キャラクター表示エリアの右側に配置され、キャラクターのステータスバー（飢え、渇き、疲労、気分など）を表示するエリアです。
...
└── ダッシュボードエリア (Dashboard Area)
    ├── 情報エリア (Info Area)
    │   ├── 状況エリア (Situation Area) [3 列分割：1/3 高さ]
    │   │   ├── 天候エリア (Weather Area)
    │   │   ├── 条件エリア (Conditions Area)
    │   │   └── 日時エリア (DateTime Area)
    │   └── キャラクターエリア (Character Area) [左右 1:1 分割、2/3 高さ]
    │       ├── キャラクター表示エリア (Character Display Area) [50% 幅、上下 2:1 分割]
    │       │   ├── ポートレイトエリア (Portrait Area) [2/3 高さ]
    │       │   └── 装備・怪我エリア (Equipment/Injury Area) [1/3 高さ、左右 1:1 分割]
    │       │       ├── 装備エリア (Equipment Area)
    │       │       └── 怪我エリア (Injury Area)
    │       └── ステータスエリア (Status Area) [50% 幅、全高]
    └── オプションエリア (Options Area)
        └── 操作ボタン（設定・図鑑・ログ・終了）
```
