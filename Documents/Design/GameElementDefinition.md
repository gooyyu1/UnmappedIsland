...
- **`stack_by`**: プロパティ名。指定すると、「同じ`ObjectDef`であること」に加え「このプロパティの現在値も
  一致すること」が同じ`ObjectStack`にまとまる条件になります（既定=省略時は`ObjectDef`が同じだけでまとまる）。
  中身の種類によって同じ道具でも別物として扱いたい容器（同じ水筒でも、水が入ったものと空のものは別スタック、
  種類を問わず同じ水同士はまとめたい）のような、「中身が同じかどうか」でスタックの単位を決めたいケースに使います。
...
```
  ...
  `stack_order`と同じく、スタックへ加わる時点の値で1度だけ判定し、その後の値の変化を追って自動的に
  別のスタックへ移し替えることはしません（容器の中身が後から変わっても、同じスロットに居続ける限り
  見た目上のスタックはそのまま）。`fixed_positions`の固定番号も、`ObjectDef`単位ではなく`ObjectStack`単位
  （＝`stack_by`込みの単位）で割り当てられるため、中身が違う容器同士は固定番号を共有しません。`traits`側
  （5節）でも宣言でき、`stack_order`と同じ規則（自身の宣言が優先、複数の`trait`が重複宣言していればエラー）
  でマージされます。
