...
- 直接の親（`parent`）固定ではなく、祖先を遡る（`ancestor`）固定である理由: 直接の親がこのプロパティを
  持たない場合に対応するためです（例: アイテムの直接の親はプレイヤーだが、`temperature` は部屋が持つ）。
  途中の親がこのプロパティを定義していれば、そこで探索が止まり、そこより上（例えば部屋）までは遡りません。
  これにより、断熱容器のように「途中で値を変えて中継する」表現も、`inherit` とは別に、その容器自身が
...
object_defs:
  ...
  food:
    props:
      temperature:
        value: -2      # 自分のオフセット
        inherit: true  # 祖先(通常はroom)のtemperatureを加算する
```
...
暖炉自身がカート等（`temperature` を持たない容器）の中に置かれていても、その容器を素通りして、実際に
`temperature` を持つ部屋（あるいはさらに上の祖先）に効果が届きます。祖先を辿った先のオブジェクトが後で
