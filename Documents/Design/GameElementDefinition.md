...
- **シンボル型プロパティ（6.8 節）の場合は `min` を書きません。** シンボル型の値は `NameRegistry` への登録順で
  ...
  数値プロパティの「最下段（min省略）＝フォールバック」という、他の値を明示的に受け止める段階を`stages`に
  書く必要はシンボル型プロパティにはありません。ただし、著者が `stages` に書き忘れた値に対して `ResolveStage`
  が `null` を返すことはありません。シンボル値自体が既に「名前の付いた離散値」であるため、`stages` にその値の
  エントリが無くても、**値自身の名前をそのままステージ名とする段階（`passives` を持たない）が都度合成されて
  返ります**。`null` を返してしまうと、呼び出し側（`WhenOwnStage` ゲート等）が「該当する段階が無いこと」と
  「著者が書き忘れただけであること」を区別できず、意図せず判定が無効化される危険があるためです。
