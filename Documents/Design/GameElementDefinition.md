...
- **シンボル型プロパティ（6.8 節）の場合は `min` を書きません。** シンボル型の値は `NameRegistry` への登録順で
  ...
  これは数値型プロパティでも本来同じで、著者がフォールバック段階（`min` 省略の段階）を書かなければ、
  該当する段階が無い値に対して `ResolveStage` は同じく `null` を返します。つまり「フォールバックの書き忘れで
  `null` になり得る」というのは数値型・シンボル型を問わない一般的な性質であり、シンボル型だけ特別に
  「値の名前をそのまま段階名として合成する」ような救済を用意しても、数値型側に残る同じ問題は解消されません。
  そのため `ResolveStage` の戻り値は**常に `null` になり得るもの**として扱い、呼び出し側（`WhenOwnStage`
  ゲートを含む）は必ず `null` チェックを行う前提とします（`Runtime.WorldObject.IsInStage` 参照）。
