...
（`destroy`しない、または`destroy`しても同種が残る場合）、挿入位置は「自分の直後」です。一般のスロットでは
これは単純な挿入で済みますが、`fixed_positions`のスロットでは、自分の固定番号+1の位置から空いている番号を
探し、間にいる他の型をすべて+1して押し出してから割り込ませます（前詰めはせず、押し出された型同士の相対順序は
変わりません）。空きが見つからない場合（`unit_capacity`分すべてが自分より後ろまで埋まっている場合）は配置に
失敗し、必ず起点自身の親へ強制的に伝播します（`fallback`が無いのと同じ扱い）。
