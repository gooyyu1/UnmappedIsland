...
- `min` の代わりに **`eq`（完全一致）**を指定することもできます。`min` は値の大小関係（半開区間）を前提に
  しますが、シンボル型プロパティ（6.8 節）の値は `NameRegistry` への登録順で決まるだけの整数であり、
  大小関係そのものに意味がありません。`eq` は「この値と完全に一致する場合だけこの段階」という判定を行うため、
  シンボル型プロパティの各値をそのまま段階名として使えます。`min`/`eq`は同じ段階に同時に指定できません
  （ロード時エラー）。最下段（他のどの段階にも該当しない場合のフォールバック）は、`eq` を使う場合も
  `min`/`eq` の両方を省略します。
