...
（`Tests/StreamingAssets/ClimateSystemTests.cs`。「calm は短い間隔で軽い雨が降り続ける」「wet はほとんど雨だが
...
- `atmospheric_moisture`（`world` の蓄積型プロパティ、range 0〜10,000）: `season` の各 stage が持つレートを
  毎 tick 加算する（calm: +10 / wet: +60 / dry: -20）。0〜10,000 というスケールは、calm の遅いレート
...
- 降雨中の自己減算: 天気（4 節）が `light_rain`/`heavy_rain`/`storm` のいずれかである間、その `weather` の
  ...
  （-50 / -100 / -200。激しい雨ほど速く消費する。耐久値や `GameElementDefinition.md` 6.4 節の
  `progress`/`feverish` 例と同じ、ステージ自身が値を変化させるパターン）。雨が降り続ければやがて水分が
  尽きて乾いた候補が選ばれやすくなる、という循環がこれで自然に成立します。
  各減算量と `wet` の季節レート（+60/tick）との大小関係が、雨季の降雨の止み方を決めます:
  `light_rain`（-50）だけは季節レートを下回る（正味 +10/tick）ため、小雨は雨季の水分を自力では
  使い尽くせず降り続きます。`heavy_rain`（-100、正味 -40）・`storm`（-200、正味 -140）は季節レートを
  上回るため、大雨・嵐のあとには水分が減って晴れ間が来うる、という緩急がつきます（4.3 節）。
...
**「乾季後半ほど暑い」は、この貯水池モデルだけで自動的に成立します。** `thermal_level` には自己減算に
相当する消費要因がないため、`dry` の間は単調に積み上がり、追加の「経過度」計算をしなくても乾季後半に
`hot` 帯へ達して暑くなります。一方、大気水分量側は雨自身の自己減算という消費要因を持つため単調には
積み上がらず、`wet` の間は各天気の正味レートが釣り合う水準（`humid` 閾値付近、4.3 節）を上下し続けます。
その結果、嵐・大雨は雨季後半に偏らず雨季全体に分布します（雨季の過酷さは「後半ほど強まる」のではなく
「季節を通じて断続的に来る」形）。`calm` は 3.1 節の通り `dry`/`wet` とは逆向き（回復方向）のレートを
持ち、後半に向かうほど穏やかになっていく季節として設計しています。
...
実装値では、`calm` 中の `light_rain` の正味レートは -40/tick（自己減算 -50 ＋季節レート +10）で、
`weather_remaining` 1 回分（16〜24 tick、4.2 節）の放電は 640〜960 に留まります。谷（0 付近）まで
落ちるのは乾季明けの登り返しの間だけで、以後の大気水分量は `moderate` 閾値 3,000 の少し下を
「+10 で登って閾値に触れる→雨が選ばれて -40 で少し押し戻される」と行き来し続けます。放電と充電の
レート比が 40:10 なので、晴れと雨の時間比はおおよそ 4:1 に落ち着き、**1 日 1 回前後の軽い雨**が
降ります。閾値のすぐ上では `heavy_rain`/`storm` の重みが 0（`moderate` 段階、4.3 節）のため雨は
ほぼ `light_rain` で、水分が稀に `humid`（6,000）まで登り切った場合にのみ大雨・嵐が起こりえます
（実測で `calm` の tick 比 0.1% 未満。「calm にもごく稀に大雨が来る」という揺らぎとして許容します）。
統計テストでは 30 シード平均で 27 日間に 24.3 回の雨イベント、雨の最大間隔は平均 2.6 日・最悪 3.5 日
でした。より長い期間（20 シード×3,600 日）の診断（`Documents/Diagnostics/ClimateSystemStats.md`）では、
`calm` の連続未降雨時間は平均 1.01 日・95%ile 2.50 日・最大 5.71 日で、蒸発への安全余裕を保っています。
...
重み比（約 38%）で頭打ちのため、`calm` の雨は 1〜2 周期（実測で平均 0.23 日、95%ile 0.42 日）で
止むのが基本形です。
...
- `weather`: 現在の天気を表す symbol プロパティ。`core.yaml` の 7 値すべてが遷移先候補です。強度順に、
  ...
  `storm` と `scorching` は両極の「極端な天候」で、それぞれ雨季・乾季後半の風物詩として扱います
  （嵐は雨季全体に断続的に、炎天は乾季後半に集中して現れる。3 節末尾参照）。
...
`atmospheric_moisture` の `humid`/`saturated` 帯（雨季の水分が高い間）で重みを持つのと対応する関係です。
...
`light_rain` +10/tick（季節レート +60 － 自己減算 50）、`heavy_rain` -40/tick、`storm` -140/tick、
非降雨時 +60/tick です。小雨だけは正味プラスのため、小雨は雨季の水分を自力では使い尽くせず降り続き、
大雨・嵐は正味マイナスのため、激しく降ったあとには水分が減って晴れ間が来うる——という緩急が、
重みの調整ではなくレートの符号だけから生まれます。大気水分量は `humid` 閾値（6,000）の少し下を
上下し続け（`humid` 帯は雨が優勢で正味マイナス寄り、`moderate` 帯は晴れが混ざって正味プラス）、
実測では `wet` の雨 tick 比率は約 72%（うち大雨・嵐が約 30%）、連続降雨時間は平均 0.68 日・
95%ile 1.79 日・最大 5.62 日、連続未降雨時間は平均 0.27 日でした
（`Documents/Diagnostics/ClimateSystemStats.md`、20 シード×3,600 日）。
「ほとんど雨だが、まとまった雨と晴れ間が交互に来る」という雨季の体感をこの構成で表現しています。

かつては季節レートを ±100〜250 の大きな値にし、自己減算も -200〜-400 と大きくして釣り合いを
取っていましたが、**季節による変動が過大**で、(1) `calm` では 1 回の小雨で水分が `moderate` 帯の外まで
吹き飛んで雨がほぼ必ず 1 周期で終わる（連続降雨時間の 95%ile が 1 周期 = 0.25 日に張り付く）、
(2) 調整のたびにレートと自己減算の両方を連動させ直す必要がある、という問題がありました。現行の
控えめな値（3 節）は「大気水分量は雨自身が消費する。季節レートは方向を与えるだけでよい」という
方針への転換で、`dry` のマイナスを小さくできるのも同じ理由です（乾季は雨が降らないこと自体が
乾燥を維持するため、季節レートで強く引き下げる必要がない）。
...
- 両極の極端な天候は貯水池の高水位帯が駆動する: `storm`（嵐）は大気水分量の `humid`/`saturated` 帯
  （雨季の水分が高い間）が、`scorching`（炎天）は蓄熱量の `hot` 帯（乾季後半）が、それぞれ重みを立てる
  （4.3 節）。ただし名残は意図的に非対称: 嵐は雨自身が水分を消費し尽くすまで乾季序盤に残るが、炎天は
  蓄熱量の上限 = `hot` 閾値により乾季の終了と同時に消える（残暑なし。雨は大気の水分を消費するが、
  晴れは大気の熱を消費しないという現実の物理と整合させ、「暑さ = 乾季」をプレイヤーの体感と一致させる）
...
- `calm` の大気水分量レートは「ほぼ0」にせず、`dry` で下がりきった水準から `wet` にふさわしい水準へ回復する
  ...
  晴れ・雨の両方が起こる（3.1 節）。さらに `calm` では上昇レートと降雨の自己減算レートのバランス（4:1）に
  より、連続未降雨時間が長くなりすぎない（＝1日1回前後、軽めの雨が挟まる）ようにしている（3.2 節）
- 季節レートは控えめな値に抑え、季節ごとの天気の対比はレートの絶対値ではなく自己減算との大小関係
  （各天気の正味レートの符号）で作る。`wet` では小雨だけが正味プラスで降り続き、大雨・嵐は正味マイナスで
  晴れ間への引き金になる（3 節・4.3 節）
