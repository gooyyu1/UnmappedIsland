...
`modify`/`accumulate` しているのと同じパターンです。「一度きりの発火」を表す専用の仕組み（フラグ・
`on_shortfall` の上書き等）は不要です。
...
- 5 節の難易度補正の検討過程では、「`on_min` で本体の処理を行い、`on_shortfall` の既定動作（`range.min` への
  `set`）を上書きして境界へ戻さず大きく上へ逃がすことで、実質的に『1 回だけ発火』を表現する」という代替案も
  検討しました。`world` のようなシングルトンは `destroy` できず、`on_min`/`on_shortfall`（6.3 節・6.5 節）が
  評価される順序（`on_min` が先、`on_overflow`/`on_shortfall` が後）を利用したこの手筋は、破棄できない
  プロパティ上で「1 回だけ」を表現する一般的なイディオムとして、季節・天気とは別の場面（積分値ではない、
  瞬間的な単発イベント）で今後使える可能性があります。ただし本節の対象は大気水分量という積分値そのものであり、
  「特定の期間ずっと加算する」という `stages` の通常の使い方（3 節・4.3 節と同型）の方が素直に当てはまるため、
  本書では不採用としました。
