...
- 降雨中の自己減算: 天気（4 節）が `light_rain`/`heavy_rain`/`storm` のいずれかである間、その `weather` の
  ...
  例と同じ、ステージ自身が値を変化させるパターン）。どの減算量も `wet` の季節レート（+90/tick）をやや上回る
...
実測（20 シード×3,600 日、`Documents/Diagnostics/ClimateSystemStats.md`）では、`calm` の連続未降雨時間は
...
非降雨時 +90/tick です。どの雨も正味マイナスのため降り続けばいずれ晴れ間を呼び込み、非降雨時の +90 が
素早く水分を回復させて次の雨へつなぐ——という充放電の往復が、`humid` 閾値（6,000）付近を上下しながら
「ほとんど雨だが、まとまった雨と晴れ間が交互に来る」という雨季の体感を作ります。実測では `wet` の
雨 tick 比率は約 68%（うち大雨・嵐が約 36%）、連続降雨時間は平均 0.54 日、連続未降雨時間は平均 0.25 日
でした（`Documents/Diagnostics/ClimateSystemStats.md`、20 シード×3,600 日）。なお `light_rain` の
