...
- `season_elapsed`: 現在の季節に入ってからの経過 tick 数。毎 tick `+1` される。季節が切り替わるたびに `0` へ
  リセットする（5.2 節の「乾季 10 日目前後」判定に使う）。
...
次の 3 つを同時に行います。`on_min` の対象は `self` のみ許可されていますが、すべて `world` 自身のプロパティなので
制約になりません。

1. `season` を巡回順の次の値へ `set` する（3 状態の固定巡回であり、天気のような確率分岐ではないため `pick`
   （10 節）は不要）。
2. `season_elapsed` を `0` へ `set` する。
3. `season_remaining` を次の期間の tick 数へ `set` する。`season_cycle` が `0`（まだ 1 周目）の間は常に固定 30 日
   （2,880 tick）。`1` 以上になった後は、複数の候補日数から等確率で 1 つを選ぶ `pick`（10 節。候補と重みの
   具体値は 7 節で未決）でばらつかせる。
4. `calm`→`dry` への遷移（＝1 周が完了した瞬間）でのみ、`season_cycle` を `+1` する。
...
## 5. 難易度の初期補正: 大気水分量への一時的な介入

序盤の理不尽な過酷さを避けるため、ゲーム開始直後に限って大気水分量を底上げし、天気（4 節）が雨に転びやすい
状態を作ります。**天気そのものを固定で雨にするのではなく**、あくまで抽選の元になる `atmospheric_moisture`
の値を操作するだけであり、それでも晴れる可能性はゼロではありません（5.3 節）。

### 5.1 ゲーム開始2日目

`world` の既存プロパティ `day`（`core.yaml`）が `2` になったタイミングで、`atmospheric_moisture` に一時的な
加算を行い、直後の天気遷移（4.3 節）が雨系候補に大きく傾くよう仕向けます。

### 5.2 最初の乾季、10日目前後

初めて `season` が `dry` になったサイクルに限り、その乾季に入ってから（`season_elapsed`）10 日相当
（960 tick）が経過したタイミングで、5.1 節と同様に `atmospheric_moisture` へ一時加算を行います。2 回目以降の
乾季ではこの補正を行わないため、初回の乾季かどうかを覚えておくフラグ（例: `dry_season_seen`）が別途必要です。
...
- `season_cycle` に応じた「固定 30 日 / ランダム」の分岐を、既存の `on_min`/`pick` の語彙でどう具体的に書くか
  （2.3 節）
- 5.1・5.2 節の一時加算の具体的な加算量、および `day == 2` のようなタイミング検出の正確な書き方
