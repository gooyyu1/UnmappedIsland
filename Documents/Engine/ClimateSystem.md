...
- 降雨中の自己減算: 天気（4 節）が `light_rain`/`heavy_rain`/`storm` のいずれかである間、その `weather` の
  ...
  （-100 / -280 / -395。耐久値や `GameElementDefinition.md` 6.4 節の `progress`/`feverish` 例と同じ、
  ステージ自身が値を変化させるパターン）。雨が降り続ければやがて水分が尽きて乾いた候補が選ばれやすくなる、
  という循環がこれで自然に成立します。3 つの減算量は「激しい雨ほど速く消費する」という単純な比例には
  なっていません。`light_rain` の減算量だけ突出して小さいのは、`heavy_rain`/`storm` は `wet` の季節レート
  （+250/tick）と拮抗・凌駕させて「降り続ければ尽きる」循環を成立させる一方、`light_rain` は現実の小雨が
  そこまで大気の水分を奪わないという想定と、`calm` の降雨頻度（3.2 節）を左右する下降側のパラメータで
  あることを踏まえ、意図的に弱く設定しているためです（4.3 節参照）。
...
あります。「概ね 3 日に 1 回くらい、大雨・嵐ではない軽い雨（`light_rain`）」という言い方は、この連続未降雨
時間に対する目安であって、雨が一定周期で規則正しく降ることを求めているわけではありません（一定周期性は
そもそも 3.3 節が扱う「固定レートによる単調さ」の一種であり、望ましくありません）。

この頻度は新しい仕組みを必要とせず、既存の 2 つのレートの**バランス**だけで決まります。

- **上昇側**: `calm` の大気水分量レート（3.1 節）が、雨が止んだ直後の水準から、4.3 節の `moderate` 段階
  （`light_rain_weight` が初めて意味のある値を持つ段階）の閾値まで登り切るのにかかる日数。これが
  連続未降雨時間の実体です。
- **下降側**: 降雨中の自己減算（3 節）が、一度 `light_rain` が選ばれてから大気水分量をどこまで押し戻すか。
  ここが深いほど、次の上昇側にかかる日数（＝連続未降雨時間）は長くなります。

実装値では、`calm` の上昇レート +10/tick は `moderate` 閾値 3,000 まで 300 tick（約 3.1 日）で登ります。
`light_rain` の自己減算 -100/tick は `weather_remaining` 1 回分（16〜24 tick、4.2 節）で 1,600〜2,400 を
消費するため、`moderate` 帯に入ってすぐ選ばれた場合でも水分量は谷（0 付近）までは落ちきらず、途中で
折り返します。結果、大気水分量は「登って `moderate` に触れる→ `light_rain` が選ばれて部分的に放電する→
また登って `moderate` に触れる」という充放電を、谷から山へ緩やかに充電し雨のたびに一部だけ放電する形で
繰り返します。`humid`/`saturated` まで登り切る前に自己減算で引き戻されるため、「大雨にはならない」という
要件も、`heavy_rain`/`storm` の重みが `moderate` 段階では 0 である（4.3 節）ことと合わせて満たされます。
統計テストでは 30 シード平均で 27 日間に 13.0 回の雨イベント、雨の最大間隔は平均 3.7 日・最悪 5.3 日でした。
より長い期間（20 シード×3,600 日）の診断（`Documents/Diagnostics/ClimateSystemStats.md`）では、`calm` の
連続未降雨時間は平均 1.92 日でした。

自己減算を弱めると「谷まで押し戻す」度合いが浅くなり、次の上昇にかかる日数（＝連続未降雨時間）はむしろ
**短くなります**。自己減算の大きさと連続未降雨時間の長さは独立ではなく、直結しています。これは、
降雨中の自己減算を軽くする調整が雨季の連続降雨時間（4.3 節）にも影響するのと対になる関係です。
...
`atmospheric_moisture` の `saturated` 帯（雨季後半）で重みを持つのと対応する関係です。

ただし**名残の振る舞いは意図的に非対称**です。嵐には名残があります: 雨季末の飽和した水分が雨自身の
...
**`wet` の連続降雨時間を伸ばすには、重みの比率よりも自己減算の大きさが本質的でした。** 雨季（`wet`）で
「まとまった雨が降り続く」体感を強めるため、`humid` 段階の晴れ系の重みを縮小した
（`sunny_weight` を廃止し `clear`/`cloudy` も小さくした）だけでなく、`heavy_rain`/`storm` の自己減算
（3 節）を当初値（-300/-400）から -280/-365 へ弱めています（`storm` はこの後 4.3 節後段でさらに -395 まで
調整）。後者がより本質的な調整です。

天気の遷移ごとに「連続降雨がそこで終わるかどうか」は `weather_remaining` の `on_min` が引く `pick` の
瞬間の大気水分量の段階だけで決まるため、素朴には `humid`/`saturated` 段階の重み比率だけを見れば十分に
思えます。しかし実際にシミュレーションで、連続降雨が終わる直前の大気水分量の段階を計測したところ、
弱める前は**約 65% が `moderate` 段階（雨がほぼ選ばれない段階）まで水分が落ちきってから終わっていました**。
`heavy_rain`/`storm` の自己減算が `wet` の季節レート（+250/tick）を大きく上回っていたため、連続降雨の
途中で `heavy_rain`/`storm` が続けて選ばれるたびに水分が `humid`/`saturated`（雨が優勢な段階）を割り込み、
`moderate`（晴れ系が優勢な段階）まで押し下げてしまっていたのが実態です。`humid` 段階の重み比率をどれだけ
雨寄りに調整しても、水分自体が `humid` の外まで落ちてしまえばその調整は効きません。

そこで自己減算を、`wet` の季節レートをわずかに下回る程度（-280/-365、正味 -30/tick 前後）まで弱めました。
これにより連続降雨の途中で水分が `moderate` まで落ちきる前に、多くの場合次の雨（多くは `humid`/`saturated`
段階のまま）へつながるようになります。自己減算自体はなくしていない（雨は依然として水分を消費し続け、
いずれは尽きる）ため、「雨が降り続ければやがて水分が尽きる」という 3 節の循環そのものは保っています。
統計テストでは、この調整で `wet` の連続降雨時間の平均が約 0.78 日から約 1.52 日（約 2 倍）に伸びました。

**`light_rain` の自己減算を弱めると、`wet` の連続降雨時間と `calm` の連続未降雨時間の両方が動きます。**
`light_rain` の自己減算を -200 から -100 へ弱めたところ（3 節・3.2 節参照）、直接の目的である `calm` の
連続未降雨時間は 2.74 日から 1.92 日へ**短くなりました**（3.2 節で述べた通り、自己減算が浅くなり折り返し
までの上昇にかかる日数が減るため）。同時に `wet` では `light_rain` 中の正味レート（+250 の季節レート
に対し自己減算 -100 なので正味 +150/tick）が強く上向きになり、連続降雨時間が 1.52 日から 2.00 日まで
伸びてしまいました（4.3 節前段で確立した約 1.5〜1.6 日の目安から外れる）。この副作用を打ち消すため、
`storm` の自己減算の絶対値を -365 から -395 へさらに強め、`wet` の連続降雨時間を 1.54 日まで戻しています
（`Documents/Diagnostics/ClimateSystemStats.md`、20 シード×3,600 日の実測値）。
`heavy_rain` の自己減算（-280）はこのラウンドでは変更していません。
...
- 両極の極端な天候は貯水池の飽和帯が駆動する: `storm`（嵐）は大気水分量の `saturated` 帯（雨季後半）が、
  `scorching`（炎天）は蓄熱量の `hot` 帯（乾季後半）が、それぞれ重みを立てる（4.3 節）。ただし名残は
  意図的に非対称: 嵐は雨自身が水分を消費し尽くすまで乾季序盤に残るが、炎天は蓄熱量の上限 = `hot` 閾値に
...
- `calm` の大気水分量レートは「ほぼ0」にせず、`dry` で下がりきった水準から `wet` にふさわしい水準へ回復する
  ...
  連続未降雨時間が長くなりすぎない（＝概ね3日に1回程度、大雨にはならない軽い雨が挟まる）ようにしている
  （3.2 節）
