...
（`Tests/StreamingAssets/ClimateSystemTests.cs`。「calm は概ね 2 日に 1 回軽い雨が降る」「wet はほとんど雨だが
...
- `atmospheric_moisture`（`world` の蓄積型プロパティ、range 0〜10,000）: `season` の各 stage が持つレートを
  毎 tick 加算する（calm: +10 / wet: +90 / dry: -20）。0〜10,000 というスケールは、calm の遅いレート
  （moderate 閾値 3,000 まで約 3 日）を整数の `accumulate` のまま表現するためのもの。
  季節レートは意図的に控えめな値に抑えている。特に `dry` のマイナスを大きくしないのは、大気水分量には
  雨自身の自己減算（後述）という減少要因が既にあり、かつ一度下がれば増加要因がなく晴れ続けるため、
  季節レートで強く引き下げなくても乾季は自然に乾くから。季節ごとの天気の対比は、レートの絶対値の
  大きさではなく、自己減算との**大小関係**（正味でどちらへ動くか）が作る。
- `thermal_level`（蓄熱量。同じく `world` の蓄積型プロパティ、range 0〜1,920）: `season` の各 stage が
  ...
  calm は乾季の 1,920 から 20 日かけて 0 へ下り「乾季の暑さからじわじわ涼しくなる」を、それぞれ表現する。
  `hot` stage は炎天（`scorching`）の抽選重み `scorching_weight` も駆動する（4.3 節）。
  **上限は `hot` 閾値 1,920 と同値**とし、残暑（calm へ持ち越される炎天）を廃止している: calm に入った最初の
  tick で蓄熱が 1,919 へ下がって `mild` になるため、炎天と気温 +8 は乾季の終了と同時に消える（stage の
  `min` は「その値を含む」判定のため、上限に張り付いている間も `hot` は有効）。「暑さ = 乾季」という対応を
  プレイヤーの体感と素直に一致させるための意図的な設計で、暑さの長さの揺らぎは季節の持続日数のランダム性
  （2.3 節）だけが表現する。
- 降雨中の自己減算: 天気（4 節）が `light_rain`/`heavy_rain`/`storm` のいずれかである間、その `weather` の
  ...
  （-110 / -130 / -200。激しい雨ほど速く消費する。耐久値や `GameElementDefinition.md` 6.4 節の
  `progress`/`feverish` 例と同じ、ステージ自身が値を変化させるパターン）。雨が降り続ければやがて水分が
  尽きて乾いた候補が選ばれやすくなる、という循環がこれで自然に成立します。
  どの減算量も `wet` の季節レート（+90/tick）をやや上回る（正味 -20 / -40 / -110）ため、雨季でも
  降り続けた雨はいずれ水分を減らして晴れ間を呼び込みます。`light_rain` の減算量は `calm` の
  連続未降雨時間（3.2 節）を決める主要パラメータでもあります。
...
自己減算に相当する消費要因がないため、`dry` の間は単調に積み上がり、追加の「経過度」計算をしなくても
乾季後半に `hot` 帯へ達して暑くなります。一方、大気水分量は雨自身の自己減算という消費要因を持つため
単調には積み上がらず、`wet` の間は各天気の正味レートが釣り合う水準（`humid` 閾値付近、4.3 節）を
上下し続けます。そこで雨季側の「後半ほど嵐・大雨」は、大気水分量ではなく、`thermal_level` と対称の
もう 1 つの単調な積分値 `monsoon_level`（雨季の深まり、4.3 節）が保証します。`calm` は 3.1 節の通り
`dry`/`wet` とは逆向き（回復方向）のレートを持ち、後半に向かうほど穏やかになっていく季節として
設計しています。
...
- 大気水分量: `dry` はマイナス（-20）、`calm` は**プラス**（+10。乾季で下がりきった水準から、雨季にふさわしい
  水準へじわじわ回復していく）、`wet` はさらにプラス（+90）。
...
確保しなければならない序盤です。晴れが続くと蓋のない容器の水は蒸発してしまうため、渇水で詰まないよう、
`calm` の**連続未降雨時間**（雨が止んでから次の雨までの長さ）が長くなりすぎないでほしい、という狙いが
あります。これは雨が一定周期で規則正しく降ることを求めるものではありません（一定周期性はそもそも
3.3 節が扱う「固定レートによる単調さ」の一種であり、望ましくありません）。

この頻度は新しい仕組みを必要とせず、既存の 2 つのレートの**バランス**だけで決まります。

- **上昇側**: `calm` の大気水分量レート（3.1 節、+10/tick）が、雨が止んだ直後の水準から、4.3 節の
...
実装値では、`calm` 中の `light_rain` の正味レートは -100/tick（自己減算 -110 ＋季節レート +10）で、
`weather_remaining` 1 回分（16〜24 tick、4.2 節）の放電は 1,600〜2,400 に達します。この放電を
+10/tick で登り返すのに 160〜240 tick（1.7〜2.5 日）かかるため、**連続未降雨時間は平均およそ 2 日**に
なります（雨のたびにほぼ底まで放電し、約 2 日かけて `moderate` 閾値 3,000 へ登り返す充放電サイクル）。
`moderate` 帯の `light_rain_weight` は 20（晴れ系 65 に対し約 24%）に抑えており、閾値に触れてから
実際に雨が選ばれるまでの待ち時間も間隔に上乗せされます。閾値のすぐ上では `heavy_rain`/`storm` の
重みが 0（`moderate` 段階、4.3 節）のため雨はほぼ `light_rain` で、水分が稀に `humid`（6,000）まで
登り切った場合にのみ大雨が起こりえます（実測で `calm` の tick 比 0.1% 前後。「calm にもごく稀に
大雨が来る」という揺らぎとして許容します）。実測（20 シード×3,600 日、
`Documents/Diagnostics/ClimateSystemStats.md`）では、`calm` の連続未降雨時間は平均 2.17 日・
95%ile 4.21 日・最大 6.62 日で、「概ね 2 日に 1 回の軽い雨」と蒸発への安全余裕を両立しています。

自己減算を弱めると「押し戻す」度合いが浅くなり、次の上昇にかかる日数（＝連続未降雨時間）は
**短くなります**。自己減算の大きさと連続未降雨時間の長さは独立ではなく、直結しています。逆に
連続降雨時間を伸ばす方向には自己減算はあまり効きません: `moderate` 帯では雨が続けて選ばれる確率自体が
重み比で頭打ちのため、`calm` の雨は 1〜2 周期（実測で平均 0.21 日、95%ile 0.25 日）で
止むのが基本形です。
...
- `weather`: 現在の天気を表す symbol プロパティ。`core.yaml` の 7 値すべてが遷移先候補です。強度順に、
  ...
  扱います（嵐は `monsoon_level`、炎天は `thermal_level` という季節内進行度が駆動する。4.3 節参照）。
...
atmospheric_moisture:
  ...
  stages:
    ...
    - name: humid          # 6000〜8499: 雨が優勢になり、大雨・嵐も現れ始める（晴れ系はclearのみ、僅かに）
      min: 6000
      passives:
        - modify:
            self:
...
原理的に外れなくなり、`saturated` 段階（雨季の水分が最も高まった状態）で「まず晴れない」を表現できなく
なるためです。上の実装では
...
サイクルなら絶対 81 日目）以降であり、炎天は**乾季後半の天気**になります。
...
単調に増えず（3 節）、水分の帯だけでは「後半ほど」を表現できないので、消費要因を持たない単調な
積分値を別に持つ、という `thermal_level`（乾季後半の炎天）と同じ解法です。

この上乗せには **`conditions`（14 節）で「大気水分量が閾値以上のとき」というゲート**が付いています
（`deepening` は 6,000 以上、`peak` は 4,500 以上。stage の WhenOwnStage ゲートと `conditions` は AND で
重なる、8.2 節）。無条件に上乗せすると、`monsoon_level` の減衰が終わらない乾季序盤に、水分が尽きた
砂漠のような空からも大雨が降ってしまうためです。逆にこのゲートのおかげで、乾季に入っても水分がまだ
高い間（雨自身が消費し尽くすまでの過渡）は上乗せが生き、嵐・大雨の名残が自然に表現されます。
実測（20 シード×3,600 日）では、`wet` を 3 等分した嵐・大雨の tick 比率は序盤 31%→中盤 38%→終盤 42%、
嵐だけでは 4%→6%→18% と、後半に向かって単調に増えます（大雨単独は嵐に置き換わられて終盤にやや減るが、
「嵐・大雨」の合計と嵐単独はどちらも単調増加。統計テスト
`WetSeason_HeavyRainAndStorms_IncreaseTowardTheEnd` がこの性質を回帰検知します）。

ただし**名残の振る舞いは意図的に非対称**です。嵐には名残があります: 雨季末の高い水分が雨自身の
自己減算で消費し尽くされるまでの乾季序盤には、嵐・大雨が残ります。一方、炎天には名残（残暑）が
ありません: 蓄熱量の上限を `hot` 閾値と同値（1,920）にしているため、calm に入った瞬間に `hot` から
外れ、炎天は乾季の終了と同時に消えます（3 節）。この非対称は現実の物理とも整合します——雨は降ること
自体が大気の水分を消費しますが、晴れることは大気の熱を消費しません（せいぜい放射冷却の程度）。
熱が「使われて減る」対応物が無い以上、雨側の自己減算・名残に対応する仕組みを熱側へ導入しない、
という判断です。プレイヤーから見た「乾季の暑さが長い年」は、残暑ではなく季節の持続日数のランダム性
（2.3 節）が表現します。ゲーム開始直後の最初の calm は、蓄熱量の初期値が中立（1,000 = `mild`）のため
炎天は発生しません。
...
`light_rain` -20/tick（季節レート +90 － 自己減算 110）、`heavy_rain` -40/tick、`storm` -110/tick、
非降雨時 +90/tick です。どの雨も正味マイナスのため降り続けばいずれ水分が減って晴れ間を呼び込み、
非降雨時の +90 が素早く水分を回復させて次の雨へつなぐ——という充放電の往復が、`humid` 閾値（6,000）
付近を上下しながら「ほとんど雨だが、まとまった雨と晴れ間が交互に来る」という雨季の体感を作ります。
実測では `wet` の雨 tick 比率は約 68%（うち大雨・嵐が約 36%）、連続降雨時間は平均 0.54 日、
連続未降雨時間は平均 0.25 日でした（`Documents/Diagnostics/ClimateSystemStats.md`、20 シード×3,600 日）。
なお `light_rain` の自己減算（-110）は雨季のためではなく、`calm` の連続未降雨時間を約 2 日にする
（3.2 節）ことから決まっている点に注意してください。1 つの定数が複数の季節の挙動に効くため、
変更時は 3.2 節の充放電サイクルと本節の正味レートの両方を確認する必要があります。
...
`boosting` の +200/tick は dry の -20/tick を大きく上回るため、正味 +180/tick で乾ききった水分量が
...
- 両極の極端な天候は、消費要因を持たない単調な季節内進行度が駆動する: `storm`（嵐）・後半の大雨は
  `monsoon_level`（雨季の深まり）の高水位帯が、`scorching`（炎天）は `thermal_level`（蓄熱量）の
  `hot` 帯（乾季後半）が、それぞれ重みを立て、「雨季後半ほど嵐・大雨」「乾季後半ほど炎天」を対称に
  表現する（4.3 節）。ただし名残は意図的に非対称: 嵐・大雨の上乗せは大気水分量のconditionsゲート越し
  のため、雨自身が水分を消費し尽くすまでの乾季序盤には名残が残るが、炎天は蓄熱量の上限 = `hot` 閾値に
  より乾季の終了と同時に消える（残暑なし。雨は大気の水分を消費するが、晴れは大気の熱を消費しないという
  現実の物理と整合させ、「暑さ = 乾季」をプレイヤーの体感と一致させる）
- 「季節後半ほど過酷になる」は、消費要因を持たない単調な積分値（`thermal_level`/`monsoon_level`）から
  生まれ、追加の経過度計算を要しない（3 節末尾）。`calm` は `dry`/`wet` と逆向きのレート（回復方向）を
  持たせることで、この法則から意図的に外れる（3.1 節）
- `calm` の大気水分量レートは「ほぼ0」にせず、`dry` で下がりきった水準から `wet` にふさわしい水準へ回復する
  ...
  連続未降雨時間が長くなりすぎない（＝概ね2日に1回、軽めの雨が挟まる）ようにしている（3.2 節）
- 季節レートは控えめな値に抑え、季節ごとの天気の対比はレートの絶対値ではなく自己減算との大小関係で作る。
  `wet` ではどの雨も正味マイナスのため降り続けば晴れ間を呼び込み、非降雨時の回復が次の雨へつなぐ
  （3 節・4.3 節）
- `dry` の雨は「一度も降らないか、多くても2〜3回」に抑える: dry帯のlight_rain_weightを最小(1)にし、
  雨季の名残（水分が高い間の嵐・大雨）と序盤補正（5.2 節）だけが乾季の数少ない雨を担う（4.3 節）
