...
- 同じ ID を持つ定義が複数のファイルに存在する場合、ファイル・ディレクトリ・読み込みのタイミングを問わず
  **常にエラー**とします（後勝ちで黙って上書きする、という規則は持ちません）。「追加のつもりだったのに
  既存の定義を誤って上書きしていた」という事故を防ぐためです。MOD による既存定義の意図的な差し替えは、
  この「追加」の文法とは別に、専用の「既存 object_def への patch」文法を用意して表現します（詳細は別途）。
