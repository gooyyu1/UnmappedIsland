...
（`Domain.Defs.PropertyDef.CreateValue` 参照。かつては毎 tick 再ロールの意味でしたが、
「開始時に 1 回引いて以後固定」へ変更されています）。
...
- **空のmappingを書く（`on_shortfall: {}`）と、「宣言だけして何もしない」**、つまり既定のクランプを打ち消して
  ...
  値を境界へ引き戻す（結果 `on_min` を二重発火させうる）ことを避けるため、この形を使います。
...
そのものは変わりません。`Slot`は中身を`ObjectStack`（1つのスタックにまとまるオブジェクトの集まり）の列として
保持し、常に「表示上の並び順」そのものを表す実データとして維持されます。個々の`ObjectStack`の中は次で説明する
`stack_order`（無ければ挿入順）で並びます。スタックの区別を畳み込んだ「このスロットの中身全部」のビュー
（`Slot.Contents`）も提供されますが、これは内部処理（タグ判定・重さ集計など、スタックの区別に興味が無い
ほとんどの処理）向けの派生ビューであり、正の情報源はあくまで`ObjectStack`の列です。
...
```
  ...
  宣言でき、`stack_order`と同じ規則（自身の宣言が優先、複数の`trait`が重複宣言していればエラー）で
  マージされます。
