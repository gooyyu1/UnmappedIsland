...
- **日時はぶら下げ式フリップカード**: 経過日数と時刻は、卓球のスコアボードのように
...
| レーン間・外周マージン | 6 |
| 状況エリア高 | 176 |
| 日時フリップカード | 日数の桁 64×88、時刻の桁 44×60 |
| オプションバー高 | 96 |
| ポートレイトカード | 260 × 399 |
| 装備・怪我カード | 146 × 224 |
| アイコンボタン | 88 × 88 |
...
日時のフリップカード表示（経過日数・時刻を左右に並べる）と天候チップを表示する
高さ 176u の帯です。両者は `justify-content: space-between` で左右に配置し、
横幅いっぱいに広げて余白を抑える。縦型ではフィールドエリアの直上、横型では
ダッシュボード列の最上段（ロケーションレーンの左隣）に置きます。
