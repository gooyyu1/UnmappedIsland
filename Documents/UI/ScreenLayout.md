...
- **キャラクター表示エリアは2×2グリッド（縦型）／カード＋縦列（横型）**: 縦型は
  「キャラクターカード｜条件」「装備｜怪我」の 2×2 グリッド。横型は左にポートレイトカードを
  置き、右の列に条件ボタンの行・装備ボタン・怪我ボタンを縦に並べる（横型はダッシュボード列の
  横幅が限られ、右の縦列にまとめた方が使い勝手が良いため）。条件・装備・怪我はいずれも
  ポートレイトカードに重ねず、通常のフロー内に配置する。
- **条件はラベルなしのアイコンボタン、装備・怪我はアイコン＋ラベルの横長ボタン**: 条件は
  タップで詳細の子ウィンドウを開く想定のアイコンのみのボタン（64×64u）で、複数同時に
  付き得るため 2 列のグリッドで折り返して並べる。装備・怪我は「装備」「怪我」という種別の
  固定ラベルをアイコンの右に表示する横長ボタンとし、列・セルの幅いっぱいに広げる
  （アイテム名は表示せず、タップ先の子ウィンドウで確認する想定。高さ 88u は
  アイコンボタンと同じ最小タップ領域）。
...
- **縦型**: 「キャラクターカード｜条件」「装備｜怪我」の 2×2 グリッド。1 列目にポートレイト
  カード、2 列目に条件ボタンの行を 1 行目の高さいっぱいに配置し、2 行目に装備ボタン・怪我
  ボタンをそれぞれ 1 個ずつ置く。
- **横型**: 左にポートレイトカードを置き、右の列（幅はキャラクター表示エリアの残り幅）に
  条件ボタンの行・装備ボタン・怪我ボタンを上から順に縦に並べる。
...
│   │   ├── ポートレイトカード
│   │   └── 右の縦列: 条件ボタンの行 → 装備ボタン → 怪我ボタン（アイコン＋ラベルの横長ボタン）
│   └── ステータスエリア（バー×6）
├── オプションバー（アイコンボタン×4）
