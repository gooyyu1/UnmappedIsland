...
中央に百分率）・補足の 1 行・「探索する」「閉じる」ボタン（高さ 88u）を置きます。探索はゲームの
主操作なので、バーの横幅はフィールドエリアと同程度（ウィンドウ幅＝フィールドエリア幅。ただし縦型では
画面の左右に余白が残るよう画面幅の 92% を上限とする）を確保します。
...
枠は常に 4 個表示し、見つかっていない分は破線の空枠のままにします。カードはレーンと同じ形のまま、
枠の幅（150u 上限）に合わせて縮めて描きます。**4 件を超えた分は横スクロール**で送ります。枠から
はみ出したカードはレーンと違って背景板では隠せないため、マスクで切り抜きます。

##### 探索にかかる実時間

「探索する」を押すと、そのアクションの `duration` が進めるゲーム内時間の分だけ**実時間をかけて**
進みます（**ゲーム内 15 分＝現実 0.5 秒**。探索は 30〜45 分なので 1〜1.5 秒）。
...
割合は一定の速さでは進めず、**tick 境界（15 分ごと）で一拍置きます**。1 tick ぶんの実時間（0.5 秒）の
うち、前半 0.25 秒で次の目盛りまで進み、後半 0.25 秒は止まります。ゲーム内の変化が起きるのは tick
境界だけなので、滑らかに増やすより実際の進み方に合います（探索の 45 分なら 3 目盛り）。

**状況エリアの時計も同じ刻みで動きます。** 塗りが目盛りへ届いた瞬間に、その時刻へ 15 分ぶん飛びます
（00:00 → 00:15 → 00:30 → 00:45）。塗りと時計が食い違って見えないよう、どちらも同じ規則から
導いています。
