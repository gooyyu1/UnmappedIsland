...
- **キャラクター表示エリアは2×2グリッド**: 「キャラクターカード｜条件」「装備｜怪我」の
  2×2グリッドで統一し、縦型・横型で同じ構造を使う。条件・装備・怪我はいずれもポートレイト
  カードに重ねず、通常のフロー内に配置する。
- **条件・装備・怪我はラベルなしのアイコンボタン**: 名前や状態のテキストは表示せず、アイコンのみの
  ボタンとする（タップで詳細の子ウィンドウを開く想定）。装備・怪我は 88×88u（アイコンボタンと
  同じ最小タップ領域）、条件は複数同時に付き得るため一回り小さい 64×64u とし、2 列のグリッドで
  折り返して並べる。
...
| アイコンボタン・装備/怪我ボタン | 88 × 88（最小タップ領域） |
| 条件ボタン | 64 × 64 |
...
「キャラクターカード｜条件」「装備｜怪我」の 2×2 グリッド（縦型・横型共通の構造）です。
1 列目にポートレイトカード（210×322u）、2 列目に条件ボタンの行を、1 行目の高さいっぱいに
配置する。2 行目には装備ボタン・怪我ボタンをそれぞれ 1 個ずつ置く。条件・装備・怪我は
いずれもラベルなしのアイコンボタンで、ポートレイトカードに重ねず通常のフロー内に配置する
（ボタンの仕様は設計原則を参照）。
...
広げる。横型はダッシュボード列の横幅をフィールドエリアに極力回すため、天候チップは同居させず
日時のみを表示する（高さは内容に合わせて自動）。縦型ではフィールドエリアの直上、横型では
...
2×2 グリッドの内容量で決まる高さ（約 422u）、残りをステータスエリアが吸収します。
`.dashboard-column` は `min-width: 0` を指定し、キャラクター表示エリアなどの内容量で
横幅が突っ張らないようにしている点に注意（`flex: 1` な列が中身の最小幅で止まってしまう
既知の挙動の回避）。画面が 16:9 より横長の端末では、余剰の幅をダッシュボード列が吸収します。
