...
| レーン間・外周マージン | 6 |
| 状況エリア高 | 176 |
| 日時フリップカード | 日数の桁（3 桁）64×88、時刻の桁 44×60 |
| オプションバー高 | 96 |
| ポートレイトカード | 縦型 170×261、横型 260×399 |
| 装備・怪我カード | 146 × 224 |
| アイコンボタン | 88 × 88 |
...
ポートレイトカード、その上に重ねる条件チップ列、装備カードと怪我カード（各 146×224u）を
表示します。ポートレイトカードは縦型 170×261u・横型 260×399u で、縦型の方が小さいのは
縦型ではオプションバーが画面上部を占有し、キャラクターエリアに使える高さが横型より
少ないため（キャラクターと天候のカードはフィールドカードより小さくてよい）。
...
[横型] ではオプションバーがダッシュボード列の最下部（キャラクターエリアの下）に移動する。
```
