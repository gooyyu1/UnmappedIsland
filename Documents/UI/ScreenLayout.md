...
- **隙間へ落とした**: 隣のレーンへ移動します。ハンドレーンは枠の位置が固定なので、落とした隙間へ
  既存の枠を押し出して入れます（まず右方向、無理なら左方向。[SlotSystem.md](../Engine/SlotSystem.md) 3 節）。
  空いている枠へ落とした場合は、他の枠を動かさずにそこへ入ります。

カード同士の実際の隙間は 12u と狭く、移したいだけなのに `combinations` と判定されやすいため、
**カードの左右 1/8 も隙間側**として扱います。同じレーンの隙間へ落とす並べ替えは扱いません。
