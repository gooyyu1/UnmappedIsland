...
namespace UnmappedIsland.Diagnostics
{
    ...
    public class ClimateStatsReportTests
    {
        ...
        private string BuildReport()
        {
            ...
            sb.AppendLine();
            sb.AppendLine($"`Tests/Diagnostics/ClimateStatsReportTests.cs` によるシミュレーション実測値のスナップショット");
            sb.AppendLine($"（シード数 {SeedCount}、各 {SimDays} 日）。`core.yaml` を変更したら以下で再生成する。");
            sb.AppendLine();
            sb.AppendLine("```");
            ...
            sb.AppendLine("## 計測方法");
            sb.AppendLine();
            sb.AppendLine("- 序盤/中盤/終盤 = 各季節インスタンスの実持続期間の3等分区間。");
            sb.AppendLine("- 天気ごとの発生時間 = 期間内の合計時間。発生しなかった期間も0時間の標本として計上（nは全天気共通）。");
            sb.AppendLine("- 連続降雨/未降雨時間 = 同じ状態が連続した1回ごとの長さ。開始tickの区間に割り当て、季節境界で打ち切り。");
            sb.AppendLine("- 標準偏差は標本標準偏差（n-1）、5%ile/95%ileは最近隣法（nearest-rank）。");
            sb.AppendLine();
            ...
            sb.AppendLine();
            sb.AppendLine("`core.yaml`の設定値の実測値（範囲端0/10,000に達したtickは除外）。");
            sb.AppendLine();
            sb.AppendLine("### 季節ごとの大気水分量レート（1tickあたり、非雨天時）");
            sb.AppendLine();
            AppendStatTable("季節", "", seasonKinds.Select(s => (SeasonName(s), seasonMoistureRate[s])));
            sb.AppendLine("（`dry`の標準偏差が0でないのは、最初の`dry`季節に難易度の初期補正=`ClimateSystem.md` 5.2節が重なるため。）");
            sb.AppendLine();
            sb.AppendLine("### 天気ごとの自己減算（1tickあたり、降雨中のみ）");
            sb.AppendLine();
            sb.AppendLine("推定自己減算 = その天気の間の正味変化量 − その季節のレート。");
            sb.AppendLine();
            foreach (int w in new[] { lightRainId, heavyRainId, stormId })
            {
