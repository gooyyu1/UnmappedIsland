...
├── Loader/                    # YAML ローダー自体の単体テスト（本ドキュメント「Loader テスト」節を参照）
├── StreamingAssets/           # Assets/StreamingAssets/WorldCodex 実ファイル検証（本ドキュメント「StreamingAssets テスト」節を参照）
├── Domain/                    # 読み込み後の振る舞いテスト（本ドキュメント「Domain テスト」節を参照）
└── Generation/                # 地形生成パイプラインのテスト（本ドキュメント「Generation テスト」節を参照）
```

> **フォルダ名と名前空間の対応ルール**
>
> フォルダ名と名前空間は必ず一致させること。
> `Tests/<FolderPath>/` に置かれたファイルの名前空間は `UnmappedIsland.<FolderPath>` とする。
> （例: `Tests/Domain/` → `UnmappedIsland.Domain`、`Tests/Loader/` → `UnmappedIsland.Loader`）
> このルールは `Assets/Scripts/` にも同様に適用される。
...
| ファイル | 検証対象 |
|---|---|
| `YamlLoaderTests.cs` | ローダーの基本動作（インライン YAML / 一時ディレクトリを使った `WorldCodex` 構築） |
| `GenerationYamlLoaderTests.cs` | 地形生成定義（`axes`/`location_types`/`generation_scopes`）のパースと参照検証 |

**新しい Loader テストを追加するとき**
- `WorldCodexYamlLoader` または `WorldCodexYamlLoaderBuilder` の動作を検証するテストをここに置く。
- `Assets/StreamingAssets/WorldCodex` の実ファイルを直接読むテストはここに置かない（後述の `Tests/StreamingAssets/WorldCodex/` に置く）。
- 「このキーワードがあれば ObjectDef にこう反映される」という形式の検証が典型例。
- `WorldObject` を Tick させたりプロパティを動的に変化させたりする操作はここに含めない。

---

### StreamingAssets テスト（`Tests/StreamingAssets/`）

**namespace: `UnmappedIsland.StreamingAssets`**

`Assets/StreamingAssets/WorldCodex` 配下の YAML 実ファイルが、ゲームとして期待する定義を実現しているかを検証します。
...
| `LocationsYamlTests.cs` | `locations.yaml`（土地・道の定義。探索→道の発見→移動の E2E 含む） |
| `TerrainGenerationYamlTests.cs` | `terrain_generation.yaml`（地形生成定義と locations.yaml の対応） |

---

### Domain テスト（`Tests/Domain/`）
...
| `ActionDurationTests.cs` | アクションの `duration`（実行によるゲーム内時間の消費） |
| `MoveEffectTests.cs` | `move` 効果動詞（インスタンスID参照によるactorの移動） |

**新しい Domain テストを追加するとき**
...
- YAML フィクスチャをインラインで定義し `WorldCodexYamlLoader` でパースする方針を維持する
  （外部ファイルへの依存を最小化するため）。
- YAML の書き方自体（ローダーの解釈ルール）を確認したい場合は `Tests/Loader/` に追加する。
- `Assets/StreamingAssets/WorldCodex` の実ファイル内容を確認したい場合は `Tests/StreamingAssets/` に追加する。

---

### Generation テスト（`Tests/Generation/`）

**namespace: `UnmappedIsland.Generation`**

地形生成パイプライン（`Assets/Scripts/Domain/Generation/`）を検証します。
実際の `terrain_generation.yaml` + `locations.yaml` を使い、複数シードに対する生成の不変条件と、
生成結果の世界への実体化を確認します。

| ファイル | 検証対象 |
|---|---|
| `TerrainGeneratorTests.cs` | 生成の不変条件（決定性・土地数10〜20・山1つ以上・海岸リング・全土地連結・辺の交差なし） |
| `IslandSpawnerTests.cs` | `IslandSpawner`/`NewGame`（土地・道の実体化、required_progressの割当、開始→探索→移動のE2E） |
