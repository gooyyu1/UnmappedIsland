            ...
            run(
                ...
                [
                    ...
                    *(["--tolerance", str(card["tolerance"])] if "tolerance" in card else []),
                    # mode=flood の指定（card_art.py の flood_core 参照）。
                    *(["--fg", str(card["fg"])] if "fg" in card else []),
                    *(["--bg", str(card["bg"])] if "bg" in card else []),
                    *(["--edge-sigma", str(card["edgeSigma"])] if "edgeSigma" in card else []),
                    *(["--slope-ref", str(card["slopeRef"])] if "slopeRef" in card else []),
                    *(["--edge-weight", str(card["edgeWeight"])] if "edgeWeight" in card else []),
                    *(["--power", str(card["power"])] if "power" in card else []),
                    *(["--gray-weight", str(card["grayWeight"])] if "grayWeight" in card else []),
                    *(["--chroma-weight", str(card["chromaWeight"])] if "chromaWeight" in card else []),
                    *(["--chroma-ref", str(card["chromaRef"])] if "chromaRef" in card else []),
                    *(["--unshade"] if card.get("unshade") else []),
                    *(["--shade-margin", str(card["shadeMargin"])] if "shadeMargin" in card else []),
