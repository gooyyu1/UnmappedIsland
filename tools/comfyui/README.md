...
[docs/ui/ScreenLayout.md](../../docs/ui/ScreenLayout.md) ボタンの絵 節）。ボタンの全面を覆うので、
ボタンと同じおよそ 2:1（生成 1024×512・保持 512×256）で作ります。

```bash
python build.py recipes/icon_map.json
```

**この 3 枚だけ LoRA を当てません**（レシピの `loraStrength` が 0）。輪郭線のはっきりした平塗りに
するためで、他の絵の水彩とは作風を分けています。小さいボタンでは、にじんだ絵より線画の方が何の
ボタンか読み取れます。同じ理由で `oilifyRadius` も 0 です（掛けると線が潰れます）。
