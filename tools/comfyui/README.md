...
仕上がりは 2048×640 ですが、**2560×640 で生成**します。

**横の余り（512px＝`blend`）は捨てずに、継ぎ目の材料として使い切ります。** 出力の x 列と入力の
x+2048 列は本来同じ絵になるべき位置なので、その 2 つをクロスフェードすると、右端の次に左端が
自然に続きます。

縦は切りません。ゲーム側がレーンの高さへ合わせて拡大縮小するため（`laneArt.ts` 参照）、仕上がりの
高さは決め打ちしなくてよく、絵を欠けさせない方が得だからです。高さ 640 は生成時の縦横比を決める値で、
仕上がりのタイル（2048×640 ＝ 3.2:1）が画面上のレーンの縦横比に近くなるように選んでいます。
これより平たくすると、SDXL が横長すぎる画面に同じモチーフを繰り返しがちになります。
