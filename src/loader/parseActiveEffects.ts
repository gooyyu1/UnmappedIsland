...
/**
 * passivesの中の transfer（8.4節）。文法はactiveのものと同一で、違うのは置かれた場所が宣言元しか
 * 用意しないこと（ReferenceScope.declaration）——持続する宣言は操作の関係に紐づかないので、その役を
 * 指す形は解決先を持たない（modify/addのpassiveと同じ理由）。
 */
