...
function tryParseInt32(raw: string): number | undefined {
  if (!INT_PATTERN.test(raw)) return undefined;
  const value = Number(raw);
  if (value < INT32_MIN || value > INT32_MAX) return undefined;
  return value;
}

/** setやmodify/accumulateの量など、mapのキー経由ではなく値ノードから直接取り出す整数リテラル。
 * yamlMapping.tsのrequireInt等はmap+key単位でしか扱えないため、値ノードを直接受け取るここだけの薄いラッパー。 */
export function parseIntLiteral(context: string, raw: string): number {
  const value = tryParseInt32(raw);
  if (value === undefined) throw new YamlLoadError(`${context}: 整数である必要があります（値: '${raw}'）。`);
  return value;
}
 ...
 * 整数→真偽値→シンボルで固定する。2番目の戻り値は、rawがシンボル名として登録された場合にtrueになる
 * （stagesの解釈分岐、6.4節）。
 */
export function parseScalarNumber(
  ...
  const intValue = tryParseInt32(raw);
  if (intValue !== undefined) return [intValue, false];
  ...
  throw new YamlLoadError(
    `${context}: 値 '${raw}' は整数・真偽値・シンボル名(識別子)のいずれかである必要があります。`,
  );
}
