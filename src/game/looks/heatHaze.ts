 ...
 *
 * 受け取るのは**居る土地の気温**（その場所の`ambient_temperature`）。空の気温——基準20に日射
 * （暗い-3／明るい+3）と季節（涼しい-5／暑い+8）が重なった実効値（core.yaml）——を土台に、海抜ぶんの
 * 差（ClimateSystem.md 1.1節）と据えた炉の暖（FireSystem.md 9.2節）が積まれるので、空だけで27へ届くのは
 * **暑い季節の日中**だけ。天気ではなく気温で決めるのは、陽炎が立つのは地面が焼けているからで、
 * 灼熱という天気の名前そのものではないため。
 */
