...
export const SIZE = {
   ...
   */
  iconButtonArt: 80,
  /** 状況アイコン。周りの事情を示すだけで押せない（ScreenLayout.md 4.1.1節）ので、桟の中で最も小さい。 */
  conditionButton: 48,
  /**
  ...
  /**
   * ボタンの列が、上下に空ける余白。**間隔はこの余白の残りから決まる**（PlayScene.addSlotButtonColumn）。
   *
   * 隣のポートレイトは上端から10.5u（PAPER_INSET + FRAME_SIDE_WIDTH）を枠に使うので、**人はカードの
   ...
   *
   * **どれも同じ大きさで敷く。** 物の大小——開いた地図 > 開いた本 > Tシャツ > 巻いた包帯——は絵の側が持ち、
   * UIはそれを測らない（測って揃えると差が消える）。**正方形にしないのも同じ理由**で、正方形だと
   * 平たい物ほど小さくしか描けない（地図が高さの半分しか使えなかった）。
   *
   * **大きさの上限は一番背の高い開いた地図（104×58u）が決める。** 共通の倍率なので、地図が
   * ボタンからはみ出す手前で頭打ちになる。
   */
  slotButtonIcon: { width: 145, height: 64 },
  /**
   * 文字のボタン（`addTextButton`）のラベルの大きさ。**ボタンの箱の幅が名前の長さの予算を決める**
   * ので、意匠が持つ——ラベルは縮まず折り返さないため、箱より長い名前はそのまま外へ出る
   * （[`Windows.md`](../../../docs/ui/Windows.md) 4節）。
   */
  textButtonLabel: 26,
  radius: 12,
  /**
...
export const COLOR = {
  ...
  /**
   * ドラッグ中に、**理由を言うためだけの落とし先**を示す枠（CardInteraction.md 2.1節）。異常を言う赤
   * （cardOverlayText）ではなく灰色にするのは、壊れたのではなく今は条件が揃っていないだけだから。
   *
   * **無彩色にする**（彩度0）。ドラッグ中は落とし先の枠・受け入れる札のふち・材料の枠の縁が同時に出て、
   * どれも色相で意味を分けている（下のcellCurrentStep）。**色を持たないものはその取り合いに加わらない**
   * ので、意味を1つ足しても既にある色相を1つも狭めない。僅かでも色を差すと、その色相が既にある意味と
   * ぶつかりうる。
   */
  cardDropRefuse: 0x757575,
   ...
   *
   * **ドラッグ中に出る枠（上の`cardDrop`で始まる色）のうち、色相を持つものとは色相を分ける。**
   * ドラッグ中は両方が同時に出るので、同じ色相だと「今どこへ落ちるか」と「何の枠か」が混ざる。
   * 無彩色のもの（cardDropRefuse）は色相を1つも使わないので、ここでは相手にならない。
   * **数え上げずに一覧で指す**——枠の種類が増えたときに、ここが古い数のまま残らないように。
   */
