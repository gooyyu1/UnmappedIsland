...
export function applyScenario(game: NewGameSession, scenario: Scenario, codex: WorldCodex): void {
  ...
  for (const [name, raw] of scenario.props) {
    game.player.instance.getProperty(propertyIdOf(codex, name)).overwrite(resolveValue(codex, name, raw));
  }
  for (const [name, raw] of scenario.worldProps) {
    game.world.instance.getProperty(propertyIdOf(codex, name)).overwrite(resolveValue(codex, name, raw));
  }
}
