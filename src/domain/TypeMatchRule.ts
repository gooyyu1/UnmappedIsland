...
export class TypeMatchRule {
  ...
  /**
   * この指定に当てはまる型を全部挙げる。**当てはまるものを数え上げるだけ**で、それが1つに定まるかも、
   * 絵にできるかも見ない。
   *
   * 1つに定まらない指定（タグ）を絵で見せるのに使うなら、先にcanBePicturedで見分ける。
   */
  ...
  /**
   * この指定を、当てはまる型のカードで見せられるか。**否定形（`not`）では見せられない**
   * ——「そのタグを持たない型すべて」を順に送っても、何を持ってくればよいかを言わない。
   *
   * **持つのは規則の側。** 否定を書ける場所（枠の`accept`、7.2節）から絵を出す側が、毎回
   * 読み上げ（reading）の形を自分で見分けることになるのを避ける。
   */
  get canBePictured(): boolean {
    return this.reading.kind !== 'not';
  }
