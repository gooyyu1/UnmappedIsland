...
export class PropertyDef {
   ...
   *
   * 完全一致で決まる段（シンボル型、6.6節）は、値の並びの上に位置を持たないので偽。**持ち主の型では
   * なく段の形で見る**ので、位置を持たない段が他の型に増えてもここは変わらない。名指した段をこの型が
   * 持たないときも偽——**綴り違いはここまで来ない**（どの型も宣言していない段名はロードで落ちる。
   * WorldCodexの名指し検査）が、同じプロパティでも段は型ごとなので、他の型の段を名指すことは在りうる。
   */
