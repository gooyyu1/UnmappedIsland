...
export function populate(session: WorldSession, map: IslandMap): void {
  ...
  for (const site of map.sites) {
    ...
    if (site.variant !== undefined)
      for (const [propertyGlobalId, value] of site.variant.props)
        location.getProperty(propertyGlobalId).overwrite(value);
  ...
  for (const site of map.sites) {
    ...
    for (let i = 0; i < touching.length; i++) {
      ...
      path.getProperty(travelMinutesId).overwrite(edge.travelMinutes);
      path.getProperty(requiredProgressId).overwrite(requiredProgress);
      path.getProperty(destinationIdId).overwrite(locations[other].instanceId);
  ...
  for (const edge of map.edges) {
    ...
    forward.getProperty(returnPathIdId).overwrite(backward.instanceId);
    backward.getProperty(returnPathIdId).overwrite(forward.instanceId);
  }
}
