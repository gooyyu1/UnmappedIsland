...
| 中身のバー | 液体容器の `content` スロットの `capacity` に対する中身の量 | 絵の下、下端から 72u 浮かせる | 15u |
...
- **どちらのバーも、減った分は赤い帯として残して遅れて縮めます**（ステータスエリアの「減った分の
  赤い帯」節と同じ動き・同じ時間で、1 回の行動の途中では縮めないところまで同じ）。バーは作り直さず
  値だけを差し替えるので、飲んだ量・使った量がそのまま帯として読めます。
...
**中身の色は液体ごとに決めます**（水は青、茶は茶緑、油は黄色）。何色に見えるかはその液体自身の
性質なので、容器側にも表示側の都合にも持たせず、液体の object_def の識別子で引く 1 か所
（`src/game/ui/liquidColor.ts`）に集めます。液体を 1 種類足したらここへ 1 行足す形で、足し忘れは
自動テスト（`tests/game/liquidColor.test.ts`）が捕まえます。
...
ダッシュボード列からサイドバーへ移した分で、フィールドエリアにカード 6 枚分の横幅
（1322u）を確保できます。サイドバー幅はバー自体の厚み（アイコンボタン 88 + 左右
パディング 16×2）と同じ 120u にし、ボタン左右の余白が広くなりすぎないようにしています。

| エリア | サイズ (u) |
|--------|-----------|
| ダッシュボード列 | 478 × 1080（上から状況エリア・天候の帯・キャラクター表示・ステータス） |
