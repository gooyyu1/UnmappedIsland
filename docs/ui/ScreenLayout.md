...
- **キャラクター表示エリアは上段／下段が独立したレイアウト（縦型）、カード＋縦列（横型）**:
  縦型は上段（キャラクターカード｜条件）と下段（装備｜怪我｜地図）を独立したレイアウトとして扱う。
  上段の列幅（ポートレイト幅）に下段が引きずられないよう、下段はグリッドの両列にまたがる
  独立の行にし、装備・怪我・地図はその行の中で幅を均等に分け合う（同じ大きさになる）。横型は
  左にポートレイトカードを置き、右の列に条件ボタンの行と装備・怪我・地図の行を縦に並べる
  （横型はダッシュボード列の横幅が限られ、右の縦列にまとめた方が使い勝手が良いため）。
  条件・装備・怪我・地図はいずれもポートレイトカードに重ねず、通常のフロー内に配置する。
- **条件はラベルなしのアイコンボタン、装備・怪我・地図はアイコン＋ラベルのボタン**: 条件は
  タップで詳細の子ウィンドウを開く想定のアイコンのみのボタン（64×64u）で、複数同時に
  付き得るため 2 列で折り返して並べる（縦型・横型共通。ダッシュボード列の幅では、横型でも
  ポートレイトの右へ横一列に並べる幅が無い）。装備・怪我・地図は「装備」「怪我」「地図」という
  種別の固定ラベルを持つボタンを 1 行に 3 つ並べ、幅は同じ大きさで行・列いっぱいに
  広げる（アイテム名は表示せず、タップ先の子ウィンドウで確認する想定。高さ 88 u は
  アイコンボタンと同じ最小タップ領域）。ラベルは収まる幅ならアイコンの右、狭い幅
  （横型の右の縦列）ならアイコンの下に置く。
...
| ダッシュボード列幅（横型） | 534（日時のフリップカード 462 + 左右パディング 20×2 + 紙の余白 32） |
| 日時フリップカード | 日数の桁（3 桁）64×88、時刻の桁 44×60、「:」の確保幅 10、桁間 8、日数と時刻の間 28（縦型・横型共通） |
...
| 装備・怪我・地図ボタン | 高さ 88（最小タップ領域）、幅は行を 3 等分 |
| 条件ボタン | 64 × 64 |
...
- **横型のダッシュボード列幅 534u**: 日時のフリップカード（462u）＋左右パディング 20u×2 ＋紙の余白 32u。
...
ポートレイトカード（205×320u）と、条件ボタンの行・装備／怪我／地図のボタンの行で構成します。
いずれもポートレイトカードに重ねず、通常のフロー内に配置する（ボタンの仕様は設計原則を参照）。

- **縦型**: 上段「キャラクターカード｜条件」・下段「装備｜怪我｜地図」の 2 段構成だが、
  上段と下段は独立したレイアウト。1 列目にポートレイトカード、2 列目に条件ボタンの行を
  1 行目の高さいっぱいに配置する（ここまでがグリッドの上段）。下段（装備・怪我・地図のボタンの行）
  はグリッドの両列にまたがる独立の行とし、ポートレイト幅に関係なく 3 個で幅を均等に分け合う
  （同じ大きさになる）。
- **横型**: 左にポートレイトカードを置き、右の列（幅はキャラクター表示エリアの残り幅）に
  条件ボタン（2 列で折り返し）と装備・怪我・地図の行を上から順に縦に並べる。
...
`stages` の `alert` と同じく**プロパティ側が宣言**し（UI は種別を知りません）、UI は次の3点を切り替えます。

| | 減ると悪い（既定） | 増えると悪い |
|---|---|---|
| 遅れて出る帯 | 減った分を、元の位置に残してから縮める | 増えた分を先に帯として出し、塗りが後から追いつく |
| 増減の記号 | ▲ 緑・▼ 赤 | ▲ 赤・▼ 緑 |
| 塗りの色 | （既定） | 満タンを良いと読ませない別の色 |

帯の規約を「**悪化した分を遅れて見せる**」に一般化したもので、溜めと縮みの時間（0.25 秒後から 0.7 秒）も、
1 回の行動の途中では動かさないことも、向きによらず同じです。並び順・明滅・固定表示も既存のままです。

`load` を宣言したキャラクタがまだ無いため未実装で、宣言のキー自体も
[GameElementDefinition.md](../engine/GameElementDefinition.md) 6 節へ足していません。
...
各桁は白い紙の画像（`flip_digit.png`）に黒い数字を載せたもの。画像はカードの枠と同じ
生成テクスチャから組み立て、留具の穴・リング・落ち影まで描き込んである
（`tools/comfyui/recipes/flip_digit.json`）。
...
- 絵は帯の高さへ合わせて縦横同率で拡大縮小し、横方向は繰り返して並べます（左右の端が繋がるように
  描きます）。
- 原寸は **192×24px** です。高さ 24px は短辺 2160px の端末での 12u に当たるので、これより大きく
  描いても縮小されるだけです。
...
時間が経過し切ってから——道具は使い終わってから手元へ戻る、という見え方になります。同じレーンの中で
並べ替えたときも同じで、指を離した場所から新しい位置へ入ります。**詰め直しとして滑るのは、掴んで
いなかったカードだけ**です。
...
（1266u）を確保できます。サイドバー幅はバー自体の厚み（アイコンボタン 88 + 左右
パディング 16×2）と同じ 120u にし、ボタン左右の余白が広くなりすぎないようにしています。

| エリア | サイズ (u) |
|--------|-----------|
| ダッシュボード列 | 534 × 1080（上から状況エリア・天候の帯・キャラクター表示・ステータス） |
| ダッシュボード列の中身 | 502 × 1032（背景のページの縁を避けた範囲） |
| フィールドエリア | 1266 × 1080（カード 5 枚分強の横幅） |
| 右サイドバー | 120 × 1080（上段: オプションバー、下段: フィルターバー） |

ダッシュボード列内の状況エリア・天候の帯は内容量に応じた高さ、キャラクター表示エリアは
右の縦列の内容量で決まる高さ（条件ボタンの行＋装備・怪我・地図の行）、残りを
...
│   │   ├── 上段: ポートレイトカード｜条件ボタン（2列で折り返し）
│   │   └── 下段: 装備ボタン｜怪我ボタン｜地図ボタン（アイコン＋ラベル、幅は均等）
│   └── ステータスエリア（バー×6）
