...
**バーの値は、アクション・combination・探索のたびに書き換えます。** 位置は引き直しません（並ぶ項目は
キャラクターのプロパティで決まり、行動では増減しないため）。中身だけを差し替えるのは、バーを作り直すと
画面の表示順が変わり、後から置かれた子ウィンドウの覆いより手前へ出てしまうためです。
...
するので、移動のように画面を作り直す行動でも、作り直した後のバーに同じ記号が出ます。
...
カードもレーンの背景の絵も入れ替わるため、カードの差し替え（`CardMotion`）ではなく画面ごと作り直します。
...
- 1 行は「名前＋バー」で、ステータスエリアと同じ部品（`StatusBar`）です。`range` を持たず割合を
  ...
  ——開いている間はワールドが変わらないためで、右の欄は空いたままになります。
