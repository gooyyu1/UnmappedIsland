...
| 地図・装備・怪我ボタンのアイコン | 80 × 80（絵でも絵文字でもこの大きさ。ボタンの中央） |
| 条件ボタン | 48 × 48 |
...
- **絵は物 1 つを中央に置いたアイコンで、背景は透過です。** ボタンの地の色はボタン自身が持ちます。
  ボタンの中央へ **80×80u** で置き、上下に余白を残します。全面を覆う絵にすると、絵がボタンの
  大きさに対して大きすぎ、何のボタンかより絵の方が目に入ります。
- **ボタンの地は、カードの紙と同じくすんだ白に色相だけを載せた色です**（`COLOR.mapButton` など）。
  3 つを見分けるのに要るのは色相の差だけで、彩度を上げると、載せるアイコンより地の方が目立ちます。
- 絵がまだ無いものは**絵文字で代用**し、同じ場所へ同じ大きさで置きます。絵は少しずつ増える前提
...
- 一覧はビルド時に作ります（`import.meta.glob`）。ファイル名がコードの引く識別子と食い違うと絵は
  ...
  [カードの絵](#カードの絵)と同じです。
- 絵は `tools/comfyui/recipes/icon_<識別子>.json` から作り直せます。切り出しのキャンバスは 256×256px
  で、ボタン上の大きさとは関係ありません（ボタン側が 80u へ縮めます）。
