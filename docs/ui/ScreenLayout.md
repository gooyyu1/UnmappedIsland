...
- **ボタンの地は、カードの紙を敷いたうえにボタンの色を乗算で載せます**（染めた紙）。平らな塗りに
  淡い色を置くとパステルに見え、汚れと滲みのある他の絵から浮きます。色は染みが見える程度に
  くすませます（`COLOR.mapButton` など）。紙が読めなければ、この色の平らな塗りが地になります。
- **敷くのはカードの枠の絵の内側の紙だけで、ボタンごとに別の場所を取ります。** 枠の絵は角丸と
  落ち影が焼き込んであるので、そのまま横長へ引き伸ばすと角が楕円に潰れます。角丸はボタン側で
  切り抜き直し、枠線もその上へ引き直します。取る場所を分けるのは、同じ場所だと 3 つに同じ染みが
  並んで模様として目に付くためです。
