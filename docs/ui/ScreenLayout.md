...
暗転は移動にかかる時間の経過（時計とドーナツグラフ）と**並行して**進み、時間が経ち切ると同時に暗く
なり切ります。そのため場面転換のぶん待たされることはありません。ワールド自体は押した瞬間に変わって
いる（[探索にかかる実時間](#探索にかかる実時間)と同じ）ので、移動を伴う行動かどうかはその時点で
分かり、時間が経ち切るのを待たずに暗転を始められます。
