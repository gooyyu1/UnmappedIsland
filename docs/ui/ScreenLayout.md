...
| 状況エリア高 | 縦型 172、横型 224（横型は幅が日時 1 つ分しかなく、天候名を別の段へ分けるため高い） |
| 空の窓 | 状況エリアから余白（縦型 16×8、横型 8×8）を引いた残り。内側の余白は縦型 24×20・横型 20×16 |
...
| キャラクター表示エリア高 | 428（余白 16 + ポートレイト 320 + ギャップ 12 + 条件の行 48 + 紙の余白 32。縦型・横型共通） |
| キャラクター表示エリアの内側余白 | 16（下だけ 32。この辺がフィールドエリア側の縁に当たるため） |
...
- **キャラクター表示エリアの下パディング 32u**: このエリアの下端はフィールドエリアに接しているので、
  紙の余白をここが受け持ちます。状況エリア（172u）との和が 600u ちょうどで、これ以上広げると
  9:16 の端末でフィールドエリアが 1080u を割ってしまいます（＝3 レーンが収まらなくなる）。

#### 縦長の画面の余り

**キャラクターエリアは内容量ぶん（428u）だけ確保し、引き伸ばしません。** 9:16 より縦長の端末で
...
（`tools/comfyui/recipes/flip_digit.json`）。**窓の下端に付ける**ので、桁の紙は絵の下側
（地平線・海面）に重なります。明るい空でも白い紙が沈みません。
...
- 一覧はビルド時に作ります（`import.meta.glob`）。理由は[カードの絵](#カードの絵)と同じです。
...
ダッシュボード列内の状況エリア（224u）とキャラクター表示エリア（428u）は固定の高さで、残りを
...
言語ごとの対応表（[Localization.md](../engine/Localization.md)）から引きます。条件・装備・怪我の
アイコンはドメイン側に表示できる形がまだ無いため、モックと同じ固定値を返すプレースホルダーの
ままです。天候の名前も、Localization がプロパティの値を引ける形をまだ持たないため固定値です
（空の絵のほうは実際の天気で切り替わります）。
