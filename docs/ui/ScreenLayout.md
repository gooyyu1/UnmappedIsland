...
- **カード基準レイアウト**: 画面を等分して領域を作るのではなく、カード寸法を基準に各エリアの
  ...
  （= 41:64、比率 0.6406）とする。枠の画像（`src/assets/card_frame.png`、820×1280）も同じ比率で、
...
- **世界の状況を縦に集約**: 状況エリア（日時・天候）を設置物レーンの上に隣接させ、
...
| 情報エリアの余白 | フィールドエリア側 25、それ以外の辺 24（背景のページの縁を避ける） |
| オプション・フィルターボタンの間隔 | 20（カード間ギャップより広めに確保し、隣接ボタンの誤タップを防ぐ） |
| 状況エリア高 | 横型 112（フリップカード 88 + 上下パディング 12×2）、縦型 148（上 20 + 下 40） |
| ダッシュボード列幅（横型） | 592（日時のフリップカード 523 + 左右パディング 20×2 + 紙の余白 25） |
...
フィールドエリアの**左（横型）・上（縦型）**にまとまっている、状況エリア・天候の帯・キャラクター
エリアの全体です。**オプションバーは含みません**（本の外側に置き、縦型ではページの上端に接します）。

**背景は 1 枚の本のページ**（`src/assets/information_background_landscape.png` /
`information_background_portrait.png`）で、内訳の各エリアは自前の背景色を持たず、このページの上に
直接載ります。

- 絵は**縦横同率で拡大縮小**し、フィールドエリアに接する辺を基準に置きます。横型は画面の高さに、
  縦型は画面の幅に合わせます。
- 反対側（横型の左・縦型の上）は**画面外へはみ出す前提**で、絵の側に十分な余白が取られています。
  極端な画面比で絵が届かない場合に備え、下地に紙の色（`COLOR.informationPaper`）の背景板を敷きます。
- 絵のフィールドエリア側 32px は**透過のグラデーション**で、ページがフィールドエリアへ落とす影です。
  この 32px 分だけフィールドエリアへ食い込ませて置きます。
...
| 辺 | 余白 | 決め方 |
|----|------|--------|
| フィールドエリア側（横型は右・縦型は下） | 25u | 表紙の縁の幅（56px）から食い込ませる幅（32px）を引いた残り |
| それ以外（横型は上下・縦型は左右） | 24u | 縁が角でカーブしていて幅が一定でないため、絵から導かず見た目の余白として置く |

いずれも絵が短辺いっぱいへ拡大縮小される（短辺は常に 1080u）ため、**画面の大きさによらず一定**です。

この制約が 2 つの寸法を決めています。

- **横型のダッシュボード列幅 592u**: 日時のフリップカード（523u）＋左右パディング 20u×2 ＋紙の余白 25u。
- **縦型の状況エリアの下パディング 40u**: エリアの下端はフィールドエリアに接しているので、高さを
  足すと中身が上へ寄ります。これ以上広げるとダッシュボード列の最小高が 600u を超え、9:16 の端末で
  フィールドエリアが 1080u を割ってしまう（＝3 レーンが収まらなくなる）ため、40u が上限です。

#### 縦長の画面の余り

**キャラクターエリアは内容量ぶん（452u）だけ確保し、引き伸ばしません。** 9:16 より縦長の端末で
...
| 向き | 位置 |
|------|------|
| 横型 | 日時の下（＝天候の帯の上）、ステータスエリアの上 |
| 縦型 | 日時・天候の上、ステータスエリアの左（キャラクター表示エリアとの境目） |
...
プロパティごとに `stages` が持ちます、`characters.yaml`）。開始直後はどのステータスも満タン＝安全域
...
**バーが減るときは、減る前の位置まで赤い帯を残し、少し遅れて（0.25 秒後から 0.7 秒かけて）縮めます。**
格闘ゲームの体力バーと同じで、塗りが一瞬で縮むと「どれだけ減ったか」が見えないためです。縮み切る前に
また減った場合は、帯の右端はそれまでで最も高かった位置に残ります。

**1 回の行動の途中では帯を縮めません。** 長い行動は tick ごとに値が減っていきますが（時間経過の再生、
「時間経過のドーナツグラフ」節）、そのつど縮め始めると 1 tick 分ずつの帯にしか見えません。**行動が
終わってから縮め始める**ことで、帯はその行動で減った合計を示します。

増えるときは帯を残しません（増えた分は塗りそのものが伸びて分かるため）。この動きはバーの部品
（`ProgressBar`）が持ちますが、探索率のバーは増えるだけなので赤い帯は出ません。

**出ていなかった行が現れるときは、その行動を始める前の値から見せ始めます。** 帯は「今この行動でどれだけ
減ったか」を見せるものなので、現れた行にもその行動での減少ぶんは帯で出ます。出ていない間（安全域だった間）
に減った分まで足すと、現れた瞬間に満タンから大きく減ったように見えてしまうため、そこは含めません。
...
表示する。縦型ではフィールドエリアの直上、横型ではダッシュボード列の最上段（設置物レーン
の左隣）に置きます。
...
**設置物のカードだけは、枠と絵の間にその土地の景色を敷きます**（`src/game/ui/cardBackgroundArt.ts`）。
持ち歩けるアイテムと違い、設置物はその土地から切り離せないもので、カード自体が土地の一部として
見えるべきだからです。道のカードもここに含みます。

置き場所は **`src/assets/card_backgrounds/<土地の object_def の識別子>.png`** で、ファイル名が
そのまま「どの土地の絵か」を表します。寸法・角の丸め・縁の透過はカードの絵と同じ規約に従います。

- カードの絵と違い、背景は**透過させません**。空や海が紙へ置き換わると景色にならないためです。
  代わりに縁だけを内側へ向けて薄くし、カードの枠へ馴染ませます。
- 絵がまだ無い土地は、**紙がそのまま地になります**。絵は少しずつ増える前提なので、絵のある土地と
  無い土地が混ざった状態を正常とします。

一覧の作り方と、ファイル名が実在の土地かどうかを自動テスト（`tests/game/cardBackgroundArt.test.ts`）で
検査する点はレーンの背景と同じです。

#### レーンの背景

各レーンには全面に絵を敷きます。置き場所は **`src/assets/lanes/`** 直下のみで、ファイル名が
そのまま「どの土地の・どのレーンの絵か」を表します（`src/game/ui/laneArt.ts`）。

| レーン | ファイル名 | 何を描くか |
|--------|-----------|-----------|
| 設置物 | `<土地の object_def の識別子>_fixture.png` | その土地の眺め |
| アイテム | `<土地の object_def の識別子>_item.png` | その土地の地面 |
| ハンド | `hand.png` | プレイヤーの手 |

上の 2 つは**現在地の土地ごとに差し替わります**。ハンドレーンはプレイヤー自身なので土地によりません。

- **土地をレーンより先に置きます。** ファイル名一覧で同じ土地の絵が隣り合い、土地を 1 つ足すときに
  何を描き足せばよいかが見て分かるためです。
- **ディレクトリでは分けません。** レーンごとのディレクトリに分けるとファイル名自体は同じ
  （`jungle.png` が 2 つ）になり、エディタのタブやログでどちらの絵か区別できなくなるためです。

- 絵は**レーン高（352u）へ合わせて縦横同率で拡大縮小**します。原寸は問いませんが、レーン高より
  低い絵は拡大されるので、`352u` が実ピクセルで最大いくつになるかを見て決めます（`u` = 画面短辺 ÷ 1080
  なので、短辺 2160px の端末で 704px）。
- 横方向は**繰り返して並べる**ので、左右の端が繋がるように描きます。
- 背景はカードと同じだけ横へ送ります。カードが地面の上に置かれているように見せるためです。
- 絵がまだ無い土地は、**レーンの背景色で塗ります**。絵は少しずつ増える前提なので、絵のある土地と
  無い土地が混ざった状態を正常とします。

一覧の作り方と、ファイル名が実在の土地かどうかを自動テスト（`tests/game/laneArt.test.ts`）で
検査する点はカードの絵と同じです。
...
**道のカードは、道そのものではなく行き先の土地の名前を出します。** 道は「どこへ繋がっているか」だけが
意味を持つためです。
...
減った分の赤い帯（ステータスエリア）も tick ごとに動きます。増減の三角は「行動開始からその時点まで」の
...
**演出中は子ウィンドウを開けません。** カードのタップ・現在地カード・ポートレイト・装備／怪我の
...
掴んでいる間は、カードの分身がポインタに追従します。分身は**不透明**です（半透明にするとカードらしさが
損なわれるため）。掴まれて場所が空いた元のカードだけを薄くします——動くのは 1 枚なので、スタックが
残っている場合は薄くしません。分身はレーンの外へも出るため、レーンの中ではなく画面の最前面に描きます。
...
差し替えの時点で指を離した場所から元の枠へ戻ります。時間のかかる `combinations` では、これは
時間が経過し切ってから——道具は使い終わってから手元へ戻る、という見え方になります。
...
ダッシュボード列からサイドバーへ移した分で、フィールドエリアにカード 5 枚分強の横幅
（1208u）を確保できます。サイドバー幅はバー自体の厚み（アイコンボタン 88 + 左右
パディング 16×2）と同じ 120u にし、ボタン左右の余白が広くなりすぎないようにしています。

| エリア | サイズ (u) |
|--------|-----------|
| ダッシュボード列 | 592 × 1080（上から状況エリア・天候の帯・キャラクター表示・ステータス） |
| ダッシュボード列の中身 | 567 × 1032（背景のページの縁を避けた範囲） |
| フィールドエリア | 1208 × 1080（カード 5 枚分強の横幅） |
...
怪我のアイコンはドメイン側に表示できる形がまだ無いため、モックと同じ固定値を返す
プレースホルダーのままです。
