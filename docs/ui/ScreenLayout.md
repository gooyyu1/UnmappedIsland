...
いまの並びは、**カードを前詰めに置き、足りない分の空枠を末尾へ個数だけ足す**という作りです
（`laneCells.ts` の `emptyCellsFor`、`ObjectWindowSlot` の `cards` と `cellCount`）。カードと空枠が
別々の概念なので、**位置と枠が対応せず、枠ごとの装飾を指せません**。

製作中オブジェクトの材料（[`RecipeSystem.md`](../engine/RecipeSystem.md)）では、枠ごとに次を出したく
なります。

- 今の工程で要る枠か、後の工程で要る枠かを、**縁の色**で表す（塗りにしない——カードが入ると隠れる）
- あと何枚要るかを、**カードに重ねて**出す（他に書ける場所が無い）

これらは**枠の情報であって、カードの情報ではありません**。`Card` は設置物・アイテム・手持ち・装備・
怪我・レシピ一覧のすべてが通る共通のクラスなので、1つの画面の都合で `CardContent` を広げると、使わない
フィールドを全レーンが運ぶことになります。

そのため、枠を一級の単位にして、**背景 → カード → 重ねる物**の3層で描く形へ移します。プレースホルダーは
「カードが居ない枠の背景」として1層目に収まり、縁も枚数も枠が持ちます。`Card` は何も知りません。

渡し方は、`cards` と `cellCount` の2本立てをやめて枠の配列1本にします。

```ts
cells: readonly {
  card?: CardContent;
  borderColor?: number;   // 今の工程 / 後の工程
  overlay?: string;       // 「2/3」など、カードに重ねる表示
}[];
```

`CardLane` は全レーンが通るため、この変更は設置物・アイテム・手持ち・装備・怪我のすべてに影響します。
まとめて1つの変更として行うこと。
