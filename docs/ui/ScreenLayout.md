...
**1 つの部品**（`ObjectWindow`）で扱います。**組み方はどれも同じ 3 段**で、変わるのは真ん中に何を置くかだけです。

| 段 | 中身 |
|---|---|
| 上 | 見出し。オブジェクトなら自分の名前、キャラクタのスロットならスロットの名前（「装備」「怪我」） |
| 中 | 左に**自分のカード**（出す対象のみ）、右に**説明文か中身の並び**のどちらか |
| 下 | 操作のボタン。アクションが無ければ「閉じる」だけ |

真ん中の分岐は 2 つだけです。

- **自分のカードを出すか。** キャラクタのスロットとコンテナは出しません——右の段を中身の並びへ譲る
  ためです。怪我は出します（何を治療しているのかが分からないと困るため）。
- **右に説明文か、中身の並びか。** 中身のスロットを持つなら並び、持たないなら説明文です。
  **両方は出しません**——縦にも横にも収まらないので、持っている方を採ります。
