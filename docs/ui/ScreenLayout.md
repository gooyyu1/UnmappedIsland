...
- **ボタンはポートレイトの右へ縦積み**: キャラクター表示エリアで最も背の高いポートレイト
  ...
  重ねず、通常のフロー内に配置する。
- **押す頻度と大きさを対応させる**: 地図・装備・怪我はよく使うので、条件（48×48u）より大きな
  ボタン（168×84u）にする。アイテム名は表示せず、タップ先の子ウィンドウで確認する。
  **列の高さを 3 等分はしない**——文字を持たないボタンなので、絵より広い面を取ると、絵ではなく
...
- **この 3 つは絵だけで表す**（下記「ボタンの絵」）。**文字を持たせない**のは、言語ごとに変わる
  文字数をボタンの内側へ収める必要が出るため。日時のフリップカードと同じ理由で、文字を持たなければ
  翻訳も要らず、表示幅も設計値だけで決まる。役割が 3 つに固定されているので、絵だけで区別が付く。
...
| 地図・装備・怪我ボタン | 168 × 84（列の中で上下中央。列の高さを 3 等分はしない） |
| 地図・装備・怪我ボタンのアイコン | 136 × 60（3 枚とも同じ。物の大小は絵の側が持つ） |
| 条件ボタン | 48 × 48 |
...
地図・装備・怪我のボタンの絵は **`src/assets/icons/<アイコンの識別子>.png`** に置きます。識別子は
...
- **3 枚とも同じ寸法（336×144px）で持ち、UI は同じ大きさ（136×60u）で敷きます。** 寸法そのものを
  物の大きさにすると、絵を差し替えるたびに画面上の大きさが暗黙に変わってしまいます。変わるのは
  中の画素だけにします。
- **物の大小は絵の側が持ちます**——開いた地図（104×58u）＞ T シャツ（65×62u）＞ 巻いた包帯
  （61×38u）。UI が物を測って一律に揃えると、この差が消えます。**厳密な比にはしません**（包帯が
  見えなくなるため）。順序が読める程度に詰めます。
...
- **敷く紙はボタン専用の絵（`slot_button_paper.png`）で、ボタン 1 つぶんが 1 枚です。** カードの枠と
  ...
  切り抜き、枠線もその上へ引き直します。1 枚ずつ別の場所から取るのは、同じ場所だと 3 つに同じ染みが
  並んで模様として目に付くためです。
...
- 絵は `tools/comfyui/recipes/icon_<識別子>.json` から作り直せます。物の大きさは `cardArt.size`、
  キャンバスは `cardArt.canvas`（3 枚とも同じ値）です。
