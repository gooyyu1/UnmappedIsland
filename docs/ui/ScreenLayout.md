...
**カードを押したとき中身も出すかは、タグではなくスロットで決めます。** 「プレイヤーが入れられる
スロット」（`contents`・`treatment`）を持っていれば、そのカードのウィンドウに中身の並びが載ります。
編み籠（`containers.yaml`）も怪我の治療具も同じ経路です。

かつては `container` タグの有無で決めていましたが、怪我のように「入れ物ではないが、プレイヤーが 1 つ
入れる場所」を持つ物が出てきました。怪我に `container` タグを付けるのは筋が悪いので、判定の側を
広げています。
