...
外の様子をフィールドエリアそのものへ映します。天候チップは情報エリアの端にある小さな札なので、
...
暑い季節の日中だけ、**地面の絵をゆらめかせます**（Phaserの変位フィルタ）。

- **カードには掛けません。** 画面全体を歪ませるとカードの名前まで揺れて読めなくなりますし、実際の
  陽炎も焼けた地面の上に立つもので、手前の物が歪んで見えるわけではありません。フィルタは掛けた
  表示物の中だけを歪ませるので、レーンに敷かれた地面の絵にだけ掛けます。
- **掛けるのはアイテムレーンだけです。** 手持ちのレーンは外にある地面ではありません。設置物レーンは
  現在地カードのぶん背景が固定と可動に分かれていて、可動側だけが揺れると境目に段差が見えます。
- **ここだけはフィルタを使うので、立っている間は描画バッファを使います**（[DesignNotes.md](../engine/DesignNotes.md)）。
  歪ませる以上は避けられないコストなので、そのぶん立つ条件を狭く取っています。
- **動いて初めて分かる程度に留めます。** 地面が大きく歪むと、陽炎ではなく描画の壊れに見えます。
- **天気ではなく気温を見ます。** 陽炎が立つのは地面が焼けているからで、炎天という天気の名前そのもの
