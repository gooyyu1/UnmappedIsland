...
| スクロールバー | 高さ 8、送られるカードの下端との間隔 3（カードの下に 11 を確保する）、つまみの最小長 48 |
...
- **バーのぶんの場所（11u = 間隔 3u + 高さ 8u）は、送られる領域の中に常に確保します。** バーが出ない
  ときも出るときも、カードの位置が動かないためです。レーンではカードの下の余白（16u）がこれを兼ね、
  区切りの帯がかぶる 3u にも掛からないので、カードにも帯にも重なりません。
...
11u を足してバーの場所を確保します（そのぶんウィンドウが縦に伸びます）。
