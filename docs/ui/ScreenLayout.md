...
エリアの全体です。縦型ではオプションバーもこの中に入ります。

**背景は 1 枚の本のページ**（`src/assets/information_background_landscape.png` /
`information_background_portrait.png`）で、状況エリア・天候の帯・キャラクターエリアは自前の背景色を
持たず、このページの上に直接載ります（オプションバーだけは、向きによらず自前の背景を持ちます）。
