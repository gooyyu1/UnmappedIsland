...
- **条件はラベルなしのアイコンボタン、装備・怪我はアイコン＋ラベルの横長ボタン**: 条件は
  ...
  ポートレイトの右へ横一列に並べる幅が無い）。装備・怪我は「装備」「怪我」という
...
- **横型**: 左にポートレイトカードを置き、右の列（幅はキャラクター表示エリアの残り幅）に
  条件ボタン（2 列で折り返し）・装備ボタン・怪我ボタンを上から順に縦に並べる。
...
切り離せないもので、カード自体が土地の一部として見えるべきだからです。道のカードもここに含みますが、
敷くのは今いる土地ではなく[行き先の土地の景色](#設置物レーン-fixture-lane)です。
...
意味を持つためです。地に敷く景色も行き先のものにし、絵には道そのものを載せます——絵が「これは道だ」を、
地が「どこへ続くか」を答えるので、2 つの役割が重なりません。行き先の景色を絵としてカード全面に出すと、
同じレーンの左端にある現在地カードと見分けが付かなくなります。
...
右の縦列の内容量で決まる高さ（条件ボタン＋装備ボタン＋怪我ボタン、約 352u）、残りを
...
│   │   │   ├── ポートレイトカード
│   │   │   └── 右の縦列: 条件ボタン（2列で折り返し） → 装備ボタン → 怪我ボタン
