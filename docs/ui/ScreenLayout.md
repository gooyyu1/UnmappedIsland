...
**名前は今より小さくなります。** これまでは 30u で 3 行まで折り返していました。名前を大きく出すより、
絵を大きく見せるほうを採っています。
...
| 中身のバー | 中身（代表）の量が、入っている容器の `capacity` をどれだけ満たしているか | 絵の下、下端から 36u 浮かせる | 15u |
| 容量のバー | 入れ物に入っている物の `volume` の合計が、そのスロットの `capacity` をどれだけ満たしているか | 中身のバーと同じ | 15u |
| 怪我のバー | `severity` プロパティの `range` に対する、残っている傷 | 下端から 72u 浮かせる（印を避ける） | 15u |
...
- **中身・容量・怪我のバーはカードの下端へ付けません。** 何がどれだけ入っているか・あとどれだけ
  入るか・あとどれだけで治るかはそのカードの主要情報で、絵と同じくらい見に行く先だからです。耐久度より
  太くし、絵の下・下端との間を空けた位置に置きます。1 枚のカードが 2 本以上を出すことは無いので、
  寸法は 3 本で共有します。**位置を共有するのは入れ物の 2 本だけ**で、怪我のバーはそのぶん高い位置に
  置きます——[手当ての印](#カードの印)と同じカードに出るため、印（下端から 70u まで）に掛からない
  高さで止める必要があります。
...
そのカードが映しているものの**状態**を、絵文字ひとつで紙の左下へ小さく重ねます。今のところ出るのは
手当て済みの怪我（🩹）だけです。

- **状態で絵を差し替えません。** 手当ての有無で絵を差し替えると、怪我の部位 × 治療具の数だけ絵が要り、
  すぐ組み合わせ爆発を起こします（[`Injuries.md`](../world/Injuries.md) 手当て節）。印なら、状態の数だけ
  絵文字を足せば済みます。
- 置き場所は紙の左下で、**怪我のバーの下**に収まる大きさにします（バーは下端から 72u、印は 18u から
  52u ぶんなので触れません）。道の矢印と同じ角ですが、道のカードと怪我のカードは同時に存在しません。
...
矢印が無いと現在地のカードや他の設置物と見分けが付きません。大きさは名前の文字（30u）の 1.5 倍ほど
（51×45u）取り、カードを縮めて並べても一目で道と分かるようにします。**白い塗りに黒い細い縁**に
