...
| 怪我のバー | `severity` プロパティの `range` に対する、残っている傷 | 中身のバーと同じ | 15u |
...
- **中身のバーと怪我のバーはカードの下端へ付けません。** 何がどれだけ入っているか・あとどれだけで
  治るかはそのカードの主要情報で、絵と同じくらい見に行く先だからです。耐久度より太くし、絵の下・
  下端との間を空けた位置に置きます。1 枚のカードが両方を出すことは無いので、寸法も位置も共有します。
