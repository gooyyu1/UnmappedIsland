...
- **ステータスエリアのバーは上寄せ**: 表示するステータスは常に全種類ではなく、`status` タグが
  付いたものだけを表示する可変長リスト。中央寄せにすると表示件数が変わるたびにバー群の位置が
...
| カード間ギャップ | 12 |
| レーン間・外周マージン | 6 |
| レーンの区切りの帯 | 高さ 12（マージンの 2 倍。中央半分が区切り、上下 3 ずつは隣のレーンへかぶる） |
...
レーンの境目には木の梁の帯（**`src/assets/lane_separator.png`**）を敷きます。**設置物レーンの上・
レーンの間 2 か所・ハンドレーンの下**の 4 本で、フィールドエリアを枠で囲う見え方になります。

**帯は中央半分だけが区切りそのもので、上下 1/4 ずつは隣のレーンへかぶせます。** そのため帯の高さは
レーン間マージン（6u）の 2 倍の 12u を取り、マージンの中心線に対して上下対称に置きます。かぶる
3u ぶんはレーン内側のカード上下の余白（15u）に収まるので、カードには重なりません。

- 絵は帯の高さへ合わせて縦横同率で拡大縮小し、横方向は繰り返して並べます（左右の端が繋がるように
  描きます）。
- **入力は遮りません。** 隣のレーンへかぶせて置くため、下のレーンのタップやドラッグを奪わないように
  しています。
- 隣接エリア（状況エリア・フィルターバー）の背景板を描き終えた**最後**に敷きます。上端・下端の帯は
  フィールドエリアの外へ 3u はみ出すので、先に敷くと隣接エリアに覆われてしまうためです。
...
- 1 行は「名前＋バー」で、ステータスエリアと同じ部品（`StatusBar`）です。`range` を持たず割合を
  定義できないプロパティは、バーの代わりに値そのものを出します。
