...
| 地図・装備・怪我ボタンのアイコンの枠 | 168 × 72（ボタンの中央。絵は縦横比のままこの中へ収める） |
| 条件ボタン | 48 × 48 |
...
- **枠は正方形にせず、ボタンと同じ横長（168×72u）にします。** 絵は縦横比のままこの枠へ収めるので、
  正方形にすると平たい物ほど小さく見えます（開いた地図が高さの半分しか使えず、巻いた包帯より
  小さく見えました）。横長にすると、どの絵も高さで揃い、幅だけが物の縦横比のぶん伸びます。
- **絵は透明な余白を切り落として持ちます**（`card_art.py --trim`）。正方形のキャンバスに余白を
  残したままだと、枠へ収めたときに余白ごと縮んで、上の工夫が効きません。
- **ボタンの地は、カードの紙と同じくすんだ白に色相だけを載せた色です**（`COLOR.mapButton` など）。
  3 つを見分けるのに要るのは色相の差だけで、彩度を上げると、載せるアイコンより地の方が目立ちます。
- 絵がまだ無いものは**絵文字で代用**し、同じ場所へ枠の高さで置きます。絵は少しずつ増える前提
...
- 一覧はビルド時に作ります（`import.meta.glob`）。ファイル名がコードの引く識別子と食い違うと絵は
  ...
  [カードの絵](#カードの絵)と同じです。
- 絵は `tools/comfyui/recipes/icon_<識別子>.json` から作り直せます。切り出しの解像度は 256px で、
  ボタン上の大きさとは関係ありません（ボタン側が枠へ収めます）。
