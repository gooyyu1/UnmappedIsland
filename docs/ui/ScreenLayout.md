...
| 地図・装備・怪我ボタンの絵 | ボタンの全面（絵が無いときの代わりの絵文字だけ 64 × 64 で中央） |
| 条件ボタン | 48 × 48 |
...
- **絵はボタンの全面を覆います。** 背景は透過にせず、絵が地の色まで持ちます。ボタンの寸法
  （211×99u）に合わせて**およそ 2:1**で描き、比率のずれは覆い切る側へ合わせて切り落とします。
  寸法は u の 2 倍で 4K に合う **512×256px** です。
- **絵は角丸で切り抜き、枠線はその上へ引き直します。** ボタンの塗りと枠線は絵の下になるためです。
- **この 3 枚だけ線画寄りの平塗りです。** 他の絵（レーンの背景・カード・空）の水彩とは作風を分けて
  います。ボタンは小さく、にじんだ絵より輪郭線のある絵の方が何のボタンか読み取れるためです。
- 絵がまだ無いものは**絵文字で代用**し、ボタンの中央へ 64×64u で置きます。絵は少しずつ増える前提
  なので、両者が混ざった状態を正常とします。ボタンの塗り（`COLOR.mapButton` など）はこのときの地に
  なるので、絵の背景の色もこれに寄せます。
- 一覧はビルド時に作ります（`import.meta.glob`）。ファイル名がコードの引く識別子と食い違うと絵は
  黙って使われないままになるので、自動テスト（`tests/game/iconArt.test.ts`）が検査します。理由は
  [カードの絵](#カードの絵)と同じです。
- 絵は `tools/comfyui/recipes/icon_<識別子>.json` から作り直せます（3 枚で 0.4MB）。
...
- **日時と天候名が載る場所を、白く抜かないようにします。** 桁の紙も天候名も白いので、そこが白いと
  沈みます。焼けた空のように全体が明るい天気では、載せ物のある側だけを濃い色で締めます。
...
矢印が無いと現在地のカードや他の設置物と見分けが付きません。大きさは名前の文字（30u）と同じくらいで、
**白い塗りに黒い細い縁**にして、明るい絵の上でも暗い絵の上でも輪郭が残るようにします。縁が名前の
縁取り（4u）より細い（2u）のは、名前が文字の隙間を埋めないよう縁取りを太らせるのに対し、矢印は
一続きの塗りなので、太らせると形そのものが鈍るためです。
...
描かれると環境ごとに色も形も変わってしまうためです。塗り潰した太い矢羽根にするのは、名前の文字ほどの
大きさしか取らないからで、細い線ではカードを縮めて並べたときに何の形か読み取れなくなります。
