...
そこに在るからです。重ねてもカード自身は動かない `combinations`（石を打ち割る等）では、離した時点で
元の枠へ戻る動きを見せます——ワールドの差し替えを待ってもカードは動かないので、そのままだと指を離した
瞬間に枠へ瞬間移動して見えるためです（時間のかかる `combinations` では、結果の反映は経過し切ってから）。
