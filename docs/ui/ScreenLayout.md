...
- **ボタンはポートレイトの右へ縦積み**: キャラクター表示エリアで最も背の高いポートレイト
  ...
  重ねず、通常のフロー内に配置する。
- **押す頻度と大きさを対応させる**: 地図・装備・怪我はよく使うので、大きなボタン（幅は列いっぱい、
  高さはポートレイトの 1/3）にする。アイテム名は表示せず、タップ先の子ウィンドウで確認する。
  一方、条件は今の状態を示しているだけ（タップで詳細を開くが頻度は低い）なので、
  48×48u に留める。並びは持ち物の近さの順で、地図と装備は身につけているもの、
...
| 地図・装備・怪我ボタンのアイコン | 168 × 72（3 枚とも同じ。物の大小は絵の側が持つ） |
| 条件ボタン | 48 × 48 |
...
- **3 枚とも同じ寸法（168×72u ＝ 336×144px）で持ち、UI は同じ大きさで敷きます。** 寸法そのものを
...
- **ボタンの地は、カードの紙と同じくすんだ白に色相だけを載せた色です**（`COLOR.mapButton` など）。
  3 つを見分けるのに要るのは色相の差だけで、彩度を上げると、載せるアイコンより地の方が目立ちます。
