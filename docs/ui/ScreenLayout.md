...
- **押す頻度と大きさを対応させる**: 地図・装備・怪我・レシピはよく使うので、条件（48×48u）より
  大きなボタン（168×84u）にする。アイテム名は表示せず、タップ先の子ウィンドウで確認する。
  **列の高さを等分はしない**——文字を持たないボタンなので、絵より広い面を取ると、絵ではなく
  面の方が目に入る。内容量ぶんに留め、余った高さは列の中で上下に分ける。並びは持ち物の近さの順で、地図と装備は身につけているもの、
  装備と怪我は開く子ウィンドウの形が同じ。押す頻度が最も低い地図が端に来る。持ち物ではない
  レシピは反対の端。
  キャラクター表示エリア自体にも内側の余白（16u）を確保し、要素が端に張り付かないようにする。
- **この 4 つは絵だけで表す**（下記「ボタンの絵」）。**文字を持たせない**のは、言語ごとに変わる
...
| 地図・装備・怪我・レシピボタン | 168 × 84（列の中で上下中央。列の高さを等分はしない） |
| 地図・装備・怪我・レシピボタンのアイコン | 136 × 60（4 枚とも同じ。物の大小は絵の側が持つ） |
| 条件ボタン | 48 × 48 |
...
- **4 枚とも同じ寸法（336×144px）で持ち、UI は同じ大きさ（136×60u）で敷きます。** 寸法そのものを
...
| 域 | 見せ方 |
|----|--------|
| 安全域 (`safe`) | 出さない（余裕があるものを並べても読み取る情報が無いため）。ただし安全域へ戻った行は、良くなった分の帯（後述）を見せ終わってから消えます |
| 留意域 (`watch`) | 出す |
...
**安全域へ戻った行は、その場では消さず、良くなった分の帯（後述）が追いつき切ってから消します。**
...
子ウィンドウも、同じ日射で同じだけ翳らせます。雨と陽炎がフィールドエリアに留まるのとは、ここが
...
- しきい値を持つのはワールド側の `stages` だけです。UI は「その名前のプロパティが安全域を外れたら
  明滅させる」とだけ知っていて、何がどれだけ危ないのかは知りません（[域による出し分け](#域による出し分け)と
  同じ分担）。
...
元から限界に近い名前がはみ出すためです。作りかけであることは青い覆いが示す（[製作中オブジェクトの
カード](#製作中オブジェクトのカード)）ので、名前で言い直す必要はありません。
...
- **敷くかどうかはオブジェクトの種類ではなく、設置物レーンに並ぶカードかどうかで決めます。**
  ...
  例外は[製作中オブジェクトのカード](#製作中オブジェクトのカード)だけで、こちらは景色より覆いを
  優先します。
...
そのカードが映しているものの状態を、[枠の下の桟](#カードの枠)の中にバーで出します。**どれも「その値を
...
| 材料のバー | 今の工程が要求する素材と道具が、どれだけ揃っているか（[製作中オブジェクトのカード](#製作中オブジェクトのカード)） |
...
- **状態で絵を差し替えません。** 手当ての有無で絵を差し替えると、怪我の部位 × 治療具の数だけ絵が要り、
  すぐ組み合わせ爆発を起こします（[`Injuries.md`](../world/Injuries.md) 手当て節）。印なら、状態の数だけ
  絵文字を足せば済みます。
...
**ステータスバーと同じく域（`alert`）から引きます**（`fillColorFor`）。耐久度と違って**減るほど良い**量
なので、値そのものから引くと「満杯の緑＝新しい傷」という逆の読み方になります。傷が引くほど緑へ寄り、
増えた分だけが赤い帯として残ります。どの重さでどの域に入るかは怪我ごとの宣言で、軽い怪我は負った
直後でも危険域に入りません（[`Injuries.md`](../world/Injuries.md)）。
...
- **枠の青は種別を覆います。** 作りかけの籠はアイテムの茶ではなく青写真の青を持ちます。まだその物に
  ...
  [キャラクタの青](#枠の色は種別で変える)より彩度を上げて、並んだときに混ざらない差を取ります。
...
- そのため製作中の設置物には[土地の背景](#背景の絵)を敷きません。青の覆いが読めるだけの無地の地が
...
上に**材料の充足率**、下に**工程の進捗**を出します（[カードの状態バー](#カードの状態バー)の桟の中）。
...
- **材料のバーだけは、カードだけで「今作業できるか」を答えます。** 同じことは材料の枠の `2/6` も
  言っていますが、そちらは[材料ウィンドウ](#製作中オブジェクトの材料)を開かないと見えません。
  色は耐久度と同じ色域（満ちるほど緑）にして、緑＝作業できる、で読めるようにします。
...
エリアの境目には木の梁の帯（**`src/assets/separator.png`**）を敷きます。敷く場所は 3 種類です。
...
**そのぶん、道であることは[枠の色](#枠の色は種別で変える)と、桟の中央の右向き矢印が示します。**
名前も絵も行き先のものなので、これが無いと現在地のカードや他の設置物と見分けが付きません。**先に
読めるのは色のほう**で、矢印は色を確かめる側です。
...
#### 子ウィンドウ (Object Window)

**カードやスロットのボタンを押すと開く子ウィンドウ**です。オブジェクト・コンテナ・キャラクタのスロットを
**1 つの部品**（`ObjectWindow`）で扱います。
...
- カードを受け入れるスロットでは、**並びの末尾に受け皿の空枠**を出します（次項）。
...
ことで隠れません。縁が前者を選ぶのは、手前へ上げるとカードが持つ[スタック数の丸](#カードの絵)を
...
[末尾の受け皿](#末尾の受け皿前詰めのレーン)ぶんの空き枠を足すだけです。
...
- **空き枠には、その枠が要求する型のカードを薄く敷きます**（[`accepts`](#枠セルは一級の単位)）。
...
空き枠は末尾の受け皿しか無いので、そちらは**その位置の隙間へ落としたもの**として扱い、並びの末尾へ
入ります。画面は枠を指すだけで、それが位置の指定になるかどうかはワールド側の宣言が決めます。
...
- **アクションを押した時点でウィンドウを閉じます。** 実行すればワールドが変わり、そのカードが消える
  ...
  経過を見せます（[製作中オブジェクトの材料](#製作中オブジェクトの材料)）。
...
| 出現 | 差し替え前のどの ID とも重ならない | **それを起こした側のカード**から飛んでくる（探索なら現在地、`combinations` なら重ねた相手、獣が生んだ怪我ならその獣） |
...
飛んでいくのは**見た目だけの分身**で、カード自身は枠に居るまま伏せて待ちます。カードの居場所を決めるのは
