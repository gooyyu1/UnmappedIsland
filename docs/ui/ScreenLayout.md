...
- 中身の並びはレーン（`CardLane`）そのものなので、横スクロール・ドラッグ＆ドロップ・並べ替えは
  レーン間とまったく同じ仕組みで動きます。
- **空けておく枠の数は、そのスロットの枠の数**（`unit_capacity`、
  [SlotSystem.md](../engine/SlotSystem.md) 2 節）**で決めます。** 3 枠なら 3 枠、1 枠なら 1 枠
  （治療具。4 枠空けると「4 つ当てられる」と誤って伝わります）。スロットの制約は 3 つあり、見るのは
  枠の数です——`capacity` は中身の `size` の合計、`accepts` の `max` は「板 1 枚と棒 4 本」のような
  型ごとの個数で、どちらも枠の数ではありません。**4 枠で頭打ち**にするのは、それ以上並べると画面から
  はみ出すためで、入り切らない分は横スクロールで送ります。枠の数まで縮めたレーンは中央へ寄せます。
...
落とし先の枠を出しません。設置物を持ち歩けないのも同じ道筋です——`accepts` が弾いているのであって、
