...
**立つのは、配れる「完成へ近づける仕事」（`goal:game`）が尽きた周です**（[`board-design.md`](../.claude/board-design.md) 2.17・2.18.1）。**在庫の数では立ちません**——スメルを拾う役の入力はPRが出るたびに生えるので、「配れる `kind:task` が尽きた」で見ると、整備の issue が在庫を満たし続け、この役は立てられません。見つからない周が続いても、間隔（`CYCLES`）のぶんは空けます。渡す本文は [`dig-prompt.md`](../.claude/dig-prompt.md)。
...
**「どうしますか」とは書けません。** 決まっていない箇所は仮決めして進むのが既定なので、具体的な案は常に手元にあります。だから**太字の1文が、そのまま仕様書へ書ける形**になっていて、人間は**同意するものにチェックを付けて、`判断待ち` を外す**だけで済みます。**ラベルを外すところまでが答え**です——機械が見ているのはラベルで、チェックのほうは見ていません（[`parallel-work.md`](../.claude/parallel-work.md)「チェックだけでは、機械は拾わない」）。
...
**覚えることは1つです**——**ラベルを外す＝動いてもらう、自分で終わらせる＝閉じる（issue）かマージする（PR）。** **issue でもPRでも、`判断待ち` の外し方は同じ意味です**——同じラベルの外し方が場所によって別の意味を持つと、覚えることが2つになります（[`board-design.md`](../.claude/board-design.md) 2.13.1）。**付けたのがレビュアーか機械かも、見分ける必要がありません。**
