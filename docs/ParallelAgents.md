...
**デーモンを起こす係と、盤面を見回る係は、立つ条件が背反です。** 前者が手を出すのはデーモンが居ないときだけ（走っていれば `daemon.sh start` は何もしません）、後者が立つのは居るときだけ（立てるのがデーモン自身です）。**同じ原因に両方が別々の手を出す形は、錠ではなくこの背反で消えています**（[`board-design.md`](../agent-ops/board-design.md) 2.21.1節）。
...
**デーモンを起こす者だけは、デーモンの外に居ます。** 落ちれば盤面は1ミリも動かないので、そこを盤面の中の仕組みで見張ると**落ちたときに見張りも立ちません**。起こすのは**このPCのタスクスケジューラ**で、ログオンのときと毎時に `daemon.sh start` を打つだけです——**モデルは1つも動かず**、生きていればあちらが自分で引き返します（[`board-design.md`](../agent-ops/board-design.md) 2.19節）。
...
| 📊 **分析係（一次）** | マージ済みPRのコメントに残ったスメル（[`board-design.md`](../agent-ops/board-design.md) 4.4節）を拾う——書き手はレビュアーと、PRを書いた側の両方。**見るのはその回の帯だけで、過去の回の記録で傾向を探さない**（その回のスメルが名指した行を確かめるために開くのは別） | **新しい issue**（`kind:` は付けない）と、**記録のPR1本**（`agent-ops/analysis/<日付>.md`）。読んだコメントには 👀 を付ける。**issue にしてよいのは、ゲームの中身を指すスメルと、盤面かCIが止まるものだけ**——残りは `## 落としたもの` へ落ち（繰り返した形は `## 傾向` へも）、どちらも二次へ回る（[`board-design.md`](../agent-ops/board-design.md) 4.4.3節） | デーモンが [`dispatch-chore.sh`](../scripts/daemon/dispatch-chore.sh)。**読まれていないスメルがあれば立つ** | セッション |
| 📈 **分析係（二次）** | 一次が回ごとに書いた記録を横断して読み、**複数の回に現れている形に根本対策を打つか決める**（[`board-design.md`](../agent-ops/board-design.md) 2.17.4節）。PRのコメントも issue も読み直さない。**整備の issue を切れるのはこの係だけ**で、1回に切る本数にも上限がある | **新しい issue**（`kind:` は付けない）と、**記録のPR1本**（`agent-ops/analysis/summary/<日付>.md`）。方針の文書へは書かない——畳むのは価値観を畳む係 | デーモンが [`dispatch-chore.sh`](../scripts/daemon/dispatch-chore.sh)。**二次がまだ読んでいない一次の記録があれば立つ** | セッション |
...
| 🔧 **盤面を見回る係** | **デーモンは生きているのに盤面が進んでいない**形を拾うため、正常の定義（不変条件）に照らして盤面が健全かを毎回確かめ、崩れていれば原因を特定して直す（[`board-design.md`](../agent-ops/board-design.md) 2.21節） | **`main` への直接 push**（PRにすると、詰まっている間はマージされないので直しが届かない）と、人にしか直せないときの issue、**見回りの記録**（`~/.claude/board-state/patrol.jsonl`） | デーモンが [`dispatch-chore.sh`](../scripts/daemon/dispatch-chore.sh)。**盤面の見え方によらず立つ**（**印で絞ると、印に掛からない壊れ方を拾えない**）。**このPCでしか調べられない**（`~/daemon.log`） | セッション |
| 📐 **割に合っているかを見る係** | **ゲームが増えた量と、それに費やした量を並べて、割に合っているかを判定する**（[`board-design.md`](../agent-ops/board-design.md) 2.23節）。合っていなければ**仕組みを変える案**を出す。**係どうしの総和を見る者はここだけ** | **記録のPR1本**（`agent-ops/payoff/<日付>.md`）と、割が落ちている回だけ **`判断待ち` の issue 1本**。**自分では仕組みを変えない**——変えた回に測ると、次の回が「効いた」と「元から動いていた」を区別できない | デーモンが [`dispatch-chore.sh`](../scripts/daemon/dispatch-chore.sh)。**割に合っているかによらず立つ**（**割に合っていないことは、盤面のどの印にも現れない**）。**このPCでしか測れない**（履歴を全部持つクローンと `gh`） | セッション |
| 📮 **値の死を告げる係** | 盤面が動くのに要る値（環境ID・資格情報）が死んだことを、**手元の見回りが自分で書けない周に代わって置く**（[`board-design.md`](../agent-ops/board-design.md) 2.22.3節）。**判断はしない**——置く本文は渡されたものをそのまま | **題で引く issue 1本の本文**（同じ題が無ければ立てる）。**閉じない**——持ち主は手元の見回りのままで、値が生き返ったのを見たあちらが閉じる | 毎時の値の見回り（[`check-values.mjs`](../scripts/daemon/check-values.mjs)）が [`dispatch-chore.sh`](../scripts/daemon/dispatch-chore.sh)。**このPCの `gh` の資格情報が死んでいる周だけ立つ** | セッション |
...
**周期で立つ係の引き金は、「仕事があるか」と「前に立ててから間隔が空いたか」です**（[`board-design.md`](../agent-ops/board-design.md) 2.17節）。**間隔そのものは [`board-move.mjs`](../scripts/daemon/board-move.mjs) の `CYCLES` が係ごとに持ちます**——ここへ書き写すと、写しだけが古くなる先が増えます。**件数のしきい値は置きません**——溜まるまで待つ形にすると、1件のまま来ない日が続いたときにそれが放置され、しきい値そのものが「そこまでは残ってよい」の宣言になります。上の表で周期の係に書いた「何で立つか」は、このうち仕事の側だけです——見回る係だけは、盤面が健全かを確かめるのが仕事なので、それがいつでも在ります。

**値の死を告げる係だけは、盤面の周の外から立ちます。** 立てるのは1周を回す側ではなく、毎時の値の見回りです——**盤面が動くのに要る値を見張る係を、その値を要る周の中に置くと、値が死んだ周に見張りも動きません**（[`board-design.md`](../agent-ops/board-design.md) 2.22.1節）。
...
- **やるかどうかから訊くもの**（未決事項と、段0を通っていない `【未実装】`）→ **チェックの一覧を1本立てます**（`判断待ち` を付けるので、答えが埋まるまで誰にも配られません。[`board-design.md`](../agent-ops/board-design.md) 2.17.2節）。**未決は「まだ決めていないこと」であって「やると決めたこと」ではない**ので、そのまま配ると乗り気でないものが実装されて戻ってきます（[`agent-ops/parallel-work.md`](../agent-ops/parallel-work.md)「未決は「やるかどうか」から訊く」）。
...
**盤面の issue は写しで、手を決める材料ではありません。** 走っているセッションもPRのラベルも GitHub 側の事実なので、[`board.sh`](../scripts/agent/board.sh) もデーモンも毎回引き直します——**写しを持つと古くなったことに誰も気づけない**ので、本文には**いつ時点かを一緒に書きます**（[`board-design.md`](../agent-ops/board-design.md) 2.20.1節）。
...
**常設の盤の値は、その本文を機械がどう扱うかで分かれます**（[`board-design.md`](../agent-ops/board-design.md) 2.17.5節）。盤面がユーザーの答えとしてチェックを拾うのは、そのうち**答えを受ける盤**からだけです。**ラベルの名前が、機械の扱いをそのまま名乗ります**——名乗らせないと、機械が本文を読んでいることを誰も想像できないまま、下ろす者の居ないチェックが `## 確定待ち` に居座ります。
...
**向かう先は別の印（`goal:game` / `goal:upkeep`）で名乗ります**（[`board-design.md`](../agent-ops/board-design.md) 2.18.1節）。盤面はこちらを読んで、完成へ近づける仕事を整備より先に配り、**整備へ回す作業者の枠を切り**（[`board-design.md`](../agent-ops/board-design.md) 2.18.2節）、**完成へ近づける仕事の供給が枠に満たない周に掘り起こし役を立てます**（同 2.18.3節）。**既定はありません**——`goal:game` を持つものだけが完成へ近づける仕事で、名乗らなければ整備として並びます。**名乗り漏れを最後に拾うのは棚卸し役です**（同 2.17.1節。`kind:task` なのに `goal:` の無い issue は未整理として拾われ、印が付くまで盤面が `NOTE` で告げ続けます）。**配る手前では止めない**ので、印が付くまでのあいだは整備として並びます。

**分類が済んだ issue は、そのまま配られます。** 棚卸し役は `判断待ち` を付けません——**人の手番に入る印を、判断の要らないところへ付けない**ためです（[`board-design.md`](../agent-ops/board-design.md) 2.13.1節）。付けたぶんだけ、外すタップが増えます。順序（`blockedBy`）を張るかどうかを決めるのも棚卸し役です——**なぜその順序かを知っているのは issue を立てた側だけ**なので、立てたセッションが本文の `## 順序` 節で申告し、棚卸し役がそれを読んで決めます。**張る手そのものはワークフローが持ちます**——依存の API はセッションの側の道具に無いので、棚卸し役は待つ側の issue へ `[順序] #<先に要るほう> の後` とコメントし、[`board-labels.yml`](../.github/workflows/board-labels.yml) がそれを張ります。**申告の無いものに順序は作りません**——推定を許すと、触る先が重なっているだけの組まで直列に落ちます。
...
**覚えることは1つです**——**ラベルを外す＝動いてもらう、自分で終わらせる＝閉じる（issue）かマージする（PR）。** **issue でもPRでも、`判断待ち` の外し方は同じ意味です**——同じラベルの外し方が場所によって別の意味を持つと、覚えることが2つになります（[`board-design.md`](../agent-ops/board-design.md) 2.13.1節）。**付けたのがレビュアーか機械かも、見分ける必要がありません。**
...
**作業セッションは、人間へ問いかけません。** 人間はそのセッションを見ないので、訊いた側は答えの来ない相手を待ち続けます。決まっていない箇所は仮決めして進み、**仮決めすらできないときだけ、仕事を issue へ返して終わります**——1行目を `[返却] <なぜ返すか>` にしたコメントを置くと、issue に `判断待ち` が付き、人間が外すまで誰にも配られません。**返さないまま止まったセッションは、盤面が一度起こして、それでも動かなければ代わりに返します。** 詳細は [`board-design.md`](../agent-ops/board-design.md) 2.15節。
...
| `RETURN` | 起こしても動かなかった `task-*` のセッション。**宛先を引けないPRを抱えた担当も**（[`board-design.md`](../agent-ops/board-design.md) 2.11.4節）、**頼み終えた差し戻しが戻ってこないPRの担当も**（同 2.13.6節） | issue へ `[返却]` のコメントを置き、**仕事を人へ返す**（5節）。**文面は返す形ごとに違います**——人がすることが違うので |
| `REVIEW` | 判定のラベルが無く、緑で、マージできると分かっている。**`直し待ち` で頼み終えても戻ってこないPRも**（[`board-design.md`](../agent-ops/board-design.md) 2.13.6節。**緑でマージできる形のときだけ**で、赤や衝突のまま止まったものは上の `RETURN` へ回ります）、**判定が書かれても札が付かないPRも**（同 2.13.7節） | [`dispatch-review.sh`](../scripts/daemon/dispatch-review.sh) |
| `UNLABEL` | 最後の判定が今の頭でない版を名乗っているのに、結論の札が残っている（[`board-design.md`](../agent-ops/board-design.md) 2.13.7節） | **前の差分の札を落としてほしい**とPRへ頼む。**外すのは `board-labels.yml`**——盤面が自分で外すと、それが「人が外した」と読まれて `却下` が付きます |
...
**人の手番の印は、効き目を1つずつ持ちます。** PRに付いた `判断待ち` が止めるのは**マージだけ**、`収束せず` が止めるのは**レビューだけ**で、**どちらの下でも、差分そのものへ向いた差し戻し（`却下`・`## 見た目` の欠け・レビューの `直し待ち`）は出ます**——人の返事を待たなくても直せるからです。**出さないのは、`main` が動いて生まれたコンフリクトとCIの赤だけ**——答えを待つ間ずっと生まれ続けるうえ、直しても答えは近づきません（[`board-design.md`](../agent-ops/board-design.md) 2.13.8節）。**どちらを越えるのも画面からのマージ**で、人が外せばそれは差し戻しになります（5節）。印そのものは push で全部外れ、必要なら次の周に導出し直されます。**外す走りが転んだ回は、盤面が同じ札を落としてもらいます**（`UNLABEL`。[`board-design.md`](../agent-ops/board-design.md) 2.13.7節）。

### どこで走らせるかは、issue に書いてある

**セッションが立つ先は2つあります**——クラウドと、このPC（ブリッジ）。**既定はクラウド**で、issue に `env:` のラベルが付いていればそこへ投入します（[`board-design.md`](../agent-ops/board-design.md) 2.16節）。
...
**そのタスク自体が走らなくなったときだけは、拾う者が居ません**（誰もログオンしていない間は起きません）。この口座から動かせるものが全部このPCの上にあり、**外から見ている者が1つもない**ためです。見張りを足しても、足した見張りが同じPCで走るので同じ場所へ戻ります。**ここだけは人の目が要ります**（[`board-design.md`](../agent-ops/board-design.md) 2.19.3節）。
