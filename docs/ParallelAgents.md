...
**立つのは、配れる「完成へ近づける仕事」（`goal:game`）が尽きた周です**（[`board-design.md`](../.claude/board-design.md) 2.17・2.18.1）。**在庫の数では立ちません**——スメルを拾う役の入力はPRが出るたびに生えるので、「配れる `kind:task` が尽きた」で見ると、整備の issue が在庫を満たし続け、この役は立てられません。見つからない周が続いても一日一回に収めます。渡す本文は [`dig-prompt.md`](../.claude/dig-prompt.md)。
