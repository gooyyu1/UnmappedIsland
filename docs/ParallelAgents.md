...
**常設の盤の値は、その本文を機械がどう扱うかで分かれます。** ユーザーの答えをチェックで受ける盤（`kind:ask`）だけが、機械に本文を読まれて盤面の `## 確定待ち` へ出ます。投入の手綱のように**チェックが設定である盤**（`kind:switch`）と、**機械が本文を読まない盤**（`kind:board`。デーモンが書き換える盤面もこれ）は、その対象になりません——**ラベルの名前が、機械の扱いをそのまま名乗ります**（[`board-design.md`](../.claude/board-design.md) 2.17.5）。兼ねさせていた間、デーモンは投入の手綱のチェックをユーザーの答えとして拾っていました。
