...
**規則そのものは [`agent-ops/parallel-work.md`](../agent-ops/parallel-work.md) が持ちます。** あちらはエージェントが従う取り決めで、こちらは全体像だけを扱います——同じ説明を二度書かないため、細かい条件はあちらへのリンクに留めます。
...
**役を分けている基準は、やっていることではなく「何に書けるか」と「何で立つか」です。** 掘り起こし役と棚卸し役はどちらも読んで issue へ出す役ですが、**既にある issue を書き換えられるのは棚卸し役だけ**です（掘り起こし役が書けるのは、自分で立てる新しい issue と、その一覧が二重に立っていた周のコメント1つだけです）。**下の表の4列目（どう走るか）は基準ではありません**——「何に書けるか」から出てくる帰結です（「立ち方は『何に書けるか』が決めている」）。
...
flowchart LR
    ...
    Daemon -->|"ゲームの仕事の供給が、作業者の枠に満たないとき"| Digger
    Daemon -->|"検めていない参照か、変わったファイルがあるとき"| RefAudit
    Daemon -->|"健全かどうかによらず"| Patrol
    Daemon -->|"割に合っているかによらず"| Payoff
    Daemon -->|"このPCの gh が死んでいて、自分で issue を書けないとき"| Values
    Worker ==>|"PRを作る前に1本"| SelfReview
    Analysis ==>|"PRを作る前に1本"| SelfReview
    Payoff ==>|"PRを作る前に1本"| SelfReview
...
**デーモンを起こす係と、盤面を見回る係は、立つ条件が背反です。** 前者が手を出すのはデーモンが居ないときだけ（走っていれば `daemon.sh start` は何もしません）、後者が立つのは居るときだけ（立てるのがデーモン自身です）。**同じ原因に両方が別々の手を出す形は、錠ではなくこの背反で消えています**（[`board-design.md`](../agent-ops/board-design.md) 2.21.1）。

**作業者とレビュアーを立てるのはモデルではありません。** [`daemon.sh`](../scripts/daemon/daemon.sh) が一定の間隔で GitHub から盤面を引き、打つ手を [`board-move.mjs`](../scripts/daemon/board-move.mjs) が決めます（8節）。**人が1本もセッションを立てていなくても、投入・レビュー・直しの再開・マージは進みます。**

**デーモンを起こす者だけは、デーモンの外に居ます。** 落ちれば盤面は1ミリも動かないので、そこを盤面の中の仕組みで見張ると**落ちたときに見張りも立ちません**。起こすのは**このPCのタスクスケジューラ**で、ログオンのときと毎時に `daemon.sh start` を打つだけです——**モデルは1つも動かず**、生きていればあちらが自分で引き返します（[`board-design.md`](../agent-ops/board-design.md) 2.19）。
...
| --- | --- | --- | --- | --- |
| 💬 **相談役** | 人間と直接やりとりして、決まっていないことを決める。**訊かれたら実装ではなく設計案を返す** | PR（合意できた分だけ）、価値観の記録（`agent-ops/decisions/`） | **人間が新しいセッションを立てて話しかける** | セッション |
| ⛏️ **掘り起こし役** | 完成の定義に照らして、**まだ issue になっていない残りを数える** | **新しい issue**——やると決まっているものはそのまま、やるかどうかから訊くものは `判断待ち` を付けたチェックの一覧に（分類は棚卸し役が付ける）。**既にある issue へ書くのは、訊く一覧が二重に立っていた周のコメント1つだけ** | デーモンが [`dispatch-chore.sh`](../scripts/daemon/dispatch-chore.sh)。**ゲームの仕事の供給（配れる `goal:game` ＋ 手が動いている `goal:game`）が、作業者の枠に満たなければ立つ** | セッション |
| 📋 **棚卸し役** | 未整理の issue を分類し、投入できる形へ翻訳する | **issue の本文とラベル**（原文は `## 元の報告` として残す） | デーモンが [`dispatch-chore.sh`](../scripts/daemon/dispatch-chore.sh)。**未整理があれば立つ** | セッション |
| 📊 **分析係（一次）** | マージ済みPRのコメントに残ったスメル（[`board-design.md`](../agent-ops/board-design.md) 4.4）を拾う——書き手はレビュアーと、PRを書いた側の両方。**見るのはその回の帯だけで、過去の回の記録で傾向を探さない**（その回のスメルが名指した行を確かめるために開くのは別） | **新しい issue**（`kind:` は付けない）と、**記録のPR1本**（`agent-ops/analysis/<日付>.md`）。読んだコメントには 👀 を付ける。**issue にしてよいのは、ゲームの中身を指すスメルと、盤面かCIが止まるものだけ**——残りは `## 落としたもの` へ落ち（繰り返した形は `## 傾向` へも）、どちらも二次へ回る（[`board-design.md`](../agent-ops/board-design.md) 4.4.3） | デーモンが [`dispatch-chore.sh`](../scripts/daemon/dispatch-chore.sh)。**読まれていないスメルがあれば立つ** | セッション |
| 📈 **分析係（二次）** | 一次が回ごとに書いた記録を横断して読み、**複数の回に現れている形に根本対策を打つか決める**（[`board-design.md`](../agent-ops/board-design.md) 2.17.4）。PRのコメントも issue も読み直さない。**整備の issue を切れるのはこの係だけ**で、1回に切る本数にも上限がある | **新しい issue**（`kind:` は付けない）と、**記録のPR1本**（`agent-ops/analysis/summary/<日付>.md`）。方針の文書へは書かない——畳むのは価値観を畳む係 | デーモンが [`dispatch-chore.sh`](../scripts/daemon/dispatch-chore.sh)。**二次がまだ読んでいない一次の記録があれば立つ** | セッション |
...
| 📐 **割に合っているかを見る係** | **ゲームが増えた量と、それに費やした量を並べて、割に合っているかを判定する**（[`board-design.md`](../agent-ops/board-design.md) 2.23）。合っていなければ**仕組みを変える案**を出す。**係どうしの総和を見る者はここだけ** | **記録のPR1本**（`agent-ops/payoff/<日付>.md`）と、割が落ちている回だけ **`判断待ち` の issue 1本**。**自分では仕組みを変えない**——変えた回に測ると、次の回が「効いた」と「元から動いていた」を区別できない | デーモンが [`dispatch-chore.sh`](../scripts/daemon/dispatch-chore.sh)。**割に合っているかによらず立つ**（**割に合っていないことは、盤面のどの印にも現れない**）。**このPCでしか測れない**（履歴を全部持つクローンと `gh`） | セッション |
| 📮 **値の死を告げる係** | 盤面が動くのに要る値（環境ID・資格情報）が死んだことを、**手元の見回りが自分で書けない周に代わって置く**（[`board-design.md`](../agent-ops/board-design.md) 2.22.3）。**判断はしない**——置く本文は渡されたものをそのまま | **題で引く issue 1本の本文**（同じ題が無ければ立てる）。**閉じない**——持ち主は手元の見回りのままで、値が生き返ったのを見たあちらが閉じる | 毎時の値の見回り（[`check-values.mjs`](../scripts/daemon/check-values.mjs)）が [`dispatch-chore.sh`](../scripts/daemon/dispatch-chore.sh)。**このPCの `gh` の資格情報が死んでいる周だけ立つ** | セッション |
...
| 🔬 **レビュアー** | PR1本の差分と、その外まで読む。**直さない** | **PRへのコメント1つだけ**（issue も立てない） | デーモンが [`dispatch-review.sh`](../scripts/daemon/dispatch-review.sh) | セッション |

**盤面を見回る係だけは、直しをPRに載せません。** 盤面が詰まっている間はPRがマージされないので、載せると直しが届きません（出どころ: ユーザーの指示・2026-09-06）。

**PRを出すのは主に作業者と相談役**で、どちらも**1セッション＝1ブランチ＝1PR**に閉じます。**一次と二次の分析係・参照を検める係・割に合っているかを見る係も、記録のPRを1本ずつ出します**が、記録だけの差分なので、**作業者の枠には数えません**（8節の `CHORE`）。**棚卸し役・レビュアー・掘り起こし役・価値観を畳む係・値の死を告げる係は作業ツリーへ書かない**ので、何本立てても互いの足元を壊しません。自己レビュー役はPRを出す側のチェックアウトをそのまま使いますが、**読むだけ**なので同じです。

**周期で立つ係の引き金は、「仕事があるか」と「前に立ててから間隔が空いたか」です**（[`board-design.md`](../agent-ops/board-design.md) 2.17）。**間隔そのものは [`board-move.mjs`](../scripts/daemon/board-move.mjs) の `CYCLES` が係ごとに持ちます**——ここへ書き写すと、写しだけが古くなる先が増えます。**件数のしきい値は置きません**——溜まるまで待つ形にすると、1件のまま来ない日が続いたときにそれが放置され、しきい値そのものが「そこまでは残ってよい」の宣言になります。上の表で周期の係に書いた「何で立つか」は、このうち仕事の側だけです——見回る係だけは、盤面が健全かを確かめるのが仕事なので、それがいつでも在ります。

**値の死を告げる係だけは、盤面の周の外から立ちます。** 立てるのは1周を回す側ではなく、毎時の値の見回りです——**盤面が動くのに要る値を見張る係を、その値を要る周の中に置くと、値が死んだ周に見張りも動きません**（[`board-design.md`](../agent-ops/board-design.md) 2.22.1）。
...
| 📐 割に合っているかを見る係 | **前の回と比べられること。** 割が落ちたかは1回の値からは言えず、**比べる先がリポジトリに残っていることだけ**が判定を成り立たせます。報告に作り替えると、次の回は毎度ゼロから測り直すことになり、**そのたびに物差しが揺れます** |
...
- **合意が仕様書の節へ落ちるPRは、`判断待ち` になります。** `【確定】` の印が増えるので、機械の関門（6節）に掛かるためです。**これは正常で、そのマージが「差分への承認」にあたります**——チャットで合意したのは文面で、まだ差分は見ていません。**取り決めの置き場（`agent-ops/**`）にも、PRにする限りは同じ関門が掛かります**——[`needs-user-review.sh`](../scripts/daemon/needs-user-review.sh) の射程は、印の条件が掛かる文書と同じ1つです（[`DocumentStyle.md`](./DocumentStyle.md) 10節）。**ただし、あの置き場は `main` へ直接 push してよいので、そちらへ載せた合意には関門が掛かりません**——PRが無い差分には掛かる先がありません。この文書のように印を置かない文書では、そもそも印が動かないので鳴りません。

### ⛏️ 掘り起こし役 — 探すのではなく、数える

**立つのは、ゲームの仕事の供給が作業者の枠に満たない周です**（[`board-design.md`](../agent-ops/board-design.md) 2.17・2.18.1・2.18.3）。**在庫の数では立ちません**——スメルを拾う役の入力はPRが出るたびに生えるので、「配れる `kind:task` が尽きた」で見ると、整備の issue が在庫を満たし続け、この役は立てられません。**尽きてからでも遅すぎます**——0で見ていた間、在庫は少数のまま滞留してこの役は一度も立たず、空いた枠は整備で埋まり続けました。見つからない周が続いても、間隔（`CYCLES`）のぶんは空けます。渡す本文は [`dig-prompt.md`](../agent-ops/prompts/dig-prompt.md)。
...
数えるのは、**残りの作業量を答えられるもの**だけです（[`agent-ops/policies.md`](../agent-ops/policies.md)「未実装のものの扱い」。「無くても完成を宣言できるもの」は [`Someday.md`](./Someday.md) に分けてあります）。

- `未決事項`・`今後の検討課題` の節
- `【未実装】` の印
- 収支の穴。**生成済みの `stats/balance.yaml` を読みます**——表は `main` への push のたびに workflow が作り直すので、`npm run stats:balance` で作り直さずに読むだけで足ります。**コンテンツ不足が本命のボトルネック**です（[`agent-ops/parallel-work.md`](../agent-ops/parallel-work.md) の「コンテンツを後回しにしない理由」）
...
- **やるかどうかから訊くもの**（未決事項と、段0を通っていない `【未実装】`）→ **チェックの一覧を1本立てます**（`判断待ち` を付けるので、答えが埋まるまで誰にも配られません。[`board-design.md`](../agent-ops/board-design.md) 2.17.2）。**未決は「まだ決めていないこと」であって「やると決めたこと」ではない**ので、そのまま配ると乗り気でないものが実装されて戻ってきます（[`agent-ops/parallel-work.md`](../agent-ops/parallel-work.md)「未決は「やるかどうか」から訊く」）。
...
flowchart TB
    ...
    subgraph Issues["issue"]
        New["未整理<br/>（結論がまだ揃わない）"]
        Task["kind:task<br/>担当・完了条件・拠り所"]
        Ask["判断待ち<br/>（人へ訊く issue。1問1本）"]
        Brake["#1515 手綱"]
    end
    PR["PR<br/>（## 仮決め を先頭に持つ）"]

    Human -->|"自分の言葉で書く"| New
    Human -->|"チェックを付けて 判断待ち を外す＝確定"| Ask
    ...
    Triage -->|"確定の宣言に反する判断が要るとき1本立てる"| Ask
...
**リポジトリの記録にも書く役が居ます**——相談役がユーザーから受け取った判断を `agent-ops/decisions/` へ、作業者が確定した仕様を `docs/**` へ（9節）、一次の分析係がその回の帯で繰り返し現れた形を `agent-ops/analysis/` へ、二次の分析係が回をまたいでいる形を `agent-ops/analysis/summary/` へ。
...
| 種類 | 例 | 何のためか | 誰が書き換えるか |
| --- | --- | --- | --- |
| **人へ訊く** | `判断待ち` ラベル | **チェックを1つ付けてラベルを外せば答えになる**問いを1本に1つ。**溜める一覧は持ちません**——答えが付いても誰も気づかないため（[#656](https://github.com/gooyyu1/UnmappedIsland/issues/656) がその形で、記録として残っています） | 気づいたセッションが立て、人間がチェックしてラベルを外す |
...
**盤面の issue は写しで、手を決める材料ではありません。** 走っているセッションもPRのラベルも GitHub 側の事実なので、[`board.sh`](../scripts/agent/board.sh) もデーモンも毎回引き直します——**写しを持つと古くなったことに誰も気づけない**ので、本文には**いつ時点かを一緒に書きます**（[`board-design.md`](../agent-ops/board-design.md) 2.20.1）。
...
**棚卸し役が出す結論は `kind:` と `goal:` の各1つです**——`kind:` は**何であるか**（1セッションで片付く作業単位（`kind:task`）か、投入する先が無い常設の盤か）、`goal:` は**何へ向かうか**（下の「向かう先」。常設の盤には要りません）。**未整理とは、この結論が揃っていないこと**——分類がまだのものと、`kind:task` なのに向かう先を名乗っていないものの両方です。分類の値を数え上げて「そのどれでも無い」という否定の列挙では表しません。出口が増えるたびに条件を書き換えることになり、書き忘れた出口の issue が毎周また拾われるからです。

**常設の盤の値は、その本文を機械がどう扱うかで分かれます**（[`board-design.md`](../agent-ops/board-design.md) 2.17.5）。盤面がユーザーの答えとしてチェックを拾うのは、そのうち**答えを受ける盤**からだけです。**ラベルの名前が、機械の扱いをそのまま名乗ります**——名乗らせないと、機械が本文を読んでいることを誰も想像できないまま、下ろす者の居ないチェックが `## 確定待ち` に居座ります。
...
**向かう先は別の印（`goal:game` / `goal:upkeep`）で名乗ります**（[`board-design.md`](../agent-ops/board-design.md) 2.18.1）。盤面はこちらを読んで、完成へ近づける仕事を整備より先に配り、**整備へ回す作業者の枠を切り**（[`board-design.md`](../agent-ops/board-design.md) 2.18.2）、**完成へ近づける仕事の供給が枠に満たない周に掘り起こし役を立てます**（同 2.18.3）。**既定はありません**——`goal:game` を持つものだけが完成へ近づける仕事で、名乗らなければ整備として並びます。**名乗り漏れを最後に拾うのは棚卸し役です**（同 2.17.1。`kind:task` なのに `goal:` の無い issue は未整理として拾われ、印が付くまで盤面が `NOTE` で告げ続けます）。**配る手前では止めない**ので、印が付くまでのあいだは整備として並びます。

**分類が済んだ issue は、そのまま配られます。** 棚卸し役は `判断待ち` を付けません——**人の手番に入る印を、判断の要らないところへ付けない**ためです（[`board-design.md`](../agent-ops/board-design.md) 2.13.1）。付けたぶんだけ、外すタップが増えます。順序（`blockedBy`）を張るかどうかを決めるのも棚卸し役です——**なぜその順序かを知っているのは issue を立てた側だけ**なので、立てたセッションが本文の `## 順序` 節で申告し、棚卸し役がそれを読んで決めます。**張る手そのものはワークフローが持ちます**——依存の API はセッションの側の道具に無いので、棚卸し役は待つ側の issue へ `[順序] #<先に要るほう> の後` とコメントし、[`board-labels.yml`](../.github/workflows/board-labels.yml) がそれを張ります。**申告の無いものに順序は作りません**——推定を許すと、触る先が重なっているだけの組まで直列に落ちます。

### 人へ訊くときは「断定文」で書く

**「どうしますか」とは書けません。** 決まっていない箇所は仮決めして進むのが既定なので、具体的な案は常に手元にあります。だから**太字の1文が、そのまま仕様書へ書ける形**になっていて、人間は**同意するものにチェックを付けて、`判断待ち` を外す**だけで済みます。**ラベルを外すところまでが答え**です——機械が見ているのはラベルで、チェックのほうは見ていません（[`parallel-work.md`](../agent-ops/parallel-work.md)「チェックだけでは、機械は拾わない」）。

**答えが無いことは「否」ではありません**（見ていない・今は決めない、も同じ）。暫定が既定なので、放置されたままで何も壊れません。
...
sequenceDiagram
    ...
    alt 直し待ち／CIが赤／コンフリクト／見た目が無い
        ...
        Note over D,R: 本文だけを直して戻ってこなければ<br/>もう1周レビューへ渡す<br/>（レビューへ渡せない形は担当の issue へ返す）
    else 通してよい かつ 緑
        alt 人の手番で止まっている
            D->>G: マージを出さない
...
- **レビューが立ちます。** [`dispatch-review.sh`](../scripts/daemon/dispatch-review.sh) はPR番号だけで立ち、誰が書いたかを見ません。
- **差し戻す相手が分かります。** **セッションは自分のIDを名乗る決まり**で、コミットに `Claude-Session: https://claude.ai/code/session_<ID>` のトレーラが入ります。盤面はそれを読みます——本文の脚注は書き直した拍子に落ちますが、トレーラは残ります。
...
flowchart LR
    ...
    C["判断待ち の issue にチェック<br/>＋ラベルを外す＝確定"] --> Mark["仕様書へ【確定】が付く"]
    D["相談役と話す"] --> Decide["合意がPRになる"]
```

**覚えることは1つです**——**ラベルを外す＝動いてもらう、自分で終わらせる＝閉じる（issue）かマージする（PR）。** **issue でもPRでも、`判断待ち` の外し方は同じ意味です**——同じラベルの外し方が場所によって別の意味を持つと、覚えることが2つになります（[`board-design.md`](../agent-ops/board-design.md) 2.13.1）。**付けたのがレビュアーか機械かも、見分ける必要がありません。**
...
**作業セッションは、人間へ問いかけません。** 人間はそのセッションを見ないので、訊いた側は答えの来ない相手を待ち続けます。決まっていない箇所は仮決めして進み、**仮決めすらできないときだけ、仕事を issue へ返して終わります**——1行目を `[返却] <なぜ返すか>` にしたコメントを置くと、issue に `判断待ち` が付き、人間が外すまで誰にも配られません。**返さないまま止まったセッションは、盤面が一度起こして、それでも動かなければ代わりに返します。** 詳細は [`board-design.md`](../agent-ops/board-design.md) 2.15。
...
**ただし、申告だけでは止める線になりません。** 直近25本（2026-08-29 の実測）で `## 仮決め` に中身のあったPRは **22本**。全部を人間へ回せば**タップを1回増やすだけの関門**になり、本当に判断が要るものが埋もれます。実際に付いた `判断待ち` は **0本**——受け取る側が使えていませんでした。

そこで、判定を次のように割ってあります。

| 何を見るか | 誰が見るか | 越え方 |
| --- | --- | --- |
| **確定の宣言の増減**（見出しに付いた `【確定】` と、文書単位の宣言） | 機械（[`needs-user-review.sh`](../scripts/daemon/needs-user-review.sh)） | **デーモンには越えられません。** `判断待ち` のラベルが付き、通すなら**画面からマージします** |
...
**触ったファイルの線では引きません。** ファイルの線は「触ったか」しか見ないので、**スキーマの `description` を変えただけの差分と、文法を1つ足した差分が同じに見えます**——止めた理由と `## 仮決め` の中身が一致せず、通すかを決める側は「何を判断すればよいのか」を読み取れません。**区別できるのは差分を読む者だけ**なので、レビュアーが持ちます。
...
**代わりに、ぶつかった実績を控えます。** [`describe-conflict.sh`](../scripts/daemon/describe-conflict.sh) が「どのファイルで・どのPRと」を出し、デーモンがそれを帳面へ残します。**錠を足すべき資源は、起きた衝突から決めます。**

**層で切った領域の地図は持ちません。** 効くのはファイルの衝突にだけで、文書は主題で並ぶため軸が直交します。**同時に1本しか動かせない資源——GPUや、動かしながら書き換えられないデーモン——だけがラベルの錠になります。** 条件は [`agent-ops/parallel-work.md`](../agent-ops/parallel-work.md) の「同時に走らせてよいか」。
...
**[`daemon.sh`](../scripts/daemon/daemon.sh) が一定の間隔で GitHub から盤面を引き、1周に1手だけ打ちます。** 1周の中身（引く・決める・打つ）は [`board-round.mjs`](../scripts/daemon/board-round.mjs) が持ち、そのうち打つ手を決めるのは [`board-move.mjs`](../scripts/daemon/board-move.mjs) で、**盤面を受けて手の並びを返すだけの純粋な関数**です（GitHubも CCR も触らないので、単体で試せます）。

| 手 | いつ打つか | 何をするか |
| --- | --- | --- |
| `TIDY` | 窓（[`board-read.mjs`](../scripts/daemon/board-read.mjs) の `MERGED_WINDOW_HOURS`）に載っているマージ済みのPRで、まだ片付けていないもの | [`tidy-merged-pr.sh`](../scripts/daemon/tidy-merged-pr.sh)。`Closes` の issue が閉じたかの確認 → 積まれていたPRの差し戻し → 本体を新しい `main` へ進める |
| `MERGE` | `通してよい` があり、緑で、コンフリクトも無く、**人の手番で止まっていない** | [`merge-pr.sh`](../scripts/daemon/merge-pr.sh)。機械の関門を通してマージするだけ |
| `ARCHIVE` | **もう誰も起こさないセッション**——作業者は担当の issue がもう自分の仕事でなくなったとき、レビューは判定を書き終えたとき（書かないまま止まれば、下の `review-stall` で1回起こしてから）。**条件は役ごとに違い、[`board-design.md`](../agent-ops/board-design.md) 2.10 が持ちます** | **セッションを畳む**（[`archive-session.sh`](../scripts/daemon/archive-session.sh)）。**役によらず同じこの手です** |
| `RESUME … mend` | `直し待ち`・CIが赤・`main` と衝突（**札が付かなかった回は、判定のコメントから読みます**。[`board-design.md`](../agent-ops/board-design.md) 2.13.7）。**CIの赤と衝突は、人の手番で止まっているPRには出しません**（同 2.13.8） | **書いたセッションを起こして直させる**（[`resume-session.sh`](../scripts/daemon/resume-session.sh)） |
...
| `RETURN` | 起こしても動かなかった `task-*` のセッション。**宛先を引けないPRを抱えた担当も**（[`board-design.md`](../agent-ops/board-design.md) 2.11.4）、**頼み終えた差し戻しが戻ってこないPRの担当も**（同 2.13.6） | issue へ `[返却]` のコメントを置き、**仕事を人へ返す**（5節）。**文面は返す形ごとに違います**——人がすることが違うので |
| `REVIEW` | 判定のラベルが無く、緑で、マージできると分かっている。**`直し待ち` で頼み終えても戻ってこないPRも**（[`board-design.md`](../agent-ops/board-design.md) 2.13.6。**緑でマージできる形のときだけ**で、赤や衝突のまま止まったものは上の `RETURN` へ回ります）、**判定が書かれても札が付かないPRも**（同 2.13.7） | [`dispatch-review.sh`](../scripts/daemon/dispatch-review.sh) |
| `UNLABEL` | 最後の判定が今の頭でない版を名乗っているのに、結論の札が残っている（[`board-design.md`](../agent-ops/board-design.md) 2.13.7） | **前の差分の札を落としてほしい**とPRへ頼む。**外すのは `board-labels.yml`**——盤面が自分で外すと、それが「人が外した」と読まれて `却下` が付きます |
...
| `CHORE` | 周期で起きる係に仕事があり、前に立ててから間隔が空いた | [`dispatch-chore.sh`](../scripts/daemon/dispatch-chore.sh)。**PRを出す係が居ても、作業者の枠には数えません**（`TASK` の行に挙がっている上限は、どちらも見ません） |
| `NOTE` | 手を打てない事情がある | `覚え書き:` の行をログへ残す。**読むのは人です** |

**機械で判定できることは、レビュアーを立てる手前で弾きます。** コンフリクトと、画面が変わるのに `## 見た目` 節が無いこと——どちらも差分と本文から機械で読めるので、**レビュアーのセッションを1本使ってから差し戻すのは無駄**です。弾いたぶんは[レビューの観点](../agent-ops/review-criteria.md)からも消してあります。両方に置くと、レビュアーが読む番には既に直っているものを挙げることになります。
...
**人の手番の印は、効き目を1つずつ持ちます。** PRに付いた `判断待ち` が止めるのは**マージだけ**、`収束せず` が止めるのは**レビューだけ**で、**どちらの下でも、差分そのものへ向いた差し戻し（`却下`・`## 見た目` の欠け・レビューの `直し待ち`）は出ます**——人の返事を待たなくても直せるからです。**出さないのは、`main` が動いて生まれたコンフリクトとCIの赤だけ**——答えを待つ間ずっと生まれ続けるうえ、直しても答えは近づきません（[`board-design.md`](../agent-ops/board-design.md) 2.13.8）。**どちらを越えるのも画面からのマージ**で、人が外せばそれは差し戻しになります（5節）。印そのものは push で全部外れ、必要なら次の周に導出し直されます。**外す走りが転んだ回は、盤面が同じ札を落としてもらいます**（`UNLABEL`。[`board-design.md`](../agent-ops/board-design.md) 2.13.7）。

### どこで走らせるかは、issue に書いてある

**セッションが立つ先は2つあります**——クラウドと、このPC（ブリッジ）。**既定はクラウド**で、issue に `env:` のラベルが付いていればそこへ投入します（[`board-design.md`](../agent-ops/board-design.md) 2.16）。
...
**判定のラベルも push で自分から消えます**（[`board-labels.yml`](../.github/workflows/board-labels.yml)）。判定は「その差分についてのもの」なので、差分が変われば偽になります。**偽になるべき状態を、自分では消えない場所に書かない**——これが盤面の作り全体を通した決め事です（[`agent-ops/board-design.md`](../agent-ops/board-design.md)）。

### 止めるのは、手綱のチェックを外すこと

**デーモン自体は止めません。** 止めるとレビューもマージも一緒に止まります。投入だけを止めたいときは、**手綱の issue のチェックを外します**（[`brake.sh`](../scripts/daemon/brake.sh) が毎周読みます）。外れている間はその種類の手だけが打たれず、残りはそのまま流れます。

**デーモンが止まっていることは、盤面にもラベルにも出ません。** 知らせる代わりに、**落ちたら起こす者を1つ置いてあります**（このPCのタスク。2節）——**この係は知らせるのではなく直します。** 人が自分で確かめたいときは [`daemon.sh`](../scripts/daemon/daemon.sh) の `status` を打ちます。

**そのタスク自体が走らなくなったときだけは、拾う者が居ません**（誰もログオンしていない間は起きません）。この口座から動かせるものが全部このPCの上にあり、**外から見ている者が1つもない**ためです。見張りを足しても、足した見張りが同じPCで走るので同じ場所へ戻ります。**ここだけは人の目が要ります**（[`board-design.md`](../agent-ops/board-design.md) 2.19.3）。
...
| 記録するもの | 置き場 | いつ読まれるか |
| --- | --- | --- |
| ユーザーの発言に含まれる判断 | `agent-ops/decisions/`（原文と、その読み取り） | **価値観を畳む係が読むとき**。セッションへは入らない |
| 進め方・設計・レビューの価値観 | [`agent-ops/policies.md`](../agent-ops/policies.md) | **セッション開始時に全文が自動で読み込まれる** |
| ゲーム内容の判断基準 | [`DesignPrinciples.md`](./concept/DesignPrinciples.md) | ゲーム内容の判断をするとき |
| 盤面を回す運用の取り決め | [`agent-ops/parallel-work.md`](../agent-ops/parallel-work.md) | 盤面を回す側だけが読む |
...
セッションは毎回まっさらです。**放っておくとばらつく所を1つずつ塞いだ結果**が、次の層です。
...
| **判断の伝わり方** | PRの `## 仮決め` 節、人へ訊く issue、記録の置き場 | 判断が**セッションの中だけに残って消える** |
...
**同じことが、盤面を回す側にも効きます。** 手で `gh` を叩く手順は、省ける確認から順に省かれます——**省かれた確認は、省かれたことすら見えません。** そこで、決まりきった並びは [`scripts/daemon/`](../scripts/daemon/)（デーモンが回すもの）と [`scripts/agent/`](../scripts/agent/)（人やセッションが自分で打つもの）に畳んであります。

| 何をするとき | 打つもの | 畳んだ中に必ず入る確認 |
| --- | --- | --- |
| 盤面を見る | [`board.sh`](../scripts/agent/board.sh) | 依存（`blockedBy`）と、もう投入したか |
| 投入する | [`dispatch-task.sh`](../scripts/daemon/dispatch-task.sh) / [`dispatch-review.sh`](../scripts/daemon/dispatch-review.sh) | issue（PR）が開いているか・空の箱で起動していないか・**指示が欠けずに届いたか** |
| 盤面を回す | [`daemon.sh`](../scripts/daemon/daemon.sh) | 8節の手・手綱・**同じ盤面へ二度打たないこと** |
| マージする | [`merge-pr.sh`](../scripts/daemon/merge-pr.sh) | 機械の関門・マージできると分かっていること |
| マージ済みのPRを片付ける | [`tidy-merged-pr.sh`](../scripts/daemon/tidy-merged-pr.sh) | `Closes` の issue が閉じたか・積まれていたPRを差し戻したか・本体の追随 |
