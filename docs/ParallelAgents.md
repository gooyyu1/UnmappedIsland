...
**向かう先は別の印（`goal:game` / `goal:upkeep`）で名乗ります**（[`board-design.md`](../.claude/board-design.md) 2.18.1）。盤面はこちらを読んで、完成へ近づける仕事を整備より先に配り、それが尽きた周に掘り起こし役を立てます。**既定はありません**——`goal:game` を持つものだけが完成へ近づける仕事で、名乗らなければ整備として並びます。**配られる issue が必ず持っていることは棚卸し役が保証します**（同 2.17.1。`kind:task` なのに `goal:` の無い issue は未整理として拾われ、印が付くまで盤面が `NOTE` で告げ続けます）。
