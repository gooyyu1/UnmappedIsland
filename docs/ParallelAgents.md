...
**分類が済んだ issue は、そのまま配られます。** 棚卸し役は `判断待ち` を付けません——**人の手番に入る印を、人が外す作業にしない**ためです（[`board-design.md`](../.claude/board-design.md) 2.13.1）。順序（`blockedBy`）を張るのも棚卸し役です——**なぜその順序かを知っているのは issue を立てた側だけ**なので、立てたセッションが本文の `## 順序` 節で申告し、棚卸し役がそれを読んで張ります（クラウドのセッションは GitHub の REST が塞がっていて張れません）。**申告の無いものに順序は作りません**——推定を許すと、触る先が重なっているだけの組まで直列に落ちます。
