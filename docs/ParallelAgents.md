...
**点線の3本は「引き金が無い」経路です**——人間が思い出して頼まないと立ちません（下の要改善点）。**司令塔自身もそこに居ます。** 残る実線の3本は、**盤面か見張りが条件を出した瞬間に立ちます。**
...
**PRを出すのは2種だけ**（作業者・相談役）で、どちらも**1セッション＝1ブランチ＝1PR**に閉じ、担当の列挙で衝突を避けます（7節）。**調査員・棚卸し役・レビュアーはリポジトリを触らない**ので、何本立てても互いの足元を壊しません。司令塔は記録を直接書きますが、**同時に1本しか走らない**ので相手が居ません。
...
**6種のうち2種は、自動化の枠組みの外に居ます。** 棚卸し役・作業者・レビュアーは投入の条件が機械で決まる（`## 未整理` に並んだ／着手できる `task` がある／PRが緑になった）のに対し、**相談役と調査員は人間が思い出して頼むまで立ちません。**
