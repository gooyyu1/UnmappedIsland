...
| ⛏️ **掘り起こし役** | 完成の定義に照らして、**まだ issue になっていない残りを数える** | **新しい issue**——やると決まっているものはそのまま、やるかどうかから訊くものは `判断待ち` を付けたチェックの一覧に（分類は棚卸し役が付ける）。**既にある issue へ書くのは、訊く一覧が二重に立っていた周のコメント1つだけ** | デーモンが [`dispatch-chore.sh`](../scripts/daemon/dispatch-chore.sh)。**配れる `goal:game` の `kind:task` が無ければ立つ** | セッション |
...
**PRを出すのは主に作業者と相談役**で、どちらも**1セッション＝1ブランチ＝1PR**に閉じます。**一次と二次の分析係と、参照を検める係も記録のPRを1本ずつ出します**が、記録だけの差分なので、**作業者の枠には数えません**（8節の `CHORE`）。**棚卸し役・レビュアー・掘り起こし役・価値観を畳む係・値の死を告げる係は作業ツリーへ書かない**ので、何本立てても互いの足元を壊しません。自己レビュー役はPRを出す側のチェックアウトをそのまま使いますが、**読むだけ**なので同じです。
...
**立つのは、配れる「完成へ近づける仕事」（`goal:game`）が尽きた周です**（[`board-design.md`](../agent-ops/board-design.md) 2.17・2.18.1）。**在庫の数では立ちません**——スメルを拾う役の入力はPRが出るたびに生えるので、「配れる `kind:task` が尽きた」で見ると、整備の issue が在庫を満たし続け、この役は立てられません。見つからない周が続いても、間隔（`CYCLES`）のぶんは空けます。渡す本文は [`dig-prompt.md`](../agent-ops/prompts/dig-prompt.md)。
