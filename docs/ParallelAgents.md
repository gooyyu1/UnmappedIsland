...
| 📊 **分析係** | マージ済みPRに残ったレビューのスメル（[`board-design.md`](../.claude/board-design.md) 4.4）を拾う | **新しい issue**（`kind:` は付けない）と、**記録のPR1本**（`.claude/analysis/<日付>.md`）。読んだコメントには 👀 を付ける | デーモンが [`dispatch-chore.sh`](../scripts/agent/dispatch-chore.sh)。**読まれていないスメルがあれば一日一回** | セッション |
...
**分類が済んだ issue は、そのまま配られます。** 棚卸し役は `判断待ち` を付けません——**人の手番に入る印を、人が外す作業にしない**ためです（[`board-design.md`](../.claude/board-design.md) 2.13.1）。順序（`blockedBy`）を張るのは人間です——張ってよいかを決めるのは、投入の列に載せるかと同じ判断だからです。
