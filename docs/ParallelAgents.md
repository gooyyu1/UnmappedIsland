...
**役を分けている基準は、やっていることではなく「何に書けるか」と「何で立つか」です。** 掘り起こし役と棚卸し役はどちらも読んで issue へ出す役ですが、**人の書いた原文を書き換えられるのは棚卸し役だけ**です（掘り起こし役が既にある issue へ触れるのは、確定待ち（[#656](https://github.com/gooyyu1/UnmappedIsland/issues/656)）へ項目を足すときだけです）。**下の表の4列目（どう走るか）は基準ではありません**——「何に書けるか」から出てくる帰結です（「立ち方は『何に書けるか』が決めている」）。
...
| ⛏️ **掘り起こし役** | 完成の定義に照らして、**まだ issue になっていない残りを数える** | **やると決まっているものは新しい issue、やるかどうかから訊くものは [#656](https://github.com/gooyyu1/UnmappedIsland/issues/656)**（分類は棚卸し役が付ける） | デーモンが [`dispatch-chore.sh`](../scripts/agent/dispatch-chore.sh)。**配れる `kind:task` が無ければ一日一回** | セッション |
...
**掘り起こし役に実装させないのは、報告と反映を同じ役に持たせないためです。** 掘り起こし役が出すのは材料で、それを配るのはデーモン、手を動かすのは配られた作業者です。棚卸し役は**既にある** issue の本文を書き換えますが、**書き換えるのは型であって中身ではありません**（原文を消させない）。**掘り起こし役が書き換える既存の issue は確定待ちだけ**で、そこへ項目を足す以外は1文字も触りません。
