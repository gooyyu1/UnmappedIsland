...
**PRを出すのは主に2種**（作業者・相談役）で、どちらも**1セッション＝1ブランチ＝1PR**に閉じます。司令塔も自分の領分でPRを出すことがあります（普段は `main` へ直接 push します）。**棚卸し役・レビュアーは作業ツリーへ書かない**ので、何本立てても互いの足元を壊しません。掘り起こし役は司令塔のチェックアウトを、自己レビュー役はPRを出す側のチェックアウトをそのまま使いますが、どちらも**読むだけ**なので同じです。掘り起こし役が収支表も作り直さずに読むのは、これが理由です（下の「探すのではなく、数える」）。司令塔は記録を直接書きますが、**同時に1本しか走らない**ので相手が居ません。
...
**司令塔は実装しません。** やるのは**issue を立てる・記録する・デーモンには決められないものを捌く**の3つで、**PRの差分も読みません**（6節）。
...
| 盤面を回す | [`daemon.sh`](../scripts/agent/daemon.sh) | 8節の6種の手・手綱・**同じ盤面へ二度打たないこと** |
