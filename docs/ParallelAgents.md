...
**層で切った領域の地図は持ちません。** 衝突を止める役は上の**担当の交わり**が担っていて、地図が別に受け持つものが残らないためです。**計算では表せないもの——同時に1本しか動かせない資源——だけがラベルの錠になります。** 条件は [`.claude/parallel-work.md`](../.claude/parallel-work.md) の「同時に走らせてよいか」。
...
| `TASK` | 書くセッションが1本も居ない | 一番古い `task` を1件投入する（**並列度1**。返されたものは配らない） |
