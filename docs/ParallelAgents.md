...
**役を分けている基準は、やっていることではなく「何に書けるか」と「何で立つか」です。** 掘り起こし役と棚卸し役はどちらも読んで issue へ出す役ですが、**既にある issue を書き換えられるのは棚卸し役だけ**です。**下の表の4列目（どう走るか）は基準ではありません**——「何に書けるか」から出てくる帰結です（「立ち方は『何に書けるか』が決めている」）。
...
flowchart LR
    ...
    Daemon -.->|"引き金がまだ決まっていない"| Digger
    Worker ==>|"PRを作る前に1本"| SelfReview
    Analysis ==>|"PRを作る前に1本"| SelfReview
    Adviser ==>|"PRを作る前に1本"| SelfReview
```

**点線は1本だけで、そこはまだ繋がっていません**——掘り起こし役を起こす引き金が決まっていないためです（[`board-design.md`](../.claude/board-design.md)「未決」）。**残りは全部、条件が出た瞬間に立ちます。** 相談役の条件は「**人間が相談したくなったこと**」で、そのとき人間はもうそこに居るので、機械の引き金を置いても待つだけになります。
...
**デーモンを起こす者だけは、デーモンの外に居ます。** 落ちれば盤面は1ミリも動かないので、そこを盤面の中の仕組みで見張ると**落ちたときに見張りも立ちません**。監視係は CCR の Routine（cron）で立ちます——**デーモンに依らずに立つ唯一の口**です（[`board-design.md`](../.claude/board-design.md) 2.19）。
...
| ⛏️ **掘り起こし役** | 完成の定義に照らして、**まだ issue になっていない残りを数える** | **やると決まっているものは新しい issue、やるかどうかから訊くものは [#656](https://github.com/gooyyu1/UnmappedIsland/issues/656)**（分類は棚卸し役が付ける） | **まだ決まっていない**（[`board-design.md`](../.claude/board-design.md)「未決」） | セッション |
...
| 🫀 **監視係** | デーモンの心拍を見て、無ければ起こす。**判断は1つもしない**（[`board-design.md`](../.claude/board-design.md) 2.19） | **どこにも書かない。** 打つのは [`watch-routine.sh`](../scripts/agent/watch-routine.sh) 1本だけで、リポジトリも issue もPRも触らない | **CCR の Routine（cron）が毎時。デーモンには依らない** | セッション |

**PRを出すのは主に作業者と相談役**で、どちらも**1セッション＝1ブランチ＝1PR**に閉じます。**分析係も記録のPRを1本出します**が、一日一回・記録だけの差分なので、**書くセッションの上限には数えません**（8節の `CHORE`）。**棚卸し役・レビュアー・掘り起こし役・価値観を畳む係は作業ツリーへ書かない**ので、何本立てても互いの足元を壊しません。自己レビュー役はPRを出す側のチェックアウトをそのまま使いますが、**読むだけ**なので同じです。
...
- **GitHubへ書く**（レビュアー・棚卸し役・分析係・掘り起こし役・価値観を畳む係）— **書いたものが記録として残り、それが次を動かす合図になります**（判定のラベル・`## 未整理`・チェックボックス）。合図を読むのはデーモンか人間で、**書いた側が生きている必要はありません**。親と運命を共にするサブエージェントには置けません。

**どこにも書かない役だけが、サブエージェントにできます。** 報告の行き先が親しかないので、渡し方を決める余地がそもそもありません。**読んだ量は子の文脈に留まり、親へ返るのは結論だけです。**

**ただし、どこにも書かなくても、親が居なければセッションです。** 監視係がそれで——見張る相手がデーモンなので、デーモンが立てる子にはできず、**親を持たない口から立つほかありません**（[`board-design.md`](../.claude/board-design.md) 2.19）。返すべき結論も持ちません。打って直して終わります。
...
**出口は分かれていて、掘り起こし役自身が振り分けます。** 全部を確定待ち（#656）へ積むと、**この役を起こしても盤面が動きません**——配れるものが1件も増えないからです。
...
**リポジトリの記録にも書く役が居ます**——相談役がユーザーから受け取った判断を `.claude/decisions/` へ、作業者が確定した仕様を `docs/**` へ（9節）、分析係が繰り返し現れる形を `.claude/analysis/` へ。
...
**理由はコメントに書けば足ります**——「−5 で」だけで構いません。足りなければ、起こされたセッションがPRのコメントで訊きます。**ただしコメントだけでは止まりません**——デーモンが読むのはラベルだけなので、**止める意思はラベルで表します。**
...
**触ってよい範囲は、どちらで走っても同じです。** 分けているのは**クラウドではできない仕事**だけ——GitHub の REST が塞がっていて**既存の issue の本文を書き換えられない**（だから棚卸し役はブリッジ）、手元の画面やこのPCにしか無いものを使う、の2つです。
...
**デーモンが止まっていることは、盤面にもラベルにも出ません。** 知らせる代わりに、**落ちたら起こす者を1つ置いてあります**（監視係。2節）——**この係は知らせるのではなく直します。** 人が自分で確かめたいときは [`daemon.sh`](../scripts/agent/daemon.sh) の `status` を打ちます。

**監視係を立てる Routine そのものが止まったときだけは、拾う者が居ません。** この口座から動かせるものが全部このPCの上にあり、**外から見ている者が1つもない**ためです。見張りを足しても、足した見張りが同じPCで走るので同じ場所へ戻ります。**ここだけは人の目が要ります**（[`board-design.md`](../.claude/board-design.md) 2.19.4）。
