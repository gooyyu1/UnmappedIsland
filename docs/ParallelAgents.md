...
sequenceDiagram
    ...
    alt 直し待ち／CIが赤／コンフリクト／見た目が無い
        ...
        alt 判断待ち が付いている
            D->>G: マージを出さない
            H->>G: 承認（【確定】の印が動いたなら画面からマージ、レビュアーが求めたならラベルを外す）／そのまま（却下）
        else 付いていない
            D->>G: マージ（人間は通らない）
        end
    end
...
**機械の線は「モデルには決めさせないもの」を名指ししたものです。** 「これは通してよい」とモデルが裁量で判断できる形にすると、**越えられる関門は越えます**。だから機械の側は、**越え方まで人間の手に固定してあります**——ラベルを外しても次の周に付き直るので、通すなら**画面からマージします**（後片付けはマージした手から切り離してあるので、それでも走ります）。レビュアーの判定のほうは、**何を判断してほしいかがコメントに書かれている**ので、ラベルを1つ外すだけで越えられます。
...
**既定はマージで、手を動かすのはほぼ止めるときだけです。** 手数の多い入口は使われなくなり、**使われない仕組みは無いのと同じ**だからです（11節）。**`判断待ち` が付いたPRだけは、通すほうにも1手が要ります**——レビュアーが付けたものはラベルを外す、機械が付けたものは画面からマージする。どちらも1手に収めてあります。**相談役との対話だけは手数が掛かります**が、そこで扱うのは**まだ形になっていない判断**なので、短くはできません。そのPRが関門に掛かるかどうかは、合意がどこへ落ちたかで決まります（2節）。
...
| 何を見るか | 誰が見るか | 越え方 |
| --- | --- | --- |
| **確定の宣言の増減**（見出しに付いた `【確定】` と、文書単位の宣言） | 機械（[`needs-user-review.sh`](../scripts/agent/needs-user-review.sh)） | **デーモンには越えられません。** `判断待ち` のラベルが付き、**ラベルを外しても次の周に付き直します**——通すなら**画面からマージします** |
...
**人の手番の印は、効き目を1つずつ持ちます。** PRに付いた `判断待ち` が止めるのは**マージだけ**、`収束せず` が止めるのは**レビューだけ**で、**どちらの下でも差し戻しは出ます**——コンフリクトやCIの赤の解消を人の返事まで待たせる理由が無いからです。機械が付けた `判断待ち` を越えるのは**画面からのマージ**で、これは差分から毎回導出される事実なので push で外れ、必要ならまた付きます。
