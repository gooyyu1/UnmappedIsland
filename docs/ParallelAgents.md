...
**層で切った領域の地図は持ちません。** 効くのはファイルの衝突にだけで、そこは上の**担当の交わり**が既に見ています。**文書には効きません**——文書は主題で並ぶので、層で切った担当とは軸が直交します。担当に文書まで書かせれば、文書の衝突も同じ交わりの計算に乗ります。**同時に1本しか動かせない資源**（GPUを専有する画像生成、動かしながら書き換えられないデーモン）だけは計算では表せないので、そこにはラベルで錠を掛けます。詳細は [`.claude/parallel-work.md`](../.claude/parallel-work.md) の「同時に走らせてよいか」。
...
**人の手番の印は、効き目を1つずつ持ちます。** `判断待ち` が止めるのは**マージだけ**、`収束せず` が止めるのは**レビューだけ**で、**どちらの下でも差し戻しは出ます**——コンフリクトやCIの赤の解消を人の返事まで待たせる理由が無いからです。`判断待ち` を越えるのは `merge-and-close.sh <PR> --user-ok` だけで、これは差分から毎回導出される事実なので push で外れ、必要ならまた付きます。
