...
**0 なのは標本のせいではありません。** 鎖の入口はすべて、生成が必ず置く土地から採れます——繊維
（バナナの株）・丸太（広葉樹）・生皮（サル・イノシシ）・小骨（ヤケイ）は森から、石は山頂から、
太い枝は海岸帯の3種のどれからも採れ、森と山頂は `terrain_generation.yaml` の `guarantees` が1つずつ
置き、海岸帯のサイトは `hull_coast` が必ず作ります。
