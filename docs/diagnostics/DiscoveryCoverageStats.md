...
| `island_missing_any` | 書き方（`written_as`）ごとに、契機が1つでも消えた島の割合 |
...
`island_missing_object` が前者の取りこぼし、`island_missing_tag` が後者の取りこぼしで、
`island_missing_any` がその2つを島の側から数えたものです。
