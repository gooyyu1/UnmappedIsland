...
見るのは **YAMLの1つのセルを、丸めだけを挟んで書き写した数値**と、**条件で絞った複数のレコードを
畳んだ数値**（下の「節から出る幅は、畳み方を印に書く」）です。**複数のセルを掛けた・引いた・裏返して導いた数値には付けません**
——導出は書き写しではなく文書の主張で、式を印に書けるようにすると同じ計算が文書と生成器の2箇所に
...
「この節から出る幅」——条件で絞ったレコードの最小・最大・その差——は、**導出ではなく生成物の読み方**
なので印に書けます。読む列の次、粗さの手前に畳み方を置きます。
