...
——実際、`1 - barren/100` で裏返した「実り」も、海抜へ気温減率を掛けた列も、直上の数を丸めた幅も、
出どころが動いた周をそのまま越えました（[issue #2163](https://github.com/gooyyu1/UnmappedIsland/issues/2163)）。

採れる道は2つで、**どちらを採ったかは印の有無でその場で読めます。**

- **導出を生成器へ移して、1つのセルにする。** 文書はそれを書き写すだけになるので印が置けます。
  計算が生成器の1箇所に在るので、入力が動けば値も一緒に動きます（`terrain.yaml` の
  `daily_budget.day_trip_one_way` がこの形）。
...
日本語で持つのは、**生成物の列名と紛れないため**（列名はどれもASCII）。
