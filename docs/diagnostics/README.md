...
効きません——``約`4,200`分`` のように囲んだ中に書いた数も、印の直前の数として読みます。
...
`4,614.8` を「約4,600分」とは書けません。この既定のままだと、定義を触るPRが**結論に効かない下位桁
まで書き直す義務**を負い、同じ行を触るPRどうしがぶつかります。

`±` を足すと、**書いた数を中心とした幅**の中にセルがあれば緑になります。幅の書き方は次のとおりで、
どちらも「どれだけ粗く書いたか」の1点だけを言います。

| 粗さ | 見方 | 「4,600」と書いたとき緑になる範囲 |
|---|---|---|
| （書かない） | 書いた桁へ丸めて一致 | 4,599.5以上 4,600.5未満 |
| `±100` | 出どころと同じ単位で±100 | 4,500〜4,700 |
| `±5%` | 書いた数の±5% | 4,370〜4,830 |

既定の行だけ上端が入らないのは、それが幅ではなく**丸めの規則**そのものだからです（`4,600.5` は
`4,601` へ丸まります）。

```text
4桁の労働がかかるのは筏（約4,600分<!-- stats: balance.yaml object_costs object=raft total_minutes ±5% -->）
```
