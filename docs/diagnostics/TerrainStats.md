...
```
その日その土地で進む仕事（分）
  = min(outdoor_window − 往復の移動 − survival_gathering, その土地で採れる時間)
```
...
- **`daily_budget.survival_gathering`** = 1日を賄う生存の採取（BalanceStats.mdの最小労働から
  睡眠を引いた分）。
- **その土地で採れる時間** = ClimateSystemStats.md「土地×季節ごとの活動時間」の `gathering` の
  季節平均。遠さは移動の項として、暗さは頭打ちとして、同じ1行に入る。
...
GameEndings.md 9.2節）。1日に進む探索がその土地で採れる時間を超えられないこと（探索も見て探す仕事
なので、採取と同じ列を見る）と、往復の移動も屋外の枠から出ることの2つだけで縛る。
