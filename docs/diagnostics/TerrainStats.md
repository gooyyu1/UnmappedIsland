...
`objects` が量の出どころ。**型の名前の並び**ならそれらを1つずつ手に入れる総労働の合計
（BalanceStats.md の `object_costs`）、**`null`** なら置いた日数（仮置き）。**同じ型が2回以上並ぶのは、
積んで段へ届かせる山**——里心を止める設えのように、タグを名乗る型を並べて段へ届かせる山は、
いちばん安い積み方がそのまま並ぶ。
...
**泊まりがけは遊びの側には入れない**（ContentSkeleton.md 8.3.1節）。どちらも残っているのは、
