...
（`IslandSpawner.placePlayer`）なので、砂浜の行がそのまま今の立ち上がりになる。**砂浜が1つも無い島
では外周の海岸へ落ちる**ので、その島の立ち上がりは岩海岸・岸壁の行のほうを読む（砂浜の出る島の
割合は`stats/terrain.yaml`の`location_type_counts`）。
