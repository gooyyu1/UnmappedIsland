...
- **道は未発見でも数える。** 判定するのは島の作りであってプレイヤーの進み具合ではない。
  ...
  **道が全部出そろうまで**の時間の和（時刻表は`src/analysis/pathDiscovery.ts`が、実体化する側と
  同じ割り当てから引く）。**着いた先の探索は含まない**（そこで目当ての物を引くまでの回数は引きの運）。
  **上限を採るので、実際に要る時間はこれより短い**——通り抜けるのに要るのは目当ての1本だけで、
  その土地の最初の道は2回目の探索で出る（TerrainStats.md「道が見つかるまで」）。
...
選抜を入れた後との差は`island_start_site`との比較で読む。**砂浜が1つも無い島では候補が外周の海岸へ
落ちる**ので、その島の開始地点は岩海岸・岸壁の行のほうに現れる（砂浜の出る島の割合は
`stats/terrain.yaml`の`location_type_counts`）。

## 島ごと

`island_`で始まる節は、その島で**選抜が選んだ開始地点**の値。島は引き直さないので
（ContentSkeleton.md 2.3.1節）、引きの厳しい島でもここから始まる——**この分布の
