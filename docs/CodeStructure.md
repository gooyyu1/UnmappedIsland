...
|---|---|---|---|
| **世界** | 何が在り、何が起きるか。**プレイヤーに見えていない土地・物も含む** | 画面のことば全部・解析 | `src/domain/`（`src/loader/` が定義を読み、`src/locale/` がことばを与える） |
| **映し** | 世界のうち、**プレイヤーに映っている分**。今の断面と、遅れて見せる変化 | Phaser・矩形・ミリ秒・解析 | `src/game/view/` |
| **意匠** | それを**どんな色・絵・寸法で**見せるか | 世界の状態・Phaser・解析 | `src/game/looks/`（色・寸法・領域・時間の見せ方） |
| **素材** | **どのファイルがどの絵か** | 画面の寸法・色・Phaser・解析 | `src/art/` |
| **汎用部品** | Phaser の足りない分・間違っている分を埋める表示物 | **このゲーム**——語彙も寸法も配色も。解析 | `src/ui/` |
...
| **層の外** | **層のどれでもないコード** | Phaser・解析 | 横断の道具（`src/util/`・`src/game/errorReport.ts`）、保存と起動時の入力（`src/save/`・`src/scenario/`・`src/game/launchSeed.ts`）、外から材料を受け取る口（`src/asset-pack/`。[`AssetPack.md`](./engine/AssetPack.md)） |
...
`解析` の行自身を除く残り8行はどれも知ってはいけません（5節）。

**層の外**の3つは性質が違います。**それでも同じ行に居るのは、どれも「何を知ってよいか」で位置が
決まらないから**であって、似ているからではありません。
...
**ノードは1節の表の10行そのもの**で、食い違っていないことは自動テストが数えます
...
**組み立ては層ではありません。** どの部品をどこへ置き、映しの答えを誰へ渡すかを決める場所で、
**解析を除く全部の層を知ってよい唯一の場所**です（置き場は1節の表）。起動（`src/main.ts`）も
組み立ての一番外側で、別の層ではありません——知ってよい範囲が組み立てとまったく同じだからです。
層として並べると一番上に太いものが乗っているように見えますが、上下ではなく結線です。

**組み立てと層の外を層と呼ばないのは、層どうしを分けている「何を知ってよいか」で位置が決まらないから**
です。組み立ては解析を除く全部の層を知ってよく、層の外はどの層からも使われます。どちらも並びの中に
居場所がありません。
