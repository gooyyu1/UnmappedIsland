...
|---|---|---|---|
| **世界** | 何が在り、何が起きるか。**プレイヤーに見えていない土地・物も含む** | 画面のことば全部・解析（5節） | `src/domain/`（`src/loader/` が定義を読み、`src/locale/` がことばを与える） |
| **映し** | 世界のうち、**プレイヤーに映っている分**。今の断面と、遅れて見せる変化 | Phaser・矩形・ミリ秒 | `src/game/view/` |
| **意匠** | それを**どんな色・絵・寸法で**見せるか | 世界の状態・Phaser | `src/game/looks/`（色・寸法・領域・時間の見せ方） |
| **素材** | **どのファイルがどの絵か** | 画面の寸法・色・Phaser | `src/art/`（絵の実体は `src/assets/`） |
| **汎用部品** | Phaser の足りない分・間違っている分を埋める表示物 | **このゲーム**——語彙も寸法も配色も | `src/ui/` |
| **部品** | Phaserの表示物を**作る・持ち続ける・触らせる** | 世界の語彙（レシピ・スロット名・プロパティ名） | `src/game/ui/` |
| **解析** | 宣言を**どう読むか**。答えは必ず近似になる（5節） | 効果・条件の木そのもの（読み上げ口だけを使う） | `src/analysis/` |
| **組み立て** | どの部品をどこへ置き、映しの答えを誰へ渡すか | **無し**——全部を知ってよい唯一の場所（2節） | `src/main.ts`・`src/game/DeviceScreen.ts`・`src/game/*Scene.ts`・`src/game/ui/*Window.ts` |
| **層の外** | どこからでも使う横断の道具 | Phaser（`src/save/`・`src/scenario/`・`src/util/` は自動テストが見張る） | `src/game/errorReport.ts`・`src/save/`・`src/scenario/`・`src/util/` |
...
矢印は「**知ってよい**」向きで、破線の枠は層ではないものです。`層の外` はどこからでも使えるので、
入ってくる矢印は描いていません——制約があるのは出ていく先だけです。
...
**組み立ては層ではありません。** `src/main.ts`・`DeviceScreen`・`PlayScene`・各 `Scene`・各 `Window` が
これにあたり、**全部の層を知ってよい唯一の場所**です。起動（`src/main.ts`）は組み立ての一番外側で、
別の層ではありません——知ってよい範囲が組み立てとまったく同じだからです。どの部品をどこへ置き、
映しの答えを誰へ渡すかを決めます。層として並べると一番上に太いものが乗っているように見えますが、
上下ではなく結線です。**層と呼ばないのは、表の「知ってはいけないもの」が空だから**です——制限が
無いものを層として並べても、区別を何も述べていません。

**この境界は自動テストが見張ります**（`tests/architecture/layers.test.ts`）。世界・映し・意匠・素材から
Phaser へ、また汎用部品（`src/ui/`）からこのゲームへ、import で到達できないことを、間に何本
...
- **部品が汎用か、このゲームのものか**: **このゲームを消しても1文字も変わらないなら汎用**
  （`src/ui/`）。名前で決めません——`Button` は汎用に見えて、押されたときの濃さを意匠から直に
  引いています（`COLOR.pressedShade`）。寸法・配色・ゲームの語彙のどれか1つでも抱えていれば、
  このゲームの部品です。
