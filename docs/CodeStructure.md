...
| **素材** | **どのファイルがどの絵か** | 画面の寸法・色・Phaser・解析 | `src/art/`（絵の実体は `src/assets/`） |
...
これにあたり、**解析を除く全部の層を知ってよい唯一の場所**です。起動（`src/main.ts`）は組み立ての
一番外側で、別の層ではありません——知ってよい範囲が組み立てとまったく同じだからです。どの部品を
どこへ置き、映しの答えを誰へ渡すかを決めます。層として並べると一番上に太いものが乗っているように
見えますが、上下ではなく結線です。
