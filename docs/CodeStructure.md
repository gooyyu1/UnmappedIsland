...
- **汎用部品か部品か**: **このゲームを消しても1文字も変わらないなら汎用部品**。名前で決めません
  ...
  **見るのはクラス自身が参照しているものだけです。** `Button` と同じファイルの
  `addTextButton`（`src/game/ui/Button.ts`）は、押されたことをゲームの操作ログへ書き出しますが
  （`noteOperation`・`uiText`）、`Button` はそれを呼びません。同じファイルに居ることも、ファイルの
  頭に並ぶ import も、そのクラスがゲームを知っている証拠になりません。
...
分かれ道の**受け方は、読み上げ（`PickReading`・`ConditionalReading`）自身が読ませる口として持ちます**
