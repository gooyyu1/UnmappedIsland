...
重みの無い二択の**受け方は、読み上げ（`ConditionalReading`）自身が読ませる口として持ちます**
（`conditional` の説明から辿れます）。読み手は自分の問いに合う口を呼ぶだけです——「それが起こりうるか」を
問うなら両方の枝を読ませ、「何がどれだけ起こるか」を問うなら枝を1つずつ受けて、著者が書いた枝だけを
数えます（どちらの枝かは枝自身が名乗るので、読み手が枝を見比べる必要はありません）。
