...
- 解析がすること: 重みを確率へ直し、分岐を直積で畳み、値域の端を割ったかを判定し、重みの無い二択
  （条件つきの効果）をどちらの枝で読むか選ぶ。

重みの無い二択の**受け方は名前の付いた選択肢**（`EffectReader.ts` の `everyBranchOf`・
`authoredBranchOf`）で、読み手は自分の問いに合うほうを名指しで選びます——「それが起こりうるか」を
問うなら両方の枝、「何がどれだけ起こるか」を問うなら著者が書いた枝。**選び分けの理由はその2つの
説明にだけ**あり、受け口ごとに書き下ろしません（読み手が増えたときに倣う先が要るため）。
