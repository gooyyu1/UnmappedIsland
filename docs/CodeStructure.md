...
重みの無い二択の**受け方は、`EffectReader` が名前を付けて持ちます**（`conditional` の説明から辿れます）。
読み手は自分の問いに合うものを名指しで選びます——「それが起こりうるか」を問うなら両方の枝、
「何がどれだけ起こるか」を問うなら著者が書いた枝。**選ぶのは解析に限りません**——ビューアも遊びの本体も
同じ選択肢から選びます。**どれが自分の問いに合うかは選択肢の側が説明する**ので、受け口には自分の問いを
書くだけで済みます（読み手が増えたときに倣う先が要るため）。
