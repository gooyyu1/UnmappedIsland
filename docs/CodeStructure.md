...
- **意匠に置くか、部品の中に置くか**: **色は、使う部品が1つでも意匠（`looks/theme.ts` の `COLOR`）へ
  ...
  **この決めは自動テストが見張ります**（`tests/architecture/colorPlacement.test.ts`）——`src/game` の
  意匠の外に色の字面が1つでも在れば落ちます。汎用部品（`src/ui`）は意匠へ到達できない層なので見ません
  （既定は起動時に外から入ります）。**外れるのは実行時エラーの報告**（`src/game/errorReport.ts`）だけ
  です——壊れた画面の上へDOMで重ねる帯なので、画面の配色の一部ではなく、むしろ画面から離して
  読ませます。
