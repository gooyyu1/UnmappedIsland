...
**解析と codex ビューアは、この2行を除くどこからも見えません。** 解析を知ってよいのは codex ビューア
...
できないことを、間に何本挟まっていても辿って確かめます。契約を**型として**輸入するぶんを数えないのは、
**Phaser が付いてくるかを見ている境界だけ**です（型はビルドで消えます）。**知っていること自体を禁じて
いる境界**では型も数えます——汎用部品がこのゲームの型を1つでも署名に置けば、そこはこのゲーム抜きでは
読めなくなるからです。
...
- **意匠に置くか、部品の中に置くか**: **色は、使う部品が1つでも意匠（`looks/theme.ts` の `COLOR`）へ
  ...
  **この決めは自動テストが見張ります**（`tests/architecture/colorPlacement.test.ts`）——`src/game` の
  意匠の外に色の字面（**6桁ちょうどの16進**と、引用符の中の `#` 記法・`rgb()`・`hsl()`）が在れば
  落ちます。桁の揃わない16進（`0xfff`）は見ていません——寸法の綴り（`410x640px`）と見分けが
  付かないからです。汎用部品（`src/ui`）は意匠へ
  到達できない層なので、この決めが掛かりません。**層の外の実行時エラーの報告**
  （`src/game/errorReport.ts`）も同じで、壊れた画面の上へDOMで重ねる帯は画面の配色の一部ではなく、
  むしろ画面から離して読ませます。
...
- **汎用部品か部品か**: **このゲームを消しても1文字も変わらないなら汎用部品**。名前で決めません
  ——`TextInput`（`src/game/ui/TextInput.ts`）は汎用に見えて、欄の枠線・地・書体を意匠から直に
  引いています（`COLOR`・`SIZE`・`FONT_FAMILY`）。寸法・配色・ゲームの語彙のどれか1つでも抱えて
  いれば、部品です。
  **見るのはそのもの自身が参照しているものだけです。** `src/game/ui/WeatherOverlay.ts` の
  `seededRandomNumbers` は種から乱数列を作るだけで、同じファイルの `WeatherOverlay` が引く意匠も
  雨の見せ方も読みません。同じファイルに居ることも、ファイルの頭に並ぶ import も、そのものが
  ゲームを知っている証拠になりません。
...
`describe(names)`・`readBy(reader)`・`SlotDef.cellCountPolicy`）。画面が増えれば口も増えますが、増えるのは口の実装であって、
...
持ちます**——ここへ書き写すと、一致を誰も検査しない2つ目の一覧が増えるので置きません。直下のまとまりが
増減したらそこが落ちるので、**宣言と実体で分けるにせよ、話題をもう1つ増やすにせよ、決めごとのほうを
先に直すことになります。**
...
日本語が折り返されない、`pointerup` が押し始めた場所と無関係に届く——**間違っている分を埋めるもの**は、
Phaser をやめれば役目ごと消えます。**足りない分を埋めるもの**は残り、次の土台もそれを持っていなければ
同じものを書き直すことになります。
...
使う物（武器）の値、条件つきの増減が同時に成立するか、抽選の重みを確率と見なしてよいか。
どれも定義の側は答えを持っておらず、**「そう置いた場合」を決めた側だけが答えられます**。

その仮定をドメインへ置くと壊れます。定義クラスは本来の宣言より解析のほうが長くなり、
...
**越えるのは問いの形だけで、答えは越えません。** 「この起点のこのプロパティは何か」という問いは
ドメインが持ち（`ReferenceValueResolver`）、世界が在る場面では `ReferenceContext` が今の実効値で、
オブジェクトが1つも無い場面では解析の解決器が近似で答えます。だから宣言の読み方——所要時間の端数を
どう落とすか、宣言が無ければ何分か——は宣言を持つ側（`InteractionDef.minutesFor`）に1つだけ在り、
解析はそこへ自分の答え手を渡して引きます。**解析だけの都合**——生成時のロールのどちらの端で読むか
（`StaticValueResolver` の `end`）——は、端を決めた後の問いになってから渡るので、境界を越えません。
...
- 解析がすること: 重みを確率へ直し、分岐を直積で畳み、値域の端を割ったかを判定し、重みの無い二択
  （条件つきの効果）を、量を問うときは著者が書いた枝で代表する。

分かれ道の**受け方は、読み上げ自身が読ませる口として持ちます**
（`pick`・`conditional` の説明から辿れます）。読み手は自分の問いに合う口を呼ぶだけです——「それが
起こりうるか」を問うなら分かれた先を全部読ませ、「何がどれだけ起こるか」を問うなら1つずつ受けて、
重みで畳むか、著者が書いた枝だけを数えます（どちらの枝かは枝自身が名乗るので、読み手が枝を見比べる
必要はありません）。
**呼ぶのは解析に限りません**——ビューアも遊びの本体も同じ口を呼びます。**どれが自分の問いに合うかは口の側が
説明する**ので、受け口には自分の問いを書くだけで済みます（読み手が増えたときに倣う先が要るため）。

読み上げのメソッドは動詞ごとに分かれていて、`readBy` は抽象です。**動詞を1つ足したときに読み手が
...
**評価**（`evaluate`）までできてしまいます。渡すのは `readBy` だけを持つ宣言
