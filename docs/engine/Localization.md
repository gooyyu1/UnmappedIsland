...
**「その型が自分で言い換えたか」を問う引き方もあります**（`ObjectTexts.renamedInteractionName`）。
`default` を見ずに自分のエントリだけを読み、書いていなければ答えません。探索のタブの見出しが
これで、海区だけが「見張り」になり、言い換えていない型は画面の既定語のままです
（[`Windows.md`](../ui/Windows.md) 5 節）。

## 書式のプレースホルダは名前で書く

差し込みのある書式（次のもの）は、`{base}` のように**名前で**書きます。TypeScript には組み込みの
...
`display_name` はスロットの名前です。**持ち主は込めません**——「マルコの装備」ではなく「装備」。
持ち主を名乗るのは名前を出す側で、子ウィンドウのタブなら窓の見出しがそれに当たります
...
`src/locale/uiTexts.ts` の `UI_TEXT_NAMES` に並びます——「この語をlocaleから消したら何が変わるか」を
1箇所で答えられるようにするためで、`WorldVocabulary` と同じ考え方です。名前と同梱の対応表が
過不足なく一致することは自動テスト（`tests/world-codex/bundledLocale.test.ts`）が見ます。

差し込みのある語は、他の節と同じく `{name}` の書式で書き、引くときに値を添えます
（`locale.uiText('survived_days', { days: '3' })`）。
...
**画面へ出る語をコードに直接書かないこと**は自動テスト（`tests/locale/uiTextLiterals.test.ts`）が
見ます。`src/game/` の文字列リテラルに日本語の字があれば落ちるので、外れるのは
「画面のラベルではない」と言い切れるものだけです（例外はそのテストが理由付きで持ちます）。

**語をつなぐ書き方もここが持ちます。** `、`（区切り）・`（）`（言い添え）・`〜`（幅）は言語をまたいで
同じではなく、英語なら `A, B`・`X (Y)`・`3-5` と、字も前後の空白も変わります。語だけを移して
つなぎ方をコードへ残すと、対応表を差し替えてもそこだけ日本語の組み方が残るため、検査は全角の記号も
見ます。

対応表を入れる前に描かれるものは、対応表の名前（`close`）がそのまま出ます——出るのは、対応表
そのものを読めずに終わった起動だけです。
...
- 例外を投げるときの文言は対応表を引きません。**読むのは開発者**（エラー報告にそのまま載る）なので、
  画面のラベルとは扱いを分けます。
