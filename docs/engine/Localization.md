...
## notes: 画面が型ごとに出す補足の1行

画面が**その型について言う 1 行**を、場面の名前ごとに書きます。今読んでいるのは探索のタブの
バーの下の 1 行（[`Windows.md`](../ui/Windows.md) 5 節）だけで、場面の名前（`exploring`・`explored`）は
画面の側が決めます。
...
object_texts:
  default:
    notes:
      exploring: 探索を続けると、アイテムや他の土地へ続く道が見つかる。
      explored: この土地に隠された道はすべて見つけた。探索を続ければ、まだ何かは見つかる。
  coastal_waters:
    notes:
      exploring: 見張りを続けると、次の海区へ続く航路が見つかる。   # 海で見つかるのは航路だけ
```

**`variation_names` と同じく `default` エントリを参照します。** ほとんどの型が同じ文でよく、言い方を
変えたい型だけが自分のエントリで上書きします。**画面に文を持たせない**のは、同じ 1 行が指すものが型で
変わるためです——探索で見つかるのは、土地なら道とアイテム、海区なら航路です
（[`Voyage.md`](../world/Voyage.md) 3 節）。

その場面の文がどちらのエントリにも無ければ、場面の名前がそのまま出ます（対応表の他の節と同じ
扱いで、書き忘れがその場に出ます）。
