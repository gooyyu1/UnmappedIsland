...
**2 とします。** 下げると行き止まりが消え、上げると島の端まで日帰りで届かなくなります。どちらの側も
測って出ます（500シード）。
...
- **上から**: 道が減るほど最短経路は伸びます。3 まで上げると、拠点から最も遠い土地への往復が1日の
  屋外の枠に収まらない島が出始め、5 では数十島に1つがそうなります。これは
  [`ContentSkeleton.md`](../world/ContentSkeleton.md) 8.3.1節の**確定**（島は端から端まで日帰りで
  届く）が破れることなので、**上限は 2.5 です**。

往復の実測は [`stats/terrain.yaml`](../../stats/terrain.yaml) の `base_farthest_round_trip`
で、破れたときに落ちるのは `tests/generation/terrainGenerator.test.ts` の
「どの島でも、拠点から最も遠い土地まで日帰りで往復できる」です。

**2 と 2.5 のどちらでもよい**ところを 2 に置いているのは、日帰りの側へ余裕を多く残すためです。
