...
（85〜95%）の間に隙間があるので、内陸のサイトは海岸帯へ届きません。**`interior_bias` は海岸の数には
効きません**——0から1まで動かしても、海岸帯に入る土地の割合も、道1本あたりの移動時間も、ほとんど
変わりません（島ごとの平均で2分ほど）。ベストキャンディデート法が、半径の分布より先にサイト同士の
間隔で候補を選ぶためです。**値は 0.6 のままとします**——動かす先に差が無いので、動かす理由もありません。
