...
**どの保証も `hard_limits` を満たす `Site` を得られる配り方が在るなら、必ずそれを採ります。** 保証を 1 件ずつ
順に見て、そのたびに候補の先頭から固定すると、先に見た保証が取った `Site` のせいで、後の保証だけが
`hard_limits` を満たさない `Site` へ回ります（最高標高の `Site` を山頂が取り、そこにしか置けない火口が
山腹へ降りる）。そうならないよう、**取ると後の保証が置き場を失う候補は飛ばします**（当てられる数は
二部グラフの最大マッチングで数えます。材料の割り当てと同じ形で、`GameElementDefinition.md` 13.1 節）。
それでも置き場の無い保証だけが、`hard_limits` を満たさない `Site` へ降ります。

**取り合いになったときは、先に宣言した保証が優先します**（宣言順は、書き手が決められる唯一の順序です）。
どちらが取っても両方が成り立つなら、先に宣言したほうが `pick` 側の `Site` を取ります。優先が効くのは
上の「置き場を失わせない」範囲までで、**先の保証の望みより、後の保証が置けることが先**です。
...
2. **内陸**: 残りのサイトを、半径75%以内へベストキャンディデート法（Mitchell's best-candidate algorithm）で
   散布します。候補は枠へ面積あたり一様に引き、既存のどのサイトからも最も離れた候補を毎回選ぶことで、
   サイト同士が均等に散らばります。Poisson-disk sampling ではなくこの方式なのは、生成するサイト数を
   直接指定できるためです（Poisson-disk は半径と面積から結果的な個数が決まるサンプラで、個数を
   指定できません）。

**海岸の数を決めているのは、この枠の分離そのものです。** 内陸の枠（半径75%以内）と外周リング
（85〜95%）の間に隙間があるので、内陸のサイトは海岸帯へ届きません。

**内陸のサイトを中心へ寄せる調整弁は、宣言に持ちません。** 候補の半径分布を中心寄りへ歪めても、
採るのは「既存のどのサイトからも最も離れた候補」なので、**寄せたぶんは間隔の側で戻されます**
——採られたサイトの半径は、候補の半径ほどには動きません。端から端まで振っても、海岸帯の土地の
割合は動かず、道の網目も島の顔が変わるほどには動きません。動くのは**どの型の土地が島に出るか**の
ほうで、
これは「中心へ寄せる」という名前からは読めない効き方です。名乗った働きが名乗りに見合わず、見合う
働きは名前から読めない弁になるので、置きません（実測は
[`DesignNotes.md`](../DesignNotes.md)「地形生成」）。内陸の散布を中心寄りへ振りたくなったら、
候補の分布ではなく**ベストキャンディデート法が候補を採る基準の側**を変えます。
...
- **下から**: 1.5 では行き止まりを持つ島が半分を切り、1.2 ではほぼ消えます。行き止まりが無い島は
  ...
  「難易度は、数値ではなく配置の散らばりで作る」）。2 では島の9割以上が行き止まりを持ちます。
  **測った刻みの中で、ここを満たすいちばん下が 2** で、上げるほど行き止まりは増えます
  （1.5 と 2 の間は測っていないので、**2 が下限だとは言えません**）。
- **上から**: 道が減るほど最短経路は伸び、いずれ拠点から最も遠い土地への往復が1日の屋外の枠に
  収まらなくなります（[`ContentSkeleton.md`](../world/ContentSkeleton.md) 8.3.1節の**確定**が破れる）。
  **ただし今の消耗では 8 倍まで上げてようやく届かない島が出ます**。

**上の線は生存の採取の量で動きます。** 往復に使えるのは屋外の枠から1日を賄う採取を引いた残り
（`stats/terrain.yaml` の `daily_budget`）なので、**採取が重くなれば上限は下りてきます**——採取が
枠の3分の1を占めるほどになると、上限は 2.5 あたりまで下ります。**この節の 2 は下の線から出ているので、
そこが動いても揺れません。**

往復の実測は [`stats/terrain.yaml`](../../stats/terrain.yaml) の `base_farthest_round_trip`
で、上の線を割ったときに落ちるのは `tests/generation/terrainGenerator.test.ts` の
「どの島でも、拠点から最も遠い土地まで日帰りで往復できる」です。
...
**代償は測ってあります。** 海岸帯のサイトが選べる型が3つから2つへ減るぶん、`max_sites_per_type`
（3.4 節）を守るとどの型も選べないサイトが出て、同じ海岸の型が上限より1つ多く並ぶ島が出ます
（500 シードのうち 2.2%、超えるのは常に1つまで）。3.4 節のとおり上限は絶対の条件ではないので、
これは仕様どおりの逃げ道です。**逃げ道が常用されていないこと**は
`tests/generation/terrainGenerator.test.ts` の「同じ地形が並びすぎず、上限を外れるのは例外に留まる」が
見張ります。

型ごとの海抜の実測は [`stats/terrain.yaml`](../../stats/terrain.yaml) の `site_elevation_by_location`
で、線が消されたときに落ちるのは同ファイルの「砂浜と岸壁は、同じ高さには出ない」です。
