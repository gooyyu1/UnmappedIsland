...
[Locationネットワーク] (距離・移動時間つきパスで接続)
   ↓ LocationType.display_name（同型が複数ならname_pool）で命名
[Location群] (命名済み)
...
**構造物内部の生成は未実装です**（3.7 節・6 節）。島の生成と構造物内部の生成は、同一の生成ロジックを
...
3. 複数あるなら、その型の `name_pool` をシャッフルして 1 つずつ配ります（例: 「花咲く草原」）。
4. `name_pool` が足りなければ、残りに漢数字の接尾辞（「草原（第四）」…）を付けます。名前として読めないので、
   プールは想定される個数ぶん用意します。

**名前に島のどこに在るかを出しません。** 地形の把握はプレイヤー自身の仕事なので、位置が分かる修飾語を
付けると、行き先の名前を見ただけで島の形が割れてしまいます。`name_pool` に入れる語も同じ制約に従います。
...
入口というプレースホルダで、内部の子 `Location` グラフの生成は今後の課題です。
