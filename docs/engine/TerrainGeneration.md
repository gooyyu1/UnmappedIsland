...
採るのは「既存のどのサイトからも最も離れた候補」なので、**サイト同士の間隔は分布より先に決まり**、
道の網目も海岸帯の土地の割合も動きません。動くのは、半径がそのまま `elevation`（距離場が重みの
大半）へ効くことで変わる**どの型の土地が島に出るか**のほうで、これは「中心へ寄せる」という名前
からは読めない効き方です。名乗った働きは出ず、出る働きは名前から読めない弁になるので、置きません
（実測は [`DesignNotes.md`](../DesignNotes.md)「地形生成」）。内陸の散布を中心寄りへ振りたく
なったら、候補の分布ではなく**ベストキャンディデート法が候補を採る基準の側**を変えます。
