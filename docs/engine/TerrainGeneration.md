...
採るのは「既存のどのサイトからも最も離れた候補」なので、分布より先に間隔の側が効き、島はほとんど
変わらないためです（実測は [`DesignNotes.md`](../DesignNotes.md)「地形生成」）。内陸の散布を
中心寄りへ振りたくなったら、候補の分布ではなく**ベストキャンディデート法が候補を採る基準の側**を
変えます。
