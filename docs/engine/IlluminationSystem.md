...
**守備範囲**: 明るさをどう持ち、行動の可否をどう決めるか。単位（EV）・行動が見る明るさ（手元・
...
繰り返しません。風雨で屋根の下でない場所での行動ができないことは明るさでは表さないので（同 8.1節）、
それを表す値（風速）としきい値も本書の外です（同 8.1.5節）。**火の暖房効果も本書の外です**——炉は明るさと同じ
...
[`GameElementDefinition.md`](./GameElementDefinition.md) 6.3節）・`base`（同 6.5節、値の土台。本書 6.1節）・
`modify`（同 8.3節）・`stages`（同 6.4節、しきい値の境目。本書 8節）・`conditions`（同 14節）だけで
組み立てます。計算式を持ち込む `derived`（同 17節）は使いません。

**在処**: 定義は `src/assets/world-codex/` の `core.yaml`（world の環境光と、場所が共通で持つ明るさ）・
`locations.yaml`（土地ごとの樹冠と反射、移動と探索の条件）・`voyage.yaml`（海区・本土・筏）・
`fire.yaml`（炉と松明）・`characters/player_character.yaml`（キャラクタの明るさ）。

**確定度と実装状況**: **確定しているのは5.1節（火の段取りは日が暮れるまでに済ませる）と6.1節（値の
...
## 2. 行動が見る明るさと、それを導く環境光

**行動の可否を決めるのは、次の明るさだけです。**
...
宣言は、オブジェクトの木（world → 土地 → キャラクタ → 持ち物）を下りながら育ちます。
...
**場所の明るさを配るのは `location` trait**（`core.yaml`）で、土地だけでなく海区・本土・筏（`voyage.yaml`）も
同じものを持ちます。**筏が持たなければ、乗り込んだ瞬間に明かりの継ぎ目が切れます**——場所を名乗る物が
...
**在るのは松明と炉だけです**（`fire.yaml`。炉は火が生きている間、火力の段によらず同じ明るさ）。
松明の灯し方は [`FireSystem.md`](./FireSystem.md) 3.1節・3.1.2節、1本で何が開くかは
[`ContentSkeleton.md`](../world/ContentSkeleton.md) 8.1.1.4節。
...
環境光と手元の明るさを分けて持つのは、この順序と、3節の据え付け光源の隔離との、独立した理由からです**
——どちらか一方をやめても、もう一方が同じ分け方を要求します。
...
**条件の側には数字を書きません。** しきい値を持つのはキャラクタ側の `stages` だけで（8節）、
...
（探索・草を刈って採る・木に登って実や葉を採る・木を伐る・海を見張る・銛で突く・海鳥を捕る）と、
陸の道を歩くこと。
手が覚えている粗い動作（食べる・くべる・石を積む・火種を落とす・焼け石を落とす・窯や実を割る・
穴を開けて飲む・餌を置く）と、海を渡ること（`voyage.yaml`。理由は
[`Voyage.md`](../world/Voyage.md) 3.10節）には書きません。

**この一覧は、風雨の線でもあります。** 明るさの条件を書いた操作は、風雨の条件（`too_stormy`）も
書きます——嵐の日、屋根の下でなければ何もできないからで、値としきい値は
[`ContentSkeleton.md`](../world/ContentSkeleton.md) 8.1.4節・8.1.5節が持ちます。上の「書きません」の
側も同じで、粗い動作と海を渡ることは風雨でも止まりません。

**見張っているのは2つが重なっていることだけです。** `tests/world-codex/stormOutdoors.test.ts`が全数で
両向きに見るので、**片方だけを書いた操作はそこで落ちます**。一方、**どちらも書き忘れた操作は拾えません**
——「この操作は明るさを要求するべきか」を見るものが無いからで、そこは上の【いつか: 明るさ条件の検査】が
受け持ちます（まだ在りません）。**風雨の側に独立した見張りは要りません**——その検査が入れば、重なりを
通して風雨の書き忘れも一緒に落ちます。
...
**火起こし具・火口・炉は、暗くなる前に用意しておかなければなりません。** どれも明るいうちにしか
揃えられないからです。**用意していなければ、朝まで何も作れません。**

**火を熾すことだけは、暗くてもできます。** 摩擦発火は手と耳の作業で、受け止める熾は自分で光るため、
暗さで難しくなりません。**ここが夜の唯一の戻り道です**——火起こし具・火口・炉が揃って
いれば、暗がりでも火を点け直して手元を取り戻せます。

**この道を閉じるのは、雨だけです。** 雨の降る夜に屋根の無い場所に居れば、道具が揃っていても点け直せ
ません（[`FireSystem.md`](./FireSystem.md) 3.1.1節）。**戻り道が消えたわけではなく、屋根の下——浅い洞窟
——へ入るという一手が要るようになります。**

一式を要求するのは、**日が暮れる前に段取りを済ませることが1日の組み立ての中身だから**です（1節）。
戻り道を1つ残すのは、暗くなった瞬間に朝までの手番が全部消えるのを避けるためで、**残す道が「用意して
あれば通れる」ものであれば、備えなかったことの結果は残ります。** 雨の夜に屋根のある土地に居るかも、
同じ「用意してあるか」の一部です。

**出どころ**: [#656](https://github.com/gooyyu1/UnmappedIsland/issues/656)（火起こし具・火口・炉を日暮れ前に用意させる）と、同（雨の日は屋外で火が起こせない）

### 5.2 段取りの要求を、どの経路で効かせるか
...
作ることはできない**ので、5.1節の要求は崩れません。

## 6. 値の土台は `base` で明示する

### 6.1 土台は、他のプロパティを指して書ける【確定】

**土台を指せる `base`（[`GameElementDefinition.md`](./GameElementDefinition.md) 6.5節）で書きます。**
文法そのものはあちらが持つので、本書はそれを使うだけです。

土台を指せる必要があるのは、**1つの世界の値から、土地ごとに違う組み合わせ（樹冠と反射だけ／
それに据え付けの光源を足したもの）を作る**ためです。土台が「祖先の同名」に固定されている限り、
もう一方の組み合わせには別の継承の道が要ります。
...
`{subject, prop}` は既にある参照の形そのもの（同 14.1節）なので、**演算は1つも増えていません。**

同じ組み合わせを作る道として、次の案を退けました。
...
見るので、境目を動かすときに書き換えるのは1箇所です。行動の可否を決める明るさが条件の側で数字と
...
**土地の `ambient_brightness` にはまだ段がありません。** 水の蒸発（9節）だけがこれを読み、しきい値を
