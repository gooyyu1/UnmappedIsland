...
**world の `ambient_brightness` は3段**——`pitch_dark`（−5未満）・`dim`（−5〜+10）・`bright`（+11以上）。
上の境目は曇りの正午（5,000 lx = +11、[`ContentSkeleton.md`](../world/ContentSkeleton.md) 8.1.3節）で、
**日射が空気を暖め始める明るさ**です。気温への寄与（9節）がこの段に乗っています。下の境目は移動の
しきい値（−5）で、ここへ落ちるのは夜だけです。

**境目は読み手が要る粗さで決めます。** world の段を読むのは気温で、そこで要るのは「陽が照っているか」
——行動のしきい値（5節）ではありません。段の側を細かくしても、できることは変わりません。

## 9. world の環境光を読んでいるのは3箇所

| 読み手 | 読み方 |
| --- | --- |
| 気温への寄与（`core.yaml`） | `ambient_brightness` の段（8節）の `pitch_dark` と `bright` が `ambient_temperature` を `modify` する |
...
なります。夜の寄与は底（−6）そのもので、天気の寄与は引く向きにしか働かないため、**夜は天気に
よらず同じ暗さ**になります。値の出どころは
[`ContentSkeleton.md`](../world/ContentSkeleton.md) 8.1.3節の照度の表です。
...
- **月あかりを項として持つか。** 今は持たず、夜は底（−6）へ均しています。満月が中天にある夜は
  実際には0.25 lx（−3）で底より3段明るいので、持たせれば「満月の夜は開けた土地を歩ける」が
  時刻で動くようになります。持たせると月齢を入れるかも続けて問題になり、夜の空が月と星の
  どちらかになって、対数の加算では合成できない場面（1節）がもう1つ増えます。
