...
- **0（2.5 lx）は、陽が地平線の下にあること。** 最も明るい満月の中天でも −3 なので、夜はすべて
  `dark` に入ります。**月の高さで夜の涼しさが変わってはいけない**ので、境目は月の届く範囲より上に
  置きます。
...
**時刻の段（`core.yaml` の `hour`）は空にある光源の高度で切ってあります。** 赤道付近の島なので
日の出6時・日没18時が動かず、太陽も月も1時間に15°動きます。昼は太陽高度で12段（正午をはさんで
対称）、夜は**満月の高度**で同じく対称の8段——満月は太陽の正反対に居るので18時に昇り0時に中天で、
中天の照度は0.25 lx（−3）です。値の出どころは
[`ContentSkeleton.md`](../world/ContentSkeleton.md) 8.1.3節の照度の表と、同 8.1.2節の夜の表です。

**夜は月あかりだけなので、雲が月を隠せば底へ落ちます。** 曇り以上の夜はどれも底（−6）に均され、
区別が付きません。
...
- **月齢を入れるか。** 今は満月固定です（`ContentSkeleton.md` 8.1.1.1節）。入れると夜の明るさが
  月齢の周期で動き、**開けた土地を夜に歩けるかが日によって変わります**。夜の空が月と星のどちらかに
  なるので、対数の加算では合成できない場面（1節）がもう1つ増えます。
