...
出入りの入口は3つで、違いは**位置をどう指すか**だけ（`WorldObject`）。
...
| 制約 | 答える問い | 書く場所 |
| --- | --- | --- |
| `accept`（7.2節） | **何が**入るか | 枠。`tag`（タグ保持）か `object`（型そのもの）でマッチ（4.1節） |
...
**身につける枠だけは、枠の宣言に加えて物の側の宣言も見る**（`covers` / `layer`、
[`GameElementDefinition.md`](./GameElementDefinition.md) 7.5節）。同じ部位の同じ階層を既に占めている
物が入っていれば断る。制約を持つのが枠ではなく中身なので、上の表には並ばない。
...
両者の違いは「空になったセルを残すか」「枠を増やせるか」「位置として枠を指せるか」の3つで、
...
**手持ち（`hand`）だけは、画面がこの安定した位置を使わない。** ハンドレーンは表示のたびに中身を前へ
詰めるので（[`ScreenLayout.md`](../ui/ScreenLayout.md) 7.3節）、枠の数は6のままだが、どの枠に居るかは
保たれない。詰めるのは**中身**であって枠ではないため、ここの規定——枠は残る・枠を位置として指せる
——はそのまま効く。詰めるのに使うのも下の `moveStackTo`（空きセルを指した並び替え）1つだけで、
専用の仕組みは無い。

**枠数を決めることと位置が安定することは同じこと**なので、この2つを別々のキーにはしない。読む側へも
...
規定どおり `CellLayout.placeSameSlot` が実装する。置き換え位置の決定（元のセル・同種が残っているか・
1つの効果から2個目以降が生まれたときの並べ先）は、効果適用の入口で捕捉した `SameSlotSpawnSite` が担う
（`ActionSystem.md` 5節）。
