...
出入りの入口は4つで、違いは**位置をどう指すか**だけ（`WorldObject`）。

- `MoveToSlot(slot, at?)` — 位置は任意。省略すると置き場所は枠に任せる。
- `ForceIntoSlot(slot)` — 枠の要件・`capacity` を無視して押し込む（spawn の伝播フォールバック、9.4節）。
  **既に世界に在る物を、置き場所が無いことを理由に消さないため**の入口。
...
意味する。**この読み替えは `Slot` 自身が行う**（`Slot.InsertAt` / `MoveStackTo`）ので、画面は指で示した
位置をそのまま渡す。
