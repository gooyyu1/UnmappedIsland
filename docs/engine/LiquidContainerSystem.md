...
**器へ水が入るもう 1 本の道が、水源から汲むことです。** 真水の湧く設置物へ器を重ねると、その器が
水入りの変種になって口まで満ちます。宣言は `water_source` トレイトが 1 つ持ち、名乗った設置物すべてに
同じ 2 つの操作が付きます（今これを名乗るのは `locations.yaml` の湧き水だけです）。
...
- **`draw_into_filled`**: 水の残っている器を重ねたとき。満ちるまで汲み足します。別の液体が入った器は
  どちらの操作にも合わず、「中身が入っている」理由で断られます（異種は混ざらない、4 節）。

**汲める器の型は名指ししません。** 見るのは `liquid_container` タグだけなので、器が増えても水源の宣言は
変わりません。
