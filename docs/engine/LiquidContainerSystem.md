...
ので、上乗せだけを落とす形ではありません——**塞いだ器は水を飛ばす口を持たない**ので、基礎の行も
上乗せの行も 1 つ残らず落ちます。
...
（[`GameElementDefinition.md`](./GameElementDefinition.md) 14.3 節）、蓋の無い器の宣言に手を入れる必要が
ありません。
...
- **`collect_rain`（`rain_catching_container` トレイトのメニュー型の操作）**: 雨天で、かつ雨の当たる場所に
  ...
  生まれません）。**空の容器でしか実行できません**——中身入りの変種も `fill` を持つため、`fill` が 0 で
  あることを条件で確かめて弾いています。**メニュー型なのでボタンごと消えはせず、押せない見た目で
  残って理由（`not_empty`）を言います**（[`Windows.md`](../ui/Windows.md) 4 節。下の蓋の `lidded` と同じ形）。押せない理由は分けてあり（`not_raining` と `sheltered`）、
