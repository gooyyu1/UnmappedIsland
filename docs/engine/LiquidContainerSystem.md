...
液体（水・茶・油）を容器で保持し、飲用・注ぎ移し・蒸発させ、雨を貯め、水源から汲む仕組みの設計ドキュメントです。定義本体は
...
器も、外が雨なら乾きません。密閉容器はどちらのゲートも成立しないため蒸発せず、
蒸発しきった液体は消滅します（1節）。
...
`ancestor.weather`・`ancestor.sheltered` で、日射のような第 2 の軸が無いぶん単純です。
