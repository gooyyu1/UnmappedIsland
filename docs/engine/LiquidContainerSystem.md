...
| 入ったまま `size` が増えて上限を超えたとき | （増えない） | **あふれた分は失われる** |
...
**注ぎ切った側は消えます。** `size` が 0 になった液体はその場で消滅し（1節）、容器の `content` が空に
...
別々に選べません。`weight` は `size × density` で、`density` は g/mL そのもの（水は 1）なので、水は「量の数値が
...
- **量（`rain_filled_liquid` トレイトの `passives`）**: 降っている間、毎 tick 自分の `size` を増やします。

生まれる水の `size` は **1mL**（`liquid` トレイトの `size` の既定値）です。量が 0 の量的オブジェクトは
...
**重さ**は `size × density` です（`density` は単位量あたりの重さ = g/mL、水は 1）。液体は
