...
しきい値は EV（[`IlluminationSystem.md`](./IlluminationSystem.md) 1節）で、**どのくらいの日射で水が
...
**いちばん下の +12 が「水が減り始める明るさ」です。** 薄日でも直射がある明るさで、雲のまったく無い空
なら日の出の1時間後に届き、曇りでは1日を通して届きません。上の2つは、雲の無い空の正午（+16）を
最上段に置いた、その間の刻みです。

**時間帯の概念は液体側に持ち込みません。** 夜は `hour` の段が月あかり（最も明るい満月の中天でも
−3）しか与えない（`core.yaml`）ので、上乗せは自動的に消えます。時刻の区切りを変えても蒸発側は
無修正で追従します。

**上乗せ側は天候を見ません。** 雨天でいちばん明るい小雨の正午でも +10 で、最も低いしきい値 +12 に
届かないためです。天候の判定が要るのは基礎の側だけで、湿度が未実装なので雨天を「湿度が高くて乾かない」の
代理として除外しています。密閉容器はどちらのゲートも成立しないため蒸発せず、蒸発しきった液体は消滅します（1節）。

しきい値が `ambient_brightness` の寄与（`core.yaml` の `hour`・`weather` それぞれの `modify` 量）に
乗っている点だけが外部への依存です。**`core.yaml` 側の寄与を変えると蒸発量が動きます。**

上乗せが効く時刻（`hour`。日の出6時・日没18時）:
...
満杯から空になるまでの日数（`ClimateSystemStats.md` の天候出現時間で加重した実測ベース）:
