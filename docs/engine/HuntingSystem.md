...
**体調が配分を押すのも1箇所で、通り道は意識です。** 痛み・失血・衝撃はどれも `consciousness` を押し
下げている（[`VitalsSystem.md`](./VitalsSystem.md) 2 節）ので、押し引きは合流した後の段に置きます。

- **1箇所に収まるのが意識だけだからです。** `blood` と `shock` は `max` が体格なので獣ごとの宣言に
