...
- **tick 駆動**: 時間経過にともなう変化は、すべて tick ごとの独立変数への加減算（`accumulate`）で表現します。
...
`on_overflow`/`on_shortfall` の中身は `active`（9 節）と同じ型の内容で、`active` の代わりに `pick`
（10 節、排他）も書けます。対象は現時点で `self` のみです（未対応: `parent`/`actor`/`dragged`、
ロード時にエラー）。
...
- **`fixed_positions`**（既定`false`）: `true`の場合、型ごとに固定番号（0始まり、`unit_capacity`個まで）が
  割り当てられ、空いた番号を保持したまま前詰めしません。新しい型が（`same_slot`経由のspawnではなく）通常の
  手段で初めて登場した際は、空いている最小番号へ自動的に割り当てられます。プレイヤーは手動で番号を
  入れ替えられます（例: プレイヤー手持ちの6枠）。`same_slot`経由での新規登場は、下記「`spawn`との関係」の
  割り当てルールに従います。
...
- `stack_order`は、新規インスタンスがスタックへ加わる際の並び位置決定にのみ使います。一度並んだ後、値の変化に
  追従した再ソートは行いません。同種は同じ速度で変化する（`accumulate`は`ObjectDef`ごとに一定量、8.4節）
...
- **`quantitative`**（既定`false`）: `true`の場合、この型のインスタンスは「1個・2個」ではなく**量**で存在し、
  ...
  `accumulate`（8.4節）・`transfer`（9.5節）・`add`（9.2節）は移動ではないので、`capacity`（7.3節）も
  量の下限も見ずに `size` を動かせます。そこで**この2つの不変条件は、`size` が動くたび（その場の書き換えの
  直後と、tickでの `accumulate` の反映後）にエンジンが戻します**: 量が尽きたインスタンスは消え、`capacity` を
...
`fixed_positions`のスロットでは固定番号）を新しいオブジェクトへそのまま引き継がせます。これにより、種類が
...
単純に自分の直後へ挿入します。`fixed_positions`のスロットでは、**まず自分の固定番号+1の位置（右側）から
...
`accepts`（7.2 節）・`capacity`（7.3 節）が「何をいくつ入れられるか」の制約であるのに対し、`auto_placement` は
...
`accepts`/`capacity` の受け入れ判定であって、「そもそも自動では入らない」という宣言ではないためです。
...
これは表示の宣言であって、`accepts`（7.2 節）のような配置の制約ではありません。名乗っていない
...
- **他の持ち主へ移せない。** いったん親に付いたら、`move`（9.5 節）でもドラッグでも別の親へは
  移りません（生まれた直後の、まだ親を持たない状態での配置は通ります）。`accepts`（7.2 節）が
...
`modify` は 8.3 節の通り単純加算で合成され、`accumulate` も有効な全ブロックの量がそれぞれ tick 毎に反映されます。
...
キーのラップを挟みません。`destroy` と同時に指定でき（`{spawn: {...}, destroy: self}`）、この場合 `destroy` が
先に実行されます（破棄によって位置が空いてから配置することで、スタック表示（7.6節）における位置の引き継ぎが
素直に実現できるため）。
...
- **`object`**: 移動させる対象。`actor`（アクション実行者）と `dragged`（`combinations` でドラッグされてきた
  カード、12.2 節）のみ対応です（`self`/`parent`/`child` 等は「一度きりの命令に対してどれを動かすか」の
...
最初に配置できたスロットへ入れます。ただし `spawn` と異なり、`move` には `accepts`/`capacity` を無視した
...
重み付き候補のリストです。`passives`（無条件／`conditions`／stage 内）には書けません。`passives` は「いつ振るか」
という瞬間を持たない、関係とゲートに基づく継続的な評価だからです。
...
入れ子は再帰的であり、候補の動詞の代わりにさらに別の `pick` を書くこともできます。
