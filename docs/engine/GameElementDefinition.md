...
**同じ tick の輸送どうしは、互いの到着を見ません。** 1 つのオブジェクトが宣言した輸送は、まず全件の
移動量が決まり、そのあとまとめて動きます。そのため上の例は 1 tick につき 1 段ずつ進み、食べた物がその
tick のうちに蓄えへ抜けることはありません。ただし**出した量は互いに見える**ので、同じ値から出す輸送を
並べても在庫を二重には動かしません（宣言順に消費します）。
