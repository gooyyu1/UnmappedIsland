...
object_defs:
  canteen:
    ...
    variation_axes:
      content: {of: {tag: liquid}, capacity: 1000}
```
...
- **`capacity`** は、その変種が抱えられる量の上限です。変種には `fill`（量、`range` は 0〜`capacity`）が
  足され、**素の型は `fill` を持ちません**——空であることは、量が 0 なのではなく量そのものが無い、
  という形で表れます。
- **`fill` が 0 になった変種は素の型へ戻ります**（エンジンの不変条件、`WorldObject.settleFill`）。
  `on_shortfall`（6.3 節）には載せられません——`transfer`（9.5 節）は `range.min` を「出せる量の床」と
  見るため、境界を `range` の外へ置くと注ぎ切ることも飲み干すこともできなくなります。
- **`become` で変種になった直後は空です。** 量を入れるのは変えた側で、注ぎ移しなら移った量、雨なら
  最少量を書きます。ここに正の既定値を置くと、注ぐたびに出どころの無い量が足されます。
