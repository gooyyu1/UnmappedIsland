...
- **`value`**: 比較対象の値。**リテラル**（整数・真偽値・シンボル名、6.6 節）か、**`{object, prop}` 参照**
...
位置**）。常に等価判定で、`op` は持ちません（否定したい場合は 14.5 節の `not` で包みます）。
...
`in_slot` は `prop` とは独立したフィールドで、同時には指定できません。フィールドを分けるのは、スロット判定に
比較演算子が意味を持たないことと、プロパティ名とスロット名は別の名前空間であり、同じ識別子でも衝突させない
ためです。
...
`object` が指すオブジェクト自身が、指定した `tag`（4 節）を持つかを判定します。常に存在判定で、`op` は
持ちません（否定したい場合は 14.5 節の `not` で包みます）。
