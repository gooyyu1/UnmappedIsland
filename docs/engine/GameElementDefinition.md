  ...
  canteen:
    ...
    variation_axes:
      content:
        of: {tag: liquid}
        props:
          fill: {range: {min: 0, max: 1000}}   # 容器ごとに違う上限は、変種の宣言として渡す
        exhausted_when: fill                   # 量が尽きたらこの軸が外れる＝素の型へ戻る
```
...
- **`props`** は、その軸の値を持つ変種にだけ足す宣言です。**素の型ごとに違う値**——容器ごとの上限——を
  渡すための口で、何を書くかは著者の裁量です。ここで書いた項目は、配られた trait の同名の宣言を
  フィールド単位で上書きします（5 節の合成規則そのまま）。**素の型はこれを持ちません**。
- **`exhausted_when`** は、尽きるとこの軸が外れるプロパティの名前です。0 以下になった変種は素の型へ
  戻ります。`on_shortfall`（6.3 節）には載せられません——`transfer`（9.5 節）は `range.min` を「出せる量の
  床」と見るため、境界を `range` の外へ置くと注ぎ切ることも飲み干すこともできなくなります。
  **どのプロパティが「量」かを決めるのは宣言で、エンジンは名前を知りません。**
- `variation_axes` と `recipes` は変種へ写しません。**変種の変種は作らず**、作れるのは素の型のほう
  です（空の容器を作ってから中身を入れる）。
