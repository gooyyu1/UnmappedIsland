...
**素の型へ戻る道も宣言の側にあります。** 量が尽きたら空へ戻る、という規則は `on_exhausted`（6.3 節）へ
...
### 6.3 range / on_overflow / on_shortfall / on_exhausted（周回・上下限）
...
**`on_exhausted` は、値が `range.min` ちょうどに着いたときに一度だけ発火します。** `on_shortfall` が
境界を `range` の外に置くのに対し、こちらは**尽きたこと**そのものへの反応です。両者を分けるのは、
`transfer`（9.5 節）が `range.min` を「出せる量の床」と見るためで、**在庫を出し切る操作は下限を割りません**
——境界を外に置く形では「注ぎ切った」「飲み干した」に反応できません。既定はありません（宣言した型だけが
反応します）。
...
- ステージは **`art`** で「その段階にいる間カードに出す絵」を接尾辞として宣言できます
  ...
  - `art` を宣言できるのは、その object_def が **`art_by_stage`** で名指ししたプロパティの段だけです
    ...
    `represented_by`（7.6節）と同じ規約で、trait 側の宣言も参照できます（自分自身の指定を優先し、
...
オブジェクトレベルのキーです（`represented_by` と同じ位置に書き、trait からの合成規則も同じ）。捻挫・液体・道がこれにあたります。
...
キーです（`represented_by` と同じ位置に書きます）。並んだスロットは子ウィンドウのタブになります
...
- **上限を持つスロットが複数あるときは、最も詰まっているものを出します。** バーが答えるのは
  ...
  見えてしまいます。
- trait と自分自身の宣言は OR で合成します（`quantitative` と同じ、5 節）。
...
- **動かす物**: `subject` と `subject_prop` の**どちらか一方**で指します（両方書く・どちらも書かないのは
  ロード時エラー）。
  - **`subject`**: `self`（`move` を宣言したオブジェクト自身）・`actor`（アクション実行者）・`dragged`
    （`combinations` で組み合わせる相手のカード、12.2 節）です（`parent`/`child` 等は「一度きりの命令に
    対してどれを動かすか」の意味論が未確定のため未対応・ロード時エラー。9.1 節で `active` の対象キー
    `child` が未対応なのと同じ理由）。
...
`move` は `actions`/`combinations`/`pick` の各候補のいずれでも書けます。`on_overflow`/`on_shortfall`
...
**インスタンスをそのままに、型だけを別の型へ差し替えます。** 水を汲んだ水筒が「水入りの水筒」になる、
...
- **プロパティ**: 同じ名前のプロパティは値を引き継ぎます。新しい型にしか無いものは初期値から始まり
  ...
  引き継いだ値が外れる場合は**クランプするだけで、`on_overflow`/`on_shortfall` は発火させません**
...
製作の完成——`progress` の `on_overflow` で作りかけが完成品になる——がこの操作です
