...
### 7.8 item_slot（物を出し入れするスロット）

**`item_slot`** は、その型の**物を出し入れするスロットを1つだけ名指しする**オブジェクトレベルのキーです
...
traits:
  item_container:
    item_slot: contents        # かごと言えば中身。押せば開いて出し入れできる
    slots:
      contents:
        accepts: [{tag: item, max: 9999}]

object_defs:
  coconut_bowl:                # 液体の容器はitem_slotを持たない。中の水は単独では存在できない
    represented_by: content
    slots:
      content:
        accepts: [{tag: liquid, max: 1}]
...
- **液体の容器は名乗りません。** 中身の液体は**物ではなく量**（`quantitative`、7.6 節）で、1 枚ずつ
  取り出せるものではないためです。名乗れば水を単独で持ち出せてしまいます。「入れ物の中身」という
  言葉は同じでも、かごの中身とは別のものだ、という線がこのキーの名前です。
- **複数のスロットを持つが、そのどれでもない型は宣言しません。** キャラクタ（手持ち・装備・怪我）が
  これにあたります。装備・怪我の子ウィンドウは、開くスロットを画面側が固定で渡すので、この宣言に
  依りません。
