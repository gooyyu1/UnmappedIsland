...
**座標から生成型を引く対応表は未実装です。** 今あるのは製作中オブジェクトから完成品を引く一方向の対応
（`WorldCodex.productOf`）だけで、2 つ以上の軸を持つ生成器と座標からの逆引きは、9.9 節の `become` と
合わせて実装します。
...
### 9.9 become（同じ個体のまま型を差し替える）【未実装: become】
...
- **座標が解決できない組み合わせでは、その操作を実行できません。** メニューにも吹き出しにも出ません
  （`conditions` の不成立と違い、出せる `reason`（14.6 節）を持ちません——不成立なのは条件ではなく
  行き先の型そのものだからです）。
...
- **スロット**: 同じ名前のスロットは中身ごとそのまま残ります。**新しい型が持たないスロットの中身は、
  `destroy`（9.3 節）と同じ規則で親へこぼれます**（`bound_to_owner` な中身は道連れ）。枠数が減って
  入りきらない分も同じです。
