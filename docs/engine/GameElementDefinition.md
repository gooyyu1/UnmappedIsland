...
- ステージは **`art`** で「その段階にいる間、カードに出す絵」を宣言できます（値は
  `src/assets/objects/<名前>.png` のファイル名）。**同じ個体のまま姿だけが変わる状態**——気を失った
  動物——を、型を差し替えずに表すためのものです（[`CardView.md`](../ui/CardView.md) 9.1 節）。1つの
  オブジェクトの複数の段が同時に絵を宣言しうるため、**`props` の宣言順で最初に見つかった1つ**を使います
  （`Runtime.WorldObject.artName`）。宣言した絵をまだ用意していない場合は、その型の絵のままになります。
