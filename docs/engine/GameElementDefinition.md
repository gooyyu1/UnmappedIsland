...
（`represented_by`（7.6 節）と同じ位置に書き、trait からの合成規則も同じ）。**そのカードへ重ねた物の
行き先**がここになります（[`CardInteraction.md`](../ui/CardInteraction.md) 2 節）。

**中身を見せるかは別の宣言です**（`visible_slots`、7.11 節）。かごのように両方を名乗る型が多いので
1 つのキーが兼ねていましたが、筏は「重ねれば積めるが、中身は外から見えない」——兼ねられません。
...
キーです（`main_item_slot` と同じ位置に書きます）。並んだスロットは子ウィンドウのタブになります
...
（`main_item_slot`、7.8 節）。中を見るには乗り込む、という違いがそのまま操作の形になります。
