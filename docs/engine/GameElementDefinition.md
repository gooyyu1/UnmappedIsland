...
### 7.8 show_contents（中身をプレイヤーに見せるか）

**`show_contents: true`**（既定 `false`）を書くと、そのスロットの中身がカードとして並び、プレイヤーの
目に触れます。並べば、そこへ入れることも、そこから取り出すこともできます。

**どこに並べるかは UI 側の決めごとです**（[`ScreenLayout.md`](../ui/ScreenLayout.md) 子ウィンドウ節）。
今はカードを押して開く子ウィンドウか、キャラクタの装備・怪我のように専用のボタンから開く子ウィンドウ
ですが、それは表示の都合であって、この宣言が依存すべきものではありません。**見せるスロットはすべて
名乗ります**——UI が「このスロットはボタンから開くから」と暗黙に扱うのは、データ側からその事実が
読めなくなるためです。

```yaml
slots:
  contents:                  # かごの中身。押せば開いて出し入れできる
    accepts: [{tag: item, max: 9999}]
    show_contents: true
  content:                   # 液体の容器。中身の水は単独では存在できないので見せない
    accepts: [{tag: liquid, max: 1}]
```

**既定を「見せない」にしているのは、見せることが取り出せることでもあるためです。** 液体の容器の中身
（`content`）のように、単独で存在してはいけない物を黙って取り出せるようになるより、見せたいスロットだけが
名乗る方が安全です。UI 側がスロット名で決めていた頃は、`content` と `contents` が 1 文字違いだったおかげで
開かれずに済んでいただけで、名前が揃えば水を取り出せてしまう状態でした。

これは表示の宣言であって、`accepts`（7.2 節）のような配置の制約ではありません。見せないスロットへも、
`spawn`/`move`（9 章）やワールド側の効果は今までどおり物を入れられます。
