...
### 7.8 show_contents（中身をカードから見せるか）

**`show_contents: true`**（既定 `false`）を書くと、そのスロットの中身が、カードを押して開く子ウィンドウに
カードとして並びます（[`ScreenLayout.md`](../ui/ScreenLayout.md) 子ウィンドウ節）。並べば、プレイヤーは
そこへ入れることも、そこから取り出すこともできます。
