...
小数を持たないため、細かい傾きは「1 単位を何分割するか」で表します。毎 tick 変化するプロパティは、次の
2 クラスのどちらかに属し、書かれている数値の読み方はクラスで決まります。

| クラス | 基準 | 値の読み方 | 例 |
|---|---|---|---|
| **時間を数えるもの** | **±1/tick** | 値がそのまま tick 数 | `tick`・`season_remaining`・`weather_remaining`・`thermal_level`・`monsoon_level`・`exploration_progress` |
| **量を溜めるもの** | **基準レートの 1 tick 分 = 100** | 値 ÷ 100 が「基準レートで何 tick 分か」 | `satiety`・`hydration`・`body_fat`・`wakefulness`・各栄養・`atmospheric_moisture` |

どちらに属するかは「**そのプロパティに『1.2 倍』のような割合の補正をかけたくなるか**」で決めます。時間の
刻みが tick である以上、時間を数えるものに小数は生じないため ±1 で足ります。量には「暑いと渇きが速い」の
ような補正が乗りうるため、`modify`（8.3 節）で刻める余地として 100 分割を確保します。
...
`on_overflow`/`on_shortfall` の中身は `on_min`（6.5 節）と全く同じ型の `active`（9 節）内容ですが、対象は
現時点で `self` のみです（未対応: `parent`/`actor`/`dragged`、ロード時にエラー）。

- `on_overflow` は値が `range` の上限を超えた瞬間、`on_shortfall` は下限を下回った瞬間に、指定した内容を
  一度だけ適用します。
...
- **空のmappingを書く（`on_shortfall: {}`）と、「宣言だけして何もしない」**、つまり既定のクランプを打ち消して
  範囲外の値をそのまま許容する、という意味になります。`on_min`（6.5 節）で値を再設定するプロパティ
  （気候の `season_remaining` 等、core.yaml参照）では、既定のクランプが `on_min` の再設定後に走って
  `on_min` を二重発火させうるため、この形を使います。
...
- `add` に書けるプロパティは、自分自身（折り返し）に限らず、同じ object_def 内の他のプロパティ
  （繰り上げ・繰り下げ先）も指定できます。存在しないプロパティ名は `on_min`/`add` と同様に黙って無視されます。
- 自分自身の折り返し・繰り下げには `set`（9.2 節）ではなく `add` を使ってください。後述の通り
  `on_overflow`/`on_shortfall` はループしないため、1 tick で `range` の幅を複数回分飛び越えることがあります。
  `set` で固定値へ代入するとその超過分を失いますが、`add` なら差分を保ったまま反映されます。
- `on_overflow`/`on_shortfall` はループしません。1 tick につき、条件を満たしたプロパティ 1 つあたり最大 1 回
  ...
  - 1 tick で `range` の幅を複数回分飛び越えた場合、1 回の適用では収まりきらず、残りは次回以降の tick に
    持ち越されます。
  - 繰り上げ・繰り下げ先自身がさらに範囲外へ出る連鎖（分→時→日）が同じ tick 内で解決されるかどうかは、
    `props` の宣言順に依存します。繰り上げ・繰り下げ先が自分より後に宣言されていれば同じ tick 内で正しく
    折り返され、先に宣言されていれば次の tick へ持ち越されます。連鎖がある `props` は、繰り上げ・繰り下げの
    元→先の順（例: `minute`→`hour`→`day`）で宣言することを推奨します。
- 1 回の判定内での適用順は `on_max`（6.6 節）→ `on_min`（6.5 節）→ `on_overflow` → `on_shortfall` です。
  値を書き換えない「観測者」（`on_max`/`on_min`）を、値を書き換える「補正者」（`on_overflow`/`on_shortfall`）
  より先に評価します。折り返しは境界ちょうどには着地しないことが多いため（例: 0-100 循環で 150 まで加算
  されると折り返し後は 50）、補正の後に判定すると「境界へ到達した瞬間」を見逃してしまうからです。
...
- **シンボル型プロパティ（6.8 節）の場合は `min` を書きません**（書いた場合はロード時エラー。シンボル値の
...
`on_min` は、プロパティが**`range` の下限以下である間、毎 tick 実行される `active`（9 節）内容**です。
「跨いだ瞬間だけ」を検出する仕組み（前 tick との比較）は持たず、現在値だけで判定します（`passives` と同じ
レベルトリガーの考え方を、`set`/`add`/`destroy`/`spawn` という一時的な命令に適用したもの）。

- 毎 tick 実行されても安全なのは、典型的な用途が自己終端するためです。`destroy` は冪等であり、`spawn` を伴う
  場合は同じ `on_min` 内で `destroy` と組み合わせるのが基本形です（破棄後は tick の対象でなくなるため、
  2回目以降の実行は起こりません）。`destroy` を伴わずに `spawn` だけを書くと、下限以下である間 tick 毎に
  生成し続けてしまう点に注意してください。
- 中身は `active`（9 節）と全く同じ型ですが、対象は現時点で `self` のみです（未対応: `parent`/`actor`/
  `dragged`、ロード時にエラー）。`range.min` を参照するため、`range` は `on_min` を使う場合に必須です
  （`on_overflow`/`on_shortfall` と同じ制約）。
- `on_overflow`/`on_shortfall`（6.3 節）とは異なり、著者が明示的に書かない限り既定の自動生成は行われません
  （「底を突いたら」の挙動は用途ごとに大きく異なり、クランプを前提にした既定値がなじまないため）。
...
（6.5 節）の上限側の鏡像で、制約もすべて同じです: 現在値だけで判定するレベルトリガーであること、自己終端しない
`spawn` 単独の使用に注意が要ること、対象が `self` のみであること、`range` が必須であること、既定の自動生成が
無いこと。
...
`ancestor` は conditions（14 節）の `object` としても、`inherit`（6.7 節）の探索ロジックとしても使われる、
共通の1つの祖先探索です。
...
**宣言順に走査し、最初に配置できたスロットへ入れます**。著者はスロット名を知る必要がありません。
