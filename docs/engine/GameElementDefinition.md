...
### 7.8 main_slot（主要なスロット）

**`main_slot`** は、その型の**主要なスロットを1つだけ名指しする**オブジェクトレベルのキーです
...
traits:
  item_container:
    main_slot: contents        # かごと言えば中身。押せば開いて出し入れできる
    slots:
      contents:
        accepts: [{tag: item, max: 9999}]

object_defs:
  coconut_bowl:                # 液体の容器はmain_slotを持たない。中の水は単独では存在できない
    represented_by: content
    slots:
      content:
        accepts: [{tag: liquid, max: 1}]
...
- **持たない型は、カードを押しても中身が出ません。** 見せることは取り出せることでもあるので、
  持たない側を既定にします。液体の容器（`content`）のように、中身が単独では存在してはいけない
  スロットを黙って開くより、主要なスロットがある型だけが名乗る方が安全です。
- **複数のスロットを持つが、そのどれもが主要ではない型は宣言しません。** キャラクタ（手持ち・装備・
  怪我）がこれにあたります。装備・怪我の子ウィンドウは、開くスロットを画面側が固定で渡すので、
  この宣言に依りません。
- **1 つに絞れるのは、画面が「1 つの対象につき 1 つの並び」しか出せないためです。** かつては
  スロット側の真偽値で「見せる」を宣言していましたが、複数が名乗ったときにどれを開くかを画面側が
  宣言順で決める必要があり、その「最初の 1 つ」という規則自体が、主要なスロットは 1 つだという
  事実を裏側から言っていました。
