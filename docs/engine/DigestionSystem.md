...
**掛かるのは、火を通す手段のある食べ物だけです。** この枝は**火を通す価値**を表すもので、**焼けば枝ごと
消えるところまでが 1 組**——火を通す手段の無い食べ物へ掛けると、**避ける手の無い罰**になって対価の形に
なりません。手段がある側では、火を通す価値が「身になる量が増える」と「当たらなくなる」の 2 つに
なります。
...
生の枝はこれと別で、**焼いた先を持つ食べ物**（丸焼きの鎖の途中、[`FireSystem.md`](./FireSystem.md)
1.1 節）のうち、**生でも食べられる物**が持ちます。どちらも `cooking_progress` の `on_max` で決まり
