...
**1 食で入る量は、食べ物の側が名乗ります。** いまそれを名乗るのは生肉（`raw_meat`）だけで、`+3`
——生の食肉の表面菌数（10³ CFU/g の桁）から置いた仮の値です。焼けば菌は死ぬので、焼いた先
（`roasted_meat`）は名乗りません。**火を通す価値が、「身になる量が増える」と「菌が入らない」の 2 つに
なります。**
...
| 寝不足 | `-15` | `wakefulness` の `overtired` |
...
止まります**。それが獲得免疫で、押し下げが 2 つ重なっても崩れない余力になります。
