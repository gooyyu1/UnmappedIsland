...
持つのは 2 本です。どちらも身体の仕組みなので個体差を持たせず、`player_character` trait が配ります。
...
終わり方は脱水と失血のままです。**傷の `festering`・`septic` 段（[`InjurySystem.md`](./InjurySystem.md)
6 節）とまったく同じ形・同じ量**で、対症療法が効く道（熱で余計に要る水は飲めば追いつく）と効かない道
...
（最も重い空腹の `-20` でちょうど 40）、2 つ重なると `weakened`、3 つ重なるか壊血病 1 つで `failing` へ
落ちます。こう置かないと、いちばん軽い `-10` でも段が落ちて、重い順に並べた意味が消えます。

**`range.min`（20）が生来の免疫です。** 押し下げを全部重ねても（`-85`）ここより下へは行かないので、
生まれ持った守りは残ります。
...
**引き戻すのはいちばん上の段だけ**（`-0.05/tick`）なので、**罹って治った体は生来より高い所（70 の少し下）で
止まります**。それが獲得免疫で、**押し下げ `-30` までは段を保てる**余力になります（生来では `-20` まで）。
...
### 6.3 傷んだ物の吐き下しは、まだ抽選のまま

**腐敗だけは、まだ `eat` の末尾の `pick` が引きます。** 上の原則からすればこちらも菌量へ移すべきですが、
**移していません**——理由は 2 つで、`eat` の `add` の量はリテラルなので**傷んだ段と腐った段の差を書けず**、
段の `passives` には `set` が無いので**「吐く」（腹が空になる）が書けない**ためです。移すには食べ物の側が
菌量を実体値として溜める形（`perishable` の作り直し）が要ります。
...
- 傷んだ物の吐き下しを、菌量へ移すか（6.3 節）。移せば `eat` から抽選が 1 つも無くなるが、食べ物の側が
  菌量を実体値として溜める形（`perishable` の作り直し）が要る
