...
**引き金は腐敗です。** 傷んだ物を食べたときだけ起こります（腐る速さは
[`DurabilitySystem.md`](./DurabilitySystem.md) 3 節）。感染症はまだ島に無いので、これが最初の 1 本です。
...
## 7. 食の偏りは、水と蓄えを削って既存の死に方へ流す【確定】【未実装: 食の偏り】
...
- たんぱく質の欠乏（筋が落ちる、3 節）。脂質の欠乏とたんぱく質過多は 7 節が決めたが、こちらは削る先が
  決まっていない——**行動力（`stamina`）が素直だが、痛み以外へ効かせる例がまだ 1 つも無い**
