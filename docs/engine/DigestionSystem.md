...
[`DurabilitySystem.md`](./DurabilitySystem.md) 3 節）と、**火を通さずに食べたとき**。感染症はまだ島に
無いので、体調を崩す道はこの 2 つだけです。
...
**焼けばこの枝ごと無くなる**ので、火を通す価値は「身になる量が増える」と「当たらなくなる」の 2 つに
なります。
...
生の枝を持つのは**生肉（`raw_meat`）だけ**です。焼ける物のうち生のまま食べられるのは肉だけで、生の
タロイモとヤシガニは食べ物のタグを持ちません（[`Animals.md`](../world/Animals.md) 1 節）。
