...
**名乗るのは、焼いた先を持つ食べ物**（丸焼きの鎖の途中、[`FireSystem.md`](./FireSystem.md) 1.1 節）のうち、
**生でも食べられる物**です。いまはそれが 1 つで、生肉（`raw_meat`）——**丸焼きの鎖の先頭に居ること**として
引くので、焦げる手前の焼けた肉は当てはまりません（焼いて生まれた物には既に火が通っています）。
**火を通す手段の無い食べ物へは掛けません**——避ける手の無い罰になり、対価の形になりません。生で食べる
葉物（`water_spinach`）が名乗らないのはこの線によるもので、あちらで危ないのは植物ではなく育った水のほう
（寄生虫）です。数え上げが増えたときはここも直します（見張るのは
