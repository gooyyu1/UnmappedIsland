...
**仕組み**（プレイヤーの目に触れるモノそのものではなく、それらを成り立たせているメカニズム）に関する
ドキュメントを格納します。

`src/` 全体の構造——どこに何を置き、何を知ってよいか——は、エンジンに限らず画面までを規定するので
...
実際のゲームデータ（`src/assets/world-codex/*.yaml`）を型・プロパティ・スロット・操作の単位で辿って読むには、
閲覧ビューア（`npm run dev:codex`、公開先は [`../../codex/`](../../codex/)）を開いてください。作りと設計判断は
`src/codex-viewer/` の各ファイルのコメントにあります。
...
- [消化システム設計](./DigestionSystem.md) — 食べた物が胃・腸を通って蓄えになるまでの経路と、実際の消化時間から見積もった輸送の速さ

- [ローカライゼーション](./Localization.md) — 表示文字列をWorldCodexから切り離し、言語ごとの対応表から引く仕組み

### 実装ガイド（コード構造の把握。「なぜ」ではなく「どこに・どう実装されているか」）

- [コードの構造](../CodeStructure.md) — 世界・映し・意匠・素材・汎用部品・部品・解析の7層と、層では
  ない組み立て・層の外・codex ビューア。新しいファイルをどこへ置くか、それぞれが何を知ってよいか
  （このフォルダの外）
...
- [アセットパック](./AssetPack.md) — 同梱されていない定義YAML・絵・表示文字列を、1つのZIPとして受け取る方針
  （未実装）
