...
**獣の候補の重みには、探索する人の狩猟の腕が倍率として掛かります**
（[`../world/Skills.md`](../world/Skills.md) 5 節。上の確率は `novice` のもので、`expert` では 3 倍の
重みになります）。土地が持つのは素の重みだけなので、**宣言していない獣には腕がいくら高くても出くわしません**。
実りの候補には掛かりませんが、**卓の合計が増えるぶん実りの出る割合は下がります**——ジャングルは
実り 144・獣 13 なので、144/157 ＝ 約 92% から、`expert`（獣 39）の 144/183 ＝ 約 79% へ。
**腕を上げた探索は獣へ寄ります。**

**海区の見張りも同じです。** 海区は土地と同じ `explorable`（[`../world/Voyage.md`](../world/Voyage.md)）で、
魚の群れと海鳥の群れ（`shoal_find`・`seabird_find`）が獣の候補にあたります——どちらもその場に湧いて
しばらくすると去り、獲る手が狩猟の腕を配ります。**拾えるもの（流木・海藻・卵）には掛かりません。**
...
- **`duration`**（`GameElementDefinition.md` 11.3 節）: アクションの実行にかかるゲーム内時間（分）。
  リテラルか `{subject, prop}` 参照（`weight` と同じ書き方。ただし積は取りません）で指定します。
  時間は効果の適用より**先**に
