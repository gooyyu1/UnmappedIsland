...
「場所」であるという構造（スロット）と、「探索できる」という性質（進捗・道の発見）は、**別々の trait**
に分けています。家・作業小屋のような探索できない場所もスロットは必要なため、スロットは `explorable` では
なく `location` trait（`core.yaml`）側に置いています。

- **`location`**（`core.yaml`）: あらゆる場所が共通して持つ構造。**スロット**（`items`/`fixtures`/
  `characters`）と、**その場を満たしているもののプロパティ**（明るさ・気温・居心地・慰め・構えられた
  間合い。一覧は `core.yaml` が持つ）と、`world.locations` スロットの枠が受け入れる型のタグ配布。
...
### 1.1 スロット（`location` trait）

すべての場所は次のスロットを持ちます。いずれも `capacity`
（サイズの合計制限、[`GameElementDefinition.md`](./GameElementDefinition.md) 7.3 節）は指定していません。

- **`items`**: アイテムが置かれるスロット。
- **`fixtures`**: 木や植物、建築物や家具、浅い洞窟などの設置物が置かれるスロット。
...
スロットを宣言順に走査する、[`GameElementDefinition.md`](./GameElementDefinition.md) 9.4 節）。`items` を
宣言順の先頭に置いているのは、`spawn` が配置に失敗した際のこぼれ落ち（起点自身の親へ、同 9.4 節）が
同じく先頭のスロットから試すためで、「持ちきれない発見物は地面（土地の `items`）に落ちる」という
直感的な結果を、追加のルールなしに実現しています。

【いつか: 洞窟内部】 探索で見つかる浅い洞窟（`shallow_cave`）は、それ自体が `location` を名乗る入れる
場所ですが、そこから奥へ続く道はまだありません（[`Someday.md`](../Someday.md)）。
...
スロット移動操作、[`SlotSystem.md`](./SlotSystem.md) 1 節）1回で表現でき、UI 側は「`fixtures` の中身をそのまま表示する」という単純な
...
**探索と罠は、同じ獣を別の形で渡します。** 探索が渡すのは**出くわした獣**で、自由に動き、その獣の
警戒しだいで逃げも襲いもします（現れたときの警戒は獣ごとに違う、[`HuntingSystem.md`](./HuntingSystem.md) 3.1 節）。
罠が渡すのは**掛かった獣**で、留守のあいだに働き、獲物は枠の中で拘束されています
（同 5 節）。ネズミ・ヤケイが両方に載っているのはこのためで、罠の値打ちは「その獣が手に入ること」ではなく
「見ていない間に、動けない状態で手に入ること」です。

**獣の候補は、その獣を1匹だけ湧かせます**（`tests/world-codex/explorationYield.test.ts` が検証）。
獣は収穫ではなく出くわすもので、木の実や枝と一緒に並ぶと実りの一部に見えてしまうためです。**重みは
他の候補より一桁低く**（2〜4 に対し 10〜30）、1回の探索で獣に出くわす確率は、ネズミしか居ない荒野で
2/114 ＝ 約 2%、4 種が居るジャングルで 13/157 ＝ 約 8% です。

**獣のつまみは、探索する人の狩猟の腕を `base` の土台にします**
（[`../world/Skills.md`](../world/Skills.md) 5 節。上の確率は `novice` のもので、`expert` はつまみ 1 つ
につき +4）。土地が持つのは素の重みだけで、**宣言していない獣は卓に無いので、腕がいくら高くても
出くわしません**。実りの候補には積みませんが、**卓の合計が増えるぶん実りの出る割合は下がります**
——ジャングルは実り 144・獣 13 なので、144/157 ＝ 約 92% から、`expert`（獣 29）の 144/173 ＝ 約 83% へ。
**腕を上げた探索は獣へ寄ります。**

**上乗せは、つまみ 1 つにつき同じ量が積まれます。** 受け取る総量は獣の種類の数に比例する
ので、**種類の多い土地ほど腕が効きます**——荒野のネズミは 2/114 ＝ 約 2% から 6/118 ＝ 約 5% までで、
ジャングルの 8% → 17% には届きません。**この偏りは狙いどおりで、土地ごとに上乗せを割る調整は
入れません**（[`../world/Skills.md`](../world/Skills.md) 5.2 節）。

**海区の見張りも同じです。** 海区は土地と同じ `explorable`（[`../world/Voyage.md`](../world/Voyage.md)）で、
魚の群れと海鳥の群れ（`shoal_find`・`seabird_find`）が獣のつまみにあたります——どちらもその場に湧いて
しばらくすると去り、獲る手が狩猟の腕を配ります。**拾えるもの（流木・海藻・卵）には積みません。**
...
ありません。**手を出した個体だけは立ち去らず**（同節）、傷が癒えるか、洗えない傷が膿んで倒れるまで
世界に残ります——溜まらないことを支えているのは、こちらが手を出していないぶんです。
...
- **`travel_minutes`/`required_progress`/`destination_id`/`return_path_id`** はいずれも `path` の通常の
  ...
  **インスタンスIDを運ぶ初期値の `0` は「該当なし」の予約値**（`WorldObject.NO_INSTANCE`）なので、
  書き込まれる前に引いても、どの個体にも当たりません。
