...
問うたびに、その場所に並ぶ札は丸ごと作り直される（`PlayScreenView.cardsIn`）ので、**1枚ずつ問いに
行くと、判定がいくら安くても枚数の2乗で伸びます。** だから走査は**どの枠が受け入れるかをまとめて
問い**（`ShownCards.acceptingCells`）、並びを場所ごとに1度だけ作ります。同じ150枚を掴んだときの
