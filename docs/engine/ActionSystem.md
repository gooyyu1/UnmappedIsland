...
- **5分は、1 tick を渡すには短すぎる手のためにだけ在る。そこへ刻みを足すと
  格子から外れる**（5 + 15 = 20）ので、5分の所要時間は何にも動かさない。
  **5分の行動だけは、跨ぐ tick が0回か1回かで揺れたままになる**——15の倍数の行動は、開始時刻がどこでも
  必ず「長さ ÷ 15」回だけ跨ぐが、5分にはそれが無い。**だから5分にしてよいのは、手番を争わない手元の
  受け渡しだけ**——獣に手番を渡すか渡さないかが押した時刻で変わっても、それが得にも損にもならない操作。
  **これは0分と同じく素の分数を決める側の判断**で、宣言から引けるのは次の項だけ。
- **プレイヤー自身を動かす手は、5分にも0分にもしない。** 場所を移るのは獣から逃げる手でもあるので、
  跨ぐ tick が揺れると**同じ逃げ方が通るか通らないかが押す前に読めない**。0分にもしないのは、移るのに
  何も要らなければ**獣から離れる手が常に無料で通る**から——上の「繰り返しても2回目以降が何も生まない」
  には当たるが、繰り返しの向きを変えれば得が出る。**刻みの倍数にして、必ず1手を渡す。**
...
- 見張るのは `tests/world-codex/actionMinutesGrid.test.ts`。**今ずれている箇所の数え上げではなく**、
  世界じゅうの行動と、所要時間を動かしうる宣言のすべてを定義から引いて見る。**場所を移る手かどうかも
  定義から引く**——`move` が `agent` を動かす宣言（9.6 節）がそれで、名前では見分けない。
  **手番を争うかどうかは引けない**ので、5分の側で機械が言えるのは「場所を移る手がそこに居ない」まで。
  **ただし「縮めきっても0分にならない」だけは、同じ名前を複数の型が名乗っているとき、その宣言の
  中しか見ない**——どの型の素へ積む分なのかが名前からは決まらないため。

**出どころ**: [#2185](https://github.com/gooyyu1/UnmappedIsland/issues/2185)（行動の所要時間を15分の格子へ戻し、腕の効き方を行動ごとの宣言にする）と [#2245](https://github.com/gooyyu1/UnmappedIsland/issues/2245)（5分は手番を争わない受け渡しだけに残し、場所を移る手はそこから外す。2026-09-18に回答）
