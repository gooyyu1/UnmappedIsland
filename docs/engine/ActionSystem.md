...
**尽きた斧でも木が倒せ**、刃を減らす効果が落ちれば**倒しても斧が減らない**。相手に触れない宣言
...
現在地に世界にあるアイテムの型を1つずつ置いた142枚で、**いちばん多くの札と噛み合う物**
（この時点では `salt`。142枚のうち48枚が要件まで届く）を掴んで全部を両向きに問うのに
**0.47ミリ秒**でした（2026-09-18 時点、Intel Xeon @ 2.10GHz・Node v22 で計測）。
...
**この幅が保たれていることは `tests/diagnostics/interactionCost.test.ts` が見ます**——同じ並びを
組んで測り、桁の変わった遅さで赤くなります。
...
- **縮める宣言は、縮めた先が15分を割ってはならない。** レシピの `deftness`
  （[`GameElementDefinition.md`](./GameElementDefinition.md) 13.6 節）はロード時に弾く——黙って下限で
...
- **レシピの工程を割ると、腕が縮める分（`deftness`、13.6節）も工程の数だけ積まれる。** 同じ仕事を
