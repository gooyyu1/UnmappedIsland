...
**`volume` の単位は液体と同じ mL で、値は実際の占有体積ではなく「その物を包む直方体の体積」です。**
石なら 12×9×7cm = 700、ヤシの実なら 25×17×17cm = 7,000 のように、寸法の当たりを付けて掛けるだけで決めます。

**実占有体積より大きく見積もるのは意図的です。** `capacity` は中身の合計が上限に収まるかしか見ず、形の
噛み合わせを考えません。実際の籠は隙間なく詰められないので、**その詰めきれなさをかさの側へ織り込んでいます**。
丸い実や脚のある蟹は、まさに隙間を作る形なので、外接直方体との差が大きく出ます——それがそのまま
「かさばる」ことになります。

この見積もり方から、次のことが従います。

- **見かけの密度（`weight ÷ volume`）は必ず実際の比重より小さく出ます。** 石でも 1.4g/mL 程度で、
  花崗岩の 2.7 には届きません。逆にこれを超える値が出たら、どちらかの数値が桁違いです
  （`tests/worldCodex/containersYaml.test.ts` が全数を検査します）。
- **入れ子（籠の中の籠）は自然に止まります。** 編み籠のかさ（35L）は自分の容量（20L）より大きいので、
  別の籠を入れることができません。エンジンが禁じているのは自己包含だけで（7.1 節）、そこに専用の禁止
  規則は要りません。
- **重い物とかさばる物で、先に効く上限が違います。** 石を担げば `load` が先に尽き、ヤシの実や葉を運べば
  `capacity` が先に尽きます。**種類を少しずつ持ち歩けば、枠数（`cell_count`）が先に尽きます**——入れ物は
  「重さ・かさ・種類」の3つの理由で満杯になり、どれが先に来るかは荷の中身が決めます。何種類入るかは
  [`../world/Containers.md`](../world/Containers.md) 1節が決めます（編み籠は10枠）。

**入れ物の `capacity` は内容積そのまま**です（編み籠は 20L）。かさの側を大きく取っている以上、容量まで
割り増すと二重に緩みます。上で述べたとおり、実際に入る数は 20L ぶんより少なくなります。

**かさと枠数は役割が違います。** かさは「その物がどれだけ場所を取るか」という物の性質、枠数は
「何種類を持ち歩けるか」という入れ物の性質です。前者は丸太や葉を籠から弾き、後者は「道具も食料も
全部持ち歩く」を止めます。片方だけでは、小さい物なら何十種類でも入る入れ物か、種類さえ絞れば
丸太が入る入れ物になってしまいます。
