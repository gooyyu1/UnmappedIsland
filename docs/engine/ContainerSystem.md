...
控え（キャッシュ）を置きません。**辿る範囲に頭打ちが在るから**です——目方を名乗らないでいられるのは
土地と `world` だけ（1.1 節）で、その 2 つは誰かの中へ入らないので、**島に物がいくつ在っても、
辿る木には入ってきません。** 担ぎ手の木に入るのは、手の枠の数と入れ物のかさ（7 節）が受け入れた
ぶんだけです。

手の枠をすべていちばん大きい入れ物（`handcart`）で埋め、その中を籠と石で満たした木——**これ以上は
積めない 1050 物**——で、`weight` と `load` を 1 回ずつ読むのに **0.14 ミリ秒**でした（2026-09-18
時点、Intel Xeon @ 2.10GHz・Node v22 で計測）。1 フレーム（16 ミリ秒）の 1% にも届かず、貯め込みで
削る先がありません。

**この幅が保たれていることは `tests/diagnostics/interactionCost.test.ts` が見ます**——同じ木を組んで
測り、桁の変わった遅さで赤くなります。
...
- **入れ子（籠の中の籠）は自然に止まります。** 編み籠のかさ（29L）は自分の容量（20L）より大きいので、
  別の籠を入れることができません。エンジンが構造として断るのは輪になる配置だけで（7.1 節）、入れ子に
  専用の禁止規則は要りません。
