...
- **重い物とかさばる物で、先に効く上限が違います。** 石を担げば `load` が先に尽き、ヤシの実や葉を運べば
  `capacity` が先に尽きます。同じ入れ物が「重さ」と「かさ」の2つの理由で満杯になります。
...
- 入れ物ごとの「何枠入るか」は `contents` の `cell_count`（7.2 節）で決まるが、現在の
  `containers.yaml` は未指定（＝無制限）。[`../world/Containers.md`](../world/Containers.md) は入る量を
  **種類数**（編み籠なら6種類）で設計しているため、かさの上限（7 節）と枠数の上限のどちらを主に置くか、
  あるいは両方を併存させるかは未決
