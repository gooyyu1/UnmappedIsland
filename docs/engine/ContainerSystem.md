...
控え（キャッシュ）を置きません。**辿る範囲が、その物 1 つに入るぶんで頭打ちだから**です——目方を
名乗らないでいられるのは土地と `world` だけ（1.1 節）で、その 2 つは誰かの中へ入らないので、
**島に物がいくつ在っても、辿る木には入ってきません。** 入るのは、入れ物のかさ（7 節）が受け入れた
ぶんだけです。

担げるだけ担いだ木（そり 1 台に籠を詰めて 146 物）で、`weight` と `load` を 1 回ずつ読むのに
**0.02 ミリ秒**でした（2026-09-18 時点、Intel Xeon @ 2.10GHz・Node v22 で計測）。1 フレーム
（16 ミリ秒）の 1% にも届かず、貯め込みで削る先がありません。
