...
どう表すか（6 節）、物のかさと入れ物の容量をどう見積もるか（7 節）。`capacity` そのものは、宣言が
[`GameElementDefinition.md`](./GameElementDefinition.md) 7.3 節、受け入れ判定での扱いが
[`SlotSystem.md`](./SlotSystem.md) 2 節にあるだけの単純な仕組みです。
...
**ここに体感の率は一切かかりません。** 籠をそりへ積めば、そりの重さは籠の重さをそのまま加えたものになります。
そりが荷物を軽くするわけではないからです。軽減は「担いだ人がどう感じるか」の話であり、物の重さの話ではないので、
接触点である 2 節にだけ現れます。
...
丸ごと加算されます。効くのは引いているそりの率だけです。**率を持つ物どうしでも同じで**、`hand` にいない
側の率は使われません——そり（315L）が台車（容量200L）に入らないのはかさの話であって、率とは別です
（7節）。
...
読むたびに導出するので、`weight` を 1 回読むと**その物の部分木を丸ごと辿ります**（`load` に並ぶ寄与は
直接の子のぶんだけですが、その 1 件ずつが子の `weight` を読むので、辿る範囲は同じです）。それでも
控え（キャッシュ）を置きません。**1 物あたりが小さいから**です。

担いだ木 8000 物で、`weight` と `load` を 1 回ずつ読むのに **1.19 ミリ秒**——**1 物あたり 0.15
マイクロ秒**でした（2026-09-18 時点、Intel Xeon @ 2.10GHz・Node v22 で計測）。遊びが作る規模なら
1 フレーム（16 ミリ秒）の 1% にも届きません。

**「担げる量で頭打ちだから安い」ではありません。** 枠（`cell_count`）とかさ（`capacity`、7 節）が
決めるのは**物の数ではなく、詰める物 1 個のかさとの割り算**なので、かさの小さい物で埋めれば数は
いくらでも増えます（籠 1 つ（20L）に骨針（5mL）なら 4000 個）。**万を超える規模まで積めば、読み取りは
1 フレームに近づきます。**

頭打ちが在るのは範囲のほうだけです——目方を名乗らないでいられるのは土地と `world` だけ（1.1 節）で、
その 2 つは誰かの中へ入らないので、**島に物がいくつ在っても、担ぎ手の木には入ってきません。**

**1 物あたりが保たれていることは `tests/diagnostics/interactionCost.test.ts` が見ます**——同じ規模の
木を組んで測り、桁の変わった遅さで赤くなります。
...
**段が駆動するのは移動の可否だけではありません。** 歩みの遅れと `stamina` の削りも同じ段が持ちます
...
閾値ではなく段の名前で見るのは、同じ境目を `stages` の `min` と条件の側に二重に書くと、片方だけ
直したときに「バーは危険域なのに歩ける」が黙って成立するためです。段の名前は `too_heavy` で固定し、
閾値だけをキャラクタごとに変えます（[`../world/Characters.md`](../world/Characters.md)）。
止めたいのは `too_heavy` の下端を越えた荷なので、演算子は「その段以上」の側です
（`in_stage_or_above`、`GameElementDefinition.md` 14.1 節）。
...
- **入れ子（籠の中の籠）は自然に止まります。** 編み籠のかさ（29L）は自分の容量（20L）より大きいので、
  別の籠を入れることができません。エンジンが構造として断るのは輪になる配置だけで
  （[`GameElementDefinition.md`](./GameElementDefinition.md) 7.1 節）、入れ子に専用の禁止規則は要りません。
...
- **荷重の段ごとの遅れと `stamina` の削り**（[`../world/Characters.md`](../world/Characters.md)
  荷重の効き方節）。「1 日ぶんと、荷を下ろして眠った一晩の回復が同じ桁」という置き方だけが決まっていて、
  `laden` と `heavy` の差をどれだけ開けるかは、**1 日にどれだけ往復するのが普通か**が見えてから決める。
  **段の削りを変えると、担いだままの休息で戻る量も一緒に動く**（回復は削りと同じ足し算に入る、
  [`GameElementDefinition.md`](./GameElementDefinition.md) 11.7 節）ので、動かすのは 2 つの数ではない
