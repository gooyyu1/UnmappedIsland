...
**割るだけでは熱は増えません。** 増えるのは形のほうです——**炉に入り切ること**（`fuel` の上限はどの炉も
丸太 1 本の 200 を下回るので、丸太をくべると入りきらない分が失われます。2 節・6 節。乾いた薪 1 本の 30 は、
いちばん小さい焚き火の上限にもそのまま入ります）と、**乾かせること**（丸太のままでは中まで乾きません）。
...
摩擦発火を通らないので雨は関係ありません（3.1.2 節）。**雨が火起こしを閉じること（3.1.1 節）が効くのは、
島から火が絶えたとき**です。
...
- **暖**: `ambient_temperature` を +8 上げます。世界が最も冷えるとき（涼しい季節 −5 ＋ 夜 −3、
  [`ClimateSystem.md`](./ClimateSystem.md) 1 節）を、ちょうど平年の気温へ戻す量です。
