...
**だから薪が先です**——薪を組む前に火種を作ると、断られた火種はそのまま燃え尽きて 1 つ失われ、火起こし
からやり直しになります。この順序は炉を見ても分からないので、**断る理由を名乗る**のは `already_lit` と
同じです（[`CardInteraction.md`](../ui/CardInteraction.md) 2.1 節）。
