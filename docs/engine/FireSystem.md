...
いうことは、**枠があるかどうか**で表せます。枠は宣言順ではなく型の合う枠へ埋まる
（[`SlotSystem.md`](./SlotSystem.md) 2 節）ので、
焼く物を重ねれば火の中へ、器を重ねれば石の上へ、条件を書かずに入ります。

**どの枠がどちらなのかは、空いているうちに枠自身が名乗ります**——空き枠へその枠が受け入れる型の
カードを薄く敷く仕組み（[`CardView.md`](../ui/CardView.md) 11 節）は**枠によって受け入れの宣言が
違うスロットで働く**ので、器を載せる枠を持つ炉だけがこれを出します。器を載せる枠を持たない炉は
どの枠も同じものを受けるので、名乗っても並びが既に言っていることの繰り返しにしかなりません。

**石の上の枠は、今は紙のままです**——`cookware` を名乗る型が世界にまだ無く（煮炊きのレシピは 11 節の
未決事項）、敷く絵が無いためです。型が入れば、条件を書き足さずにそのまま出ます。
...
枠に残れるように持ちます（`GameElementDefinition.md` 7.2 節）。焼け石（9 節）を実装するときは、石が
このタグを得ます。
...
スロットを 2 つに分けないのは、**分けると炉の中身が一度に見えなくなる**ためです。外から見えると
名乗ったスロットは子ウィンドウのタブになる（`GameElementDefinition.md` 7.11 節、
[`Windows.md`](../ui/Windows.md) 1.2 節）ので、2 つ目を足せば火の中と石の上が別のタブへ分かれます。
どちらの枠の物も同じ火から熱を受け続けている（7 節）のに、見えるのは開いているほうのタブだけです
——石の上を覗いている間も火の中の物は進み、目を離せば焦げます（7.2 節）。枠なら 1 つの並びのままで、
受け入れの違いだけが残ります。

炉ごとの差も、枠なら `cells` の並び 1 つに収まります（6 節の表）。分けると、器を載せられるかは
「スロットを持つか」、いくつ載るかは「枠がいくつか」と、1 本の梯子が 2 つの軸に散ります。
...
interactions:
  add_fuel:
    ...
    no_room_reason: hearth_full  # 丸ごと入らないときに断る理由（14.6節）
    transfer:
      amount: 999                # 1本ぶんを丸ごと
      from: instrument
      from_prop: fuel
      to_prop: fuel
    destroy: instrument          # くべた薪は物として残らない
```

**何本くべられるかは書きません。** 移送先の `fuel` は `range` を持つので、その残りから決まります
（同 12.4 節）。**上限は `range` が既に言っている**ので、束から何本入るかを条件で数え直す必要は
ありません。

**丸ごと入らない薪は受け取りません。** 運ばれてきた薪を `destroy` で消す操作は、移し切れない分が薪ごと
失われるので、エンジンが端数を受け取らずに断ります（`GameElementDefinition.md` 9.5 節）。**満杯も、残りが
半端なのも、この 1 つの線で断ります**——炉ごとに上限を書き写した条件は要りません（書き写すと、
`range.max` が 2 箇所に分かれます）。

**断る理由の名前だけは書きます。** 断られた組み合わせは落とし先として残らないため、名乗らなければ
プレイヤーには「重ねても何も起きない」としか見えません
（[`CardInteraction.md`](../ui/CardInteraction.md) 2.1 節）。罠の餌・囲いの飼葉と同じ形で、**この形の
操作が名前を宣言していなければロード時に落ちます**（`GameElementDefinition.md` 9.5 節）。
...
説明のつかない物が生まれます。**残った分を薪ごと捨てることもしません**——`destroy` を併記したことが、
上の「丸ごと入らないなら受け取らない」を呼びます。焚き火（上限 30）が太い枝（20）を 2 本は受けない、
丸太（200）はそれ専用の炉（覆い焼きの炉、`pottery.yaml`）でしか焚けない、という線はここから出ます。

### 2.1 燃料を枠へ置かない理由

くべた薪を `fire` の枠に置き続ける形も書けますが、採りません。

- **1 つの枠には同じ型しか積めない**（枠は 1 つのスタックを持ち、スタックは同種だけを束ねます。
  [`SlotSystem.md`](./SlotSystem.md) 1 節・5 節）ので、枠の数がそのまま同時に燃やせる燃料の種類数に
...
炉のプロパティへ溶かせば、どれも消えます。**くべた薪は炉の中に「在る」ものではなく、火が「食べて
しまった」もの**だからです。

### 2.2 火力は、薪を食って育ち、薪が尽きると衰える

**炉が持つ数は `fuel`（薪）と `heat`（火力）だけで、どちらも `add` で動く実体値です。**
...
残っているのに火が衰える区間ができます。衰えを呼ぶ条件は段ではなく `fuel: 0` を見ます。

**その区間では育ちもしません。** 燃え残りはあっても、火を大きくするには塊 1 つぶんが要るからです。
育ちも衰えもしない区間になりますが、下の打ち消しは起きません——育ちは 1 以上、衰えは 0 ちょうどで、
重なりません。
...
突き合わせます）。最も忙しいのは焚き火と三石のかまどを炎で焚いたときで、**火力の上限が炎までしか
...
上限は丸太 1 本の 200 を下回るので、**丸太はそのままでは受け取られません**。2 節・6 節。乾いた薪 1 本の
30 は、いちばん小さい焚き火の上限にもそのまま入ります。丸太を丸ごと受け切れるのは土器を焼く覆い焼きの
炉だけで、そこは土器しか載りません）と、**乾かせること**（丸太のままでは中まで乾きません）。
...
満たさない組み合わせはそもそも候補にならないので、**外した回と違って火口は減りません**——雨の中で
...
**火種は短命です。** 1 tick で燃え尽きて消えるので、届くのは**同じ土地の中**までです。洞窟の中で
起こして外の炉へ運ぶことはできますが、**土地を移動すれば必ず燃え尽きます**——道 1 本には最低 1 tick
かかるからです（[`TerrainGeneration.md`](./TerrainGeneration.md) 3.5 節）。**火起こしからの火は、その土地で
使い切ります**——火を別の土地へ運ぶには、燃え尽きない入れ物、つまり灯した明かりが要ります（3.1.2 節）。

**薪の入っていない炉も火種を断ります。** 火を育てるのは薪で、種火だけでは残らないからです（2.2 節）。
**断る理由を名乗る**のは `already_lit` と同じです（[`CardInteraction.md`](../ui/CardInteraction.md) 2.1 節）。

**だから薪が先です。** 断られているあいだも火種は燃え尽きへ向かうので、**その場に薪が無ければ、組み直す
より先に火種を失います**——薪を取りに土地を移れば、上のとおり必ず失います。火起こしからやり直しです。

**雨の日に、火を起こして戻すには屋根の下が要ります**（3.1.1 節）。**浅い洞窟の無い土地では、雨が上がるまで
火を起こせません**——火種を作れる場所がないからです。**これが洞窟に「火を絶やさないための価値」を
与えます。** 石囲いの炉の種火が翌朝まで生きること（6 節）が最も効くのも、雨の日です。

**ただし、生きている炉がどこかに在れば、雨でも炉の火は戻せます**——そこで灯した明かりを運べばよく、
摩擦発火を通らないので雨は関係ありません（3.1.2 節）。**戻した種火が残るかは薪の段しだいです**
——野ざらしの炉は雨に `heat` を毎 tick −4 削られる（8 節）ので、`few`（`fuel` 9 以下）の育ち +2 では
立てた種火が次の tick で消えます。**`some`（`fuel` 10 以上、太い枝 1 本で届く）なら +6 が上回って育ちます**
（2.2 節）。屋根の下（`sheltered`）ならこの差は要りません。**雨が火起こしを閉じること（3.1.1 節）が
効くのは、島から火が絶えたとき**です。

**夜の暗さの側は動きません**（[`IlluminationSystem.md`](./IlluminationSystem.md) 5.1 節）。
あちらが「唯一の戻り道」と呼ぶのは**暗がりから手元の明るさを取り戻す道**で、明かりを運ぶには
その明かりが既に灯っている——つまり暗がりに落ちていない——必要があるからです。

#### 3.1.1 雨の日は、屋根の下でしか火を起こせない【確定】

**雨の降っている屋外では、火が起こせません。** 成功率が下がるのではなく、できない側に線を引きます
——雨に打たれた火口は、摩擦で得た熾を受け止められないからです。**屋根の下——浅い洞窟
（[`../world/Dwellings.md`](../world/Dwellings.md) 5.1 節）——なら、降っていても起こせます。**
...
#### 3.1.2 炉と明かりの間では、火の在る側から無い側へ移る

**炉と持ち歩ける明かり（`lightable`）を重ねると、火の在るほうから無いほうへ移ります。** どちらの向きも
火種は要りません——火種は火の無いところへ火を立てるための物で、既に在る炎を分けるのに摩擦発火を通す
...
**この順は選び取ったもので、ただ取りではありません。** 先頭を取れるのは片方だけなので、**どちらも
火を持っていない場合**——灯っていない明かりを消えた炉へ重ねる——に届くのが「この炉は消えている」から
「この明かりは灯っていない」へ入れ替わります。どちらも真で、どちらも相手の側の事実は言いません。
**薪の無い炉に火を点けようとする場面のほうが起きやすい**（火種でも同じ断りが「薪が先」を教えている、
3.1 節）ので、そちらを取っています。

**火起こしをやり直させないのは、そうすると松明が割に合わないからです。** 灯すたびに火口と火起こし具を
通すと、**松明1本にかかる手間が、その1本が燃える時間を超えます**
（[`ContentSkeleton.md`](../world/ContentSkeleton.md) 8.1.1.4節が両方の数を持ちます）。

**日暮れ前の段取り（[`IlluminationSystem.md`](./IlluminationSystem.md) 5.1節）は、どちらの向きでも
崩れません。** どちらの向きも**生きている炉から始まる**からです——明かりを灯すにも、その明かりで別の
炉を灯すにも、まず分けてもらえる炎が要り、**その炉を明るいうちに立てておくことが要求そのもの**です。
炉が無い場所で明かりが要るなら、火起こしから通す道（3.1節）が残っています。

**炉を灯す向きが開いていることで、火が土地を越えます。** 松明は道を何本も渡れるだけもつので、
**生きている炉がどこかに在れば、そこから火を運んで別の土地の炉に立てられます**。火起こし具と火口を
持ち歩く理由が消えるわけではありません——運べるのは松明が燃え尽きるまでの間だけで、**島から火が絶えれば
火起こしへ戻ります**。どちらを選ぶかは、生きた炉までの遠さと天気が決めます（3.1.1 節）。
...
**火口は `moisture`（吸った水）を持ち、その段が着火の重みを押し下げます。** 名前を素焼きの壺
（`pottery.yaml`）と共有するのは、同じ「含んでいる水が抜けていく」を指すからで、**効かせ方は違います**
...
（[`ContainerSystem.md`](./ContainerSystem.md) 6 節の `sheltered`）でも、鞄や籠の中でも濡れません
...
素の値は型ごとに固定で、カードの名前と絵が既に言っている——**1 枚ごとに動くのは湿りのほう**なので、
読ませるのもそちらだけにします。`ignition_chance` そのものをバーにする道はありません：ゲージは
`range` を要求し（同 8.1 節）、`range` は実効値をクランプする（[`GameElementDefinition.md`](./GameElementDefinition.md)
8.3 節）ので、腕の上乗せ（`expert` で +120）が頭打ちになります。

**火口は束ねられるままです。** 束ねた札が映すのは代表 1 つの湿りなので、乾いた束へ濡れた 1 つを
混ぜると、その 1 つは札の上では見えなくなります（[`CardView.md`](../ui/CardView.md) 8 節）。**干して
いる食べ物が既に持っている形と同じ**（`drying.yaml` の `drying_remaining`）で、1 枚ずつの札に割ると、
束で拾う火口が枠を食い尽くします。

**押し下げは、どの火口にも同じ量です。** 割合で効かせないのは、腕の上乗せが加算である
（[`../world/Skills.md`](../world/Skills.md) 5.2 節）のと同じ器に揃えるためです。同じ量なので、
**素の値の小さい火口を足すと重みが 0 以下へ潰れえます**——潰れた火口はその段で絶対に付かなくなり、
確率のはずが可否の線になります。`tests/world-codex/fireYaml.test.ts` が全火口について見張ります。
...
食っている薪の量そのもの**です。これらは同じ現象の表と裏で、「熱いほど速く燃える」のではなく「速く
...
仕組みを持ち込まずに済みます。**上位の焼成設備（恒久窯）は足しません**
...
**一度に焼ける量は枠の数が決めます**（段ごとの枠は 6 節の表）。薪は枠を
...
生 → 焼き → 焦げ を成り立たせているのは、`spawn` の既定の振る舞いです
...
の中（[`../world/Dwellings.md`](../world/Dwellings.md) 5.1 節）や将来の屋根の下では `sheltered` が 1 になり、
雨の効果自体が消えます（`ContainerSystem.md` 6 節）。**雨をしのげるかを見る道はこれだけで、洞窟が天気を
宣言し直すような別の道は持ちません。**

風は独立したプロパティを持ちません。風を切る効果は石囲いの炉の燃料消費（6 節）に、嵐の強風はこの節の
`heat` の減少に畳んであります。

### 8.1 炉そのものは、雨で崩れない【確定】
...
**火が与えるものの品揃えは 9.3 節が決めます。**

### 9.1 焼け石は、炉から取り出せる唯一の産物
...
**熱の受け渡しは料理と同じ 1 本です。** 炉は火力の段ごとに、子の `cooking_progress` と並べて
`heat_soak` を進めます（7 節）。持たない側では黙って捨てられるので、炉は相手が料理か石かを知りません。
...
**器の中と外で減り方は変えません**（11 節にあった未決）。湯へ落とした石は溜めた熱を一度に使い切り、
その場で普通の石へ戻ります。急に冷めることは、減る速さではなく**使い切ること**が表しています。

**沸かした湯は `hot_water_liquid` で、水とは別の液体です**（`liquid_containers.yaml`）。湯へさらに
焼け石は落とせず、水と混ぜて薄まることもありません。残っている熱が抜け切れば水へ戻ります。

**湯は注ぎ分けられません。** `heat_soak` が持っているのは量ではなく熱さそのもので、注いだ先が何度から
始まるかを既存の宣言では書けません——量に比例して移す `transfer`（[`GameElementDefinition.md`](./GameElementDefinition.md)
...
（`core.yaml` の `location` trait。[`IlluminationSystem.md`](./IlluminationSystem.md) 2 節・6 節）。
そのため**炉のある場所だけが暖まり、隣の土地も世界も動かず**、据えた炉の明かりは手元にしか届きません。
炉自身はどこへ効くのかを知りません。

### 9.3 火が与えるのは、産物と、置かれた場所へ届くもの【確定】

**火が与えるものは次のとおりです。** 炉から取り出せる産物（焼け石、9.1 節）と、置かれた場所へ届くもの
（暖と明かり、9.2 節）で、どちらも火の連鎖（2 節）の外にあります。

**出どころ**: ユーザーの指示（火が与えるのは、産物と、置かれた場所へ届くもの。[#2026](https://github.com/gooyyu1/UnmappedIsland/issues/2026)）

## 10. 火スキルとの分担

**炉は「今どれだけ燃えているか」を、火スキルは「火を扱えるか」を持ちます**（`Skills.md` 2.2 節）。
どちらも段を持ちますが、見ているものが違います。

| | 決めるもの | 表し方 |
|---|---|---|
| 炉の `heat` の段 | 今この炉で何ができるか | 薪が育て、火が冷ます釣り合い（2.2 節） |
| 火スキルの段 | 着火が成功するか | 着火の `pick` の重み |

スキルの段が効くのは火起こしの `pick` の重み（3.1 節）だけです。**火がついた後の火力にスキルは
効きません**——熟練者の焚き火が同じ薪でより熱くなることはないためです。`Skills.md` 2.2 節が火の担う
こととして挙げている残り——種火を保つことと高温を出すこと——を持つのは**炉の側**で、保つ長さは
炉ごとの冷める速さ（6 節）、高温は石で囲うこと（5 節）が決めます。**どちらも腕を要求しません**（石を
...
**火口ごとの差が腕とともに縮むのも狙いどおりで、火口ごとに上乗せを変える調整は入れません**
（[`../world/Skills.md`](../world/Skills.md) 5.2 節）。**上の水準は
`tests/world-codex/fireYaml.test.ts` が全火口 × 全段で見張ります**——外れの重みを緩めても、火口の素の
値を下げても赤くなります。**腕の上乗せを削っても、この水準は破れません**——縛っているのは上乗せの
無い `novice` の側なので、そちらは刻みそのものを `tests/world-codex/skillsYaml.test.ts` が見張ります。

## 11. 未決事項・今後の検討課題

- 火が燃え移ること（延焼）。炉の火を意図して分ける道はある（3.1.2 節）が、意図せず燃え移る経路は無い
- 雨が、灯った明かりを消すか。今は消さないので、**雨の屋外でも松明を運べば火は戻せる**（3.1.2 節）。
  3.1.1 節が確定として閉じているのは着火の 1 点だけなので、ここを閉じるなら別の判断が要る
- 煙。虫除けと、遠くから見えることのどちらを表すか。**燻製による保存は煙を持たずに入りました**
  ——燻し小屋（`smoking.yaml`）が読むのは火力そのもので、煙という概念は世界にまだ無い
- 沸かした湯の使い道。今は水と同じく飲めるだけで、煮炊きのレシピがまだ無い（9.1 節）
