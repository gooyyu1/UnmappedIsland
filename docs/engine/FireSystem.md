...
かかるからです（[`TerrainGeneration.md`](./TerrainGeneration.md) 3.5 節）。火を起こすのは炉のある土地です。
...
**雨の日に火を戻すには、屋根の下が要ります**（3.1.1 節）。**浅い洞窟の無い土地では、雨が上がるまで
火が戻りません**——火種を作れる場所がないからです。**これが洞窟に「火を絶やさないための価値」を
与えます。** 石囲いの炉の種火が翌朝まで生きること（6 節）が最も効くのも、雨の日です。

**炉に火を点ける物は、土地を越えられません。** 炉から松明へ火を分ける道はありますが（3.1.2 節）、
**その逆——灯った松明を別の土地へ運んで炉に火を点ける——は開けません。** 松明は道を何本も渡れるだけ
もつので、開けた瞬間に火が島を移動できるようになります。**雨の線から戻る道が 2 本になり**（屋根の下で
起こすか、別の土地の炉から運ぶか）、**遠征に火起こし具と火口を持つ理由が消えます**
（[`DesignPrinciples.md`](../concept/DesignPrinciples.md)「線を引いたら、そこから戻る道を 1 本だけ
開けておく」「後から足す手段は、既存の手段を残す強さに抑える」）。**「火を絶やさない」が意味するのも、
1 つの炉を守ることのままです**——どこか 1 つを守ればよいことにはなりません。

**現実の模型としてはずれます**——灯った松明で焚き火に火は点きます。ずれは**プレイヤーへ厳しい側**なので
置いたままにします（同「模型のずれは、罰の向きが正しければ許容する」）。

**閉じているのは、火種が土地を越えられないことだけによります。** 炉の `heat` を 0 から立ち上げる操作は
`ignite` だけで、それが受けるのは火種だけだからです。**この形は
`tests/world-codex/fireYaml.test.ts` が見張ります**——火種の寿命を延ばしても、炉の火を立てられる相手を
増やしても落ちます。

**燃えている炉そのものは、筏に据えれば筏ごと動きます**（[`ContainerSystem.md`](./ContainerSystem.md)
1 節）。**これは別のことです**——動くのは火を点ける物ではなく炉自身で、着いた先の炉に火が移るわけでは
ありません。
...
#### 3.1.2 燃えている炉からは、火起こしを通さずに火を分けられる

**燃えている炉へ、持ち歩ける明かり（`lightable`）を重ねれば灯ります。** 火種は要りません——火種は
火の無いところへ火を立てるための物で、既に在る炎を分けるのに摩擦発火を通す理由がないからです。

```yaml
# 炉の側が宣言する。分けても炉は何も失わないので、火力にも薪にも触れない
interactions:
  light_from_flame:
...
**日暮れ前の段取り（[`IlluminationSystem.md`](./IlluminationSystem.md) 5.1節）は崩れません。**
分けてもらえるのは生きている炉の炎だけで、**その炉を明るいうちに立てておくことが要求そのもの**
だからです。炉が無い場所で明かりが要るなら、火起こしから通す道（3.1節）が残っています。
