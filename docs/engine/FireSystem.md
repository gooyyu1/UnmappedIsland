...
腕（`ignition_ease`）が `base` の土台として積まれたものです——枯れ草なら `novice` で 6 割、
`expert`（+120）で 8 割強になります。**外れの重み（40）は動かしません**：外した回に失われるのは
火口 1 つで、その損は腕によらないからです。**火口ごとの差が腕とともに縮むのも狙いどおりで、火口ごとに
上乗せを変える調整は入れません**（[`../world/Skills.md`](../world/Skills.md) 5.2 節）。
