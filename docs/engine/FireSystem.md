...
267 分<!-- stats: terrain.yaml daily_budget survival_gathering -->（[`ContentSkeleton.md`](../world/ContentSkeleton.md)
...
起こして外の炉へ運ぶことはできますが、**土地を移動すれば必ず燃え尽きます**——道は最低 15 分＝1 tick
だからです（[`TerrainGeneration.md`](./TerrainGeneration.md) 3.5 節）。火を起こすのは炉のある土地です。
...
- 灯った松明から炉へ火を戻せるようにするか（3.1.2 節の逆向き）。今は `ignite` が火種しか受けないので、
  火は土地を越えない。開けると「火を絶やさない」の意味が変わるので、遊びの側の判断が要る
