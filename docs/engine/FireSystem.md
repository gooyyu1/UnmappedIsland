...
[`Windows.md`](../ui/Windows.md) 1.2 節）ので、2 つ目を足せば火の中と石の上が別のタブへ分かれ、
片方を見ている間はもう片方が見えません。どちらも同じ火から熱を受けていて（7 節）、目を離せば焦げる
（7.2 節）ので、今どうなっているかを見るのにタブの切り替えを挟む形にはできません。枠なら 1 つの並びの
ままで、受け入れの違いだけが残ります。
