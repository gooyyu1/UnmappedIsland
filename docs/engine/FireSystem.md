...
**満杯を拒む条件だけは書きます。** 残りが 0 なら組み合わせは候補にならないので、拒むだけなら容量で
足ります。それでも書くのは、**断る理由（`reason`）を届けるため**です——容量で落ちた組み合わせは
落とし先として残らないため、プレイヤーには「重ねても何も起きない」としか見えません
（[`CardInteraction.md`](../ui/CardInteraction.md) 2.1 節）。罠の餌・囲いの飼葉と同じ形です。

薪の上限は炉ごとに違うので、この条件を持つのは `hearth` trait ではなく炉の側です。trait が配った
