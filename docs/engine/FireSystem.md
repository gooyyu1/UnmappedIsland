...
```yaml
# 火口の側が宣言する。成功の重みは火口ごとの素の値に起こす人の腕を積んだもの（10 節）で、
# 天気は可否を決める（3.1.1 節）
interactions:
  light:
...
**火口は `moisture`（吸った水）を持ち、その段が着火の重みを押し下げます。** 形は素焼きの壺
（`pottery.yaml`）と同じで、値そのものではなく**段**が効き、乾き切れば押し下げは無くなります。
...
**乾かす道は 2 本あります。** 地面に広げて強い日差しに当てれば速く（`-3/tick`、境目は塩田・干し場と
同じ [`IlluminationSystem.md`](./IlluminationSystem.md) 1 節の EV +14）、どこに在っても抜けていきます
（`-1/tick`）。後者があるので、**濡れた火口が詰みになることはありません**——雨宿りした洞窟に日は
差しませんが、待てば戻ります。
