...
実測（[`stats/climate.yaml`](../../stats/climate.yaml) の `non_rain_streak`、`season: calm`）でも、連続
未降雨時間の平均はこの 1.7〜2.5 日の帯に収まります。
...
`rain_streak`、`season: calm`）で止むのが基本形です。
...
`rain_streak`・`non_rain_streak`）。なお `light_rain` の
