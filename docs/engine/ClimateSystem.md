...
`core.yaml` で既に時間・天気・環境光・気温を直接プロパティとして
持つ）自身のプロパティとして表現し、専用の `WorldObject` インスタンスは作りません。

- **季節（長期サイクル）**: 穏やか・雨季・乾季を固定順で巡回する。滞在期間は初回サイクルのみ固定、
  2 周目以降はランダムに決まる（3 節）。
- **天気（短期変動）**: 灼熱（`scorching`）・快晴（`sunny`）・晴れ（`clear`）・曇り（`cloudy`）・
  小雨（`light_rain`）・大雨（`heavy_rain`）・嵐（`storm`）を段階として並べる。季節が育てる貯水池
  （大気水分量・蓄熱量）に重み付けされた抽選で決まる、季節から独立したプロパティ（4 節)。

季節・天気とも、「現在の値」と「残り時間」のプロパティの組で表現し、残り時間が 0 になった瞬間に次の値へ
...
| 土地 | 平均海抜 | `value` |
| --- | --: | --: |
| 砂浜 | 7.5<!-- stats: terrain.yaml site_elevation_by_location location=sandy_beach mean ±5% --> m | — |
| 岩礁海岸 | 15<!-- stats: terrain.yaml site_elevation_by_location location=rocky_coast mean ±5% --> m | — |
| 断崖海岸 | 40<!-- stats: terrain.yaml site_elevation_by_location location=cliff_coast mean ±5% --> m | — |
...
- `season_cycle`: 季節を何周したかを表すカウンタ。`0` から始まり、1 周（`calm`→`wet`→`dry`→`calm`）ごとに
...
現在値を参照して動的に変えることはできません（同 9.2 節）。**毎 tick の量を現在値で動かせるのは
`transfer` だけ**で（同 8.4.1 節。付随する加減算を実際に移動した量へ比例させる `linked_add` を含む、
同 9.5 節）、その「実際に移動した量」を決めるのは移送元の残量（`allow_overflow` が既定の `false` なら
...
「乾いている間・穏やかな季節の小雨は大雨・嵐にならない」（3.2 節の保証）を表現しています。`clear` の重みが
...
補正（ゲーム開始 2 日目、最初の乾季 10 日目前後）は無関係な別々の意図によるものなので、1 つのプロパティに
...
- 季節・天気とも、専用の `WorldObject` インスタンスではなく `world` 自身のプロパティ（現在値＋残り時間）として
  表現する
- 季節は穏やか/雨季/乾季を固定順で巡回する。天気は季節から独立した symbol プロパティであり、季節の
  値そのものからは直接決まらない
...
- 大気水分量は、天気そのものの stages 閾値としては使わず（4.1 節）、天気候補ごとの重み専用プロパティを
  ...
  しない（極端な条件下で「絶対に晴れない」「絶対に嵐にならない」を表現できるようにするため）。`clear` の
  重みが立つ段階では晴れ系の中で `clear` を最も重くし、この島の標準的な天気にする
