...
実測（20 シード×3,600 日、[`stats/climate.yaml`](../../stats/climate.yaml) の `non_rain_streak`）では、
`calm` の連続未降雨時間は平均 2.17 日・95%ile 4.21 日・最大 6.62 日です。
...
でした（[`stats/climate.yaml`](../../stats/climate.yaml) の `weather_hours`・`rain_streak`・
`non_rain_streak`、20 シード×3,600 日）。なお `light_rain` の
