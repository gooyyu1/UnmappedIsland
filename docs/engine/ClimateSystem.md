...
（`tests/worldCodex/climateSystem.test.ts`。「calm は概ね 2 日に 1 回軽い雨が降る」「wet はほとんど雨だが
...
- **季節（長期サイクル）**: 穏やか・雨季・乾季の 3 状態を固定順で巡回する。滞在期間は初回サイクルのみ固定、
  2 周目以降はランダムに決まる（3 節）。
- **天気（短期変動）**: 炎天・快晴・晴れ・曇り・小雨・大雨・嵐の 7 段階。季節が育てる貯水池
...
- `atmospheric_moisture`（`world` の蓄積型プロパティ、range 0〜10,000）: `season` の各 stage が持つレートを
  毎 tick 加算する（calm: +10 / wet: +100 / dry: -20）。スケールの基準（1 tick 分 = 100、
...
- `thermal_level`（蓄熱量。同じく `world` の蓄積型プロパティ、range 0〜1,920）: `season` の各 stage が
  ...
  **上限は `hot` 閾値 1,920 と同値**とし、残暑（calm へ持ち越される炎天）は無い: calm に入った最初の
...
- 降雨中の自己減算: 天気（4 節）が `light_rain`/`heavy_rain`/`storm` のいずれかである間、その `weather` の
  ...
  （-110 / -130 / -200。激しい雨ほど速く消費する。`GameElementDefinition.md` 6.4 節の `progress`/`feverish`
  例と同じ、ステージ自身が値を変化させるパターン）。どの減算量も `wet` の季節レート（+100/tick）をやや上回る
  （正味 -20 / -40 / -110）ため、降り続けた雨はいずれ水分を減らして晴れ間を呼び込みます。`light_rain` の
...
登り切った場合にのみ大雨が起こりえます（実測で `calm` の tick 比 0.1% 前後。許容する揺らぎ）。
実測（20 シード×3,600 日、`docs/diagnostics/ClimateSystemStats.md`）では、`calm` の連続未降雨時間は
平均 2.17 日・95%ile 4.21 日・最大 6.62 日です。

自己減算を弱めると「押し戻す」度合いが浅くなり、連続未降雨時間は**短くなります**（両者は独立ではなく
直結しています）。逆に連続降雨時間を伸ばす方向には自己減算はあまり効きません: `moderate` 帯では雨が
続けて選ばれる確率自体が重み比で頭打ちのため、`calm` の雨は 1〜2 周期（実測で平均 0.21 日、
95%ile 0.25 日）で止むのが基本形です。
...
生き、嵐・大雨の名残が自然に表現されます。実測（20 シード×3,600 日）では、`wet` を 3 等分した嵐・大雨の
tick 比率は序盤 31%→中盤 38%→終盤 42%、嵐だけでは 4%→6%→18% と後半に向かって単調に増えます
（統計テスト「雨季(wet)は後半ほど嵐・大雨が増える」（`tests/worldCodex/climateSystem.test.ts`）が回帰検知します）。

ただし**名残の振る舞いは意図的に非対称**です。嵐には名残があります（雨季末の高い水分が消費し尽くされる
までの乾季序盤には嵐・大雨が残る）が、炎天には名残（残暑）がありません（蓄熱量の上限 = `hot` 閾値のため、
calm に入った瞬間に炎天は消える。3 節）。プレイヤーから見た「乾季の暑さが長い年」は、残暑ではなく季節の
持続日数のランダム性（2.3 節）が表現します。ゲーム開始直後の最初の calm は、蓄熱量の初期値が中立
（1,000 = `mild`）のため炎天は発生しません。
...
`light_rain` -10/tick（季節レート +100 － 自己減算 110）、`heavy_rain` -30/tick、`storm` -100/tick、
非降雨時 +100/tick です。どの雨も正味マイナスのため降り続けばいずれ晴れ間を呼び込み、非降雨時の +100 が
素早く水分を回復させて次の雨へつなぐ——という充放電の往復が、`humid` 閾値（6,000）付近を上下しながら
「ほとんど雨だが、まとまった雨と晴れ間が交互に来る」という雨季の体感を作ります。実測では `wet` の
雨 tick 比率は約 75%（うち大雨・嵐が約 42%）、連続降雨時間は平均 0.73 日、連続未降雨時間は平均 0.24 日
でした（`docs/diagnostics/ClimateSystemStats.md`、20 シード×3,600 日）。なお `light_rain` の
自己減算（-110）は雨季のためではなく、`calm` の連続未降雨時間を約 2 日にする（3.2 節）ことから決まって
...
（2 日目の 96 tick 分）にいる間だけ `atmospheric_moisture` への追加加算（+60/tick。calm の +10 と合わせて
+70/tick）が有効になり、3 日目に入ると自動的にオフへ戻ります（`stages` は現在値だけで区間を決めるため、
...
`boosting` の +200/tick は dry の -20/tick を大きく上回る（正味 +180/tick）ため、乾ききった水分量が
...
- 両極の極端な天候は、消費要因を持たない単調な季節内進行度が駆動する: 嵐・後半の大雨は `monsoon_level` が、
  炎天は `thermal_level` の `hot` 帯（乾季後半）が重みを立てる（4.3 節）。名残は意図的に非対称: 嵐・大雨は
  乾季序盤に名残が残るが、炎天は蓄熱量の上限 = `hot` 閾値により乾季の終了と同時に消える（残暑なし）
...
- 各レート・閾値・重みの数値は統計テストを満たす初期値であり、実プレイでの体感に基づくバランス調整は今後の
  課題（数値を変えたら `tests/worldCodex/climateSystem.test.ts` が要件を満たし続けるかで回帰を検知できる）
