...
| 段 `frozen` | 熱を失い切った域。`exsanguinated` と並ぶ、本書が `fatal` を置く 2 つの段 |
...
——渇き・飢え・失血・凍えの4つで死に、死ねばセーブデータごと消えます。**2 節から 6 節までは4種の動物で
...
`consciousness` はまだ配っていません（7 節）。8 節が挙げる**新しい死因を既存の死に方へ流す道**のうち、
**食の偏りは実装済み**（[`DigestionSystem.md`](./DigestionSystem.md) 7 節）で、感染と発熱は未実装です。
**血が戻る条件（3.1 節）も未実装です**。**寒さを防ぐ側のうち入っているのは火と洞窟だけで、
衣服・寝床が `chill_point` を押し下げる側はまだありません**（8.3 節）。
未決事項は末尾に整理しています。
...
### 3.1 血が戻るのは、水と食べ物が足りている間だけ【確定】【未実装: 血の戻る条件】
...
**致死性プロパティは「死に方」の数だけ持ち、「死因」の数だけ持ちません。** 死に方は4つ——
...
4つとも**現実の量そのもの**である点は偶然ではありません。水・脂肪・血・熱はいずれも体から出入りする物で、
...
- **死因は `frozen`** で、名乗るのは `on_min` の `destroy` です（6 節）。既存の 3 つと同じ形なので、
  画面は名前から文言を引くだけです。
...
（[`StatusArea.md`](../ui/StatusArea.md)）。健康な間は本数が増えないので、致死性の値が4つあることが
