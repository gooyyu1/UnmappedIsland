...
- **勢いは桁で書き分けます。** 1 tick は15分なので、`-15` は毎時60mL——尖った石で裂けた傷の桁です
  （潰れた血管は縮んで塞がるので、切れる刃で切った傷ほどは出ません）。**槍が残す刺し傷は `-200`**
  ——固まるまでの4 tick で800mL を奪うので、400mL のサルは30分で尽き、4,600mL のイノシシでも数突きで
...
- **値に比例した効き**が書けない。「`severity` が重いほど意識が下がる・速く出血する」は `modify` /
  ...
  相当する「比例する寄与」を検討する
- 傷ごとの出血レート（4 節）。裂傷の `-15` と刺し傷の `-200` が並んだので桁は決まったが、**大血管を
