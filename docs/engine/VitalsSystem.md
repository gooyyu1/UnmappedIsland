...
**戻らなくなる境目（3.1 節）も、このままとします。** 水と体脂肪を安全域に保つのに払うのは
1 日 607 分<!-- stats: balance.yaml daily_minimum place=島全体 total_minutes -->（うち 360 分は睡眠。
段が余計に削るぶんは入っていない下限で、[`DigestionSystem.md`](./DigestionSystem.md) 9 節）で、
**渡り歩ける島には賄えない値がありません。** ゲートが閉じるのは暮らしが崩れているときだけで、深手を
負ったこと自体では閉じません。
