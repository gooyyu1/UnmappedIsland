...
**押し下げている当人へは、意識から押し返せません。**「意識が薄いほど痛みを感じない」は、`pain` の
`modify` のゲートが `consciousness` を読み、その `consciousness` が `pain` から決まる循環になり、実行時に
例外が飛びます（同 14.1節）。**押し下げていない値へなら書けます**——獣の1手の重み配分がそれで、濁った
意識が様子見の重みを太らせています（[`HuntingSystem.md`](./HuntingSystem.md) 5.5 節）。
...
- **血は自分で戻ります。** ステータスの中で唯一、tick で増える値です。**速さの非対称が効き目のすべて**
  で、削るのは一瞬（毎時60mLの裂傷でも、大血管なら数分）なのに戻るのは1日およそ200mL——**牙の傷1つが
  奪う600mLに3.1日**かかります（3.3 節）。「気をつけていれば治る」と「一度の失敗が何日も響く」が
  同じ1本で両立します。
...
**`blood` の戻りに、水分と体脂肪のゲートを付けます。** どちらかが安全域を外れている間、血は
戻りません。

- **削る速さと戻る速さの非対称が、さらに開きます**（3 節）。深手から戻り切るまでにかかる日数は
  そのままで、**その日数は水と食べ物を切らさずに過ごした日数**でなければなりません。深手を負った
  後に狩りへ出ず水を汲みに行くのは、そのほうが速く治るからになります。
- **同じ形のまま人にも獣にも配れます。** 獣も `hydration`・`body_fat` を持ち、閉じ込められている
  間だけ減る（[`TrapSystem.md`](./TrapSystem.md) 5.4 節）ので、人と獣で別々の条件を持たずに済みます。
  **野生の個体では減らないので、条件は常に満たされます**——1 つの宣言のまま、罠の中の獣にだけ効きます。
**出どころ**: [#656](https://github.com/gooyyu1/UnmappedIsland/issues/656)（閉じ込めた獣だけが渇く）

### 3.2 ゲートを、条件の文法へどう写すか

3.1 節のゲートを今の文法へ写したもので、覆すのに人間の判断は要りません。
...
- **罠で回す暮らしは、その 3.1 日を持っています。** 落とし穴が返すイノシシは
  ...
  済みません。
- **だから「仕掛けて待つ」と「正面から殴る」の差が、この 1 本の速さに出ます。** 罠は時間で払い、
  立ち合いは血で払う——どちらも数日ぶんです。**戻りを速くすると、その選択ごと消えます。**

**戻らなくなる境目（3.1 節）も、このままとします。** ゲートが見る水と体脂肪は、**島の暮らしを 1 日ぶん
賄う献立に丸ごと入っています**——その献立が要求するのは
1 日 513 分<!-- stats: balance.yaml daily_minimum place=島全体 total_minutes -->（満腹とビタミンも含めた
...
——手番を飛ばす側は入りました（6 節）が、**画面の側——操作を受け付けず時間だけ進める——がまだ
ありません。** 飛ばす判断は意識を持つ物すべてに掛かるので、今のまま人へ配ると、倒れた人は限界の休息
（[`Characters.md`](../world/Characters.md) 限界節。どれも時間が起こす操作）まで止まり、押せる手も、
勝手に進む時間も無い画面が残ります。そのため人の `blood` は、段からの意識への寄与も持ちません。
...
- **増えて 100% で死ぬ形は採りません**（8.2 節）。持つのは**残っている熱**で、水・脂肪・血と同じ、
  **減れば死ぬ現実の量**です。単位は熱量そのもの（kcal）で、**`max` がそのまま体格**になるのは
  `blood` と同じです。
- **寒いかどうかは、比較 1 つで決まります。** 居る場所の気温と、自分が持つ寒さの境目を比べるだけです。
  気温にはあらゆる場所が持つ土地ごとの差と、季節・日射・据えた炉の暖が既に積み上がっています
  （[`ClimateSystem.md`](./ClimateSystem.md) 1 節、[`FireSystem.md`](./FireSystem.md) 9.2 節）。
  **新しい寒さの概念は要りません。**
- **守る側は、境目を押し下げます。** 衣服は身につけている間、寝床は寝ている間、境目を下げます。
  **火だけは境目ではなく気温の側を上げます**が、効き方が違うだけで、
  合流先は同じ 1 つの比較です。着込むほど寒さの入口が遠のき、焚けばその場が暖まる——という違いが、
  どちらを押すかの違いとしてそのまま出ます。
- **雨は速さに効きます。** 屋根も蓋も無い所で雨に打たれている間
  （[`ContainerSystem.md`](./ContainerSystem.md) 6 節）は、熱を削る速さが上がります。
  **雨で気温の側を下げる形は採りません**——濡れた体から熱が抜けるのは空気の温度の話では
  ないうえ、気温を動かすと炉の暖も焚き火の値打ちも雨の日だけ別の意味を持ってしまいます。削る側の
  ブロックを分ければ、**濡れることと寒いことが別々のまま 1 つの値へ合流します**。
- **戻るのは、境目より上に居る間だけ**です。**血とは逆で、熱は取り返せます**——だから凍死は「一度の
  失敗が何日も響く」ではなく、**「一晩をどう越すか」**の死に方になります。
...
**押し下げる深さは、空が作る気温の刻みへ端を合わせます。** 空の気温は平年の 20℃ に日射と季節が重なる
だけなので（[`ClimateSystem.md`](./ClimateSystem.md) 1 節、`core.yaml`）、刻みは粗く、素の 16℃ を下回るのは
**涼しい季節の夜（12℃）と、その薄明・雨天の昼（15℃）** です。**刻みより細かい差は、今の空では現れません**
——守る側の深さを決めるときは、そのどちらを跨ぐかで端を取ります。**最も深い衣服が 12℃ とちょうど
釣り合う**のは、炉の暖（+8）が最も寒い夜をちょうど平年へ戻すのと同じ置き方です
（[`FireSystem.md`](./FireSystem.md) 9.2 節）——**釣り合うだけなので、土地ごとの気温差が入れば足りなく
なります**（[`ClimateSystem.md`](./ClimateSystem.md) 1 節）。深さそのものは
...
＝放っておくな」、意識のバーが言うのは「弱っている＝あと何手か」です。どちらも1手へ効きますが、
**反撃してくるかを決めるのは警戒**で、意識が決めるのは手を出すか立ち尽くすか（同 5.5 節）——
**バーが下がっても、落ち着いていない相手は手を出してきます。**

## 未決事項・今後の検討課題

- **値に比例した効き**が書けない。「`severity` が重いほど意識が下がる・速く出血する」は `modify` /
  `add` の量がリテラルのため、段ごとのブロックで階段状に刻む。**量の側を豊かにする道は無い**
  （`src/domain/PassiveAmount.ts`。**寄与として登録される量**に積を使えるのはエンジンだけで、YAMLが書ける
...
- 倒れたプレイヤーの画面で何 tick 進めるか。**倒れている間に周りが何をするか**——同じ土地の動物が
  襲い続けるのか、興味を失うのか（倒れた当人が何もしないことは 6 節で決まっている）
