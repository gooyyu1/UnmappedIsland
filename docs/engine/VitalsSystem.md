...
**新設する文法はありません。** 導入する語彙は次の3つのプロパティと2つの段の名前だけで、
...
戻ります。気を失った相手を仕留める一撃（5 節）も入っており、**死体が出る道は既にあります**
（[`HuntingSystem.md`](./HuntingSystem.md) 1.4 節）。血（3 節）と出血（4 節）・失血死（6 節）はまだ
入っていません。キャラクタ側もまだ配っていません（7 節）。未決事項は末尾に整理しています。
...
stab_wound:
  passives:
    - modify:
        parent: {pain: 60}
    # 止血していない間だけ、宿主の血が失われていく。これが出血そのもの。
    - conditions:
...
- **勢いは桁で書き分けます。** 1 tick は15分なので、`-200` は毎時800mL——2時間半で意識が落ちはじめる
  （3 節）速さです。大血管を切れば `-1000` の桁で数 tick しか保たず、にじむ程度の擦り傷は `-2` で
  実質何も起きません。**同じ1つの量に対するレートの違い**だけで、致命傷から掠り傷までが並びます。
...
- **値に比例した効き**が書けない。「`severity` が重いほど意識が下がる・速く出血する」は `modify` /
  ...
  相当する「比例する寄与」を検討する
- 傷ごとの出血レート（4 節）。mL/tick で書くので現実の桁がそのまま使えるが、**致命傷は数 tick で、
  放置できる傷は数十 tick で**効くよう、傷の種類ぶんを並べて配分し直す必要がある
