...
- **血は自分で戻ります**（tick で増えます）。**速さの非対称が効き目のすべて**
...
1 日 549 分<!-- stats: balance.yaml daily_minimum place=島全体 total_minutes -->（満腹とビタミンも含めた
...
警戒（`wariness` が 1 以上）の 1 行だけですが、`unconscious` の段が `wariness` を −100 する
