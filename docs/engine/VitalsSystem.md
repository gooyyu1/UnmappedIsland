...
——渇き・飢え・失血の3つで死に、死ねばセーブデータごと消えます。**2 節から 6 節まではサルで実装済みです**
（検証は `tests/worldCodex/animalsYaml.test.ts`）——殴れば一撃で気を失い、裂けた傷から血が流れ、
放っておけば固まって止まり、失いすぎれば目を覚まさず、尽きれば死体になります。**キャラクタが持つのは
...
- **条件は付けていません。** 血が水と食事から作られるのは事実で、`hydration`・`satiety` をゲートに
...
| 起きること | 誰が起こす・読むか |
| --- | --- |
| 死 | **世界の側**（`on_shortfall` の `destroy: self`）。プレイヤーが死んだことは「もうどこにも居ない」ことがそのまま表す（`PlayerCharacter.isDead`）ので、死んだという旗はどこにも持たない |
| 気絶 | **画面**（操作を受け付けず時間だけ進める）と、**動物の1手を与える側**（[`HuntingSystem.md`](./HuntingSystem.md) 5.2節）が、段 `unconscious` を読んで手番を飛ばす |

**死因を名乗るのはワールドの側です。** `on_shortfall` は既定のクランプを置き換えるので、命を絶った値は
...
**プレイヤー自身には新しい見せ方を作りません。** `blood` に `status` タグを付けただけで、ステータス
エリアの既存の規約に乗り、**安全域を外れたときだけ**行が現れます（[`StatusArea.md`](../ui/StatusArea.md)）。
健康な間は本数が増えないので、致死性の値が3つあることがそのまま画面の複雑さにはなりません。
...
- **血が戻るのに条件を付けるか**（3 節）。水と食事をゲートにすれば「渇きと飢えを断ち切ってから初めて
  血が戻る」が書けるが、動物に餌をやる仕組みが無いため、人と獣で別々の条件を持つことになる。動物の
  生活（何を食べ、どこで水を飲むか）を作るときに、両方へ同じ形で配れるかを見て決める
...
- **飢えを避ける手立てが無い**（8 節）。`body_fat` は満腹度に関わらず `-1/tick` で減り、戻す手段が
  どこにも無いため、標準体格（`max` の1/4）から必ず十数日で餓死する。食事が蓄えを戻すのか、それとも
  満腹度が尽きている間だけ蓄えが削られるのかは、食事の仕組みを詰めるときに決める
