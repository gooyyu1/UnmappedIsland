...
| この型を材料・道具に使うのはどのレシピか | `ObjectDef.describeRecipesUsing` |

「生み出す操作」は**場所の名前だけ**を出します。探索のような大きな `pick` の木を丸ごと並べても、
「これはどこから手に入るのか」を知りたい読み手には多すぎるためです（中身は宣言元の型のページにあります）。
