...
- 工程はラベルを持たない小さな丸です。名前を並べても情報価値の割に横幅を食うため、読み手には
  「ここが合流・分岐の結節点」という印だけを見せ、名前はツールチップ（重ねると表示）に置きます。
  押すと宣言元の型のページへ飛びます。
- 型のクリックは2段階です。**まだ強調されていない型はまず強調表示**（その型のチェーンを見せる）、
  **強調中の型を押すとその型のページ**へ飛びます。図を眺めている段階でいきなりページへ
  飛ばされるより、まず文脈を見せるほうが探索の流れに合うためです。
- 図は横に長いので、mainの幅制限を抜けて**画面幅いっぱい**に表示し、ツールバーで**拡大・縮小**できます。
