...
| この型を生み出すのはどの操作か | `ObjectDef.describeCreationsOf` |
| この型を材料・道具に使うのはどのレシピか | `ObjectDef.usesInRecipes` |

**逆引きは箇条書きで、行き先を指すだけにします**（「砂浜 の 探索する」「編み籠」）。数が増えるほど
一覧性のほうが要りますし、詳細はリンク先の型のページで見られるためです。例外は「影響元」で、
そこだけは量と条件まで出します——「どれだけ動くのか」がその節を見る目的そのものだからです。
...
| `#/object/<識別子>` | 型の詳細（絵・説明文・props・slots・passives・actions・combinations・recipes・土地の亜種と、上記2つの逆引き） |
