...
（[`GameEndings.md`](../concept/GameEndings.md) 12.4 節）。**嵐が屋外の行動を止める線
（[`ContentSkeleton.md`](./ContentSkeleton.md) 8.1.4 節）は、渡ることには掛かりません**——あの線が止めるのは
その場に立って手を出す仕事（海の上では見張りと釣り、3.10 節）で、**筏は乗り手が何をしていようと浮かんで
いる限り動く**からです。止めてしまえば、嵐の海区は待つ以外にすることの無い区間になります
（[`DesignPrinciples.md`](../concept/DesignPrinciples.md) の「時間経過に任せる区間を作らない」節）。**押し流す先を持つのは海区の側**です——筏は
...
（[`ContentSkeleton.md`](./ContentSkeleton.md) 8.1.4 節）。**見張りも、嵐の日は止まる側です**——同じ節が
屋外の行動をすべて止めると決めているためで、**今その条件を持っているのは採取の操作だけ**です（同 8.1.5 節）。
