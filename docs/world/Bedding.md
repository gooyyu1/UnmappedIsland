...
素材まで遡った手間は次のとおりです。**段1・段2を素手で通すと約830分**で、1日ぶんの自由時間
（816分。[`ContentSkeleton.md`](./ContentSkeleton.md) 8節）とほぼ同じです。
...
- **寝床が持つプロパティの名前と本数**（4節）。快適性1本か、乾き・虫・圧迫へ分けるか。分けるほど工法の
  違いは出ますが、その数だけ部品の宣言が増えます
- **`add` の量にプロパティ参照を許すか**（4節）。許さないなら、部品の寄与を回復量へ届かせる別の道が要ります
- **雨。** 屋根の無い寝床は雨季にほぼ無価値ですが、住居も `sheltered`
  （[`ContainerSystem.md`](../engine/ContainerSystem.md) 6節）も未実装なので、寝床だけを先に出すと雨を
  表せません
