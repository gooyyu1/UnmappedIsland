...
**確定度と実装状況。** 検討結果であり確定仕様ではありません。**入っているのは敷物（2節の段1）・骨組み
...
通した合計は765分**（刃物ありの135分＋630分）——1日ぶんの自由時間
（[`ContentSkeleton.md`](./ContentSkeleton.md) 8節）の大半を使う量です。

| 完成品 | 直接の材料 | 工程 | 素材から通した時間 |
| ------ | ---------- | ---- | ------------------ |
| 寝床 | 編んだ葉 ×3 | 15分 | 素手で195分／刃物があれば135分 |
| 寝台の骨組み | 太い枝 ×6、紐 ×3 | 180分 | ＋630分（うち紐に450分） |
| ハンモック | 紐 ×5 | 120分（据えるのは無料。6.2節） | 870分（うち紐に750分） |
...
- **ハンモックは、段1と段2を通した手間とほぼ並びます**（765分に対して870分）。**同じだけ戻すのに、
...
**押し下げは「体の下に乾いた層が在ること」と「地面から離れていること」の足し算**で、どちらも1℃です。
2節のとおり**横になっている間の熱の逃げ道は空気より地面が主**なので、割るならこの2つになります
——敷物は前者だけ、据えたハンモックは後者だけを持ち、両方を持つのは骨組みを差した寝台だけです。
...
上へ伸びます。** それでも**最も寒い土地の夜は残り**、そこに残るのは炉の暖
（[`../engine/FireSystem.md`](../engine/FireSystem.md) 9.2節）です。
...
浅いこと）と `tests/world-codex/landTemperature.test.ts`（伸びる先と、残る土地）。
...
- **詰め物の押し下げ**（4.2節）。段3は体の下の層を厚くする側なので、押し下げをもう1℃足す先になりえます。
  足せば**最も寒い土地の夜も火なしで越せる**ようになるので、そこを火だけに残すかと一緒に決まります。
  物として無いうちは置けません
