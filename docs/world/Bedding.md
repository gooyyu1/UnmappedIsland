...
**在処。** 定義は `src/assets/world-codex/bedding.yaml` です。材料はすべて既存で、編んだ葉は
`weaving.yaml`、紐は `fiber.yaml`、太い枝は `locations.yaml` にあります。

**確定度と実装状況。** 検討結果であり、**確定しているのは 4.2.1 節（火なしでは越せない夜を、寝床が
無くす）だけ**です。**本書の段はすべて入っています**——敷物（2節の段1）・骨組み（同 段2）・
詰め物（同 段3）・ハンモック（6節）と、段ごとの回復の上積み（4節）、それに寝ている間の寒さの入口の
押し下げ（4.2節）です。未決事項は8節。
...
| 段 | 形 | 何が変わるか |
| -- | -- | ------------ |
| 0 | 地面に直に寝る | 基準線。**まとまっては眠れず、取れるのは `nap` まで**（4.1節） |
| 1 | 編んだ葉の敷物 | 圧迫と、地面への伝導。**`sleep` が押せるようになる** |
| 2 | ＋骨組み（高床） | **湿気・虫・浸水から一度に離れる。ここが最大の跳躍** |
| 3 | ＋詰め物 | 圧迫。**体重で潰れない厚みが入る**（4.2節）。沈み込みは中身で分かれる（5節） |
...
**その保温の分を数へ落としたものが 4.2節**で、大半ではないことがそのまま押し下げの浅さになります。
...
そうでないと、下の「`nap` 2回と `sleep` 1回がちょうど同じだけ戻る」が成り立ちません——キャラクタ側は
長い休息ほど毎 tick の量が大きいので、足し合わせると差が残ります。
...
回復で、`agent` はそのアクションを押した者を指します。**「眠っている間か」を問うゲートは要りません**
——アクションが動いている間しか効かないので、条件で言い直す先がありません。**部品を見るゲートの主語は
`self`**（既定）で、操作の宣言が乗っている側、つまり寝床そのものを指します（同 11.5節）。

**`add` の量はリテラル数値しか受け付けません**
（[`DesignPrinciples.md`](../concept/DesignPrinciples.md)「計算式はエンジンに取っておき、宣言へ開かない」）。
それでも部品の寄与が届くのは、**量を1箇所へ集めず、段ごとのブロックへ散らしている**ためです。有効な
...
| 3 ＋詰め物（植物繊維） | 3.25 / 4 | 39 / 48 | 78 / 96 |
| 3 ＋詰め物（羽毛） | 3.5 / 4 | 42 / 48 | 84 / 96 |
...
担ぎ通すと −72。寝床の上の1日は「18時間起きて6時間眠る」なので、**一晩（24 tick）でそれを返すには
3/tick が要ります——そこが段2**です。**段1はそこへ届きません**（+48）。骨組みを差す理由が、
数字の上でもここに出ます。**どの段の一晩も、いちばん薄い体力の上限**（`characters/engineer.yaml` の
...
**寝床の段のうち跳躍が最も大きいのは段2**（2節。骨組みで +1、詰め物で +0.25〜+0.5）。**覚醒度は骨組みと
詰め物では動きません**——`sleep` で
...
**詰め物の種類ごとの差**（5節の羽毛と植物繊維）は、`matches` を `{object: ...}` にしたブロックで
書いてあります。**種類が1つ増えるごとに、寝床の宣言もブロックを1つ増やします**——量を部品の側から引く形は
採りません（上の `add` の量）。**寒さの入口の押し下げだけは種類で分けず、`stuffing` タグを見る1つの
ブロックが持ちます**（4.2節）——火なしで最も寒い夜を越せるのは詰め物まで仕上げた者、という約束
（4.2.1節）が、詰め物として現れる物すべてに掛かるためです。

**それでも、回復として積む先はどの段も同じ `stamina` です。** 段の違いが別の値へ出るのは寒さの入口だけで
（4.2節）、骨組みが持つ「湿気・虫・浸水から一度に離れる」（2節）のうち**虫と浸水は、まだ数字の大小でしか
表せていません**。乾きと虫を寝床のプロパティとして持ち、それをブロックのゲートに使えば別の効き方に
なりますが、**名前と本数は未決です**（8節）。

### 4.1 寝床が無い側——`sleep` は寝床の在る場所でしか押せない
...
**要件は1つも要りません。`sleep` を配るのが寝床だけだからです**（4節）。寝床の無い場所には押す札が
無く、キャラクタ側が持つのは `nap` までです（`characters/player_character.yaml`）——「寝床が在るか」を
問う条件を書くと、**同じことを配り手と要件の2箇所で言う**ことになります。
...
**ハンモックを担いでいくほうが軽く、据え直して使い回せます**（6.2節）**が、支点のある土地でしか
寝床になりません**（6.1節）。道が1本に保たれているのは、**どこへ向かっても通るのが編んだ葉だけ**
だからです。
...
### 4.2 寝ている間だけ、寒さの入口を押し下げる

**眠っている間、寝床は寒さの入口（`chill_point`、[`Characters.md`](./Characters.md)）を押し下げます。**
衣服が身につけている間に押し下げるのと同じ境目で（[`../engine/VitalsSystem.md`](../engine/VitalsSystem.md)
8.3節）、**衣類の押し下げとは加算で重なります。** 宣言は段ごとのブロックに乗ります（4節）——効くのは
`nap`/`sleep` が時間を進めている間だけなので、「眠っている間か」を問うゲートは要りません。**詰め物だけは
押し下げと回復の上積みが別のブロックに分かれます**——押し下げは `stuffing` タグを見る1つが持ち、
上積みは中身を名指しするほうが持つためです（4節）。
...
厚みが残るのは詰め物が入ってからです。
**ハンモックが回復では寝台と並ぶのに、ここで並ばないのは同じ理由から**です（6節。虫と湿気に最も強い
理由が、背中の下を風が抜けることなので）。
...
**長さには比例しません。** 押し下げは境目への寄与なので、`nap` と `sleep` のどちらを取っても削られるか
戻るかは同じに決まります——回復の上積み（4節）が長さに正比例するのと、ここだけ形が違います。

検証は `tests/world-codex/beddingYaml.test.ts`（段ごとの深さ・起きている間は効かないこと・衣類より
浅いこと）と `tests/world-codex/landTemperature.test.ts`（伸びる先と、詰め物で越せるようになる土地）。

#### 4.2.1 詰め物まで仕上げれば、最も寒い土地の夜も火なしで越せる【確定】

**火なしでは越せない夜を、寝床が無くします。** いちばん深い一着を着て、詰め物まで仕上げた寝台で眠る者は、
島でいちばん寒い土地の夜も越せます——寝床は衣類の上へ段のぶんを足すだけなので（4.2節）、この一着が
前提です。詰め物を作ることが、夜通し火を守る仕事から抜ける道になります。

**火だけに残す道は採りません。** 残すと、寝床をどこまで良くしても最も寒い土地の夜には炉が要り、
寝床の段は「火の要る夜を減らす」ところで止まります。**火の要らない夜を寝床が作れる**ことが、
手間の重い詰め物（5節）まで通す理由になります。

どれだけ押し下げてそこへ届かせるかは上の表が持ちます。**`stuffing` を名乗る物それぞれについて、それを
差した寝台でその夜を越せること**を `tests/world-codex/landTemperature.test.ts` が要求します——詰め物を
足して押し下げを足さなければ落ちます。**種類が増えても、同じ検査がその1つずつに同じことを要求します。**

**出どころ**: [#2286](https://github.com/gooyyu1/UnmappedIsland/issues/2286)（いちばん寒い土地の夜を、寝床の詰め物で火なしで越せるようにする。2026-09-19に回答）

## 5. 詰め物は羽毛と植物繊維の2択にする

| 詰め物 | 取り分 | 払う先 |
| ------ | ------ | ------ |
| 羽毛（`feather_stuffing`） | 沈み込みが大きく、回復の上積みが厚い（4節の +0.5） | ヤケイ30羽。罠を回して待つ日数 |
| 植物繊維（`plant_fiber_stuffing`） | 獲物を待たずに集まる | 繊維15束。掻き取るか叩き解す手間 |

**寒さの入口の押し下げはどちらも同じ**です（4.2節）——4.2.1節の約束が詰め物すべてに掛かるので、
どちらを詰めても最も寒い土地の夜は越せます。**分かれるのは沈み込みと、集め方だけ**で、上の実測
（3節）どおり羽毛のほうが重い代わりに上積みが厚くなります。**同じだけ暖まるのに、払う先が罠から
手間へ移る**——ハンモックと寝台の関係（6節）と同じ形です。
...
叩き解したコイア（`fiber.yaml`・`coconut.yaml`）です。どちらも `plant_fiber` なので、**どの島に流れ着いても
詰め物へ届きます**。
...
| 寝床になる場所 | どこでも | 支点が2つ要る |
| 虫・湿気 | 強い | 最も強い |
| 保温 | 強い | **弱い**（4.2節） |
| 伸ばしろ | 詰め物を足せる | 無い |
| 持ち出し | 敷物は据え置き。骨組みは抜けるが籠に入らない嵩 | **持ち上げればそのまま次の土地へ**（6.2節） |

熱帯アメリカのハンモックは、背中の下を風が抜けることがそのまま利点になります（温帯では逆に欠点で、下へ
断熱を足す必要が出ます）。**保温で寝台に並ばないのはこの裏返し**で、虫と湿気に最も強い理由がそのまま
弱さの理由になります（4.2節）。この島では素直に強い選択肢ですが、網を編むのに紐を大量に食い、支点2つを土地の
側へ要求するため、寝台と上下にはなりません。[`Dwellings.md`](./Dwellings.md) 4節が工法へ課した「どれも
全域で他を上回らない」を、寝床でも守ります。

### 6.1 支点は土地のプロパティで持つ

**「木が2本ある土地か」は、土地が `hanging_anchor` として名乗ります**（`locations.yaml`。木立ちの土地と
ヤシの並ぶ浜が持ちます）。**その土地に立っている木を数える形は採れません**——スロットの中身を見る条件
（[`GameElementDefinition.md`](../engine/GameElementDefinition.md) 14.3節）は存在判定だけで、「2本ある」を
言えないためです。数える文法を足せば書けますが、本書は文法を新設しません（概要）。

**探索で見つけた木の札の数とは別物です。** 支点が在るかはその土地の性質で、木を1本倒しても、まだ
探索していなくても変わりません——**倒せる木が在ることと、間を渡せる木の組が在ることは別の事実**です。

**支点を要求するのは、据える操作ではなく `nap`/`sleep` のほうです。** 網は据えるだけなら支点の無い土地
へも置けて、そこでは寝床になりません——線は「置けるか」ではなく「寝られるか」に引きます。見るのは
`ancestor`（同 8.6節）で、**もう1つの要件が「その土地の設置物として在ること」**（`in_slot: fixtures`）
です。担いだままでは寝られないので、支点のある土地を探す理由が残ります。

**確定の宣言は付けていません。** 二択の一方が書けないことから決まったもので、人間の判断を経ていません
——数える文法を足すなら、もう一方が生き返ります。

### 6.2 網は item と fixture を兼ねる1つの型

**ハンモックは、籠・そり・手押し車（`containers.yaml`）と同じく `item` と `fixture` を兼ねます。**
据えることも持ち上げることも、レーンの間を行き来させるだけでできるので、**そのための操作を持ちません**。

**巻いた網と吊った網を別々の型にはしません。** 分けると、据えるたびに個体が入れ替わって傷んだぶんが
消えます（[`GameElementDefinition.md`](../engine/GameElementDefinition.md) 9.4節。引き継ぐのは位置だけ
...
無い土地では据えても寝床にならない**ので（6.1節）、遠出の道中で `sleep` が押せるかは、その土地が支点を
持つかにそのまま掛かります。
...
- **敷くまでの1日目。** `sleep` を配るのが寝床だけ（4.1節）なので、敷物ができるまでは仮眠で回すことに
  なります。
  素手なら195分・刃物があれば135分（3節）で、水と火と食い物を立ち上げる初日と正面から競合します。
  **通しで眠れない日が何日続くのが妥当か**は測っていません
...
- **据えるのに手間を課すか。** ハンモックはレーンを移すだけで据わり、時間がかかりません（6.2節）
  ——現実には2本の木の間へ渡して張りを見る手間があります。罠も同じく置くだけで仕掛かるので
  （[`TrapSystem.md`](../engine/TrapSystem.md)）、**課すなら両方まとめてです**。枠の `put_in`
  （[`GameElementDefinition.md`](../engine/GameElementDefinition.md) 7.10節）は土地の `fixtures` 枠
  そのものに付くので、設置物ごとに値段を変えることは今の文法ではできません
- **骨組みの劣化。** 寝床そのもの——敷物（`bed`）とハンモック（`hammock`）——は
  素材を名乗りました（前者が短命、後者は綯った紐なので長持ち。
  [`DurabilitySystem.md`](../engine/DurabilitySystem.md) 2節）。**骨組みは物として在るのに名乗って
  いません**——同節が名乗らせるのは道具・身につける物・入れ物・寝床までで、寝床へ差す部品は
  その外だからです。**詰め物も同じ理由で名乗っていません**——差した部品だけが朽ちない形をこのまま
  残すかは決めていません。名乗らせるなら、羽毛と植物繊維で湿気への強さに差が出ます（5節）
- **革の蒸れと、詰め物が湿気を吸って固まること**（5節）。どちらも段を置くことの代償として言葉では
  書きましたが、数へは落としていません。落とすなら、天日に干す手間（[`DurabilitySystem.md`](../engine/DurabilitySystem.md)）か、
  寝床が持つプロパティ（4節。名前と本数はこの節の未決）のどちらかに乗ります
