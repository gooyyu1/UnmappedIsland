...
無くす）だけ**です。**入っているのは敷物（2節の段1）・骨組み
（同 段2）・ハンモック（6節）と、段ごとの回復の上積み**（4節）、それに**寝ている間の寒さの入口の
押し下げ**（4.2節）です。詰め物（同 段3）は未実装で、受け皿になる袋（5節）がまだありません。
未決事項は8節。
...
支えるのに要る割は寝床の 2/3 です——地面の仮眠がその線の下（[`Characters.md`](./Characters.md) 休息節）に
置いてあるので、敷物を敷けば体力の側でも必ず得になります。
...
**詰め物の種類ごとの差**（5節の羽毛と植物繊維）は、`matches` を `{object: ...}` にしてブロックを分ければ
書けます。**種類が1つ増えるごとに、寝床の宣言もブロックを1つ増やします**——量を部品の側から引く形は
採りません（上の `add` の量）。
...
（[`SurvivalItems.md`](./SurvivalItems.md) 5.1節）、段2まで通す手間（3節）より桁の小さい手間で作れる
位置です。**払う手間に対して浅い**ことが、保温のために寝床を建てる選択を潰します。
...
浅いこと）と `tests/world-codex/landTemperature.test.ts`（伸びる先と、詰め物が入るまで残る土地）。
...
どれだけ押し下げてそこへ届かせるかは上の表が持ちます。**`stuffing` を名乗る物が世界に現れたら、それを
差した寝台でその夜を越せること**を `tests/world-codex/landTemperature.test.ts` が要求します——詰め物を
足して押し下げを足さなければ落ちます。**現れるまでは、同じ検査が「越せない」ことを確かめています。**
...
蒸れます——**段そのものは置くと決まった**（4.2.1節）ので、この蒸れは足を止める理由ではなく、詰め物まで
通すことの代償になります。詰め物は湿気を吸って固まり黴びるので、天日に
...
- **骨組みの劣化。** 寝床そのもの——敷物（`bed`）とハンモック（`hammock`）——は
  ...
  その外だからです。差した骨組みだけが朽ちない形をこのまま残すかは決めていません。詰め物は
  まだ物として無いので、「腐りにくい」という現実どおりの優位を得るかも、置くときに決まります
