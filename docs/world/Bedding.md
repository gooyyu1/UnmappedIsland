...
| 3 | ＋詰め物 | 圧迫。**体重で潰れない厚みが入る**（4.2節）。中身が羽毛でも植物繊維でも同じ（5節） |
...
（5節）は同じだけ効きます——火なしで最も寒い夜を越せるのは詰め物まで仕上げた者、という約束（4.2.1節）が
詰め物すべてに掛かるうえ、**分かれているのは効きではなく集め方だから**です（5節）。

**種類ごとに差を付けるなら、`matches` を `{object: ...}` にしてブロックを分ければ書けます**が、
**同じ枠へ入る物どうしは排他なのに、収支の解析はそれを読めません**（`src/analysis/balanceTables.ts` の
`expectedDeltas` は条件を見ずに足すので、`stats/balance.yaml` が「どの寝床でも起きない量」を出します）。
差を付けるなら、解析の側に排他を教えるのが先です（8節）。
...
払う先が罠から手間へ移ります**——ハンモックと寝台の関係（6節）と同じ形です。上の実測（3節）で羽毛が
...
- **詰め物の種類ごとに効きを変えるか**（4節・5節）。今はどれも同じで、差は集め方だけです。変えるには
  先に、**同じ枠へ入る物どうしが排他であることを収支の解析へ教える**のが要ります
  （`src/analysis/balanceTables.ts` の `expectedDeltas` は条件を見ずに足すので、排他なブロックを
  並べると `stats/balance.yaml` がどの寝床でも起きない量を出します）
