...
| **それ以降** | 払う労働の量 | 1日（自由時間949分<!-- stats: terrain.yaml daily_budget surplus -->。8節）以上の労働がかかる山を1段と数える |
...
日数にして68.65日<!-- stats: terrain.yaml work_piles_total days -->ぶんです。**
...
（1日949分<!-- stats: terrain.yaml daily_budget surplus -->）に対する概算で、**実測**と書いたものは量を
その型の値段そのもので採っています。

**この68.65日<!-- stats: terrain.yaml work_piles_total days -->は、明るさと移動時間を無視した下限です**——どちらも入れると同じ山が
89.5日<!-- stats: terrain.yaml cycle base=shortest_mean metric=steady_days mean -->ぶんに広がり、島を開く15.2日<!-- stats: terrain.yaml exploration_phase metric=day_trip_days mean -->を足すと
1周回は約105日<!-- stats: terrain.yaml cycle base=shortest_mean metric=total_days mean ±1 -->になります（8節）。
...
| 7 | 運ぶ入れ物 | 肩掛けの袋・編み籠 | なめし革の背負い袋（1<!-- stats: terrain.yaml work_piles pile=なめし革の背負い袋 days -->）／**そり（1.98<!-- stats: terrain.yaml work_piles pile=そり days -->・実測）** | 2<!-- stats: terrain.yaml work_piles_by_system system=7 piles --> | 3.0<!-- stats: terrain.yaml work_piles_by_system system=7 days --> |
| 8 | 衣類 | 束ねた葉・編んだ葉 | **なめし革の一式（1.7<!-- stats: terrain.yaml work_piles pile=なめし革の一式 days -->・実測）** | 1<!-- stats: terrain.yaml work_piles_by_system system=8 piles --> | 1.7<!-- stats: terrain.yaml work_piles_by_system system=8 days --> |
...
| 11 | 船と海図 | — | **筏（4.5<!-- stats: terrain.yaml work_piles pile=筏 days -->・実測）**／**帆（3.3<!-- stats: terrain.yaml work_piles pile=帆 days -->・実測）**／櫂と舵（2<!-- stats: terrain.yaml work_piles pile=櫂と舵 days -->）／沿岸航海（3<!-- stats: terrain.yaml work_piles pile=沿岸航海 days -->）／海図を仕上げる（5<!-- stats: terrain.yaml work_piles pile=海図を仕上げる days -->） | 5<!-- stats: terrain.yaml work_piles_by_system system=11 piles --> | 17.8<!-- stats: terrain.yaml work_piles_by_system system=11 days --> |
| 12 | 家具・娯楽 | — | **設えを積んで里心を止める（1.56<!-- stats: terrain.yaml work_piles pile=設えを積んで里心を止める days -->・実測）** | 1<!-- stats: terrain.yaml work_piles_by_system system=12 piles --> | 1.56<!-- stats: terrain.yaml work_piles_by_system system=12 days --> |
| | | | **合計** | **27**<!-- stats: terrain.yaml work_piles_total piles --> | **68.65**<!-- stats: terrain.yaml work_piles_total days --> |
...
4. **浜まで運ぶ手段。** そり（運ぶ入れ物の段3）。備蓄は拠点にあり、出航地点は海岸なので、積み込みの前に
   ...
   [`Voyage.md`](./Voyage.md) 3.9.3節）が乗るので、**長い側では、水と食料だけで通れるのがいちばん力の
   ある者だけになり、アーティファクトを1つ載せればその者も通れません。** 短い側は水と食料だけなら4人とも
   通れますが、そこでもいちばん非力な者はアーティファクト1つで線を越えます。**持ち帰る物が、そのまま
...
**同じ27個<!-- stats: terrain.yaml work_piles_total piles -->の山が、明るさ・移動・土地の差を入れると89.5日<!-- stats: terrain.yaml cycle base=shortest_mean metric=steady_days mean -->ぶんになり、島を開く15.2日<!-- stats: terrain.yaml exploration_phase metric=day_trip_days mean -->を足して
1周回は約105日<!-- stats: terrain.yaml cycle base=shortest_mean metric=total_days mean ±1 -->です。1日949分<!-- stats: terrain.yaml daily_budget surplus -->という自由時間は上限であって、実際に使える時間ではありません。**

1日を賄う最小労働は491分<!-- stats: balance.yaml daily_minimum place=島全体 total_minutes -->（[`stats/balance.yaml`](../../stats/balance.yaml) の `daily_minimum`）なので、差し引きの自由
時間は1日949分<!-- stats: terrain.yaml daily_budget surplus -->。4節の山の合計は約65,100分<!-- stats: terrain.yaml work_piles_total minutes ±100 -->なので、
**そのまま割れば68.65日ぶん**<!-- stats: terrain.yaml work_piles_total days -->です。しかしこの949分<!-- stats: terrain.yaml daily_budget surplus -->は、
...
**嵐を引くのは後者だけ**です（8.1.4節）。720分<!-- stats: terrain.yaml daily_budget outdoor_window -->は屋外の枠、131分<!-- stats: terrain.yaml daily_budget survival_gathering -->は1日を賄う生存の採取
（[`stats/balance.yaml`](../../stats/balance.yaml) の `daily_minimum` の491分<!-- stats: balance.yaml daily_minimum place=島全体 total_minutes -->から睡眠360分<!-- stats: terrain.yaml daily_budget sleep -->を引いた分）です。
...
**同じ27個<!-- stats: terrain.yaml work_piles_total piles -->の山が、局面ごとに数え直すと約105日<!-- stats: terrain.yaml cycle base=shortest_mean metric=total_days mean ±1 -->ぶんになります。** 内訳は
**島を開く15.2日<!-- stats: terrain.yaml exploration_phase metric=day_trip_days mean -->**と、**開き切った後の89.5日<!-- stats: terrain.yaml cycle base=shortest_mean metric=steady_days mean -->**です。
...
| --- | --: | --: | --: | --: |
| **探索**（島を開き切るまで） | 162分<!-- stats: terrain.yaml exploration_phase metric=day_trip_travel_per_day mean --> | 探索 185分<!-- stats: terrain.yaml exploration_phase metric=day_trip_exploration_per_day mean --> | 全土地を探索率100%まで（2,799分<!-- stats: terrain.yaml exploration_phase metric=exploration_minutes mean -->） | **15.2日<!-- stats: terrain.yaml exploration_phase metric=day_trip_days mean -->** |
| **定常**（開き切った後） | 62分<!-- stats: terrain.yaml steady_phase metric=travel_per_day mean --> | 山 492分<!-- stats: terrain.yaml steady_phase metric=work_per_day mean --> | 屋外の山（約43,400分<!-- stats: terrain.yaml work_piles_total outdoor_minutes ±100 -->） | **89.5日**<!-- stats: terrain.yaml cycle base=shortest_mean metric=steady_days mean --> |
| 合計 | | | | **104.6日**<!-- stats: terrain.yaml cycle base=shortest_mean metric=total_days mean --> |

27個<!-- stats: terrain.yaml work_piles_total piles -->の山（約65,100分<!-- stats: terrain.yaml work_piles_total minutes ±100 -->）のうち、屋外での採取・伐採・運搬を2/3
（約43,400分<!-- stats: terrain.yaml work_piles_total outdoor_minutes ±100 -->）、拠点での加工を1/3
（約21,700分<!-- stats: terrain.yaml work_piles_total base_minutes ±100 -->）と置いています。**この割合と、下の土地の配分は
仮置きです。**
...
**律速は屋外です**——拠点での加工は約21,700分<!-- stats: terrain.yaml work_piles_total base_minutes ±100 -->で、夜の枠に対して
60日ぶん<!-- stats: terrain.yaml work_piles_total base_days -->なので、約105日<!-- stats: terrain.yaml cycle base=shortest_mean metric=total_days mean ±1 -->の中に余裕で収まります。**夜が足りなくなることはなく、
足りないのは昼です。**

**この約105日<!-- stats: terrain.yaml cycle base=shortest_mean metric=total_days mean ±1 -->は上限としても下限としても読めません。** 上へ振れる材料は、4節の山の多く
...
——1日の枠427分<!-- stats: terrain.yaml exploration_phase metric=day_trip_window_per_day mean -->に対して探索は
185分<!-- stats: terrain.yaml exploration_phase metric=day_trip_exploration_per_day mean -->しか進まないので、
余る242分<!-- stats: terrain.yaml exploration_phase metric=day_trip_spare_per_day mean -->×15.2日<!-- stats: terrain.yaml exploration_phase metric=day_trip_days mean -->は山へ回せます。
...
土地ごとに日帰りと泊まりの安いほうを採っても15.14日<!-- stats: terrain.yaml exploration_phase metric=mixed_days mean -->で、**日帰りだけの15.15日<!-- stats: terrain.yaml exploration_phase metric=day_trip_days mean -->とほとんど変わりません**。
...
生存の採取131分<!-- stats: terrain.yaml daily_budget survival_gathering -->を引いた残りを使い切らなくなったためです。

**端まで届くことも同じ枠で出ます。** 拠点から最も遠い土地への往復は、最も広い島でも
420分<!-- stats: terrain.yaml base_farthest_round_trip base=shortest_mean max -->で、上の残り（589分）に
収まります（同 `base_farthest_round_trip`）。**補給を持ち出す理由が島の広さからは生まれない**という
...
払うのは選んだ側のほうで、8.3.1節の確定もこの範囲を指しています——**山頂に住めば届かない**のは、
島が広いからではありません。
...
- 1周回の日数（8節）。局面ごとに数えると**約105日<!-- stats: terrain.yaml cycle base=shortest_mean metric=total_days mean ±1 -->**（探索15.2日<!-- stats: terrain.yaml exploration_phase metric=day_trip_days mean -->＋定常89.5日<!-- stats: terrain.yaml cycle base=shortest_mean metric=steady_days mean -->、8.3節）。**残る仮置きは
