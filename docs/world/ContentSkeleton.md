...
| **それ以降** | 払う労働の量 | 1日（自由時間813分<!-- stats: terrain.yaml daily_budget surplus -->。8節）以上の労働がかかる山を1段と数える |
...
日数にして71.82日<!-- stats: terrain.yaml work_piles_total days -->ぶんです。**
...
（1日813分<!-- stats: terrain.yaml daily_budget surplus -->）に対する概算で、**実測**と書いたものは量を
その型の値段そのもので採っています。

**この71.82日<!-- stats: terrain.yaml work_piles_total days -->は、明るさと移動時間を無視した下限です**——どちらも入れると同じ山が
103.2日<!-- stats: terrain.yaml cycle base=shortest_mean metric=steady_days mean -->ぶんに広がり、島を開く16.7日<!-- stats: terrain.yaml exploration_phase metric=day_trip_days mean -->を足すと
1周回は約120日<!-- stats: terrain.yaml cycle base=shortest_mean metric=total_days mean ±1 -->になります（8節）。
...
| 7 | 運ぶ入れ物 | 肩掛けの袋・編み籠 | なめし革の背負い袋（1<!-- stats: terrain.yaml work_piles pile=なめし革の背負い袋 days -->）／**そり（2.31<!-- stats: terrain.yaml work_piles pile=そり days -->・実測）** | 2<!-- stats: terrain.yaml work_piles_by_system system=7 piles --> | 3.3<!-- stats: terrain.yaml work_piles_by_system system=7 days --> |
...
| 11 | 船と海図 | — | **筏（5.3<!-- stats: terrain.yaml work_piles pile=筏 days -->・実測）**／**帆（3.9<!-- stats: terrain.yaml work_piles pile=帆 days -->・実測）**／櫂と舵（2<!-- stats: terrain.yaml work_piles pile=櫂と舵 days -->）／沿岸航海（3<!-- stats: terrain.yaml work_piles pile=沿岸航海 days -->）／海図を仕上げる（5<!-- stats: terrain.yaml work_piles pile=海図を仕上げる days -->） | 5<!-- stats: terrain.yaml work_piles_by_system system=11 piles --> | 19.1<!-- stats: terrain.yaml work_piles_by_system system=11 days --> |
| 12 | 家具・娯楽 | — | **設えを積んで里心を止める（1.82<!-- stats: terrain.yaml work_piles pile=設えを積んで里心を止める days -->・実測）** | 1<!-- stats: terrain.yaml work_piles_by_system system=12 piles --> | 1.82<!-- stats: terrain.yaml work_piles_by_system system=12 days --> |
| | | | **合計** | **27**<!-- stats: terrain.yaml work_piles_total piles --> | **71.82**<!-- stats: terrain.yaml work_piles_total days --> |
...
出航に適した季節の窓（同3.1節）が、これらに期限を与えます。
...
**同じ27個<!-- stats: terrain.yaml work_piles_total piles -->の山が、明るさ・移動・土地の差を入れると103.2日<!-- stats: terrain.yaml cycle base=shortest_mean metric=steady_days mean -->ぶんになり、島を開く16.7日<!-- stats: terrain.yaml exploration_phase metric=day_trip_days mean -->を足して
1周回は約120日<!-- stats: terrain.yaml cycle base=shortest_mean metric=total_days mean ±1 -->です。1日813分<!-- stats: terrain.yaml daily_budget surplus -->という自由時間は上限であって、実際に使える時間ではありません。**

1日を賄う最小労働は627分<!-- stats: balance.yaml daily_minimum place=島全体 total_minutes -->（[`stats/balance.yaml`](../../stats/balance.yaml) の `daily_minimum`）なので、差し引きの自由
時間は1日813分<!-- stats: terrain.yaml daily_budget surplus -->。4節の山の合計は約58,400分<!-- stats: terrain.yaml work_piles_total minutes ±100 -->なので、
**そのまま割れば71.82日ぶん**<!-- stats: terrain.yaml work_piles_total days -->です。しかしこの813分<!-- stats: terrain.yaml daily_budget surplus -->は、
...
——山の合計の5%です。ところが**日数では16.7日<!-- stats: terrain.yaml exploration_phase metric=day_trip_days mean -->**（1周回の14%）になります。1日に探索できる時間が土地の
...
**嵐を引くのは後者だけ**です（8.1.4節）。720分<!-- stats: terrain.yaml daily_budget outdoor_window -->は屋外の枠、267分<!-- stats: terrain.yaml daily_budget survival_gathering -->は1日を賄う生存の採取
（[`stats/balance.yaml`](../../stats/balance.yaml) の `daily_minimum` の627分<!-- stats: balance.yaml daily_minimum place=島全体 total_minutes -->から睡眠360分<!-- stats: terrain.yaml daily_budget sleep -->を引いた分）です。
...
**同じ27個<!-- stats: terrain.yaml work_piles_total piles -->の山が、局面ごとに数え直すと約120日<!-- stats: terrain.yaml cycle base=shortest_mean metric=total_days mean ±1 -->ぶんになります。** 内訳は
**島を開く16.7日<!-- stats: terrain.yaml exploration_phase metric=day_trip_days mean -->**と、**開き切った後の103.2日<!-- stats: terrain.yaml cycle base=shortest_mean metric=steady_days mean -->**です。
...
| **定常**（開き切った後） | 63分<!-- stats: terrain.yaml steady_phase metric=travel_per_day mean --> | 山 381分<!-- stats: terrain.yaml steady_phase metric=work_per_day mean --> | 屋外の山（約38,900分<!-- stats: terrain.yaml work_piles_total outdoor_minutes ±100 -->） | **103.2日**<!-- stats: terrain.yaml cycle base=shortest_mean metric=steady_days mean --> |
| 合計 | | | | **119.9日**<!-- stats: terrain.yaml cycle base=shortest_mean metric=total_days mean --> |

27個<!-- stats: terrain.yaml work_piles_total piles -->の山（約58,400分<!-- stats: terrain.yaml work_piles_total minutes ±100 -->）のうち、屋外での採取・伐採・運搬を2/3
（約38,900分<!-- stats: terrain.yaml work_piles_total outdoor_minutes ±100 -->）、拠点での加工を1/3
（約19,500分<!-- stats: terrain.yaml work_piles_total base_minutes ±100 -->）と置いています。**この割合と、下の土地の配分は
仮置きです。**
...
**律速は屋外です**——拠点での加工は約19,500分<!-- stats: terrain.yaml work_piles_total base_minutes ±100 -->で、夜の枠に対して
54日ぶん<!-- stats: terrain.yaml work_piles_total base_days -->なので、約120日<!-- stats: terrain.yaml cycle base=shortest_mean metric=total_days mean ±1 -->の中に余裕で収まります。**夜が足りなくなることはなく、
足りないのは昼です。**

**この約120日<!-- stats: terrain.yaml cycle base=shortest_mean metric=total_days mean ±1 -->は上限としても下限としても読めません。** 上へ振れる材料は、4節の山の多く
...
——1日の枠280分<!-- stats: terrain.yaml exploration_phase metric=day_trip_window_per_day mean -->に対して探索は
168分<!-- stats: terrain.yaml exploration_phase metric=day_trip_exploration_per_day mean -->しか進まないので、
余る112分<!-- stats: terrain.yaml exploration_phase metric=day_trip_spare_per_day mean -->×16.7日<!-- stats: terrain.yaml exploration_phase metric=day_trip_days mean -->は山へ回せます。
...
土地ごとに日帰りと泊まりの安いほうを採っても15.14日<!-- stats: terrain.yaml exploration_phase metric=mixed_days mean -->で、**日帰りだけの16.68日<!-- stats: terrain.yaml exploration_phase metric=day_trip_days mean -->とほとんど変わりません**。
...
生存の採取267分<!-- stats: terrain.yaml daily_budget survival_gathering -->を引いた残りを使い切らなくなったためです。
...
- 1周回の日数（8節）。局面ごとに数えると**約120日<!-- stats: terrain.yaml cycle base=shortest_mean metric=total_days mean ±1 -->**（探索16.7日<!-- stats: terrain.yaml exploration_phase metric=day_trip_days mean -->＋定常103.2日<!-- stats: terrain.yaml cycle base=shortest_mean metric=steady_days mean -->、8.3節）。**残る仮置きは
