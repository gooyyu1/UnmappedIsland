...
**火（5番）の山は、薪棚です**（`src/assets/world-codex/firewood.yaml`）。炉の段
...
炉の薪の上限を超えるので、くべると入りきらない分が失われます。割って棚で乾かせば燃料1点あたりの
手間が1.26分から0.77分へ落ちます（[`FireSystem.md`](../engine/FireSystem.md) 2.5節）。同じ形だった
移動の距離は、系統から落としました（3.1節）。
...
4. **浜まで運ぶ手段。** そり（運ぶ入れ物の段3）。備蓄は拠点にあり、出航地点は海岸なので、積み込みの前に
   ...
   （[`Voyage.md`](./Voyage.md) 3.9.6節。蒸発と食べ方がここから押し上げます）で、通れなくなる線
   ...
   運べなくなる理由**です。甕の数は下限なので、実際はここから増えます。
...
**嵐を引くのは後者だけ**です（8.1.4節）。720分<!-- stats: terrain.yaml daily_budget outdoor_window -->は屋外の枠、187分<!-- stats: terrain.yaml daily_budget survival_gathering -->は1日を賄う生存の採取
