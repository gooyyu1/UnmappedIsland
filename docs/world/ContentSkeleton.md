...
| **それ以降** | 払う労働の量 | 1日（自由時間927分<!-- stats: terrain.yaml daily_budget surplus -->。8節）以上の労働がかかる山を1段と数える |
...
日数にして66.72日<!-- stats: terrain.yaml work_piles_total days -->ぶんです。**
...
（1日927分<!-- stats: terrain.yaml daily_budget surplus -->）に対する概算で、**実測**と書いたものは量を
その型の値段そのもので採っています。

**この66.72日<!-- stats: terrain.yaml work_piles_total days -->は、明るさと移動時間を無視した下限です**——どちらも入れると同じ山が
87.7日<!-- stats: terrain.yaml cycle base=shortest_mean metric=steady_days mean -->ぶんに広がり、島を開く15.2日<!-- stats: terrain.yaml exploration_phase metric=day_trip_days mean -->を足すと
1周回は約103日<!-- stats: terrain.yaml cycle base=shortest_mean metric=total_days mean ±1 -->になります（8節）。
...
| 7 | 運ぶ入れ物 | 肩掛けの袋・編み籠 | なめし革の背負い袋（1<!-- stats: terrain.yaml work_piles pile=なめし革の背負い袋 days -->）／**そり（1.86<!-- stats: terrain.yaml work_piles pile=そり days -->・実測）** | 2<!-- stats: terrain.yaml work_piles_by_system system=7 piles --> | 2.9<!-- stats: terrain.yaml work_piles_by_system system=7 days --> |
...
| 11 | 船と海図 | — | **筏（4.0<!-- stats: terrain.yaml work_piles pile=筏 days -->・実測）**／**帆（2.8<!-- stats: terrain.yaml work_piles pile=帆 days -->・実測）**／櫂と舵（2<!-- stats: terrain.yaml work_piles pile=櫂と舵 days -->）／沿岸航海（3<!-- stats: terrain.yaml work_piles pile=沿岸航海 days -->）／海図を仕上げる（5<!-- stats: terrain.yaml work_piles pile=海図を仕上げる days -->） | 5<!-- stats: terrain.yaml work_piles_by_system system=11 piles --> | 16.7<!-- stats: terrain.yaml work_piles_by_system system=11 days --> |
| 12 | 家具・娯楽 | — | **設えを積んで里心を止める（1.55<!-- stats: terrain.yaml work_piles pile=設えを積んで里心を止める days -->・実測）** | 1<!-- stats: terrain.yaml work_piles_by_system system=12 piles --> | 1.55<!-- stats: terrain.yaml work_piles_by_system system=12 days --> |
| | | | **合計** | **27**<!-- stats: terrain.yaml work_piles_total piles --> | **66.72**<!-- stats: terrain.yaml work_piles_total days --> |
...
**同じ27個<!-- stats: terrain.yaml work_piles_total piles -->の山が、明るさ・移動・土地の差を入れると87.7日<!-- stats: terrain.yaml cycle base=shortest_mean metric=steady_days mean -->ぶんになり、島を開く15.2日<!-- stats: terrain.yaml exploration_phase metric=day_trip_days mean -->を足して
1周回は約103日<!-- stats: terrain.yaml cycle base=shortest_mean metric=total_days mean ±1 -->です。1日927分<!-- stats: terrain.yaml daily_budget surplus -->という自由時間は上限であって、実際に使える時間ではありません。**

1日を賄う最小労働は513分<!-- stats: balance.yaml daily_minimum place=島全体 total_minutes -->（[`stats/balance.yaml`](../../stats/balance.yaml) の `daily_minimum`）なので、差し引きの自由
時間は1日927分<!-- stats: terrain.yaml daily_budget surplus -->。4節の山の合計は約61,900分<!-- stats: terrain.yaml work_piles_total minutes ±100 -->なので、
**そのまま割れば66.72日ぶん**<!-- stats: terrain.yaml work_piles_total days -->です。しかしこの927分<!-- stats: terrain.yaml daily_budget surplus -->は、
...
灯すと、燃える時間より長くかかります**——火種1つが83.1分<!-- stats: balance.yaml object_costs object=burning_tinder total_minutes -->
なので、下の松明そのものと合わせて148.6分。**炉から分けてもらう道があるのはこのためで**、その道を
通るなら払うのは松明のぶんだけです。

**素材込みの手間は65.5分**<!-- stats: balance.yaml object_costs object=torch total_minutes -->
——**明かり1分あたり0.55分**です。それでも作る価値があるのは、**買えるのが時間の延長ではなく、
...
**嵐を引くのは後者だけ**です（8.1.4節）。720分<!-- stats: terrain.yaml daily_budget outdoor_window -->は屋外の枠、153分<!-- stats: terrain.yaml daily_budget survival_gathering -->は1日を賄う生存の採取
（[`stats/balance.yaml`](../../stats/balance.yaml) の `daily_minimum` の513分<!-- stats: balance.yaml daily_minimum place=島全体 total_minutes -->から睡眠360分<!-- stats: terrain.yaml daily_budget sleep -->を引いた分）です。
...
**同じ27個<!-- stats: terrain.yaml work_piles_total piles -->の山が、局面ごとに数え直すと約103日<!-- stats: terrain.yaml cycle base=shortest_mean metric=total_days mean ±1 -->ぶんになります。** 内訳は
**島を開く15.2日<!-- stats: terrain.yaml exploration_phase metric=day_trip_days mean -->**と、**開き切った後の87.7日<!-- stats: terrain.yaml cycle base=shortest_mean metric=steady_days mean -->**です。
...
| **定常**（開き切った後） | 62分<!-- stats: terrain.yaml steady_phase metric=travel_per_day mean --> | 山 475分<!-- stats: terrain.yaml steady_phase metric=work_per_day mean --> | 屋外の山（約41,200分<!-- stats: terrain.yaml work_piles_total outdoor_minutes ±100 -->） | **87.7日**<!-- stats: terrain.yaml cycle base=shortest_mean metric=steady_days mean --> |
| 合計 | | | | **102.9日**<!-- stats: terrain.yaml cycle base=shortest_mean metric=total_days mean --> |

27個<!-- stats: terrain.yaml work_piles_total piles -->の山（約61,900分<!-- stats: terrain.yaml work_piles_total minutes ±100 -->）のうち、屋外での採取・伐採・運搬を2/3
（約41,200分<!-- stats: terrain.yaml work_piles_total outdoor_minutes ±100 -->）、拠点での加工を1/3
（約20,600分<!-- stats: terrain.yaml work_piles_total base_minutes ±100 -->）と置いています。**この割合と、下の土地の配分は
仮置きです。**
...
**律速は屋外です**——拠点での加工は約20,600分<!-- stats: terrain.yaml work_piles_total base_minutes ±100 -->で、夜の枠に対して
57日ぶん<!-- stats: terrain.yaml work_piles_total base_days -->なので、約103日<!-- stats: terrain.yaml cycle base=shortest_mean metric=total_days mean ±1 -->の中に余裕で収まります。**夜が足りなくなることはなく、
足りないのは昼です。**

**この約103日<!-- stats: terrain.yaml cycle base=shortest_mean metric=total_days mean ±1 -->は上限としても下限としても読めません。** 上へ振れる材料は、4節の山の多く
...
- 1周回の日数（8節）。局面ごとに数えると**約103日<!-- stats: terrain.yaml cycle base=shortest_mean metric=total_days mean ±1 -->**（探索15.2日<!-- stats: terrain.yaml exploration_phase metric=day_trip_days mean -->＋定常87.7日<!-- stats: terrain.yaml cycle base=shortest_mean metric=steady_days mean -->、8.3節）。**残る仮置きは
