...
| **それ以降** | 払う労働の量 | 1日（自由時間909分<!-- stats: terrain.yaml daily_budget surplus -->。8節）以上の労働がかかる山を1段と数える |
...
日数にして70.78日<!-- stats: terrain.yaml work_piles_total days -->ぶんです。**
...
（1日909分<!-- stats: terrain.yaml daily_budget surplus -->）に対する概算で、**実測**と書いたものは量を
その型の値段そのもので採っています。

**この70.78日<!-- stats: terrain.yaml work_piles_total days -->は、明るさと移動時間を無視した下限です**——どちらも入れると同じ山が
93.9日<!-- stats: terrain.yaml cycle base=shortest_mean metric=steady_days mean -->ぶんに広がり、島を開く15.2日<!-- stats: terrain.yaml exploration_phase metric=day_trip_days mean -->を足すと
...
| 7 | 運ぶ入れ物 | 肩掛けの袋・編み籠 | なめし革の背負い袋（1<!-- stats: terrain.yaml work_piles pile=なめし革の背負い袋 days -->）／**そり（2.16<!-- stats: terrain.yaml work_piles pile=そり days -->・実測）** | 2<!-- stats: terrain.yaml work_piles_by_system system=7 piles --> | 3.2<!-- stats: terrain.yaml work_piles_by_system system=7 days --> |
| 8 | 衣類 | 束ねた葉・編んだ葉 | **なめし革の一式（1.9<!-- stats: terrain.yaml work_piles pile=なめし革の一式 days -->・実測）** | 1<!-- stats: terrain.yaml work_piles_by_system system=8 piles --> | 1.9<!-- stats: terrain.yaml work_piles_by_system system=8 days --> |
...
| 11 | 船と海図 | — | **筏（5.1<!-- stats: terrain.yaml work_piles pile=筏 days -->・実測）**／**帆（3.7<!-- stats: terrain.yaml work_piles pile=帆 days -->・実測）**／櫂と舵（2<!-- stats: terrain.yaml work_piles pile=櫂と舵 days -->）／沿岸航海（3<!-- stats: terrain.yaml work_piles pile=沿岸航海 days -->）／海図を仕上げる（5<!-- stats: terrain.yaml work_piles pile=海図を仕上げる days -->） | 5<!-- stats: terrain.yaml work_piles_by_system system=11 piles --> | 18.8<!-- stats: terrain.yaml work_piles_by_system system=11 days --> |
| 12 | 家具・娯楽 | — | **設えを積んで里心を止める（1.69<!-- stats: terrain.yaml work_piles pile=設えを積んで里心を止める days -->・実測）** | 1<!-- stats: terrain.yaml work_piles_by_system system=12 piles --> | 1.69<!-- stats: terrain.yaml work_piles_by_system system=12 days --> |
| | | | **合計** | **27**<!-- stats: terrain.yaml work_piles_total piles --> | **70.78**<!-- stats: terrain.yaml work_piles_total days --> |
...
**同じ27個<!-- stats: terrain.yaml work_piles_total piles -->の山が、明るさ・移動・土地の差を入れると93.9日<!-- stats: terrain.yaml cycle base=shortest_mean metric=steady_days mean -->ぶんになり、島を開く15.2日<!-- stats: terrain.yaml exploration_phase metric=day_trip_days mean -->を足して
1周回は約109日<!-- stats: terrain.yaml cycle base=shortest_mean metric=total_days mean ±1 -->です。1日909分<!-- stats: terrain.yaml daily_budget surplus -->という自由時間は上限であって、実際に使える時間ではありません。**

1日を賄う最小労働は531分<!-- stats: balance.yaml daily_minimum place=島全体 total_minutes -->（[`stats/balance.yaml`](../../stats/balance.yaml) の `daily_minimum`）なので、差し引きの自由
時間は1日909分<!-- stats: terrain.yaml daily_budget surplus -->。4節の山の合計は約64,300分<!-- stats: terrain.yaml work_piles_total minutes ±100 -->なので、
**そのまま割れば70.78日ぶん**<!-- stats: terrain.yaml work_piles_total days -->です。しかしこの909分<!-- stats: terrain.yaml daily_budget surplus -->は、
...
**嵐を引くのは後者だけ**です（8.1.4節）。720分<!-- stats: terrain.yaml daily_budget outdoor_window -->は屋外の枠、171分<!-- stats: terrain.yaml daily_budget survival_gathering -->は1日を賄う生存の採取
（[`stats/balance.yaml`](../../stats/balance.yaml) の `daily_minimum` の531分<!-- stats: balance.yaml daily_minimum place=島全体 total_minutes -->から睡眠360分<!-- stats: terrain.yaml daily_budget sleep -->を引いた分）です。
...
**同じ27個<!-- stats: terrain.yaml work_piles_total piles -->の山が、局面ごとに数え直すと約109日<!-- stats: terrain.yaml cycle base=shortest_mean metric=total_days mean ±1 -->ぶんになります。** 内訳は
**島を開く15.2日<!-- stats: terrain.yaml exploration_phase metric=day_trip_days mean -->**と、**開き切った後の93.9日<!-- stats: terrain.yaml cycle base=shortest_mean metric=steady_days mean -->**です。
...
| --- | --: | --: | --: | --: |
| **探索**（島を開き切るまで） | 163分<!-- stats: terrain.yaml exploration_phase metric=day_trip_travel_per_day mean --> | 探索 184分<!-- stats: terrain.yaml exploration_phase metric=day_trip_exploration_per_day mean --> | 全土地を探索率100%まで（2,799分<!-- stats: terrain.yaml exploration_phase metric=exploration_minutes mean -->） | **15.2日<!-- stats: terrain.yaml exploration_phase metric=day_trip_days mean -->** |
| **定常**（開き切った後） | 62分<!-- stats: terrain.yaml steady_phase metric=travel_per_day mean --> | 山 462分<!-- stats: terrain.yaml steady_phase metric=work_per_day mean --> | 屋外の山（約42,900分<!-- stats: terrain.yaml work_piles_total outdoor_minutes ±100 -->） | **93.9日**<!-- stats: terrain.yaml cycle base=shortest_mean metric=steady_days mean --> |
| 合計 | | | | **109.1日**<!-- stats: terrain.yaml cycle base=shortest_mean metric=total_days mean --> |

27個<!-- stats: terrain.yaml work_piles_total piles -->の山（約64,300分<!-- stats: terrain.yaml work_piles_total minutes ±100 -->）のうち、屋外での採取・伐採・運搬を2/3
（約42,900分<!-- stats: terrain.yaml work_piles_total outdoor_minutes ±100 -->）、拠点での加工を1/3
（約21,400分<!-- stats: terrain.yaml work_piles_total base_minutes ±100 -->）と置いています。**この割合と、下の土地の配分は
仮置きです。**
...
**律速は屋外です**——拠点での加工は約21,400分<!-- stats: terrain.yaml work_piles_total base_minutes ±100 -->で、夜の枠に対して
60日ぶん<!-- stats: terrain.yaml work_piles_total base_days -->なので、約109日<!-- stats: terrain.yaml cycle base=shortest_mean metric=total_days mean ±1 -->の中に余裕で収まります。**夜が足りなくなることはなく、
足りないのは昼です。**
...
——1日の枠386分<!-- stats: terrain.yaml exploration_phase metric=day_trip_window_per_day mean -->に対して探索は
184分<!-- stats: terrain.yaml exploration_phase metric=day_trip_exploration_per_day mean -->しか進まないので、
余る202分<!-- stats: terrain.yaml exploration_phase metric=day_trip_spare_per_day mean -->×15.2日<!-- stats: terrain.yaml exploration_phase metric=day_trip_days mean -->は山へ回せます。
...
土地ごとに日帰りと泊まりの安いほうを採っても15.14日<!-- stats: terrain.yaml exploration_phase metric=mixed_days mean -->で、**日帰りだけの15.22日<!-- stats: terrain.yaml exploration_phase metric=day_trip_days mean -->とほとんど変わりません**。
...
生存の採取171分<!-- stats: terrain.yaml daily_budget survival_gathering -->を引いた残りを使い切らなくなったためです。
...
- 1周回の日数（8節）。局面ごとに数えると**約109日<!-- stats: terrain.yaml cycle base=shortest_mean metric=total_days mean ±1 -->**（探索15.2日<!-- stats: terrain.yaml exploration_phase metric=day_trip_days mean -->＋定常93.9日<!-- stats: terrain.yaml cycle base=shortest_mean metric=steady_days mean -->、8.3節）。**残る仮置きは
