...
光源が要ります（8.1.2節）。**「移動できる」列が明るい時間ぶん丸ごと開かないのは雨季です**——嵐の時間が
引かれるためで、砂浜なら夜も含む24時間から、他の土地なら昼の12時間から削れます。**密林だけは明るさの
側でも落ちます**——樹冠が−9と深いぶん、雨季の荒天では正午でも移動のしきい値を割ることがあります。
...
**海の上で渡ることだけは、この線の外です。** 屋根の下という逃げ場が海区には無いためで、そちらは
[`Voyage.md`](./Voyage.md) 3.8節が扱います。
...
- 嵐が1周回の日数へどう効くか（8.2節）。**屋外の枠（720分）を嵐で縮めていない**ので、嵐が屋外の行動を
  すべて止めても日数は1日も動きません。往復の移動が嵐に当たる分を数えるには、1日の割り付けの側から
  作り直すことになります。
