...
経路で当てても同じだけかかる。** 当てる操作を `combinations` として書くと、スロットへ直接落とす経路だけが
無料になり、同じことが経路で違う値段になる。
...
- 悪化（放置した怪我が別の怪我へ変わる）
- 動物にも同じ構造の怪我を負わせる（[`HuntingSystem.md`](../engine/HuntingSystem.md) 3節）。部位別の状態・
  手負いの自然治癒・飼いならしの入口が、この対称性から出る
