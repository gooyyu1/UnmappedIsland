...
1つだけ当てておける。当てるのは怪我のカードへ治療具を重ねる操作（`treat`）。

**外す経路はまだ無い。** スロットの移動としては出せるが（テストはそうしている）、当てた治療具は
画面のどこにも現れないので、プレイヤーには触れない。怪我のカードをタップしても出るのは名前と説明
だけで、`container` タグを持たないため中身の子ウィンドウは開かない。
...
- **当てた治療具を外す経路。** 一番素直なのは、怪我のカードをタップしたら治療具の子ウィンドウが開く形
  （装備・コンテナと同じ操作になる）。そのためには「中身を見せるカードか」の判定を `container` タグから
  「プレイヤーが入れられるスロットを持つか」へ広げる——怪我に `container` タグを付けるのは筋が悪い。
  アクションとして書く道もあるが、`move` の対象は `actor`/`dragged` だけなので、子を外へ出す言い方が
  今の語彙に無い（[`GameElementDefinition.md`](../engine/GameElementDefinition.md) 9.6節）
- 出血と感染。止血帯の得意・不得意はこれが無いと書けない
- **治った怪我に当てていた治療具の行き先。** 今は怪我と一緒に世界から外れる（`destroy` は自分が親から
  外れるだけで、中身は付いてくる）。使った包帯が無くなるのは筋が通るが、添え木のように残ってほしい
  ものが出たら決め直す
