...
あり（[`Dwellings.md`](./Dwellings.md) 2.1 節）、`snug` を払えるのは設えだけです。
...
90日目からの増え（+3.84/日）を止めるには `snug` まで積むか、**設えと家畜の両方**が要るので、
...
場所は、設えの見せ方が決まるまで置きません**（[#1642](https://github.com/gooyyu1/UnmappedIsland/issues/1642)）。
