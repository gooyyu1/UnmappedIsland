...
食べ物は `eat` の `add` に定数を1つ持ちます。**量ではなく質で決まります**——かさは `satiety` が、
身になる量は栄養素が既に持っているので、ここが数えるのは食べた喜びだけです。同じ「火の通った1食」
なら、丸焼きのネズミも焼いた肉と同じだけ戻します。
...
**食べ物のほかに、打ちひしがれている間の戻しがあります**（下の[限界](#限界)節）——0 まで落ちた後に
しか効かないので、食事で保てている間は現れません。
