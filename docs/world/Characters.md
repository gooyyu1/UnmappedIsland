...
| `fall_asleep`（眠り込む） | `wakefulness` | 360分 | `wakefulness` +48・`stamina` +90 | 8・15 |
| `despair`（打ちひしがれる） | `happiness` | 120分 | `happiness` +20 | 10 |

- **戻る割に、まとめて休んだ割増し**（上の[休息](#休息)節）**は付けません。** どれも休み方を選んだことに
  ならないためで、体力と幸福度は `rest` の割（10/時間）です。**眠気を戻せるのは眠る休息だけで、どれも
  割増しを持っている**ので、そのうちいちばん薄い `nap` の実質の割（8/時間）を使います。長さを掛けて、
  それぞれ +20・+20・+48。
...
- **眠気だけは実質で数えます。** 強制の間の減り（`-1/tick`）は下限のクランプが吸うので、眠り込みは
  宣言どおりの量が残ります（体力と幸福度は自分では減らないので、この差が出ません）。**割増しを付けると
  「倒れるまで起きていること」が最良の手になります**——`sleep` と同じ +96 なら起きている割合は 80% で
  寝床の上の 75% を上回り、その実質の +72 でも寝床の上とちょうど並びます。**どちらでも寝床を敷く理由が
  消えるので、`nap` の割まで落としてあります。** 限界は罰であって、選ばなかったことへの褒美にはしません。
