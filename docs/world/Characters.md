...
`company` も持たない**——常時見せるのは間の `homesickness` だけで、どれもその詳細から辿って読む
...
- **`happiness`（幸福度）**: **メンタルの不調を代表する1本**で、単位を持たない 0〜100。自分では動かず、
  食べた物が上げ、`pain` と `homesickness` の段が下げる（下の[幸福度](#幸福度)節）。個体差は持たせず trait が配る。
...
- **`pathogen`（全身の菌）／`immunity`（免疫）**: 菌は **log₁₀ CFU**（0〜9）で持ち、菌が居ることになる段
  ...
  削るので**死に方は増えない**。免疫を押し下げるのは生活の側だけで、**ビタミンが最も重く、しかも2段で
...
- **`load`（荷重）**: 持ち物と装備の重さ（g）。自分では動かず、中身から導出される
  ...
  ——歩みの遅れと `stamina` の削りも段が持つ（下の荷重の効き方節）。
...
**効かせ方は 1 箇所——`load` の段**（[`ContainerSystem.md`](../engine/ContainerSystem.md) 5 節）**で、
閾値を別に持つ量は 1 つも足しません。** 荷が何に効くかは「荷は、歩みの遅れと疲れにも効く」節が
決めます。
...
- **軽くする道具の値打ちは損なわれません。** そりのような率を下げる道具は `load` そのものを下げるので、
  **移動の可否・遅れ・疲れに同時に効きます**（[`ContainerSystem.md`](../engine/ContainerSystem.md)
...
**1 日ぶんと、荷を下ろして眠った一晩の回復が同じ桁であること**だけです。

### 荷は、歩みの遅れと疲れにも効く【確定】

**荷は、担げるかどうかだけでなく、歩みの遅れと疲れにも効きます。**

**出どころ**: [#656](https://github.com/gooyyu1/UnmappedIsland/issues/656)（重いほど移動が遅くなる／重いほど疲れる）
...
既に持っているので、ここが数えるのは食べた喜びだけです。同じ「火の通った1食」なら、丸焼きのネズミも
焼いた肉と同じだけ戻します。

**持つのは `eat` だけで、飲む操作は1つも持ちません**——`food` を名乗る青い実（刃で穴を開けてその場で
水を飲む）も同じです。**飲んで上がるかどうかは決めていません。**
...
**常時のバーに出るのは `homesickness` だけです。** 溜める側の `loneliness` も、抑える側の `comfort`・
`company` も `status` タグを持たないので、増減の理由は「里心が募っている」の1本に集まります。
**どれも、そのホームシックの詳細から辿れば読めます**——影響の枠として並んでいるので、押せばその段が
出ます（下の[抑えるのは、今いる場所](#抑えるのは今いる場所)節）。
...
（洞窟と住居は土地の `fixtures` に据わる場所で、筏は海へ出れば海区の `fixtures` に浮かぶ場所です）。継がないと、**浜に建てた囲いのすぐ隣の
洞窟へ入った瞬間に慰めが消えます**。継ぐのは下向きだけなので、**外に立っている間、住居の中の設えは
効きません。**

**`comfort`（居心地）は、その場に据えた設えが押し上げます**（家具・娯楽、`furnishings.yaml`。据えれば
効き続け、手入れは要りません）。住居の部品も同じ形で押しますが、そちらはまだ実装がありません
（[`Dwellings.md`](./Dwellings.md) 2 節）。

**設えは、据えること以外に何の役にも立ちません**——生きるのに要る物が居心地を兼ねると、対策が
どうせ作る物になって圧が素通りします（[`ContentSkeleton.md`](./ContentSkeleton.md) 3 節の系統12）。
...
**90日目からを止める `snug` へは、設えを積んで届きます。** 単体で届く物は1つも無いので必ず積むことに
なり、**何をいくつ並べるかは自由で、要るのは合計の手間のほう**です（点数を掛かる手間に比例させてある
ので点あたりの手間はおおむね揃いますが、**払う総手間は並べ方で上へ振れます**——点あたりの手間の差と、
線を越える手前で行き過ぎる量の差が、どちらも乗るためです。いちばん安い積み方は
[`stats/terrain.yaml`](../../stats/terrain.yaml) の `work_piles` に出ます）。
**刃物1つで届く道もある**ので、斧も狩りも通らずに `snug` まで積めます——里心の対策は、ほかの系統の
進み具合に縛られません。

**どちらもプロパティのタブには出ません**（タグを1つも持たないため、[`Windows.md`](../ui/Windows.md) 6 節）。
**読む場所はホームシックの詳細で、そこに並ぶ影響の枠を押すと、今どの段に居るかが名札で出ます**
（同 8.1 節・8.2 節）。出すのは段の名前だけで、点数は出しません——要るのは「今どれに届いているか」で、
あと何点かではないためです（腕前と同じ、[`Skills.md`](./Skills.md) 5.1 節）。

**専用の場所を作らないのは、押し上げているのが場所の側だからです。** 周りの事情を身の値と同じ並びへ
混ぜると、`homesickness` が動いた理由を確かめる先が割れます
（[`DesignPrinciples.md`](../concept/DesignPrinciples.md)「周りの事情と、身に起きていることは、別の
場所に出す」）。**結果から原因へ降りる道だけを開けておけば、問いを持った人にだけ答えが出ます。**
気にし続けたい人は、その詳細から固定表示にすれば常時のバーへ出せます
...
11.7 節）。経過中の減り（荷・痛み・里心）と同じ足し算に入るので、**受け取る量は「宣言 − その間の減り」**
になります。休んでいる間も荷は担いだままで、その重さのぶんだけ戻りが薄くなります。

入口は日時のフリップカードで、押すとキャラクタ自身の子ウィンドウが開きます
（[`Windows.md`](../ui/Windows.md) 4 節）。

| | 長さ | `stamina` | 1時間あたり | `wakefulness` | 1時間あたり | 眠気の実質 | 要るもの |
| --- | --- | --- | --- | --- | --- | --- | --- |
| `wait`（待機） | 15分 | +2/tick | 8 | — | — | −1 | — |
| `rest`（休憩） | 60分 | +2.5/tick | 10 | — | — | −4 | — |
| `nap`（仮眠） | 180分 | +3/tick | 12 | +3/tick | 12 | **+24**（6時間ぶん） | — |
| `sleep`（睡眠） | 360分 | +3.75/tick | 15 | +4/tick | 16 | **+72**（18時間ぶん） | **寝床** |

- **睡眠だけが寝床を要ります。** 寝床の在る場所でしか押せず、地面の上で取れるのは仮眠までです
  （[`Bedding.md`](./Bedding.md) 4.1節）。**釣り合う点がずれます**——寝床の上では18時間起きて6時間
  眠るところ、仮眠だけだと16時間起きて8時間で、**1日に起きていられる時間が2時間短くなります。**
- **まとめて休むほど1時間あたりの回復が大きい。** 同じ6時間でも、空身なら仮眠2回（体力72・眠気の実質48）
  より睡眠1回（90・72）のほうが多く戻ります。細切れに休むより通しで休むほうが得、が数値だけで出ます。
...
| --- | --- | --- | --- | --- |
| `collapse`（倒れ込む） | `stamina` | 120分 | `stamina` +2.5/tick | 10 |
| `fall_asleep`（眠り込む） | `wakefulness` | 360分 | `wakefulness` +3/tick・`stamina` +2.5/tick | 12・10 |
| `despair`（打ちひしがれる） | `happiness` | 120分 | `happiness` +2.5/tick | 10 |

- **戻る割に、まとめて休んだ割増し**（上の[休息](#休息)節）**は付けません——どの値についても。** どれも
  休み方を選んだことにならないためで、体力と幸福度は `rest` の割（10/時間）です。**眠気を戻せるのは
  眠る休息だけで、どれも割増しを持っている**ので、そちらはいちばん薄い `nap` の割（12/時間）を使います。
- **眠り込みが戻すのは眠気だけではないので、体力もこの割に従います。** `sleep` と同じ 15/時間に
  すると、**寝床の無い場所で体力を戻す最良の手が眠り込みになります**——寝床を要らない休息のうち最も
  割のよい `nap`（12/時間）を上回ってしまうためです。
- **戻る量は個体差を持ちません**（休息と同じく trait が配ります）。`stamina` の `max` が大きい
  キャラクタほど1回で戻る割合は小さく、倒れる回数の重みは体格の側に出ます——**空身で倒れれば**、
  `max` が 100 の船長は要注意域（`tired`、20）まで戻りますが、120 の農夫は危険域（`exhausted`）に
  居たまま起き上がります。**荷を担いだままなら、どちらも危険域に居たままです**——`laden` で +17.6、
  `heavy` で +12、`too_heavy` で +4 しか戻らず、船長の 20 にも届きません。
...
- **戻すのが tick 毎なのは、限界だけが下限から始まるからです。** 経過し終えてから足す形だと、下限に
  ...
  引かれるので、同じ長さなら限界のほうが多く戻ることになっていました。tick 毎なら減りと同じ足し算に
  入るので、荷を担いだままの倒れ込みは担いだぶんだけ薄くなります。限界は罰であって、選ばなかった
  ことへの褒美にはしません。
- **どれも寝床を要りません**（見るのは尽きた値だけです）。自発の `sleep` は寝床を要りますが
  （上の[休息](#休息)節）、その要件を眠り込みへ写すと、**寝床の無い場所で眠気が尽きた時点で覚醒度を
  戻す手が世界から消えます**——限界は逃げ場であって線ではありません。
...
動いたときにまた起きます。ただし**限界の手番が起きている最中に尽きた値は、同じ値でも別の値でも、その
切れ目では続きません**（倒れ込んでいる最中に眠気が尽きても、眠り込むのは次に時間が動いたときです）
——戻り切らない値があると、手が戻らないまま時間だけが進み続けることになるためです。同時に尽きていた値は、
どれもその切れ目で順に引き直されます——先に起きた手番の間に戻り切っていれば、そちらは起きません。

**どれも、強制の時間が過ぎる前に自分から告げます**（`announce`、
