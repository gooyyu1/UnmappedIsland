...
**時間を進める操作は、キャラクタ自身のアクションです。** どれも同じ形（`duration` と `add`）で
...
| `sleep`（睡眠） | 360分 | +90 | 15 | +96 | 16 | **+72**（18時間ぶん） | **寝床** |
...
| `fall_asleep`（眠り込む） | `wakefulness` | 360分 | `wakefulness` +48・`stamina` +60 | 8・10 |
...
- **戻る割に、まとめて休んだ割増し**（上の[休息](#休息)節）**は付けません——どの値についても。** どれも
  ...
  眠る休息だけで、どれも割増しを持っている**ので、そちらはいちばん薄い `nap` の実質の割（8/時間）を
  使います。上の表の量は、その割に長さを掛けたものです。
- **眠り込みが戻すのは眠気だけではないので、体力もこの割に従います。** `sleep` と同じ +90（15/時間）に
...
- **クランプが吸うぶんを割へ織り込めているのは、眠気だけです。** 強制の間も減りは走りますが、下限から
  始まるのでクランプが吸い、宣言した量がそのまま残ります。眠気の減りは `-1/tick` の定数なので織り込め
  ました——`sleep` と同じ +96 なら起きている割合は 80% で寝床の上の 75% を上回り、その実質の +72 でも
  寝床の上とちょうど並びます。**どちらでも寝床を敷く理由が消えるので、`nap` の割まで落としてあります。**
  **体力と幸福度の減りは荷や痛みで変わるので、同じようには織り込めていません**
  （[#1548](https://github.com/gooyyu1/UnmappedIsland/issues/1548)）。限界は罰であって、選ばなかった
  ことへの褒美にはしません。
...
[`GameElementDefinition.md`](../engine/GameElementDefinition.md) 11.6 節）。1時間の道を選んで押した
プレイヤーには、ボタンが言った 60 分ではなく 180 分が過ぎたことしか見えないので、告げなければ時計が
飛んだ理由が画面のどこにも出ません。**告げるのを結果の側（`signal`）に置けない**のはここで、眠り込みの
360 分では理由を聞くのが 6 時間後になります。

| 手番 | 告げる出来事 |
| --- | --- |
| `collapse` | `exhausted` |
| `fall_asleep` | `dozed_off` |
| `despair` | `disheartened` |

出す場所と見え方は札の上の文字で、他の出来事と同じです（[`../ui/CardView.md`](../ui/CardView.md) 14 節）。
