...
| `fall_asleep`（眠り込む） | `wakefulness` | 360分 | `wakefulness` +72・`stamina` +90 | 12・15 |
| `despair`（打ちひしがれる） | `happiness` | 120分 | `happiness` +20 | 10 |

- **戻る量は、その値を戻せる自発の操作に「実質で」合わせます。** 眠気を戻せるのは睡眠だけなので、
  眠り込みが戻すのも睡眠1回ぶん（実質 +72）。体力と幸福度は `rest` と同じ割（10/時間）で2時間ぶんです
  ——倒れ込むのは休み方を選んだことにならないので、**まとめて休んだ割増し**（`nap`・`sleep`、
  上の休息節）**は付きません。**
...
- **強制の間の減りは、下限のクランプが吸います。** 自発の睡眠は経過ぶん（`-1/tick`）を引いた実質の
  回復になりますが、下限から始まる眠り込みは宣言どおりの量が残ります。**だから宣言を `sleep` の
  実質（+72）に合わせています**——`sleep` と同じ +96 と書くと、眠り込みのほうが自分で選んだ睡眠より
  効率よく眠気を戻すことになり（起きている割合が 75% から 80% へ上がります）、**覚醒度を戻す最良の手が
  「倒れるまで起きていること」になります。** 限界は罰であって、選ばなかったことへの褒美にはしません。
