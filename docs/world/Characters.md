...
| プロパティ | `weight` / `pace` / `pain` / `blood` / `warmth` / `chill_point` / `satiety` / `carbohydrate` / `protein` / `lipid` / `vitamin` / `happiness` / `hydration` / `body_fat` / `wakefulness` / `stamina` / `load` と、腕前の11本（[`Skills.md`](./Skills.md) 2 節）、その段が押し上げる倍率 `ignition_ease` / `quarry_sense`（同 5 節。`pace` と同じ形）（いずれも個体差を持たず `player_character` trait が配る） |
