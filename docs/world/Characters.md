...
**押し上げる物が、据えること以外に何の役にも立たないのは、そこが値打ちだからです**——生きるのに要る物が
居心地を兼ねると、対策がどうせ作る物になって圧が素通りします
（[`ContentSkeleton.md`](./ContentSkeleton.md) 3 節の系統13）。だから寝床は `comfort` を押しません。
...
**90日目からを止める `snug` へは、設えを積んで届きます。** ひと通り据えて初めて届く量にしてあり
（1つでは届かない、`tests/world-codex/furnishingsYaml.test.ts`）、最後の1つは獣を仕留めていないと
作れないので、**終盤の圧に終盤の仕事で答える形**になります。
