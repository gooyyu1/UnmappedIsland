...
**このリポジトリは 2026-07-13 に始まり、7週間で 2,508 コミットになりました。** その間に**作り方そのものを5回変えています**。
...
timeline
    ...
    section 8月
        ...
        08-28 2つ目のエージェントを同格にする : Copilot CLI にも同じ既定が載る<br/>ただしクレジットは前日に尽きていた
        08-29 司令塔の手順を仕組みへ出す : 盤面・投入・マージを3本のスクリプトへ<br/>差分はレビュアーのセッションが読む
        08-30 確定の印を規約と検査にする : 実装 4.6万行 / 試験 4.5万行
        08-30 一次レビューを出す側へ足す : 差し戻しが1本あたり平均2.2周<br/>導出値の写しをやめる
```
...
| 08-30 | 45,853 | 45,178 | 17,906 | 10,893 | 330 | 853 | 6.6ファイル / 237行 | 55% |
...
**出したものの量（上の表）に対して、入れたものの量です。** 数字は GitHub の使用レポートの
エクスポートで、**リポジトリからは読めません。**

| 日 | 使ったクレジット | その時の月の枠 | 枠を超えた課金 |
| --- | --: | --: | :--: |
| 06-20 | 85 | 1,500 | |
| **07-13** | **1,516** | **1,500** | **あり** |
| **07-13** | 1,585 | **7,000** | |
| 07-14 | 136 | 7,000 | |
| 07-17 | 204 | 7,000 | |
| 07-20 | 1,759 | 7,000 | |
| **07-21** | **1,812** | 7,000 | **あり** |
| 07-25 | 2 | 7,000 | **あり**（全額） |
| 08-03・08-04・08-06 | 10・501・76 | 7,000 | |
| **08-27** | **6,424** | 7,000 | **あり** |

**表に出ている行の合計は枠に届きません**（7,000 の枠に対して、7月の行は 5,496）。**枠を埋めた分の
一部はこのエクスポートに出ていない**ので、ここから読めるのは総量ではなく**どの日に枠を超えたか**です。
...
### 枠は2回尽きていて、どちらの日も記録に残っている

**Copilot は枠を2回使い切っています**（上の「Copilot に使ったクレジット」）。

- **1回目は初日の 07-13。** 1,500 の枠に 1,516 を使い、超えた分が課金されています。**同じ日の次の
  行から枠が 7,000 に変わっている**ので、**上位プランへ変えたのはこの日**です——当事者の証言では
  「1か月分を4時間ほどで使い切った」。
- **2回目は 07-21。** ここも枠を超えて課金され、**07-25 の2クレジットは割引が1円も掛からず全額
  課金**されています。枠が残っていません。
...
使い切っており（上の「Copilot に使ったクレジット」）、**08-28 以降に Copilot 名義で入ったコミットは
1本だけ**です。**この期に書いているのは、実質 Claude だけ**になります。
...
| レビューを立てたPR | **117本** |
| 立てたレビューのセッション | **256本** |
| PR1本あたりの投入 | **平均 2.19回 / 中央値 1回** |
| 2回以上になったPR | **55本（47%）** |
| 最多 | [PR #1298](https://github.com/gooyyu1/UnmappedIsland/pull/1298) の**11回**（うち「直しが要る」が7回） |

**（08-29 21:38〜08-30 23:34。レビューのセッションは1回の投入につき1本立ち、使い回しません。）**
...
入った本数の約4倍**（373対95）。**増えた300本のうち240本がレビュー**です。

**セッションの数はトークンの量そのものではありませんが、比例します**——レビューのセッションは毎回
まっさらな文脈へ、差分・観点の一覧・仕様書を読み込むところから始まります。**時間あたりの本数には
出ません**（この時期に契約を最上位プランへ変えており、同時に流せる本数が増えているため）。**見るべきは
トークンあたりです。**
...
| | 一次レビューの前 | 後 |
| --- | --: | --: |
| 最初のレビューが立ったPR | 98本 | 19本 |
| PR1本あたりの投入 | 平均 2.36回 | 平均 1.32回 |
| 2回以上になったPR | 52% | 21% |

**この差はまだ読みません。** 回り始めて数時間で、後の19本は**まだ往復の途中**です（これから2周目が
