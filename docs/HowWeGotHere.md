...
**このリポジトリは 2026-07-13 に始まり、8週間で 2,887 コミットになりました**（2026-09-07 時点）。その間に**作り方そのものを何度も変えています**——その区切りが下の年表です。
...
**区切りの日**を渡すと、そのまま出ます。**先頭の 07-13 は渡していません**——どこで作り方が
変わったかは選ぶものですが、**いつ始まったかは履歴の側に在る**ので、道具が付けます。
...
**`issue` の列だけはgitに無い**ので、先に `npm run stats:issues`
（[`collectIssueHistory.mjs`](../scripts/collectIssueHistory.mjs)）がGitHubから集めて
[`stats/issues.tsv`](../stats/issues.tsv) へ置き、表を作る側はそれを読みます。**表を作る側は網へ
出ません**——出ると、`gh` も網も無い環境ではこの列だけが落ちた表が出て、貼り直した先で今ある値を
失います。**集め直していない日の行には `（持ち越し）` が付く**ので、付いたまま貼らないこと。
...
| 文書 | `Documents/**.md` | `docs/**.md` ＋ 取り決めとひな形（`agent-ops/` 直下と `agent-ops/prompts/` の `*.md`。置き場が `.claude/` 直下だった頃のぶんも同じ列） |
...
置き場が道具と同じでもデータなので `定義` へ、運用の取り決め（`agent-ops/` の `*.md`）は
文書なので `文書` へ入ります。

**上の表の範囲に当たらないものは、どの列にも数えません。** `agent-ops/` から `文書` へ数えるのは
**取り決めとひな形だけ**で（実行されるものは階層を問わず `道具` へ入ります）、**判断の履歴
（`agent-ops/decisions/`）と分析の書き出し（`agent-ops/analysis/`）、skill の本文**はどの列にも
入りません。文書とは別のもので、
分ける必要が出たときに分類を作ります。`.github/**` とリポジトリ直下の `*.md` も入っていません。

| 日 | 実装 | 試験 | 文書 | 定義 | 道具 | issue | PR | 1PRあたり | 変更行 | Claudeのコスト | 文書と実装が同じPR |
| --- | --: | --: | --: | --: | --: | --: | --: | --: | --: | --: | --: |
| 07-13 | 488 | 0 | 274 | 0 | 0 | 12 | 12 | 7.6ファイル / 402行 | 4,819 | $0（1本 $0.0・千行 $0.0） | 0% |
| 07-18 | 4,337 | 4,300 | 2,074 | 63 | 0 | 18 | 48 | 9.3ファイル / 680行 | 24,481 | $34（1本 $0.9・千行 $1.4） | 39% |
| 07-25 | 9,473 | 8,596 | 4,472 | 1,527 | 180 | 48 | 124 | 13.4ファイル / 1,020行 | 77,533 | $356（1本 $4.7・千行 $4.6） | 32% |
| 08-01 | 15,523 | 11,381 | 5,532 | 3,490 | 1,556 | 48 | 210 | 13.9ファイル / 859行 | 73,889 | $1,270（1本 $14.8・千行 $17.2） | 62% |
| 08-13 | 26,906 | 18,583 | 9,113 | 6,772 | 3,085 | 51 | 369 | 11.1ファイル / 326行 | 51,800 | $2,108（1本 $13.3・千行 $40.7） | 69% |
| 08-23 | 38,494 | 29,038 | 14,909 | 13,242 | 5,162 | 58 | 589 | 19.0ファイル / 529行 | 116,375 | $3,351（1本 $15.2・千行 $28.8） | 60% |
| 08-26 | 41,213 | 33,622 | 17,099 | 13,976 | 5,590 | 152 | 699 | 5.0ファイル / 229行 | 25,137 | $1,203（1本 $10.9・千行 $47.9） | 44% |
| 08-30 | 46,805 | 48,879 | 22,286 | 18,919 | 8,231 | 499 | 950 | 6.2ファイル / 203行 | 51,052 | $5,117（1本 $20.4・千行 $100.2） | 47% |
| 09-04 | 47,607 | 50,186 | 23,035 | 19,040 | 8,963 | 508 | 975 | 7.5ファイル / 283行 | 7,073 | $727（1本 $29.1・千行 $102.8） | 32% |
| 09-07 | 49,241 | 59,187 | 26,469 | 20,405 | 12,026 | 645 | 1,114 | 9.5ファイル / 306行 | 42,544 | $3,238（1本 $23.3・千行 $76.1） | 55% |

**PR は `main` へ入った累計**（マージコミットと squash の両方を数え、`main` へ直接 push した分は
入りません）。**1PRあたりと「同じPR」の割合は、1つ上の行からの区間の平均**です（先頭の行だけは、
その日1日ぶん）。**区間は1つ上の行の翌日から、その行の日まで**で、日数もこの数え方です——本文で
区間を名指しするときも、1日あたりを出すときも、この範囲で数えます。

**右端の列の「文書」は、`文書` の列とは範囲が違います**——仕様の置き場（`docs/`・`Documents/`）だけで、
`agent-ops/` の取り決めは入りません。実装と対で書かれるものではないので、**対になっているかを見る
この割合には数えません。**

**`変更行` は、その区間のPRが動かした行（追加＋削除）の合計**で、行数の列の増分（純増）とは
別のものです。**書き直しと削除は純増に現れませんが、掛かったコストは同じ**なので、値段を割る
分母にはこちらを使います。`1000 × コスト ÷ 変更行` が括弧の `千行` で、
`変更行 ÷ その区間のPR本数`（`PR` の列の、1つ上の行からの増分）が `1PRあたり` の行数です。

**09-04 で終わる区間（08-31〜09-04）は、ほとんど止まっています**（下の「幕間（08-31〜09-03）」）。PRが
25本しか増えていないので、**この行の「1本あたり」と「千行あたり」は、他の行より少ない標本の上に
乗っています。**

**コストは [`stats/usage/`](../stats/usage) から引いています**（集め方と限界は
[`scripts/usage/README.md`](../scripts/usage/README.md)）。**クラウドのセッションも手元も入った額**で、
括弧の中は同じ額をその区間の量で割ったもの（上の `変更行` の説明）。**手元の分だけは換算値**（ローカルの記録は金額を
...
**時点の量と区間の量は、同じ日に置いていません。** 行数や累計はその日の状態なので区切りの日に
置きますが、**区間を平均した量は区間の真ん中へ置きます**——区切りの日に置くと、区間の右端へ
張り付いたまま、区間の長さのぶんだけ点ごとに違うずれ方をします。区切りは「作り方を変えた日」で
決めていて幅が揃っていないので、ずれ方も点ごとに違います。
...
**並列化した直後の区間では、行数が4分の1以下・ファイル数が半分以下**まで落ちました（その後の
戻りは下の「表の最後の区間」）。**1本がぐっと小さくなっているので、本数が増えたぶんがそのまま
量ではありません。** 1つのタスクを1セッションが持つ形にしたことで、PRの粒が「1セッションで
片付く大きさ」に揃ったのがこの列です。

![PR1本あたりの大きさ](./HowWeGotHere_pr_size.svg)

**08-23 で終わる区間の山**（19.0ファイル・529行）は、負債の棚卸しで見つかった置き場の直しが
1本に固まったぶんです（点は区間の真ん中に在ります）。**その次の区間で5.0ファイル・229行まで
落ちる**のが、並列化でPRの粒が揃ったところです。

**表の最後の区間（09-05〜09-07）では 9.5ファイル・306行へ戻っています。** この区間の139本を、
**盤面を作り直すもの**（`.claude/**`・`scripts/**`・`.github/**` を触る54本）と**それ以外**（85本）で
割ると、**10.0ファイル・436行 対 9.2ファイル・224行**。
...
**この図だけは、期の区切りを使っていません。** 区切りは「作り方を変えた日」で決まっていて
幅が揃わないので、区間の量を区切りごとに出すと**区切りを1日動かすだけで値が半分になり**、総額の上下は
使ったペースではなく区間の長さを描くことになります。**幅を固定した移動窓で、日ごとに出しています**
（`scripts/historyStats.mjs` の `WINDOW_DAYS`）。
**表の区間ごとの値とは、割った幅が違うぶん食い違います。** 右端の点は、まだ走っているセッションを
含まないぶん下側に出ます。

**図の段ごとに割る先が違います**（何で割ったかは段の見出しに出ています）。1本あたりは8月の半ば
（08-03〜08-19）を **$7.9〜$20.1** で動いていましたが、**08-27 の $10.9 から 08-31 の $29.2** へ、
4日で3倍近くになっています。**レビューを別のセッションへ出したのがこの間**（08-29 21:38）です
（第6期の「往復1周の値段」）。同じ4日で**変更1千行あたりも $45 から $169** へ上がっているので、
**PRが大きくなったからではありません。**

**その後も、レビューを出す前の水準へは戻っていません。** 司令塔を廃した後（第7期）は
**$35 から $20 へ**下がってきていますが、8月半ばの $10 前後には届いていません。
**1本 $167 の司令塔が丸ごと消えてもここまで**で、消えたぶんは下の内訳で別のところへ移っています。

**枠が尽きて止まった 09-01〜09-02 が、1日あたりの段にそのまま出ています**（$48 → $8）。

**ただし1本あたりは、値段と粒度が混ざった数です。** PRの粒はこの期間で動いている（上の
「PRの数も、単独では読めません」）ので、**同じ額でも1本が小さければ1本あたりは下がります。**
動かした行で割ると別の形が出ます——**変更1千行あたりは、08-31 の $169 から $80 前後へ
下がっています。** 表の段で見ても、1本あたりが $20.4 から $23.3 へ上がったのは**1本が 203行から
306行へ大きくなったから**で、**動かした行で見た値段（$100.2 → $76.1）はむしろ下がっています。**

**Copilot は桁が2つ違います。** 全期間で **$141** に対し、Claude は **$17,404**（2026-09-07 時点）
——**1%を切ります**（下の「Copilot に使ったクレジット」）。

### 試験は、実装が始まった週から実装とほぼ同じ量ある

**表で試験が0なのは 07-13 だけ**です。この日に入っているのはUnityの骨組みと文書で、`Tests/` と
CIは**実装が本格的に増える前の 07-16 に同時に入りました**。**その次の区切り（07-18）で
4,337行 対 4,300行**、**そこから追い越すまでは**どの時点でも試験は実装の3分の2を超えています
（追い越した後は下の節）。**入った後は跳ねた日がありません**——TS移行でも、[PR #166](https://github.com/gooyyu1/UnmappedIsland/pull/166) の本文が
...
### 8月末に試験が実装を追い越したのは、相手が実装の外へ出たことと、コードを試す分が厚くなったこと

**08-26 と 08-30 のあいだで、試験の行数が実装を追い越します。** 実装も止まってはいません——1日
あたり 948行（〜08-13）・1,159行（〜08-23）・1,187行（〜08-30）と、少しずつ速くなって
います。**同じ区間で試験は 600行/日 → 1,046行/日 → 2,834行/日。** 伸びの差はここで開きました。

**この節で「〜08-30」のように呼ぶ区間は、下の内訳表の列の刻み**です——上の表の 09-04 の行は
挟みません（それ以外は上の表と同じ刻みです）。

試験を**何を相手にしているか**で割ると、こう伸びています。**この内訳は手で測ったもの**で、
`npm run stats:history` からは出ません（測ったのは 2026-09-07。上の表の最後の行と同じ地点）。

**割り方は置き場です**——同梱の中身を試す置き場はリポジトリ自身が名前を持っており
（[`tests/architecture/testKinds.test.ts`](../tests/architecture/testKinds.test.ts) の
`BUNDLED_CONTENT`）、盤面の道具は `tests/scripts/`、文書は `tests/docs/`、残り全部がコードです。
**過去の列は、その時点の置き場の名前で割ってあります**——置き場は改名されるので、**今の
`BUNDLED_CONTENT` を過去のリビジョンへ当てても同じ数にはなりません。**

| 試験が相手にしているもの | 08-13 | 08-23 | 08-26 | 08-30 | 09-07 |
| --- | --: | --: | --: | --: | --: |
| コード（`src/**.ts`） | 13,930 | 19,612 | 21,765 | 27,168 | 30,753 |
| 同梱の中身（`src/assets/` のYAML・絵・対応表） | 4,403 | 9,022 | 11,066 | 17,998 | 20,753 |
| 盤面の道具（`scripts/`・`.claude/`） | 0 | 0 | 0 | 2,453 | 6,063 |
| 文書（`docs/`） | 250 | 404 | 791 | 1,260 | 1,618 |

**各列の合計は、上の表の `試験` と一致します。**

**追い越しの区間（〜08-30）で増えた試験 15,257行の行き先**を、それまでの厚み（実装1行に
...
理由は測れていません。** 同じ時期に起きたのは、**実装を足したPR1本あたりの実装が 216行（〜08-23）
→ 96行（〜08-26）→ 81行（〜08-30）→ 74行（〜09-07）** へ小さくなったことと、08-28 に
[`CLAUDE.md`](../CLAUDE.md) へ「再現手順を最初にテストへ落とす」が入ったことです（**この数だけは
`実装` の列と同じ範囲への追加行**を、それを足したPRの本数で割ったもの。刻みと測った地点は上と同じ）。
...
（[`scripts/usage/README.md`](../scripts/usage/README.md)「数字の限界」）。**デーモンを起こす係はこの表にも、上の時系列にも出ません**
——Routine で立ったセッションはセッションの一覧に載らず、集計はその一覧から作っているためです。
**抜けているのは 09-07 01:56 から 09-11 までの毎時1本ぶん**（係をこのPCのタスクへ移して、
セッションが立たなくなるまで）なので、額としては小さく収まります。
...
下の表はその写しで、`$` は**割引前の額**＝使った量の値段です）。**クレジットが動いた日だけを並べます**
——動かなかった日のほうが、圧倒的に多いです。
...
**2回目が、PRの空白と一致します。** `copilot/` からのPRが途切れた区間を並べます。
...
`agent-ops/parallel-work.md` がやっていることと、狙いは同じです。
...
**08-22、`src` 配下の宣言を全部棚卸ししました**（`review/2026-08-22-placement/`）。対象は **229ファイル / 約37,200行 / 宣言 4,178件**。1件ずつ「そこに居るのが適切か」を5段階で判定しています。翌 08-23 には**名前だけを見る調査**（`review/2026-08-23-naming/`）も独立の観点として立ちました。

**やり方**: 1つのセッションが複数のサブエージェントへ領域を割り、結果を1箇所へ集める。**読むだけの作業なので衝突しません。** 第3期の移植と同じ形を、調べる側で使っています。

**負債の整理として始めたものが、副産物を残しました。**
...
| 入れたもの | 何のために |
| --- | --- |
| `agent-ops/policies.md` ＋ 読み込むフック | **同じ問いを二度訊かない。** セッションは毎回まっさらなので、判断を置いておく場所が要る |
| `agent-ops/parallel-work.md` | 担当の割り方・PRの型・段の分け方 |
...
1か月（08-13 まで）で51件しかありません。**タスクを配る器にしてからの1週間（08-24〜08-30）で、
400件以上増えています**（上の表）。
...
| **08-29** | `needs-user-review.sh`・`dispatch-review.sh` ＋ `agent-ops/prompts/review-prompt.md` | **関門とレビューが機械で引けるようになった。** 宣言文法や `【確定】` の印に触るPRは自動で止まり、レビューは専用セッションへ出て結果がPRのコメントで返る——**司令塔が差分を読まずに振り分けられる** |
...
| **08-30** | PRを出す前の一次レビュー（[#1358](https://github.com/gooyyu1/UnmappedIsland/pull/1358)）と、観点の唯一の一覧 `agent-ops/review-criteria.md` | **外のレビュアーと同じ観点を、外へ出す前にサブエージェントへ1回通す。** 往復の回数を手前で削る |
...
`agent-ops/review-criteria.md` は1つで、**出す側とレビュアーが同じものを
...
**第7期の列が期の始まり（09-04）ではなく 09-05 から始まるのは、司令塔が消えたのが翌日だからです**
——09-04 に入ったのは作り直す設計で、盤面はまだ司令塔が回していました。
...
**設計が既に一揃いある状態から始まります**——`agent-ops/board-design.md` は立った当日のうちに、
「状態の持ち方」「機械ループの実行基盤」「使用量による自動の手綱」まで積まれています。

**上の表で 09-04 で終わる区間（08-31〜09-04）だけ「1本あたり」が上に出ているのは、この空白です。** 額そのものは
**$727** と前後の区間よりずっと小さいのですが、**分母のPRが25本しかない**ので1本あたりでは高く出ます。
...
**09-04 23:28、盤面を作り直す設計の置き場ができます**（`agent-ops/board-design.md`）。向かう先は
...
**それでも1本あたりの値段は下がっていません。** 09-05〜09-07 は区間で **$23.3**
（末日は測った時点でまだ走っているセッションがあるので、下側の値）。司令塔が居た 08-27〜08-30 の
**$20.4** より高い水準です。**移った先は
`bridge`**——このPCで人間が開いた
...
| 落ちていないか見る | ⏰ このPCのタスク（ログオンと毎時。**デーモンの外**。モデルは動かない） |
...
出たことと、コードを試す分が厚くなったこと」）。

**並列で初めて要ったのは、[`ParallelAgents.md`](./ParallelAgents.md) 1節が挙げているもの**です。
試験がここに関わるとすれば量ではなく、**壊れたときの届く先**のほうです。1人なら次の一手で気づく
ものが、同時に走っていると、気づく前に他の作業がその上へ積まれます。だから並列で足したのは試験そのものではなく、**人を通さずに必ず走る
こと**（CIの許可リストを外し、どのファイルを触っても検査が全部走る）と、**揃わない所を検査に
変えること**（同 10節）です。

## 変えなかったもの

**作り方は変わりましたが、変えていない軸があります。**
...
3〜4割、それ以降は3〜7割**で、**リポジトリを作った日を除けば0になりません。** 設計を全部
終えてから実装へ移る形なら、文書だけの期と実装だけの期に分かれるので、この列は両端でほぼ0に
なり、混ざる区間が生まれません。

**0%なのは 07-13 の1日だけ**です。この日は12本すべてが Copilot で、Unityの骨組み・コンセプト
文書・画面のモックがそれぞれ別のPRになっています。**まだ実装と対にできる仕様が無い日**で、
段階が分かれていたのとは違います。

**割合が上がるのは 08-01 で終わる区間から**で、Unity 期とWeb期の境目に当たります。
**リポジトリを作った日を除けば、どの区間でも3割は混ざっている**ので、段階が分かれていた時期が
あるわけではありません。
