...
**上の表の範囲に当たらないものは、どの列にも数えません。** `.claude/` から `文書` へ数えるのは
**直下の `*.md` だけ**で（実行されるものは階層を問わず `道具` へ入ります）、**下の階層の `*.md`**
——判断の履歴・skill の本文・分析の書き出し——はどの列にも入りません。文書とは別のもので、
...
`.claude/` の取り決めは入りません。実装と対で書かれるものではないので、**対になっているかを見る
この割合には数えません。**
