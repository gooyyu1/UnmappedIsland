...
あたり 948行（08-01〜08-13）・1,156行（〜08-23）・1,186行（〜08-30）と、少しずつ速くなって
...
| 盤面の道具（`scripts/**`） | 0 | 0 | 0 | 2,453 | 5,817 |
| 文書（`docs/**`） | 250 | 404 | 791 | 1,260 | 1,615 |
...
- **コードを試す分のうち、従来の厚みで説明できる分が 2,737行（18%）。**
- **コードを試す分が厚くなった分が 2,666行（17%）。**

**3分の2は、実装の列が数えない相手のものです。** 実装の列は `src/**/*.ts` しか数えないので、
世界の中身をYAMLで書けば `定義` の列へ移り、盤面を回す道具（`scripts/**`）は**どの列にも
入りません。** 一方で試験の列は `tests/**` を全部数えるので、**相手がどこにあっても試験だけは
見えます。** この区間で定義は 1.6倍・道具は 4.4倍に伸びており（実装は 1.1倍）、増えた定義1行には
試験が1.7行、道具1行には試験が0.9行付いています。

**残る3分の1は、コードを試す分が厚くなったぶんです。** 実装1行あたりの試験は、区間ごとに
**0.49行（〜08-23）→ 0.79行（〜08-26）→ 0.97行（〜08-30）→ 1.49行（〜09-07）**。**厚くなった
