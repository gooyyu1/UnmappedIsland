...
timeline
    ...
    section 9月
        ...
        09-07 デーモンを起こす係を外に置く : 実装 4.9万行 / 試験 5.8万行
```
...
| 08-30 | 46,751 | 48,879 | 18,541 | 11,498 | 499 | 950 | 6.2ファイル / 203行 | $4,880（1本 $19.4） | 47% |
| 09-04 | 47,553 | 50,186 | 18,760 | 11,619 | 508 | 975 | 7.5ファイル / 283行 | $1,165（1本 $46.6） | 32% |
| 09-07 | 49,076 | 58,374 | 20,015 | 12,514 | 637 | 1,090 | 10.0ファイル / 317行 | $3,037（1本 $26.4） | 54% |
...
**第7期の区間（09-05〜09-07）では 10.0ファイル・317行へ戻っています。** この区間の115本を、
**盤面を作り直すもの**（`.claude/**`・`scripts/agent/**`・`scripts/usage/**`・`.github/**` を
触る45本）と**それ以外**（70本）で割ると、**10.2ファイル・441行 対 9.8ファイル・238行**。
...
### 8月末に試験が実装を追い越したのは、相手が変わったから

**08-26 と 08-30 のあいだで、試験の行数が実装を追い越します。** このとき実装の伸びは前の区間と
変わっていません。**変わったのは、試験が何を相手にしているかです。**

試験の置き場は、**何を試すか**で分かれています
（[`tests/architecture/testKinds.test.ts`](../tests/architecture/testKinds.test.ts)）。相手ごとに
割ると、試験の行数はこう伸びました。**この内訳だけは手で測ったもの**で、`npm run stats:history`
からは出ません（測ったのは 2026-09-07）。

| 試験が相手にしているもの | 08-13 | 08-23 | 08-26 | 08-30 | 09-07 |
| --- | --: | --: | --: | --: | --: |
| コード（`src/**/*.ts`） | 13,930 | 19,612 | 21,765 | 27,168 | 30,753 |
| 同梱の中身（YAML・絵・対応表） | 4,403 | 9,022 | 11,066 | 17,998 | 20,570 |
| 盤面の道具と文書（`scripts/**`・`docs/**`） | 250 | 404 | 791 | 3,713 | 7,432 |

**09-07 の列だけは、合計が上の表の `試験` より少し大きく出ます**——数え直したぶんです（上の
「最後の行は、その日の最終コミットに追いつきません」）。

**相手1行あたりの試験の厚みは、どれもほとんど動いていません。** コードは 0.5〜0.6行、同梱の中身は
1.4〜1.6行、盤面の道具は 0.7〜0.8行です。**試験の書き方は変わっていない**、ということです。

**変わったのは、何が書かれているか。** 08-23 から 08-30 の区間で増えた行は、実装が **+8,304** に
対して**定義が +4,842**・**`scripts/` の道具が +2,757**。倍率にすると実装 1.2倍に対し、定義 1.7倍・
道具 4.4倍です。**定義1行には試験が1.5行、道具1行には試験が1行付く**ので、定義と道具のほうが速く
伸びれば、試験の合計は実装より速く伸びます。

**そして実装の列は `src/**/*.ts` しか数えません。** 世界の中身をYAMLで書けば `定義` の列へ移り、
盤面を回す道具（`scripts/**`）は**どの列にも入りません。** 一方で試験の列は `tests/**` を全部
数えるので、**相手がどこにあっても、試験だけは見えます。** 8月末に開いたこの差は、試験が厚く
なったのではなく、**書く場所が実装の列の外へ広がったのに、列がそのままだった**ということです。
...
試験に至っては並列化の5週間前（07-16）から回っており、**コードを試す分に、並列化が足したものは
ありません**（並列化が足したのは**盤面の道具を試す分**で、道具そのものが並列化の産物です。上の
「8月末に試験が実装を追い越したのは、相手が変わったから」）。
