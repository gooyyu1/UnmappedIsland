...
並列化した直後は **5〜6ファイル・200〜230行**まで落ちました（その後の戻りは下の「表の最後の
区間」）。**1本がぐっと小さくなっているので、本数が増えたぶんがそのまま量ではありません。**
1つのタスクを1セッションが持つ形にしたことで、PRの粒が「1セッションで片付く大きさ」に揃ったのが
この列です。
...
**表の最後の区間（09-04〜09-07）では 9.5ファイル・306行へ戻っています。** この区間の139本を、
...
**この図だけは、段の区切りを使っていません。** 段の区切りは「作り方を変えた日」で決まっていて
幅が揃わないので、区間の量を段ごとに出すと**区切りを1日動かすだけで値が半分になり**、総額の上下は
使ったペースではなく区間の長さを描くことになります。**幅を固定した移動窓で、1日あたり・1本あたり・
1千行あたりを日ごとに出しています**（`scripts/historyStats.mjs` の `WINDOW_DAYS`）。
**表の段ごとの値とは、割った幅が違うぶん食い違います。** 右端の点は、まだ走っているセッションを
含まないぶん下側に出ます。

**段ごとに割る先が違います**（何で割ったかは段の見出しに出ています）。1本あたりは8月の半ば
（08-03〜08-19）を **$8〜$20** で動いていましたが、**08-27 の $10.9 から 08-31 の $29.2** へ、
...
4,337行 対 4,300行**、**そこから8月末までは**どの時点でも試験は実装の7割〜同量です（追い越した
後は下の節）。**入った後は跳ねた日がありません**——TS移行でも、[PR #166](https://github.com/gooyyu1/UnmappedIsland/pull/166) の本文が
...
**上の表で 08-30〜09-04 の区間だけ「1本あたり」が上に出ているのは、この空白です。** 額そのものは
...
**割合が上がるのは 08-01 の区間から**で、Unity 期（39%・32%）とWeb期（44〜69%）の境目に当たります。
