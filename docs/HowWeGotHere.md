...
| 道具 | — | `scripts/**`（説明の `*.md` を除く）＋ `.claude/**.sh`・`.claude/**.mjs` |

**`道具` は、リポジトリの運用を回すもの**——盤面のデーモン、使用量を集める道具、CIから呼ぶ検査、
セッションのフック。**本番のプログラムではないので `実装` とは別の列**ですが、**書かれた量としては
数えます。** `.claude/` の `*.md` は取り決めの文書なので入りません。
...
| 08-01 | 15,523 | 11,381 | 5,532 | 1,851 | 198 | 48 | 210 | 13.9ファイル / 859行 | $1,270（1本 $14.8） | 62% |
| 08-13 | 26,906 | 18,583 | 9,113 | 3,151 | 294 | 51 | 369 | 11.1ファイル / 326行 | $2,108（1本 $13.3） | 69% |
| 08-23 | 38,494 | 29,038 | 14,443 | 6,656 | 1,159 | 58 | 589 | 19.0ファイル / 529行 | $3,351（1本 $15.2） | 60% |
| 08-26 | 41,213 | 33,622 | 16,148 | 7,349 | 1,587 | 152 | 699 | 5.0ファイル / 229行 | $1,203（1本 $10.9） | 44% |
| 08-30 | 46,805 | 48,879 | 19,934 | 11,498 | 4,222 | 499 | 950 | 6.2ファイル / 203行 | $4,880（1本 $19.4） | 47% |
| 09-04 | 47,607 | 50,186 | 20,361 | 11,619 | 4,954 | 508 | 975 | 7.5ファイル / 283行 | $1,165（1本 $46.6） | 32% |
| 09-07 | 49,216 | 58,755 | 21,908 | 12,537 | 7,708 | 642 | 1,098 | 9.9ファイル / 316行 | $3,037（1本 $24.7） | 54% |
...
**第7期の区間（09-04〜09-07）では 9.9ファイル・316行へ戻っています。** この区間の123本を、
**盤面を作り直すもの**（`.claude/**`・`scripts/**`・`.github/**` を触る48本）と**それ以外**（75本）で
割ると、**10.2ファイル・441行 対 9.7ファイル・236行**。
...
| --- | --: | --: | --: | --: | --: |
| コード（`実装` の列） | 13,930 | 19,612 | 21,765 | 27,168 | 30,753 |
| 同梱の中身（`定義` の列と、絵・対応表） | 4,403 | 9,022 | 11,066 | 17,998 | 20,570 |
| 盤面の道具（`道具` の列） | 0 | 0 | 0 | 2,453 | 5,817 |
| 文書（`文書` の列） | 250 | 404 | 791 | 1,260 | 1,615 |
...
この区間で定義は 1.6倍・道具は 2.7倍に伸びており（実装は 1.1倍）、増えた定義1行には試験が1.7行、
道具1行には試験が0.9行付いています。**`実装` だけを見ていると、この伸びはどこにも出てきません。**
