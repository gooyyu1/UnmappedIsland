...
分母にはこちらを使います。`コスト ÷ 変更行` が括弧の `千行` で、`変更行 ÷ その区間のPR本数`
（`PR` の列の、1つ上の行からの増分）が `1PRあたり` の行数です。
...
**上が区間の合計（Claude と Copilot）、下が `main` へ入ったPR1本あたりと、動かした1千行あたり**です。8月に入ってからの1本あたりは
...
**ただし1本あたりは、値段と粒度が混ざった数です。** PRの粒はこの期間で4分の1になっている（上の
