...
**このリポジトリは 2026-07-13 に始まり、7週間で 2,500 コミットになりました。** その間に**作り方そのものを5回変えています**。
...
| 08-30 | 45,837 | 44,976 | 17,906 | 10,893 | 330 | 851 | 6.6ファイル / 240行 | 56% |
...
**Copilot は一度トークンが枯渇し、上位プランへ変えています**（当事者の証言）。**リセットは月単位**なので、
**翌月まで登場しない途切れがあれば、そこが枯渇に当たります。**

`copilot/` からのPRが途切れているのは2箇所です。
...
**枯渇に当たるのは後者です。** 前者は3日で戻っているので、月のリセットを待った空白ではありません
——**上位プランへ変えたのがこちらだとすれば、辻褄が合います**（変えれば即座に使えるので、翌月を
待つ必要がない）。

**ただし、これで確定とは言えません。** 07-21 の時点で Claude 側が十分に回っており、**枯渇したから
ではなく、主戦力としてそちらへ移しただけ**という説明も同じ空白を作ります。**履歴だけでは、この2つを
見分けられません。**

**戻ってからは 08-04 のYAML修正1本と 08-07 のCI修正2本だけで、実装の割り当て先としてはここで
終わります。** 08-28 に Copilot CLI として戻るまで（第6期）、書いていたのは Claude だけです。

> **注**: この期の判断（モックの壊れ方に手を焼いたこと、枯渇して上位プランへ変えたこと）は**当事者の証言**で、リポジトリから読めるのは**PRの本数・題・時刻・空白の長さ・クローズされた事実**までです。上の2つの表はその範囲で裏付けが取れたものだけを並べています。
