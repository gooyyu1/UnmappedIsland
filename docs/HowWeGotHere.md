...
| 道具 | — | `scripts/`・`.claude/`・`tools/` の `*.mjs`・`*.mts`・`*.sh`・`*.py` |

**`道具` は、リポジトリの運用を回すもの**——盤面のデーモン、使用量を集める道具、CIから呼ぶ検査、
セッションのフック、絵を出す道具。**本番のプログラムではないので `実装` とは別の列**ですが、
**書かれた量としては数えます。** 入るのは**実行されるものとその型だけ**で、傍らの `*.md`
（`.claude/` の取り決め）や `*.json`（絵のレシピ）は入りません。
...
| 08-01 | 15,523 | 11,381 | 5,532 | 1,851 | 1,556 | 48 | 210 | 13.9ファイル / 859行 | $1,270（1本 $14.8） | 62% |
| 08-13 | 26,906 | 18,583 | 9,113 | 3,151 | 3,085 | 51 | 369 | 11.1ファイル / 326行 | $2,108（1本 $13.3） | 69% |
| 08-23 | 38,494 | 29,038 | 14,443 | 6,656 | 5,162 | 58 | 589 | 19.0ファイル / 529行 | $3,351（1本 $15.2） | 60% |
| 08-26 | 41,213 | 33,622 | 16,148 | 7,349 | 5,590 | 152 | 699 | 5.0ファイル / 229行 | $1,203（1本 $10.9） | 44% |
| 08-30 | 46,805 | 48,879 | 19,934 | 11,498 | 8,231 | 499 | 950 | 6.2ファイル / 203行 | $4,880（1本 $19.4） | 47% |
| 09-04 | 47,607 | 50,186 | 20,361 | 11,619 | 8,963 | 508 | 975 | 7.5ファイル / 283行 | $1,165（1本 $46.6） | 32% |
| 09-07 | 49,244 | 58,907 | 21,943 | 12,540 | 11,758 | 642 | 1,108 | 9.4ファイル / 298行 | $3,037（1本 $22.8） | 53% |
...
**第7期の区間（09-04〜09-07）では 9.4ファイル・298行へ戻っています。** この区間の133本を、
...
| 同梱の中身（`定義` の列と、絵・対応表） | 4,403 | 9,022 | 11,066 | 17,998 | 20,649 |
| 盤面の道具（`道具` の列のうち `scripts/`・`.claude/`） | 0 | 0 | 0 | 2,453 | 5,887 |
| 文書（`文書` の列） | 250 | 404 | 791 | 1,260 | 1,618 |
...
この区間で定義は 1.6倍・道具は 1.5倍に伸びており（実装は 1.1倍）、増えた定義1行には試験が1.7行、
