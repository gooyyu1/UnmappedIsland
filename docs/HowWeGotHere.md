...
| 09-07 | 49,241 | 59,162 | 26,453 | 20,405 | 11,875 | 645 | 1,113 | 9.5ファイル / 303行 | 41,790 | $3,238（1本 $23.5・千行 $77.5） | 55% |
...
張り付いたまま、区間の長さ（3日〜12日）のぶんだけ点ごとに違うずれ方をします。
...
**表の最後の区間（09-04〜09-07）では 9.5ファイル・300行へ戻っています。** この区間の136本を、
**盤面を作り直すもの**（`.claude/**`・`scripts/**`・`.github/**` を触る51本）と**それ以外**（85本）で
割ると、**10.0ファイル・428行 対 9.2ファイル・224行**。
...
幅が3日〜12日と揃わないので、区間の量を段ごとに出すと**区切りを1日動かすだけで値が半分になり**、
総額の上下は使ったペースではなく区間の長さを描くことになります。**幅を固定した移動窓で、
1日あたり・1本あたり・1千行あたりを日ごとに出しています**（`scripts/historyStats.mjs` の
`WINDOW_DAYS`）。**表の段ごとの値とは、割った幅が違うぶん食い違います。**

**段ごとに割る先が違います**（何で割ったかは段の見出しに出ています）。8月に入ってからの1本あたりは
**$10〜$20** の間を動いていましたが、**08-29 から 08-31 にかけて $20 から $29** へ上がります。
**レビューを別のセッションへ出したのがこの時期**（08-29 21:38）です（第6期の「往復1周の値段」）。

**その後も下がっていません。** 司令塔を廃した後（第7期）も **$20〜$35** で、
**1本 $167 の司令塔が丸ごと消えても下がっていません**——消えたぶんは、下の内訳で別のところへ
移っています。

**枠が尽きて止まった4日間が、1日あたりの段にそのまま出ています**（09-01〜09-02 で $48 → $8）。
