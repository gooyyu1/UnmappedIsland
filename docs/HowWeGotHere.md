...
| --- | --: | --: | --: | --: | --: | --: | --: | --: | --: |
| 07-18 | 4,337 | 4,300 | 2,043 | 63 | 18 | 52 | 9.0ファイル / 604行 | $34（1本 $0.7） | 33% |
| 07-25 | 9,454 | 8,596 | 4,438 | 1,527 | 48 | 128 | 13.7ファイル / 1,022行 | $356（1本 $4.7） | 33% |
| 08-01 | 15,506 | 11,381 | 5,498 | 1,851 | 48 | 213 | 14.4ファイル / 857行 | $1,168（1本 $13.7） | 60% |
| 08-13 | 26,885 | 18,583 | 9,000 | 3,151 | 51 | 388 | 10.3ファイル / 305行 | $1,925（1本 $11.0） | 70% |
| 08-23 | 38,447 | 29,038 | 14,307 | 6,656 | 58 | 606 | 18.9ファイル / 530行 | $3,351（1本 $15.4） | 56% |
| 08-26 | 41,166 | 33,622 | 15,456 | 7,349 | 152 | 705 | 5.2ファイル / 244行 | $1,203（1本 $12.2） | 45% |
| 08-30 | 46,751 | 48,879 | 18,541 | 11,498 | 499 | 954 | 6.2ファイル / 211行 | $4,880（1本 $19.6） | 48% |
| 09-04 | 47,553 | 50,186 | 18,760 | 11,619 | 508 | 980 | 9.1ファイル / 300行 | $1,165（1本 $44.8） | 46% |
| 09-07 | 49,076 | 58,374 | 20,015 | 12,514 | 635 | 1,090 | 9.7ファイル / 302行 | $3,037（1本 $27.6） | 52% |

**PR は `main` へ入った累計**（マージコミットと squash の両方を数え、`main` へ直接 push した分は
入りません）。**1PRあたりと「同じPR」の割合は、1つ上の行からの区間の平均**です。

**08-30 から 09-04 までの区間は、ほとんど止まっています**（下の第7期）。PRが26本しか増えていない
のに額が積まれているので、**この行の「1本あたり」だけは他の行と比べられません**——止まっている
...
**PRの数も、単独では読めません。** 07-25 の週は1本あたり **13.7ファイル・1,022行**でしたが、
並列化してからは **5〜6ファイル・200〜250行**です。**1本が3分の1以下になっているので、本数が
増えたぶんがそのまま量ではありません。** 1つのタスクを1セッションが持つ形にしたことで、PRの
粒が「1セッションで片付く大きさ」に揃ったのがこの列です。

![PR1本あたりの大きさ](./HowWeGotHere_pr_size.svg)

**08-23 の山**（18.9ファイル・530行）は、負債の棚卸しで見つかった置き場の直しが1本に固まった
ぶんです。**その次の区間で5.2ファイル・244行まで落ちる**のが、並列化でPRの粒が揃ったところです。

**9月の区間では 9〜10ファイル・300行へ戻っています。** 盤面を作り直すPR（`.claude/**`・
`scripts/agent/**`・`.github/**` を触るもの）は行数こそ大きいのですが（441行 対 235行）、
**触るファイルの数はそれ以外と変わりません**（どちらも約10ファイル。09-04〜09-07 の実測）。
...
**$11〜$15** の間を動いていましたが、**08-27〜08-30 の区間で $19.6** へ跳ねています。
...
**Copilot は桁が2つ違います。** 全期間で **$141** に対し、Claude は **$17,119**（2026-09-07 時点）
...
**比べる相手として、当時の測定をそのまま残します。** 生データはこのPCに残っておらず、
**取り直したのは 08-30 以降だけ**なので、この表は測った日のまま据え置きです。
...
**下は 2026-09-07 に測り直した値です**（当時は回り始めて数時間の値しか無く、往復の途中のPRが
混ざって低く出る側に偏っていました。第7期の列は、司令塔が居なくなった後の同じ測り方）。

| | 一次レビューの前 | 後 | 第7期（09-05〜09-07） |
| --- | --: | --: | --: |
| レビューが立ったPR | 98本 | 45本 | 103本 |
| PR1本あたりの投入 | 平均 2.22回 | 平均 1.31回 | 平均 1.46回 |
| 2回以上になったPR | 47% | 24% | 32% |
| 最多 | 11回 | 3回 | 5回 |

**往復は半分近くまで減り、時間を置いても戻っていません。** 当時のまま置いた表より高く出ている
のは、**測った時点でまだ立っていなかった2周目が、その後に立った**ぶんです。
