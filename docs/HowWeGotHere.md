...
**絵の枚数**: 07-31 に14枚 → 08-15 に131枚 → 08-26 に152枚（`src/assets/**` の画像。`docs/ui` の画面写真は数えていません）。

### なぜ「生データを別リポジトリへ」が効いたか

**生データは1枚1.2MB前後で、189枚・220MBあります**（[UnmappedIsland-art-raw](https://github.com/gooyyu1/UnmappedIsland-art-raw)・08-26）。本体へ入れると clone が重くなり、**セッションを
立てるたびにその重さを払う**ことになります。本体に入っているのは後処理を通した152枚・32MBです。
