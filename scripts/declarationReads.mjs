...
// **書かれてはいるが誰も読まないフィールド**を「使われている」と数える。宣言と、返り値の
// オブジェクトリテラルのキーだけで自分のファイルに現れる形が、それでは読み手が居るように見える。
// ここが除くのは**宣言の名前と、書き込みの位置**。
//
 ...
 *
 * 読みでないのは、**宣言の名前**（`readonly discards: readonly C[]`）・**オブジェクトリテラルの
 * キー**（`{ discards: ... }`・`{ discards }`）・**代入の左辺のプロパティ**（`this.discards = ...`）。
 * 分割代入（`const { discards } = plan`）は読み。
 */
