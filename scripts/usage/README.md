...
python sweep_events.py 2026-08-25     # メッセージ単位の時刻を落とす
                                      # （セッション1本につき数十回のHTTP）
...
- **過去は測り直せない。** ローカルの transcript は古いものから消えるので、**一度測った時間を
  ...
  落とし直した範囲の先頭に合わせる。
- **境目は、前に渡したものと同じにする。** 持ち越す値は**その時の配り方で書かれている**ので、
  境目を跨ぐセッションを別の配り方で数え直すと、持ち越した取り分と噛み合わずに**額が二重に乗るか
  消える**。`timeline.py` は境目より前に始まったセッションを均等割りのまま扱ってこれを避けるが、
  **境目を後ろへ動かすと、その間にイベント基準で配ったセッションが跨ぐ側へ回る。** 動かすなら
  引数なしで全部を測り直す（ローカル分が痩せるのと引き換え）。
...
- **時間ごとの分解は、イベントを落としてある範囲だけ正確。** CCR のメタデータは
  `created_at`〜`updated_at` しか持たないので、`sweep_events.py` で落としたイベントが在れば
  その時刻ごとのトークン量で額を配り、**無いセッションは区間へ均等に割る**。均等割りのほうは、
  枠が尽きるなどで間が空き、同じセッションが再開して `updated_at` が数日後まで伸びると、
  **1件も動いていない日にも額が乗る。** ローカル分はメッセージ単位の時刻なので正確。
