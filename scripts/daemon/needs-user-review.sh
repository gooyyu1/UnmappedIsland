...
#
# **`MARK` と `UNANSWERED` は、受け取った人がすることが違う。** 前者は「あなたが決めたのか」を
# 訊いているので、そうなら画面からマージすればよい。後者は**指された issue にまだ答えが無い**ので、
# 先にあちらへ答える（答えると `判断待ち` が外れ、次の周でこの関門を通る）。
#
...
#
# **確かめられるのはここまで。** 答えの出た**別の**issue を指す嘘は、節の中身と issue の中身を
# 突き合わせないと分からないので、ここでは見ない——差分を読むレビュアーの側
# （[`review-criteria.md`](../../agent-ops/review-criteria.md)「人の判断へ回す」の、確定節の射程）。
#
