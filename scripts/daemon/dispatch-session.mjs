...
// 投入する前の関門はそちら。**種類で割るのは、題と、起こすリビジョンと、送る本文を埋めるかどうか。**
// 埋めるのはレビューだけ（何回目の判定かも、前の周が読んだ版も、ここでしか出せない）——だから
// **`--prompt` を読むのは種類ごとの組み立てより後**で、この順は崩せない。
//
// ## なぜ1つの node に入れるのか
//
...
/**
 * 投入するものごとに違う分。**題は頭の語で種類が分かる形**（`agent-ops/board-design.md` 2.9節）で、
 * `revision` は起こすリビジョン。
 */
const KINDS = {
  task: (given) => ({
