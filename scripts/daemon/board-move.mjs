...
/**
 * PRを**人の手番へ移す**印（2.13.2）。**答えを出せるのは人だけ**なので、付いている間はこのPRの
 * 版が動かない——盤面はここを見て、`main` の動きから生まれた直しを頼まない（2.13.8）。
 *
 * **`却下` は入らない。** あれは人が答えた後に付く印で、手番はもう書いた本人へ戻っている。
 * **`通してよい` も入らない**——止める印ではなく、レビューが書いた事実（1.3）。
 */
export const HUMAN_TURN = ['判断待ち', '収束せず'];

/**
 * **PRの版が動かないまま `main` が動いて生まれる**差し戻しの理由（下の `cause`）。**この2つだけが、
 * 人の手番で止まっている間は頼む意味を持たない**——直しても答えは近づかず、`main` が次に動けば
 * 同じところへ戻る（2.13.8）。
 *
 * 残り（`reject`・`look`・`RETURNED`）は**そのPRの差分そのものへ向いている**ので、人の返事とは
 * 関わりなく直せる。
 */
const MAIN_MOVED = ['mend:conflict', 'mend:red'];
...
const CYCLES = [
  ...
  {
    name: 'refs',
    // **1日1回。** 入力は `main` へ入った差分なので、溜めてから捌く利が無い——窓が広がるほど、
    // 1周で読む量が増えるだけ。
    hours: 24,
...
export function moves(input) {
   ...
   *
   * **全員を見て、先頭を返す。** `menders` が引くのは**名乗りの1つのID**なので、返りが2本以上に
   * なるのは**一覧に同じIDが重なった周だけ**——どれを返しても同じ相手で、選ぶ線は無い。全員を見る
   * ほうは線が在る：**1本でも動いていれば、まだ返事を待っている。**
   */
