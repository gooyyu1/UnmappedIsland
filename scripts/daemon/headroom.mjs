 ...
 *
 * **`resume` は測れないので、いちばん重い実測で揃える**（2.5.2 の「起こす経路も同じ関門を通る」）。
 * 起こされたセッションが食ったぶんは**そのセッション自身の種類**として積まれるので、`resume` の行は
 * 永久に0件——分けて測る値が記録に無い。**多いほうへ倒す**のは 2.5.3「既定は大きめに置く」と同じ理由
 * で、外したときの損が釣り合わないから（緩すぎると上限に当たって何も出ないまま返され、きつすぎても
 * 起こすのが遅れるだけで、起こす手は枠が明けた周に出し直される）。
 */
