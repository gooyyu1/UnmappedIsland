...
#
# **相手を出せるのは、squash マージの件名が `(#番号)` で終わるから**（`merge-pr.sh`）。
# 分岐点から `main` の先頭までのうち、衝突したファイルを触った件名だけを引く。
#
# 使うのは [`board-round.mjs`](board-round.mjs) で、**手を打つためではなく控えるため**
# （`agent-ops/board-design.md` 3.1節）。盤面は同じファイルを書く issue を並べて投入するので、
# ぶつかった実績を残しておかないと、`area:` の錠を足すべき資源が後から分からない。

set -euo pipefail

# **調べられなかったことは、どこで転んでも同じ答え。** `git` の返す番号をそのまま漏らすと、冒頭で
# 宣言した終了コードと食い違う。
trap 'exit 2' ERR
