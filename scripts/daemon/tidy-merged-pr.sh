...
# 指示・2026-09-05。[`board-design.md`](../../agent-ops/board-design.md) 2.10節）。畳むのは盤面が毎周
# 見て打つ。
#
# ## 本体を追随させるのは、共有している版を食い違わせないため
#
...
#
# 誰も本体では作業しないので、本体が自分から追いつくことはない。後片付けの1回でここも進めるが、
# **寄せる口はここだけではない**——[`daemon.sh`](daemon.sh) が同じ判定で周の終わりにも寄せるので、
# マージ以外で `main` が動いたぶん（人がGitHubの画面から入れたぶん・盤面を回す仕組みの直接 push・
# `DIRTY` で寄せられなかった周）も、次の周には本体へ届く（あちらの「本体を `origin/main` へ寄せるのは」）。
#
