#!/usr/bin/env bash
# デーモンを起こす係を、このPCのタスクとして登録する（`agent-ops/board-design.md` 2.19節）。
#
...
#
# **発火するのは毎時0分。** 繰り返しは `StartBoundary` から数えるので、そこを日付の頭へ置いてある
# ——**起点そのものに意味は無い**（過ぎていてよい）。登録した時刻を起点にすると、**打ち直すたびに
# 起きる分が動き**、いつ起きるかを人が読めなくなる。
#
# ## 登録は XML で渡す
#
...
<Task version="1.2" xmlns="http://schemas.microsoft.com/windows/2004/02/mit/task">
  <RegistrationInfo>
    <Author>$WHO</Author>
    <Description>盤面のデーモンを起こす。生きていれば何もしない（UnmappedIsland の agent-ops/board-design.md 2.19節）。</Description>
  </RegistrationInfo>
  <Principals>
    <Principal id="Author">
      <UserId>$WHO</UserId>
