...
# 判定は [`checked-items.sh`](checked-items.sh) が持ち、`watch-prs.sh` の `CHECKED` も同じものを
# 呼ぶ。**起こす側と読む側で判定が違うと、片方にしか出ない項目ができる**——`CHECKED` が増分の窓
# だった間に、13件が起こす側へ一度も乗らなかった（#1246）。
#
# それでもここに出すのは、**見張りが起こすのは動きがあったときだけ**で、引き継いだ直後の司令塔は
# 誰にも起こされないから。盤面は自分から現在地を訊く側の道具。
#
