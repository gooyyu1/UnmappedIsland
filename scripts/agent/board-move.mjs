 ...
 * の `synchronized`）。**盤面も同じ集合を剥がせる**——あの段は**出来事**で動くので、転んだ回は
 * 二度と来ず、前の差分への結論が今の差分の顔として残り続ける（issue #2144）。**打つ側
 * （[`board-round.mjs`](board-round.mjs)）もここから綴りを取る。**
 *
...
export function moves(input) {
  ...
  for (const pr of [...input.prs].sort((a, b) => a.number - b.number)) {
    ...
    if (!asked && verdictOn(pr, false) !== undefined) {
      ...
      if (verdictsOn(pr, pr.headRefOid, false).length > 1) {
        notes.push(
