...
/**
 * PRの頭が動いたときに落ちる結論の札（[`board-labels.yml`](../../.github/workflows/board-labels.yml)
 * の `synchronized`）。**盤面はこれを、前の差分のものと分かった時点で無いものとして読み、落として
 * ほしいと頼む**（`UNLABEL`）——あの段は**出来事**で動くので、転んだ回は二度と来ず、前の差分への
 * 結論が今の差分の顔として残り続ける（issue #2144）。
 *
 * **落とすのはワークフロー**（同 `swept`）。**盤面が自分で外すと `却下` になる**（`board-round.mjs`
 * の `UNLABEL`）ので、ここに在るのは**読むための集合**で、外す綴りはあちらが持つ。
 *
 * **`却下` は入れない。** 人がPRを止めている印を外したことの写しで、**外したのがどの差分を読んだ
 * 後かは盤面に出ない**——剥がすと人の停止を消す。残っても差し戻し（`reject`）が出るので、誰の
 * 手番でもない形にはならない。
 */
export const STALE_ON_PUSH = ['直し待ち', '通してよい', '判断待ち', '収束せず'];
...
export function moves(input) {
   ...
   *
   * **判定の名乗りと今の頭を突き合わせて読む。** 結論の札はどれも判定から付くので、**判定が在る
   * のに、そのどれも今の頭を指していないなら、札のほうが古い**——コメントは外されない側なので、
   * ラベルが嘘になっても判定は残っている（2.13.5）。
   *
   * **名乗りの無い判定は今の版のものとして数える**（`verdictOn` の `true`）。数えないと、名乗りを
   * 書き忘れた周の札が**まだ効いているのに古いと読まれる**——倒れる先は、剥がさないほう。
   *
   * **判定が1つも無いPRでは言えない。** 札の出どころは判定だけではなく（`直し待ち` は後片付けも
   * 付ける。2.10.5）、突き合わせる相手が無い。
   */
  function staleLabels(pr) {
    if (verdicts(pr.comments).length === 0 || verdictOn(pr, true) !== undefined) return [];
  ...
  for (const pr of [...input.prs].sort((a, b) => a.number - b.number)) {
    ...
    // **前の差分の札が残っているうちは、この先へ進めない**（issue #2144）。剥がす前にレビューへ
    // 出すと、**新しい判定が載った時点で古いと言えなくなる**——今の頭を名乗る判定が在るので
    // `staleLabels` が空になり、`判断待ち` が付いたままマージが止まる。**差し戻しの3つ
    // （`却下`・コンフリクト・CIの赤）は上で先に出る**——札と関わらない理由なので待たせない。
    //
    // **指紋は頭**（`headRefOid`）。剥がし終えた後も一覧が古ければ同じ札が見えるので、**一度
    // 打った頭では止めない**——剥がれていなければ次の手が出ないだけで、止めると誰の手番でもなくなる。
    if (leftover.length > 0 && taken[`unlabel:${pr.number}`] !== pr.headRefOid) {
      // **落ち着くまでは打たない。** push の直後は `board-labels.yml` が外している最中で、転んだ回と
      // 見分けが付かない——待てば向こうが外す（同じ札を2箇所から打つと、どちらが外したのか
      // ログに残らない）。
      if (pr.updatedAt < input.settledBefore) {
        unlabels.push(`UNLABEL ${pr.number} ${pr.headRefOid}`);
      } else {
        notes.push(`PR #${pr.number} は前の差分の札が残っている（外れるのを待っている）`);
      }
      continue;
    }
    ...
    if (!asked && verdictOn(pr, false) !== undefined) {
      ...
      //
      // **ここは名乗りの無い判定も数える**（`true`）。数えないと、**読ませ直したレビューが名乗りを
      // 書き忘れた周だけ歯止めが外れ**、落ち着くたびに1本立ち続ける——この訊き手の倒れる先は
      // 「出さないほう」（2.13.5・2.13.7）。
      if (verdictsOn(pr, pr.headRefOid, true).length > 1) {
        notes.push(
