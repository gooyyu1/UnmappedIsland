...
# - `直し待ち` … 書いたセッションの手元。レビューが「直しが要る」を出した状態。**緑のまま
#   放置すると毎周報告される**ので、差し戻したら必ず付ける（2026-08-25 に PR #771 で実際に空振りした）。
#   **付け外しは [`board-labels.yml`](../../.github/workflows/board-labels.yml) が行う。**
#
...
#
# **`board-labels.yml` が入った後、`FIXED` が出るのは短い窓と、セッションの一覧を引けない周だけ。**
# あちらは push の数秒後に `直し待ち` を外すので、外れた後のPRは `UNREVIEWED` の側へ回る。
# `UNREVIEWED` はセッションの一覧を引けた周にしか見ないので、**引けない周の落ち穂拾いとして残す。**
#
