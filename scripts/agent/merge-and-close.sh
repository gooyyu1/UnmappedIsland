...
# ## `\r` を落とす側と落とさない側
#
# **落とすのは、複数行を出して、その行を「シェルが」受け取るとき。** Windowsの外部 jq は標準出力を
...
# パイプの途中で流れているものだけが `| tr -d '\r' |` を要る（下の `base64 -d` の後）。
#
# **落とさない側が2つある。**
#
...
#
# ここまでは jq が付ける `\r` の話。**GitHubの画面で編集された本文は、中身そのものが CRLF で返る**
# ので、どのOSで動かしても `\r` が乗る。上の「受け取るのが `grep`・`awk` だけなら落とさなくてよい」は
# MSYS2 の `grep`・`awk` が `\r\n` を行の終わりとして扱うことに乗っているだけで、**Linuxの `awk` は
# `\r` を行の中身として残す**——`$` で留めた見出しに当たらなくなる
# （[`brake.sh`](brake.sh) が `## 手綱` の節を引く形）。中身が持つぶんは、受け手に関わらず落とす。
