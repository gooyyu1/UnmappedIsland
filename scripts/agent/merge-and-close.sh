...
#
# **文書の切れ目に `---` を挟む。** 下の awk は複数の文書を1本に連結して読むので、切れ目が見えないと
# **末尾に `## 司令塔へ` を置いた文書の節が、次の文書へそのまま伸びる**（節を閉じるのが `##` の
# 見出しだけなので）。`review-prompt.md` はレビュー側にこの節を**末尾**へ置かせるため、これは
# たまたま起きるのではなく常に起きる。
...
#
# **節を閉じるのは `##` の見出しと、水平線（`---`）。** 水平線は文書の切れ目に自分で挟んだもの
# （上の `review_comments`）と、レビューのコメントに必ず付く Claude Code の署名の頭。**どちらも
# 「ここから先は別の文書」を意味する**ので、同じ1つの規則で閉じる。閉じないと、レビューが節を末尾へ
# 置いたとき署名の行が中身として残り、「なし」と書いても非空になる。
