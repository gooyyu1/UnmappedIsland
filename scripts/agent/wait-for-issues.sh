...
#
# **「issue が閉じたか」では足りない。** 閉じていても main には何も入っていない場合がある——
# 取り下げで閉じた issue がそれで、その状態で次のタスクを投入すると、前のタスクの変更が無い
# main を土台に作業を始めてしまう。そこで見るのは、その issue を閉じる PR が**マージされたか**。
#
# 裏を返すと、PR を伴わずに閉じた issue は検出できず TIMEOUT になる。出力を見て人が判断する。
#
