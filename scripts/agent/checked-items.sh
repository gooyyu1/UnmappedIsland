#!/usr/bin/env bash
# 確定待ちの盤（`kind:ask` の issue）の本文で、チェックの付いている項目を `<番号> <項目>` の行にする。
# 標準入力は `gh issue list --json number,labels,body ...` が返す配列。
#
#   gh issue list --state open --limit 300 --json number,labels,body |
#     bash scripts/agent/checked-items.sh
#
# **300は1回目に渡す数で、上限ではない。** 渡した数と並んだ件数が返ったら、そこで切られている
# ので、倍にして引き直す（`agent-ops/github-access.md`「issue を読む・書く」）。切られるのは
# 古い側なので、黙って切ると答えの付いた盤ごと消える。
#
# ## 判定を1つにする
#
# ユーザーの答えは本文のチェックとして付き、**拾われるまでそこに残る**。今これを読むのは
# [`board.sh`](board.sh) の `## 確定待ち` だけだが、判定を呼び手の中へ書かないのは、**読む場所が
# 増えたときに同じ jq が2つになる**から。増分の窓で見ていた間に13件が一度も拾われなかったのは、
# 「いま答えが待たれている項目はどれか」を2箇所が別々に答えていたため。
#
# **デーモンはこれを読まない。** 拾うのは判断の要る仕事で、届ける口はまだ無い
# （[`board-design.md`](../../agent-ops/board-design.md) の未決「人間への入力の口」）。
#
# ## 見るのは `kind:ask` の issue だけ
#
# **ここが拾うのは、拾われたら項目ごと消えるチェックだけ。** 答えを受ける盤（`kind:ask`）の
# チェックがそれで、`kind:task` の issue が本文に持つ手順の一覧はただの覚え書き。区別せずに拾うと、
# 下ろす手立ての無い項目で盤面が毎回埋まる。
#
# **`kind:task` にも、ユーザーの答えそのものであるチェックはある**——周期の係が起票する諾否の一覧
# （[`board-design.md`](../../agent-ops/board-design.md) 2.17.2節）。あれを拾わないのは覚え書きだからでは
# なく、**下ろす先が既に在る**から（ユーザーが `判断待ち` を外せば、投入されたセッションが読む）。
#
# **常設の盤が全部その対象ではない**（分け方は
# [`board-design.md`](../../agent-ops/board-design.md) 2.17.5節）。**下ろす者の居ないチェック**を拾うと、
# 下りない項目が `## 確定待ち` に居座り、本物の答えがその中に埋もれる。
#
# **見分けるのはラベルの綴り1つで、issue の番号では持たない。** 番号で持つと、盤が1つ増えるたびに、
# 書き足し忘れたものが既定で拾われる側へ落ちる。
#
# ## 黙るのは、項目が一覧から下りたとき
#
# 出続けるのが正しい——チェックは出来事ではなく状態で、**拾われるまで残る**。拾った者は答えの
# 行き先をその盤の中へ書いてから項目を消すので、そこで `[x]` の箇条書きでなくなる。
# **消したことがそのまま黙る条件になる**ので、判定の側に「処理中」の置き場は要らない。
# **`【確定】` の印は待たない。** 印を付けるのは答えを受けた issue の担当者なので、待つと手番が
# 終わったのに一覧が残り続ける。

set -euo pipefail

# jq は Windows では改行を CRLF で書く（msys の text mode）。番号の突き合わせに使う側があるので、
# ここで `\r` を落としておく。
jq -r '.[]
       | select([.labels[].name] | index("kind:ask"))
       | .number as $number
       | (.body // "")
       | split("\n")[]
       | select(test("^[[:space:]]*[-*][[:space:]]+\\[[xX]\\][[:space:]]"))
       | sub("^[[:space:]]*[-*][[:space:]]+\\[[xX]\\][[:space:]]+"; "")
       | "\($number) \(.)"' | tr -d '\r'
