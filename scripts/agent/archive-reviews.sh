...
# だから見るのはタグと、上の2つの出来事だけ（`.claude/parallel-work.md`「状態で判定しない」）。
...
# 標準出力をテキストモードで開くので、行の区切りが CRLF になる。**`$(…)` が落とすのは末尾だけ**なので、
# 2行目以降の頭に `\r` が残る。ここでは `read` が拾って `archive_session` へ渡すので、JSON として
