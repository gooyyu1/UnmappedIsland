...
#   SOURCED   <パス> <見出し>     … 印を足したが、その節が #656 を指す `**出どころ**:` を持っている
...
# （[`review-criteria.md`](../../.claude/review-criteria.md)「人の判断へ回す（止める理由ではない）」）。
#
# ## 機械が引くのは印の増減だけ
#
...
# [`policies.md`](../../.claude/policies.md)「仕組みの作り方」の、**既に壊した実績のある操作は機械で
# 止める**にあたる。
#
# **差分を読まないと決められないことは、ここでは決めない。** 宣言文法・スキーマ・ゲームコンセプト・
# 確定節に抵触するかは、**差分を読むレビュアーが判定する**（[`review-criteria.md`](../../.claude/review-criteria.md)
...
# `#656` を指す `**出どころ**:` の1行を持つなら、決めたのはユーザー本人で、答えは既に出ている。
...
#
# **出どころは申告なので、嘘は書ける。** `SOURCED` は止めないので、申告が #656 で本当にチェック
# 済みかを確かめる決まりが無い。
#
# **`ユーザーの指示` と書いた出どころは、意図して緩めない**（`DocumentStyle.md` 6.1節。#656 を
...
while IFS= read -r path; do
  ...
  *) continue ;;
  ...
  # 新しく印が付いた節が、答えの出どころを1行で書いているか。**見るのは #656 を指していることだけ**
  # ——項目番号は答えの済んだものから詰め替わるので、指すのは決めた中身で、番号は書けない
  # （[`DocumentStyle.md`](../../docs/DocumentStyle.md) 6.1節）。`#656` は生（`#656（…）`）とリンク
  # （`[#656](…)（…）`）の両方が実在する。
  sourced() {
    printf '%s\n' "$3" | strip_marks | grep -qxFf "$WORK/new-marks" - || return 1
    sed -n "$1,$2p" "$WORK/doc.md" | grep -qE '^\*\*出どころ\*\*:.*#656'
  }
