...
#
# **渡さないことは「無効」ではなく「手元の `settings.json` の `defaultMode` に従え」。** だから
# 同じ「渡さない」が、投入先によって別のモードになる。
#
# - **クラウドは `auto`。** 箱に `settings.json` が無いので、渡さないと既定の `ask` で立つ。
#   `.claude/**` を**読むだけ**の `bash` が「機微なファイルの編集」と判定されて承認を待ち、
#   **その承認は降りない**（見ている人が居ない）ので、そこで手番が終わる——2026-09-05、PR #1567 の
#   レビューが判定を書く前にこれで止まった。
# - **ブリッジは空。** 手元の `~/.claude/settings.json` とリポジトリの `.claude/settings.json` が
#   `defaultMode: bypassPermissions` を持つので、**渡さなければそれになる**（出どころ: ユーザーの
#   指示・2026-09-06）。この経路から明示はできない——`requires a CCR parent session` で撥ねられる。
#
