...
- **立てたセッションは、現に働いている。** 投入できたことと働いたことは別で、立てた先に走る者が
  ...
  （`live-sessions.tsv` の末尾の列が `unserved` なら、そのセッションは一度も働いていない）ので、
...
| 立てたセッションが働いたか | `~/.claude/board-state/live-sessions.tsv` の**末尾の列**（`unserved` は一度も働いていないセッション。`status_bucket` では言えません——`..._FAILED` は、働いたあとに手番が転んだものにも付きます）。ブリッジなら `.claude/worktrees/bridge-cse_<ID>` が**作られたか**でも見られます |
