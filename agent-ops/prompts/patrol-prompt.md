...
- **立てたセッションは、現に働いている。** 投入できたことと働いたことは別で、**盤面が記録するのは
  前者だけ**——立てた先に走る者が付かなければ、セッションは作られたまま空で、盤面には「手が空いて
  いる」としか映らない。**値の見回りも気づかない**（`CLOUD_ENV`・`BRIDGE_ENV` の `alive` は
  **その環境IDが生きている一覧に在る**ことで、そこへ立てたものが働くかではない）。見るのは
  `live-sessions.tsv` の `status_bucket` と、働いた跡（`external_metadata`）の有無
- **人へ渡す手は、盤面の出口の中では例外である。** `RETURN`（`判断待ち` を付けて issue を返す）が
  ...
  返したぶんを戻せるのは人だけなので、**環境が直っても盤面は自力で戻らない**。配れる `着手可` が
...
| 立てたセッションが働いたか | `~/.claude/board-state/live-sessions.tsv` の `status_bucket`（`SESSION_STATUS_BUCKET_FAILED` が並ぶ周は、立てる側ではなく走らせる側が転んでいます）。ブリッジなら `.claude/worktrees/bridge-cse_<ID>` が**作られたか**でも見られます |
