...
- **盤面は、間隔ごとに1周している。** 測るのはこの係の間隔ではなく**周の間隔**で、物差しは
  ...
  **直っていることを、止まっていなかったことと取り違えない**——引けなかった区間は別に数える
  （下の「調べる先」）
...
| 盤面を引けなかった区間 | `grep -n '盤面を引けなかった（' ~/daemon.log`（**上の空きでは出ません**。5分おきに回り続けるため時刻の上では埋まります）。回数が連なっている区間がそのまま、1手も打たれなかった時間です。理由は直前の `失敗:` の行に在ります |
