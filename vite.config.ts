...
export default defineConfig({
  ...
  test: {
    ...
    environment: 'node',
    // 1件あたりの上限。**戻ってこないものを捕まえる線であって、速さの予算ではない。** 正しく
    // 走っている最も長い検査の何倍も上に置く——線が実測のすぐ上に在ると、同じ検査が混んだ回だけ
    // 越えて、本物の赤と見分けが付かなくなる（issue #2376）。越えたときに読めるのは「遅い」では
    // なく「止まっている」。
    //
    // **ファイルの中から `vi.setConfig` で渡しても効かない**（`vi` はワーカーに1つで、掴んでいるのは
    // 読み込まれた回の状態）。動かせるのはここと、`it` の第3引数だけ。
    testTimeout: 30_000,
