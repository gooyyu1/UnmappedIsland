...
export default defineConfig({
  ...
  test: {
    ...
    environment: 'node',
    // 1件あたりの上限。**速さの予算**であって、戻ってこないものを捕まえるためだけの線ではない。
    // 検査が何倍も遅くなったことを咎めるものはこの線しか無いので、全部をまとめて緩めない。
    //
    // **この予算に収まらないものは、`it`（または `describe`）へ名乗らせる。** 名乗りは差分に出るので、
    // 緩めたことが1件ずつ見える。**名乗る値は実測のすぐ上に置かない**——そこへ置くと、同じ検査が
    // 混んだ回だけ越えて、本物の赤と見分けが付かなくなる（issue #2376）。
    //
    // **フックはこの線に掛からない**（`hookTimeout`。動かすのは `beforeAll` などの第2引数）。
    // **ファイルの中から `vi.setConfig` で渡しても効かない**（`vi` はワーカーに1つで、掴んでいるのは
    // 読み込まれた回の状態。tests/architecture/testTimeouts.test.ts が見張る）。
    testTimeout: 5_000,
