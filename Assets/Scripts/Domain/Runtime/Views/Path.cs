...
namespace UnmappedIsland.Domain.Runtime.Views
{
    ...
    /// World と同じ理由で継承ではなくラップにしている。
    ///
    /// travel_minutes/required_progress/destination_idは生成時（IslandSpawner）にインスタンスごとへ
    /// 上書きされる値。移動そのもの（actorの所属差し替え・時間消費）はYAML側のtravelアクションが担う。
    /// </summary>
    public sealed class Path
    {
        ...
        /// <summary>未登録の名前は-1（LocalIndexMap.Missing扱い）にする（理由はLocation.IdOrMissing参照）。</summary>
