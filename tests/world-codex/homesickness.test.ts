...
describe('ホームシック(docs/world/Characters.md ホームシック節)', () => {
  ...
  /**
   * 打ちひしがれる周期を決めるのは、**despairで持ち出せた量を正味の削りで割った時間＋強制の2時間**。
   * **強制の2時間にも削りは走る**（戻しはtick毎、docs/engine/GameElementDefinition.md 11.7節）ので、
   * 持ち出せる量は `(2.5 - 正味の削り/tick) × 8` になる。正味の削りは口にした物で変わるので、
   * **食べているかどうかで奪われる時間も変わる**（docs/world/Characters.md ホームシック節）。
   *
   * | 食事 | 正味の削り | 持ち出せる量 | それが保つ | 周期 | 1日あたり |
   * | --- | --- | --- | --- | --- | --- |
   * | 火を通した3食（+18/日） | -30/日 | +17.5 | 14時間 | 16時間 | 3時間 |
   * | 何も食べない | -48/日 | +16 | 8時間 | 10時間 | 4.8時間 |
   */
  it.each([
    ['火を通した3食を通しても', 3, 2.5, 3.5],
    ['何も食べなければ', 0, 4.3, 5.3],
