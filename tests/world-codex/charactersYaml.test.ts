 ...
 * 覚醒度だけは誰も担がなくても -1/tick で減るので、経過ぶんを差し引いた実質の回復になる。
 *
 ...
 * 測れなくなる。体力は空から測ってよい——空身では削るものが無く、どの休息も正味で体力を戻すので、
 * 切れ目では限界を抜けている。
 */
function takeRest(
  character: string,
  ...
  expect(action()?.tryExecute() === true).toBe(true);
...
describe('プレイヤーキャラクタの定義', () => {
  ...
  describe.each(characters)('%s', (character) => {
    ...
    it.each([
      ...
      ['pain', 0],
      // 血は自分で戻るが、満タンで始まるので上限で頭打ちになる（次のテスト）。
      ['blood', 0],
    ...
    // 行う**（同 3.2節）ので、ここで置く値は「その段に入る位置」であって、条件の側の閾値ではない。
    it.each([
      ['hydration', 'dryish'],
      ['body_fat', 'starved'],
