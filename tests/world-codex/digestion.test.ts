...
describe('消化（かさ・栄養素・蓄え）', () => {
   ...
   * 段そのもの——見るのは、絶てば段まで落ちること・段が痛みを押し上げること・食べれば戻ること・
   * ビタミンそのものは命を絶たないこと。
   *
   * **壊血病を抱えた体が死なない、ではない。** この段は免疫も-40押し下げる（同6.2節）ので、菌源が
  ...
  describe('壊血病', () => {
    ...
    it('ビタミンは尽きても、それ自体では命を絶たない', () => {
      // 致命的域を持つのは命を絶つ値だけで、ビタミンは持たない（VitalsSystem.md 8節）。
      // **見ているのはビタミン単独の筋書き**——菌源を置いていないので、上のdocコメントの経路は通らない。
      endure(DEPLETION_TICKS + DAY);

      expect(vitamin().number, '0で止まる').toBe(0);
      expect(player.parent, '尽きたまま置いても世界から外れない').toBeDefined();
      expect(vitamin().alert, '尽きても危険域までで、致命的域には入らない').toBe('danger');
    });
  });

  /**
   * 脂が尽きたまま肉ばかり食べ続ける道（DigestionSystem.md 7節）。壊血病と同じ形で、内側の不調を
   * lipidの一番下の段が持つ——見るのは、絶てば段まで落ちること・水の保ちが半分になること（同8節）・
   * 食べたたんぱく質が身にならないこと・脂を摂れば全部止まること。
   *
  ...
  describe('脂が尽きた域', () => {
    ...
    it('脂を絶つと30時間で在庫が尽き、その3 tick手前で段に入る', () => {
  ...
  /**
   * 傷んだ物を食べたときの吐き下し（DigestionSystem.md 6.3節）。引き金は腐敗だけ（durabilityの段が
