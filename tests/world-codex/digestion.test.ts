...
describe('消化（かさ・栄養素・蓄え）', () => {
  ...
  function tick(count: number): void {
    ...
    for (let i = 0; i < count; i++) {
      player.tick();
      player.getProperty(hydrationId).init(held);
    }
  }

  /** 在庫へ直接入れる（食べ物を用意せずに量だけを置きたいとき）。 */
  function stock(amount: number): void {
    player.getProperty(carbohydrateId).init(amount);
    for (const name of ['protein', 'lipid']) player.getProperty(codex.propertyNames.getId(name)).init(0);
  }
  ...
  it('食べた物は、かさと栄養素の両方に入る', () => {
    ...
    player.getProperty(satietyId).init(0);
    stock(0);
  ...
  it('在庫は時間をかけて蓄えになり、尽きれば蓄えが削られる', () => {
    player.getProperty(bodyFatId).init(1000);
    stock(4);
  ...
  it('満腹感はかさで、1食が8時間もつ', () => {
    ...
    player.getProperty(satietyId).init(512);
  ...
  it('1日3食（512mLのイモ）で体脂肪は横ばいになる', () => {
    ...
    for (let day = 0; day < 5; day++) {
      for (let meal = 0; meal < 3; meal++) {
        player.getProperty(carbohydrateId).init(valueOf(carbohydrateId) + 32);
        tick(DAY / 3);
      }
    }
  ...
  it('食べ続けても際限なく太らない（基礎代謝が体格で上がる）', () => {
    ...
    const rates = [0, 96, 480, 2880, 4320].map((fat) => {
      player.getProperty(bodyFatId).init(fat);
      stock(0);
