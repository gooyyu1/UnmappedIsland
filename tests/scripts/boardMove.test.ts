...
describe('board-move.mjs', () => {
  ...
  // **返す手は取り消せない**（issue へ `[返却]` が載り、次の周にワーカーが畳まれてそのPRは宛先の
  // 無いPRになる）。**`main` が動くたびに開いているPRの `mergeable` は全部 `UNKNOWN` へ落ち**
  // （2.12.2）、CIが走り直している最中の色は `running` ——**次の周には決まる値**なので、レビューへ
  // 渡せないと読んで返すと、渡すはずのPRが人の手番へ落ちる。
  it('マージできるか読めていない周は、頼み終えたPRを返さずに待つ', () => {
    const board = {
  ...
  // **`直し待ち` でも、レビューへ渡せないと分かっていれば返す。** 入口は緑でマージできることなので、
  // 赤いまま落とすと 2.13.6 の経路そのものが無い状態に戻る。
  ...
  // 次の周に畳まれるので、頼んでも届く先が消える。**ワーカーが自分で返した形（2.15.2）は盤面の覚えに
  // 出ない**ので、ここは**現物のラベル**の側が拾う（台帳の側は
  // `返し終えていれば、判断待ちがまだ付いていなくても手を出さない` が見る）。
  ...
  // **人の手番の断りは、レビューへ渡す段を止めない**（2.13.6）。あの断りが言っているのは「直しを
  // 頼めない」ことで、**もう1周読ませる手はそれではない**——止めると、担当が人の手番に在る間だけ
  // 2.13.6 の経路が消える。
   ...
   * なので、**塞がりが明けても盤面は自力で戻れない**（issue #2206）。
   */
