...
import type { WorldSignal } from '../../src/domain/runtime/WorldSignal';
...
`;
  ...
  function open(roll: number): void {
    ...
    ground = spawn('ground');
    expect(ground.moveToSlot(worldInstance, codex.slotNames.getId('locations'))).toBeUndefined();
  ...
  function placeOnGround(name: string, slotName = 'items'): WorldObject {
    ...
    expect(object.moveToSlot(ground, codex.slotNames.getId(slotName))).toBeUndefined();
    return object;
  }
  ...
  it('世界の形が変わらない回でも、起きたことが観測できる', () => {
    ...
    const seen = observe(() => {
      session.observeChanges(
        ...
        () => {
          expect(beast.tryExecuteCombination(stick, undefined, 'hit_me', session)).toBe(true);
        },
      );
    });
  ...
  it('対象を書けば、宣言した側とは別のオブジェクトに起きたこととして告げる', () => {
    ...
    const seen = observe(() => {
      expect(beast.tryExecuteCombination(stick, undefined, 'shrug_off', session)).toBe(true);
    });
  ...
  it('解決できない対象へは何も告げない', () => {
    ...
    const seen = observe(() => {
      expect(beast.tryExecuteAction('roar', undefined, session)).toBe(true);
    });
  ...
  it('どの候補を引いたかで、違う出来事が告げられる', () => {
    const first = observe(() => {
      expect(beast.tryExecuteCombination(placeOnGround('stick'), undefined, 'hit_me', session)).toBe(true);
    });
    open(MISSES);
  ...
  it('rangeイベントからも告げられる（狩猟に限らない汎用の語彙）', () => {
    ...
    const seen = observe(() => {
      expect(beast.tryExecuteAction('exhaust', undefined, session)).toBe(true);
    });
