...
`,
  ...
  function accepted(owner: WorldObject, items: readonly WorldObject[]): number {
    return items[0].acceptedCountForMoveTo(items.slice(1), owner, contentsId, codex.wellKnown);
  }
  ...
  it('既に入っているぶんだけ減る', () => {
    const [basket, coconuts] = open('basket', 'coconut', 10);
    expect(coconuts[0].moveToSlot(basket, contentsId, codex.wellKnown)).toBeUndefined();
  ...
  it('1個も入らないなら0で、1個だけ入るかを訊いた答えと食い違わない', () => {
    ...
    for (const coconut of coconuts.slice(0, 6))
      expect(coconut.moveToSlot(basket, contentsId, codex.wellKnown)).toBeUndefined();

    const last = coconuts[6];
    expect(accepted(basket, [last])).toBe(0);
    expect(last.rejectionForMoveTo(basket, contentsId, codex.wellKnown)).toBeDefined();
  });
});
