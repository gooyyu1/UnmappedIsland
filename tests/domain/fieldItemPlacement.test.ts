...
`;
  ...
  function fill(...names: string[]): WorldObject[] {
    return names.map((name) => {
      ...
      expect(location.receiveItem(spawned)).toBe(true);
      return spawned;
    });
  }
  ...
  it('左向きの並び替えでは、落とした隙間の右へ入る', () => {
    ...
    expect(c.reorderInParentSlot({ kind: 'gap', index: 1 }), 'aとbの隙間へ動かす').toBe(true);
  ...
  it('位置を指定した受け入れは、その隙間へ入る（前詰めなので押し出しは要らない）', () => {
    ...
    expect(location.receiveItem(item('c'), { kind: 'gap', index: 1 })).toBe(true);
  ...
  it('位置を指定しても、同種のアイテムは既存スタックへ合流する', () => {
    ...
    expect(location.receiveItem(item('b'), { kind: 'gap', index: 0 })).toBe(true);
