...
`;
  ...
  beforeEach(() => {
    ...
    wip.moveToSlot(ground, slotOf('items'), codex.wellKnown);
  });

  /** 素材をn個、指定の親のスロットへ置く。 */
  function place(objectName: string, count: number, parent: WorldObject, slotName: string): void {
    for (let i = 0; i < count; i += 1)
      session.spawn(idOf(objectName)).moveToSlot(parent, slotOf(slotName), codex.wellKnown);
  }
  ...
  it('入れ物の中までは探さない', () => {
    ...
    basket.moveToSlot(player, slotOf('hand'), codex.wellKnown);
