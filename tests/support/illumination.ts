 ...
 */
export function createBrightEnoughAgent(session: WorldSession): WorldObject {
  const character = session.createObject(session.codex.objectNames.getId(SAMPLE_CHARACTER));
  makeBrightEnoughForAnyAction(character, session.codex);
  return character;
}
