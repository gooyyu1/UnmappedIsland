 ...
 * `on_max`）と、その航路が渡す先（`cross` の `to_object`）を突き合わせると、どの海区とどの海区が
 * 繋がっているかが1つに決まる——**盤面に実際に出る辺**がこれなので、他の宣言（本土までの残り海区数・
 * 荒天の押し流し先）はこれと突き合わせて検査できる。
 *
 * 行き先を型で名指しできる口（`to_object`/`into_object`）はYAMLに直接書いた識別子しか受け取らない
 * ので、隣は海区ごとに手で書くしかない（プロパティへ覚えさせる手段が無い）。**手書きが増えるぶんの
 * 守りがこれ。**
 */
...
export function readSeaChart(): SeaChart {
  ...
  for (const [name, body] of defs) {
    ...
    for (const candidate of asList(nodeAt(body, 'interactions', 'cross', 'pick'))) {
      for (const move of asList(nodeAt(candidate, 'move'))) {
        const destination = nodeAt(move, 'to_object');
        if (typeof destination === 'string') routeDestinations.set(name, destination);
      }
    }
  }
  ...
  for (const zone of zones) {
    for (const spawned of asList(
      ...
      const placedInto = nodeAt(spawned, 'into_object');
      connect(typeof placedInto === 'string' ? placedInto : zone, destination);
    }
  }
