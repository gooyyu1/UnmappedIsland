 ...
 * シミュレーションは1分強かかるので、他のレポートと違って`npm test`では丸ごと作り直しては比べない。
 * 丸ごとの突き合わせは`.github/workflows/regenerate-stats.yml`が`main`で持つ。再生成する
 ...
 *
 * 土地の側の変更まで指紋に含めると、食べ物を1つ足すたびに1分強の再生成を要求することになる。
...
function createClimateStats(
  ...
  rainWeatherKinds: ReadonlySet<number>,
): ClimateStats {
  ...
  return [
    ...
    },
    {
      ...
      records: activityHoursOf(codex, seasonWeatherHours).map((row) => ({
        ...
        exploration: rounded(row.explorationHoursPerDay, 1),
...
async function buildReportFromDefinitions(): Promise<string> {
  ...
  const rainWeatherKinds: ReadonlySet<number> = new Set([lightRainId, heavyRainId, stormId]);
 ...
 *
 * 再生成に1分強かかるので、`npm test`では作り直さずに**入力が変わったことだけを軽く見る**。
...
describe('climate.yamlの鮮度', () => {
  ...
  it('活動時間の節の行が、今の定義から出る行と過不足なく一致する', () => {
    ...
    for (const row of rows) {
      ...
      expect(row.explorationHoursPerDay, `${label} の探索できる時間`).toBeCloseTo(
        Number(record.exploration),
        1,
      );
