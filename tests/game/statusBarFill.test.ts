...
import { COLOR, gainBandFill, statusFillColorFor } from '../../src/game/looks/theme';
...
describe('ステータスバーの塗りの色', () => {
  ...
  it('増えた分の帯は、塗りをトラック側へ薄めた色になる', () => {
    ...
    const band = channels(gainBandFill(water));
    ...
    expect(gainBandFill(water), '塗りそのものではない（帯と塗りの境目が見える）').not.toBe(water);
    expect(gainBandFill(water), '減った分の赤とは別物').not.toBe(COLOR.statusBarLag);
  });
