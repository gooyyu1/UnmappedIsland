...
describe('PlayScreenView(ゲーム状態から画面の表示内容を作る)', () => {
  ...
  /** その区画のレーンに並んでいる札（PlayScreenView.cardsIn）。 */
  function lane(view: PlayScreenView, game: NewGameSession, screen: 'items' | 'fixtures') {
    return view.cardsIn(place(game, screen));
  }
