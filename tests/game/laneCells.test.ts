...
import { cellsFor, LANE_CELLS_MAX, unboundedSlot } from '../../src/game/ui/laneCells';
...
describe('レーンの枠', () => {
  it('カードは位置を保ったまま枠に入る', () => {
    const cells = cellsFor([card('丸太'), undefined, card('石')], 3, false);
    expect(cells.map((cell) => cell.card?.name)).toEqual(['丸太', undefined, '石']);
  });

  it('枠数の決まったスロットは、埋まるまで常にその数の枠を見せる', () => {
    expect(cellsFor([], 1, true), '1枠のスロットは空なら1枠').toHaveLength(1);
    expect(
      cellsFor([card('包帯')], 1, true),
      '埋まれば1枠——2枠目は「もう1つ当てられる」と誤って伝わる',
    ).toHaveLength(1);
    expect(cellsFor([card('丸太')], 3, true)).toHaveLength(3);
    expect(emptyCells(cellsFor([card('丸太')], 3, true))).toBe(2);
  });
  ...
  it('受け入れないスロットは空枠を出さない', () => {
    expect(emptyCells(cellsFor([], 1, false)), '怪我のように外から入れられない場所').toBe(0);
    expect(emptyCells(cellsFor([card('捻挫')], undefined, false))).toBe(0);
  });

  it('枠数を超えて入っていても、空枠は増えない', () => {
    expect(emptyCells(cellsFor([card('丸太'), card('石'), card('葉')], 1, true))).toBe(0);
  });
});
