...
`;
  ...
  it('条件は、周期を決めた増減のものになる', () => {
    // 24を-1で削るので周期は24 tick（6時間）。その-1を縛るのは日差しのほうで、雨は周期を決めて
  ...
  it('遅くするだけの条件は、周期を決めていても「いつ働くか」にならない', () => {
    // 最も遅いのは-3と+2が重なった-1なので周期は24 tick（6時間）。雨はその周期を決めているが、
  ...
  it('常時効く分が正味0でなくても、遅くするだけの条件は出ない', () => {
    // 最も遅いのは雨の-1で24 tick（6時間）だが、雨が降らなくても-3で進む。ここに雨を出すと
  ...
  it('片方だけで足りるなら、相方は条件にならず「または」で並ぶ', () => {
    // どちらか片方なら-1で24 tick（6時間）、両方揃えば-2で半分。**どちらも欠かせないわけでは
...
`;
  ...
  it('労働0でしか手に入らない道具を要求する経路も、時間を払える上流を持つなら表に出る', () => {
    // 茶を飲むのに要る道具は労働0でしか手に入らないが、道具の時間は経路へ按分しない（#550）ので、
    // 手に入りさえすれば経路は成り立つ。解き直した側で前提が解けないことを理由に落とすと、
    // **道具の側の都合で、時間を払える経路が表から消える**（issue #2150）。
    expect(routesThrough('draw_tea').map((route) => route.route.untimed)).toEqual([false]);
  });

  it('その道具は、島のどこにも入手経路が無いもの（穴）にはならない', () => {
