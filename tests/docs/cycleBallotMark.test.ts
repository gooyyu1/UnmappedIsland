 ...
 * - **印の行の外にも、綴りが書いてある。** どの係の一覧も `判断待ち` を付けて立つので、**候補には
 *   他の係の一覧が必ず入る**。「印が在るか」としか書いていないひな形は、**他の係の1本を自分のものと
 *   読んで黙る**か、自分の1本と合わせて2本と読んで他の係の一覧へ書き込む。
 *   **見ているのは綴りが在ることだけで、それが判定の文に在るかは見ていない**——別の節へ綴りを書けば
 *   通るので、判定の側から綴りが落ちたことは、ここでは捕まらない。
 *
...
describe('諾否の issue に置く印', () => {
  ...
  it.each(BALLOT_PROMPTS)('%s は、印の行の外でも綴りを書いている', (file) => {
    const body = bodyOf(file);
    const spelling = marksIn(body)[0];

    expect(body.replaceAll(MARK, ''), '印の行の外に綴りが無い').toContain(spelling);
  });
