 ...
 *
 * **見ているのはここに並べた綴りだけで、言い回しそのものではない。** 数字を書かずに頻度だけを言う形
 * （「毎日」「毎週」「週が明けるまで」）も写しなので並べてあるが、**並べていない言い換えは間隔が
 * 動いても落ちない**——落ちないぶんは、綴りを1つ足すことでしか埋まらない。
 */
const INTERVAL_SPELLINGS: ReadonlyMap<number, readonly string[]> = new Map([
  [1, ['一時間に一回', '1時間に1回', '一時間ごと', '1時間ごと']],
  [12, ['一日二回', '1日2回', '半日ごと', '半日に一度', '半日に一回', '12時間ごと']],
  [24, ['一日一回', '1日1回', '毎日一回', '一日に一回', '1日に1回', '毎日', '24時間ごと']],
  [
    168,
    ['週一回', '週1回', '一週間に一回', '毎週', '週に一回', '週に1回', '週が明ける', '168時間ごと'],
  ],
]);
