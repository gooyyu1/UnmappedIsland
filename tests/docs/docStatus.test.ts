 ...
 * 同じように落ちたときに気づけない。フェンスの外を採る形も変えて書く（`markdownFences.mjs` は
 * 行を1本ずつ状態機械で追い、こちらは**手前に在るフェンス行の数の偶奇**で決める）。
 *
 * フェンスを追うのは、`agent-ops/prompts/**` がセッションへ渡す本体を囲みに入れて持っており、
 * **囲みの中に深さ2の見出しが並ぶ**ため。追わないと、渡す文面の見出しを節として数えてしまう。
 */
