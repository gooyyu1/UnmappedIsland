...
| TitleScene | `drawBackground` | `src/ui/shapes` | **縦グラデーションを敷く**。Phaser の `fillGradientStyle` が WebGL 専用で使えないため行ごとに自分で混ぜている——これは CodeStructure.md 4節が言う「Phaser の足りない分を埋める＝`src/ui/`」の型 | 消える | 無し |
