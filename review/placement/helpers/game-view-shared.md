...
| src/game/view/recipeList.ts | `actorOnly` | `RecipeDef.unmetUnlockRequirement` | 「まだ実体が無いので agent しか解決できない」という**ドメインの規則**（GameElementDefinition.md 13.3節）を、B が自分で持てない。B は汎用の `resolveRoot` を要求する | 消える。`unmetUnlockRequirementFor(agent)` を足せば呼び出しは1行 | 無し。`unmetUnlockRequirement` の呼び出し元は src 内でここ1箇所だけ |
