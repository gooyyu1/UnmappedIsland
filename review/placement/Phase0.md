...
- `docs/CodeStructure.md` — 決定2・3・4と、`Button` の判定例の訂正、`src/domain/views/` の注記
...
| A-11 見せ方の宣言 | **却下。** 方針は既にコードに書かれている。CodeStructure.md に参照を1行足すだけ |
...
欠けているのは概念ではなく、`CodeStructure.md` にこの方針が載っていないことだけ。
→ `CodeStructure.md` 3節の「意匠か素材か」に1行足して `rainStyle.ts` を指す。**コード変更なし。**
...
これは CodeStructure.md 2節が言う組み立て——「どの部品をどこへ置き、映しの答えを誰へ渡すかを決める」——
そのもので、その一番外側にあたる。

→ 2節の「`PlayScene`・各 `Scene`・各 `Window` がこれにあたり」に **`src/main.ts` と `DeviceScreen` を
足す**。1行。**新しい層も、新しいディレクトリ（`src/boot/` 等）も作らない。**

却下した案: **表に「起動」の行を足す。** 層を1つ増やすことになるが、`main.ts` が知ってよい範囲は
組み立てとまったく同じ（現在の1節の表では「解析を除く全部の層」）。**区別しても何も変わらないので、
増やす理由が無い。**

---

## `docs/CodeStructure.md` への反映
...
| 2-5 `src/domain/views/` が「映し」と名前衝突 | 1節の表に「`src/domain/views/` は映しではない」を書き添える（改名は段4以降の判断） |
