...
段0〜段4まで実施済み。**残っているのは A-7（保留）だけ**。実施の記録は
[`Stage3.md`](./Stage3.md)（節ごとに1レーン）、段0の決定は [`Phase0.md`](./Phase0.md) にある。

| 段 | 状態 |
| -- | ---- |
| 段0 決定 | 済（Phase0.md。A-11・A-12 は却下） |
| 段1・段2 | 済 |
| 段3 定義側の口 | 済（Stage3.md 1〜7節。レーンR で A-9 も消えた——`*_CONDITION_ROOTS` は `src` にもう無い） |
| 段4 A-1 画面のことば | 済（Stage3.md 10節） |
| 段4 A-2 部品1つぶんの意匠 | 済（同 11節） |
| 段4 A-3 観測の器 | 済（同 12節） |
| 段4 A-4 中身から受ける寄与 | 済（同 13節） |
| 段4 A-5 入れ子を読み上げる語彙 | 済（同 14節） |
...
| 段4 A-7 実体化された島 | **保留**（同 18節。セーブの形式が決まってから——追記を参照） |
| 操作のきっかけ（`interactions` への統合） | 済（同 19節。[`InteractionTrigger.md`](./InteractionTrigger.md)） |
| `Slot` のシフトアルゴリズム（`CellLayout`） | 済（同 21節） |
| H-8・H-9（札とレーンの口） | 済（同 22節。**B-4 は取り下げ**——診断が逆向き） |
| H-17（区画の中の位置） | 済（同 23節） |
| H-16（レーンと場所の対） | 済（同 24節） |
| B-8（宣言の不変条件） | 済（同 25節。阻害要因は段2のH-1で既に外れていた） |
| B-7（効果クラスの歪み） | 済（同 26節。**4件中3件は判定1へ取り下げ**） |
| `balanceTables.isLocation` | 済（同 27節。コメントが正しく、実装を寄せた） |
| `cardLooks` のタグ名10個 | 済（同 28節。**着手前に既に解消していた**） |
| 影響の一覧の縮退表示 | 済（同 29節。`docs/ui/Windows.md` 8.2節） |
| 名前不一致（`areas/*.md` の最終列） | 済（同 30節。**この段の計画に入っていなかった27件**。23件を改名、2件は移動で消滅、2件は取り下げ） |

### 残っている宿題（レーンの外）

**A-7（実体化された島、Stage3.md 18節）だけ。** セーブの形式が決まるまで保留。
