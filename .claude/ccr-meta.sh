...
# 引ける全部。
#
# **本文を渡す引数は、ファイルへ書いてから流す**（理由と踏み方は下記）。文字化けは考えなくてよい——
# node は argv も環境変数も標準入力もUTF-8で受ける（2026-09-05 に、コードページ932のまま実測）。
#
# ## なぜこれが要るのか — 入口はこれ1つ
#
...
#
# **ヒアドキュメントで組み立てない。** 区切りを引用しない `<<EOF` の中身は、**二重引用符の中と同じに
# 展開される**——`` `...` `` と `$(...)` は中身が**コマンドとして実行されて**その出力へ置き換わり、`$VAR` は
# 変数の中身に化ける（2026-09-18 に実測）。**危ないのは特定の文字ではなく展開そのもの**なので、構文ごとに
# 載せてよいかを判断せず、載せないほうを決めておく。
#
# **このリポジトリの指示は必ず踏む。** 識別子をバッククォートで囲む書き方なので、囲んだ識別子がコマンド
# として実行され、見つからずに消える（2026-08-25 に実測。「`ClimateSystem.md` のような」が「 のような」
# になったまま届いた）。
#
...
# 読み始めるので、**動いている様子は「無事に届いた」の根拠にならない**（2026-08-25 に、丸ごと壊れた
...
# 日本語もバッククォートもそのまま通るので**壊れたようには見えない**が、`=>` や `->` を含むコードを
...
#   **どれを渡すかは投入先で決まる**ので、[`ccr-env.sh`](../scripts/daemon/ccr-env.sh) から取る
...
# 起こす必要は無い。
#
# **node から呼ぶ側は、ここを通らずに `ccr-meta.mjs` を直に import する**（同じ理由。あちらの
# 「node から呼ぶ側は、シェルの入口を通らない」）。この入口が要るのは、手で叩く側とシェルの呼び手。
#
