...
# 展開される**（2026-09-18 に実測。`` `...` `` と `$(...)` は空になり、`$VAR` は変数の中身に化けた）。
# **危ないのは特定の文字ではなく展開そのもの**なので、構文ごとに載せてよいかを判断せず、載せないほうを
# 決めておく。
#
# **このリポジトリの指示は必ず踏む。** 識別子をバッククォートで囲む書き方なので、囲んだ識別子が黙って
# 消える（2026-08-25 に実測。「`ClimateSystem.md` のような」が「 のような」になったまま届いた）。
#
