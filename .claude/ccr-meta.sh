...
# ## なぜこれが要るのか — 入口はこれ1つ
#
# **`mcp__ccr_meta__*` の登録は消してある**（`~/.claude.json` の `mcpServers`）。あれは**起動時のヘッダを
# 掴んだまま**なので、走っている最中にトークンが切れると、登録を直しても**そのセッションからは二度と
# 使えない**（2026-08-25 に2回起きた）。しかも登録には固定のアクセストークンが平文で載る。
#
