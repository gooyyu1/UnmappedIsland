...
立てた issue には `origin:agent` を付ける」）。この印は、向かう先（`goal:`）が名乗られていないときの既定を分けるのにも使われます（`.claude/board-design.md` 2.18.1）——**`origin:agent` を付けた子 issue が `goal:` を名乗っていなければ、整備として読まれ、完成へ近づける仕事より後ろに並びます。** 分解した子が完成の定義を埋めるものなら、元の issue の `goal:` を子へも引き継いでください。
