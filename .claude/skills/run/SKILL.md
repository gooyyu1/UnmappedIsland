...
`tests/game/slotCells.test.ts`が言っている）。
...
変わりうるため、`.claude/skills/run/scripts/find-chromium.sh`で毎回解決すること（決め打ちしない）。
...
**別々の実行を比べるなら、URLに `?seed=` を付ける。** `--url 'http://localhost:<port>/?seed=12345'`
で開くと新規ゲームの乱数シード欄がその値から始まり、島も開始時刻も漂着地も毎回同じになる
（別々の実行で撮った2枚がPNGとしてバイト一致することを実測済み）。

**付け忘れると比べられない。** 種を渡さなければ実行ごとに島も開始時刻（8:00〜12:00からランダム）も
変わる。実際に、別々の実行の時計を比べて回帰を疑い、無駄に調べたことがある。
...
```
[450,270] はじめる → [228,210] 空きスロット → [400,307] キャラクタ
→ [299,213] 島の名前のサイコロ（未入力だと「入力を確認してください」で止まる）
→ [668,607] はじめる → [347,105] 現在地の札 → [556,458] 探索する
```
