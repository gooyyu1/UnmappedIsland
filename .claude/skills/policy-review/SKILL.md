...
後の発言が前の解釈をひっくり返したなら、覆られた側の箇条書きへ「覆っている」と書き添え、**覆した
記録を `[[<拡張子を除いたファイル名>]]` で指す**。断りだけでは裏が取れない——指し先が実在するかは
`tests/docs/policies.test.ts` が見ている（**見ているのは解釈の節だけ**。原文の側は書き換えないと
決めてあるので、そこは通す）。

**覆したのが後の発言ではなく、今の運用そのもののこともある**（当時かぎりの手順を、解釈が常設の
既定として書いていた場合など）。そのときも同じく書き添えるが、指せる記録が無いので、今はどうなって
いるかを本文で書く——**そちらは機械では見ていない。**

**書くのは、気づいた者すべて。** `declined:` と違って棚卸しの持ち物ではない。**履歴は誰にも読まれ
ないまま溜まる場所**なので、気づいた回を逃せば、次の棚卸しが今と逆の既定を全セッションへ配る。
...
4. 承認されたぶんを反映し、元になった履歴を `archive/` へ `git mv` する。**一般則にならずとも、
   ...
   **生きている解釈が残っていない履歴も `archive/` へ**——覆った断りを書き添えてから（上の
   「覆った解釈には、覆した記録を指す」）。`declined:` は付けない。見送られたのではなく、
   **後の発言に覆られた**ので、付けると見送った場所を訊かれて答えられない。
