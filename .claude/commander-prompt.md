...
- **`判断待ち` の付いたPR**（`needs-user-review.sh` が `MARK` を出した）… ラベルを
  外しても、次のマージの試みでデーモンが付け直します。越えるのは
  `bash scripts/agent/merge-and-close.sh <PR> --user-ok` だけで、**ユーザーの許可を得てから**
  打ちます（許可を得た旨がPRのコメントに残ります）。受け付けないなら `却下` を付けます。
  **レビューが `[レビュー] 通してよい（人の判断が要る）` で付けたほうは別**（`board-design.md`
  2.13.4）——**ラベルを1つ外せば通ります**。何を判断してほしいかはそのコメントに書いてあります。
- **`収束せず` の付いたPR**（3周してもレビューが畳めなかった）… そのPRに残っているレビューの
  コメントを読み、決めてユーザーへ渡すか、自分で決める。通すなら `通してよい`、書き直させるなら
  `直し待ち` を付け直します。
- **`ユーザーへ` の付いたPR** … PR本文の `## ユーザーへ` を捌いてラベルを外す。issue を立てる・
...
- **ユーザー本人の発言から受け取った判断は、その場で `.claude/decisions/` へ原文で記録します。**
  エージェントの発言からは作りません。一般則へ畳むのは棚卸しの仕事です（`CLAUDE.md`「価値観の記録」）。
